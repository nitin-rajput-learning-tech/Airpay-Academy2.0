...
function local_custom_category_output_fragment_new_custom_category_form($args){
    ...
    $mform = new local_custom_category\form\custom_category_form(null, array('id' => $args->repositoryid, 'editoroptions' => $editoroptions), 'post', '', null, true, $formdata);

    $data->name = $data->fullname;
    $costcentername = $DB->get_records_menu('local_costcenter', array('id' => $data->costcenterid),'id', 'id,fullname');
    $data->open_costcenterid = $costcentername;
    $mform->set_data($data);
