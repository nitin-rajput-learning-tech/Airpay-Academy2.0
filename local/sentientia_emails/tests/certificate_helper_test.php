...
/**
 * @covers \local_sentientia_emails\certificate_helper
 *
 * Sprint B unit tests.
 *
...
class certificate_helper_test extends \advanced_testcase {
    ...
    public function test_cleanup_deletes_real_temp_file(): void {
        ...
        // Drop a real file in $CFG->tempdir/sentientia_emails and verify
        // cleanup_materialised removes it.
        $dir = $CFG->tempdir . '/sentientia_emails';
        if (!is_dir($dir)) {
            mkdir($dir, 0755, true);
        }
        ...
        certificate_helper::cleanup_materialised([
            'abs_path' => $path,
            'rel_path' => 'temp/sentientia_emails/' . basename($path),
            'display_name' => 'x.pdf',
        ]);
