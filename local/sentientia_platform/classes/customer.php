...
class customer {
    ...
    public static function assert_valid(int $customer_id): void {
        ...
        if ($customer_id !== self::AIRPAY && $customer_id !== self::DEFAULT) {
            throw new \moodle_exception('error_invalidcustomer', 'local_sentientia_platform',
                '', $customer_id);
        }
    }
     ...
     * `local_sentientia_customer_brand` table (single row per customer id),
    ...
    public static function branding(?int $customer_id = null): array {
        ...
        try {
            $cache = \cache::make('local_sentientia_platform', 'customer_brand');
        ...
        // exist during a multi-step upgrade where sentientia_platform hasn't
        // run its 2026052201 savepoint.
        $row = null;
        try {
            $row = $DB->get_record('local_sentientia_customer_brand',
     ...
     * (`/local/sentientia_platform/pix/...`) and absolute (`https://cdn...`)
     ...
     * `local_sentientia_customer_brand`. Future Phase 2 admin UI will wire
    ...
    public static function known_customers(): array {
        return [
            [
                'id'         => self::DEFAULT,
                'name'       => get_string('customer_default_label', 'local_sentientia_platform'),
                'is_default' => true,
            ],
            [
                'id'         => self::AIRPAY,
     ...
     * under the `local_sentientia_platform` namespace, keyed as
     ...
     *
     * Resolution:
     *   1. Look up `local_sentientia_platform/customer_<id>_<key>`.
    ...
    public static function get_customer_config(string $key, int $customer_id, $default = null) {
        ...
        $value = get_config('local_sentientia_platform', $config_key);
     ...
     * `local_sentientia_platform/customer_<id>_<key>` setting name directly.
     *
    ...
    public static function set_customer_config(string $key, int $customer_id, ?string $value): void {
        ...
        $config_key = 'customer_' . $customer_id . '_' . $key;
        if ($value === null) {
            unset_config($config_key, 'local_sentientia_platform');
            return;
        }
        set_config($config_key, $value, 'local_sentientia_platform');
    }
}
