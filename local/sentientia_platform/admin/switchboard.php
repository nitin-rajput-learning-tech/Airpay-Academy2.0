 ...
 *
 * URL: /local/sentientia_platform/admin/switchboard.php?customer=C&tenant=T
 *
...
$customer_layer_on = \local_sentientia_platform\feature_flags::is_enabled_for(
...
if (!$customer_layer_on) {
    ...
    $known_customers = \local_sentientia_platform\customer::known_customers();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_array($changes) || empty($changes)) {
        redirect(
            ...
            get_string('switchboard_no_changes', 'local_sentientia_platform'),
            2,
    ...
    redirect(
        new moodle_url('/local/sentientia_platform/admin/switchboard.php',
            ['customer' => $customer_id, 'tenant' => $tenant_id]),
        get_string('switchboard_applied', 'local_sentientia_platform', $applied),
        2,
...
$PAGE->set_url(new moodle_url('/local/sentientia_platform/admin/switchboard.php',
    ['customer' => $customer_id, 'tenant' => $tenant_id]));
$PAGE->set_title(get_string('switchboard_pagetitle', 'local_sentientia_platform'));
$PAGE->set_heading(get_string('switchboard_pagetitle', 'local_sentientia_platform'));
$PAGE->set_pagelayout('admin');

// Load all flags + their resolved state for this (customer, tenant) pair.
$all_flags = \local_sentientia_platform\feature_flags::all($tenant_id, $customer_id);
...
$customer_tabs = [];
if ($customer_layer_on) {
    foreach (\local_sentientia_platform\customer::known_customers() as $c) {
        $customer_tabs[] = [
            'id'         => $c['id'],
            'name'       => $c['name'],
            'is_active'  => $customer_id === $c['id'],
            'is_default' => $c['is_default'],
            'url'        => (new moodle_url('/local/sentientia_platform/admin/switchboard.php',
...
foreach ($tenants as &$t) {
    $t['url'] = (new moodle_url('/local/sentientia_platform/admin/switchboard.php',
...
if (!$customer_layer_on || $customer_id === 0) {
    if ($tenant_id === 0) {
        $scope_label = get_string('scope_global', 'local_sentientia_platform');
        $scope_banner = get_string('scope_banner_global', 'local_sentientia_platform');
    } else {
        ...
        $scope_banner = get_string('scope_banner_legacy_tenant',
            'local_sentientia_platform', format_string($scope_label));
    }
} else {
    $customer_label = \local_sentientia_platform\customer::label_for($customer_id);
    $tenant_label = $tenants[array_search($tenant_id,
        array_column($tenants, 'id'), true)]['name'] ?? 'Unknown';
    if ($tenant_id === 0) {
        $scope_label = $customer_label;
        $scope_banner = get_string('scope_banner_customer',
            'local_sentientia_platform', format_string($customer_label));
    } else {
...
$data = [
    ...
    'sesskey'              => sesskey(),
    'post_url'             => (new moodle_url('/local/sentientia_platform/admin/switchboard.php',
        ['customer' => $customer_id, 'tenant' => $tenant_id]))->out(false),
    'history_url'          => (new moodle_url('/local/sentientia_platform/admin/flags_history.php'))->out(false),
    'flag_count'           => count($all_flags),
];

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_sentientia_platform/switchboard', $data);
echo $OUTPUT->footer();
