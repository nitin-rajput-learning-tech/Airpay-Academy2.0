...
class statement_validator {
    ...
    private function validate_timestamp(string $ts): void {
        ...
        // createFromFormat() silently rolls over out-of-range fields (month 99, day 99),
        // so a parse "success" is not enough: reject any parse that raised warnings.
        $dt = false;
        foreach ([\DateTime::ATOM, 'Y-m-d\TH:i:s.uP', 'Y-m-d\TH:i:sP'] as $format) {
            $candidate = \DateTime::createFromFormat($format, $ts);
            $problems = \DateTime::getLastErrors();
            $clean = ($problems === false) || (empty($problems['warning_count']) && empty($problems['error_count']));
