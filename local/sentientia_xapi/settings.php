...
if ($hassiteconfig) {
    ...
    // H3 fix (UAT-SECURITY-POSTURE-2026-09-03) — per-client rate limit on
    // the /lrs/statements endpoint. Same defaults as local_sentientia_api's
    // rate_limiter (600 requests / 60s per client).
    $settings->add(new admin_setting_configtext(
        'local_sentientia_xapi/lrs_rate_limit',
        get_string('setting_lrs_rate_limit', 'local_sentientia_xapi'),
        get_string('setting_lrs_rate_limit_desc', 'local_sentientia_xapi'),
        '600',
        PARAM_INT
    ));

    $settings->add(new admin_setting_configtext(
        'local_sentientia_xapi/lrs_rate_window',
        get_string('setting_lrs_rate_window', 'local_sentientia_xapi'),
        get_string('setting_lrs_rate_window_desc', 'local_sentientia_xapi'),
        '60',
        PARAM_INT
    ));
