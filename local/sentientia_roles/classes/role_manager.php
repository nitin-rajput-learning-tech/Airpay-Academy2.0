...
/**
 * Role manager — the single point of truth for the sentientia_roles UI's
 * read + write paths.
 *
...
class role_manager {
    ...
    public static function permission_from_string(string $perm): int {
        return match (strtolower($perm)) {
            ...
            default => throw new \invalid_parameter_exception(
                get_string('err_invalid_permission', 'local_sentientia_roles')),
        };
    }
    ...
    public static function update_capability(int $roleid, string $capability,
        ...
        if ($role->shortname === 'manager' && $capability === 'moodle/site:config'
            ...
            throw new \moodle_exception('err_cannot_modify_admin', 'local_sentientia_roles');
        }

        $allcaps = get_all_capabilities();
        if (!isset($allcaps[$capability])) {
            throw new \moodle_exception('err_capability_not_found', 'local_sentientia_roles', '', $capability);
        }
        ...
        try {
            ...
            $entry->id = $DB->insert_record('local_sentientia_roles_auditlog', $entry);
            $tx->allow_commit();
        } catch (\Throwable $e) {
            $tx->rollback($e);
        }
    ...
    public static function assign_user_to_role(int $roleid, int $userid,
        ...
        try {
            ...
            $DB->insert_record('local_sentientia_roles_auditlog', (object) [
                'roleid'        => $roleid,
                'roleshortname' => $role->shortname,
