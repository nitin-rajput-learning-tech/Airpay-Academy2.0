...
/**
 * CRUD tests for sentientia_exams.
 *
...
final class crud_test extends \advanced_testcase {
    ...
    public function test_toggle_status_persists(): void {
        ...
        global $DB;
        $eid = $this->seed_exam();
        $original = (int) $DB->get_field('local_sentientia_exams', 'status', ['id' => $eid]);

        exam_manager::toggle_status($eid);
        $newval = (int) $DB->get_field('local_sentientia_exams', 'status', ['id' => $eid]);
    ...
    public function test_delete_removes_exam(): void {
        ...
        $this->assertFalse($DB->record_exists('local_sentientia_exams', ['id' => $eid]));
    }
