...
class exam_overdue extends \core\task\scheduled_task {
    ...
    public function get_name(): string {
        return get_string('task_exam_overdue', 'local_sentientia_exams');
    }

    public function execute(): void {
        global $DB;

        if (!(int) get_config('local_sentientia_exams', 'overdue_enabled')) {
            mtrace('local_sentientia_exams exam_overdue: disabled '
                . '(overdue_enabled = 0)');
            return;
        }

        $buckets = $this->parse_buckets((string) get_config(
            'local_sentientia_exams', 'overdue_days_after') ?: '1,7,14');
        if (empty($buckets)) {
            mtrace('local_sentientia_exams exam_overdue: no valid buckets');
            return;
        }
        sort($buckets);
        $max = max(1, min(5000, (int) get_config(
            'local_sentientia_exams', 'overdue_max_per_run') ?: 500));

        mtrace('local_sentientia_exams exam_overdue: starting. '
        ...
        set_config('overdue_last_run', $now, 'local_sentientia_exams');
        set_config('overdue_last_sent', $sent, 'local_sentientia_exams');

        mtrace('local_sentientia_exams exam_overdue: finished. '
    ...
    private function send_one_escalation(\stdClass $row, int $bucket,
        ...
        if (class_exists('\\local_sentientia_pwa\\notification_bridge')) {
            \local_sentientia_pwa\notification_bridge::also_push(
                ...
                get_string('overdue_push_title', 'local_sentientia_exams', $a),
                get_string('overdue_push_body',  'local_sentientia_exams', $a),
