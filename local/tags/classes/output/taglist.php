...
/**
 * Contains class local_tags\output\taglist
 *
 * @package   local_tags
...
class taglist implements templatable {
     ...
     * Constructor
     *
     * @param array $tags list of instances of \local_tags_tag or \stdClass
    ...
    public function __construct($tags, $label = null, $classes = '', $limit = 10, $pagecontext = null) {
        ...
        foreach ($tags as $idx => $tag) {
            ...
            $this->tags[$idx]->name = local_tags_tag::make_display_name($tag, false);

            if ($canmanagetags && !empty($tag->flag)) {
                $this->tags[$idx]->flag = 1;
            }

            $viewurl = local_tags_tag::make_url($tag->tagcollid, $tag->rawname, 0, $fromctx);
