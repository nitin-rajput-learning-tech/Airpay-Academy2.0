...
/**
 * Contains class local_tags_manage_table
 *
 * @package   local_tags
...
/**
 * Class local_tags_manage_table
 *
 * @package   local_tags
 * @copyright 2019 eabays
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class local_tags_manage_table extends table_sql {
    ...
    public function __construct($tagcollid) {
        ...
        $baseurl = new moodle_url('/local/tags/manage.php', array('tc' => $tagcollid,
        ...
        $tableheaders = array(get_string('select', 'local_tags'),
                              get_string('name', 'local_tags'),
                              get_string('owner', 'local_tags'),
                              get_string('count', 'local_tags'),
                              get_string('flag', 'tag'),
                              get_string('timemodified', 'local_tags'),
        ...
        $PAGE->requires->js_call_amd('local_tags/tag', 'initManagePage', array());
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if(is_siteadmin()){
            ...
            $where .= " AND  lt.open_costcenterid = :open_costcenterid";
            $joinparams['open_costcenterid'] = $USER->open_costcenterid;
        } elseif(has_capability('local/costcenter:manage_owndepartments',$context)){
            $where .= " AND  lt.open_departmentid = :open_departmentid";
            $joinparams['open_departmentid'] = $USER->open_departmentid;
        } else {
            $where .= " AND  lt.open_departmentid = :open_departmentid";
            $joinparams['open_departmentid'] = $USER->open_departmentid;
        }
        $this->sql->params = array_merge($this->sql->params, $joinparams);
        $this->countparams = array_merge($this->countparams, $joinparams);
    ...
    public function col_name($tag) {
        global $OUTPUT;
        $tagoutput = new local_tags\output\tagname($tag);
    ...
    public function col_flag($tag) {
        global $OUTPUT;
        $tagoutput = new local_tags\output\tagflag($tag);
        return $tagoutput->render($OUTPUT);
    }
    ...
    public function col_isstandard($tag) {
        global $OUTPUT;
        $tagoutput = new local_tags\output\tagisstandard($tag);
        return $tagoutput->render($OUTPUT);
    }
    ...
    public function col_controls($tag) {
        ...
        $o = '';
        // Edit.
        $url = new moodle_url('/local/tags/edit.php', array('id' => $tag->id, 'returnurl' => $this->baseurl));
