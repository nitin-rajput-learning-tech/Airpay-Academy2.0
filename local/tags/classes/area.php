...
/**
 * Class local_tags_area for managing tag areas
 *
 * @package   local_tags
...
class local_tags_area {
    ...
    public static function get_areas($tagcollid = null, $enabledonly = false) {
        ...
        if (($itemtypes = $cache->get('tag_area')) === false) {
            $colls = local_tags_collection::get_collections();
    ...
    public static function get_collection($component, $itemtype) {
        ...
        return local_tags_collection::get_default();
    }
    ...
    public static function get_showstandard($component, $itemtype) {
        ...
        return local_tags_tag::BOTH_STANDARD_AND_NOT;
    }
    ...
    protected static function delete($record) {
        ...
        local_tags_tag::delete_instances($record->component, $record->itemtype);
    ...
    protected static function create($record) {
        ...
        if (empty($record->tagcollid)) {
            $record->tagcollid = local_tags_collection::get_default();
        }
        $DB->insert_record('tag_area', array('component' => $record->component,
            ...
            'showstandard' => isset($record->showstandard) ? $record->showstandard : local_tags_tag::BOTH_STANDARD_AND_NOT,
            ...
            foreach ($tagareas as $tagarea) {
                ...
                if (!empty($record->collection)) {
                    ...
                    if (!array_key_exists($key, $existingcolls)) {
                        $existingcolls[$key] = local_tags_collection::create($collectiondata);
                    } else {
                        local_tags_collection::update($existingcolls[$key], $collectiondata);
                    }
            ...
            if (!isset($itemtypes[$key]->tagcollid)) {
                foreach ($todeletecoll as $tagcoll) {
                    if ($tagcoll->id == $tagarea->tagcollid) {
                        $itemtypes[$key]->tagcollid = local_tags_collection::get_default();
                    }
                }
            }
    ...
    public static function uninstall($pluginname) {
        ...
        local_tags_tag::delete_instances($component);
    ...
    public static function move_tags($component, $itemtype, $tagcollid) {
        ...
        if ($relatedtags) {
            $tags = local_tags_tag::get_by_name_bulk($tagcollid, array_keys($relatedtags));
            foreach ($tags as $tag) {
        ...
        if ($cleanupcollections) {
            local_tags_collection::cleanup_unused_tags($cleanupcollections);
        }
