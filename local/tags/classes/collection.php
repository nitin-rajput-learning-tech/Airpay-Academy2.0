...
class local_tags_collection {
    ...
    public static function display_name($record) {
        ...
        if (!empty($record->name)) {
            ...
            return get_string('managetags', 'local_tags');
        } else {
            return $record->id;
        }
    }
    ...
    public static function get_areas($tagcollid) {
        $allitemtypes = local_tags_area::get_areas($tagcollid, true);
        $itemtypes = array();
    ...
    public static function get_areas_names($tagcollid, $enabledonly = true) {
        $allitemtypes = local_tags_area::get_areas($tagcollid, $enabledonly);
        $itemtypes = array();
        foreach ($allitemtypes as $itemtype => $it) {
            foreach ($it as $component => $v) {
                $itemtypes[$v->id] = local_tags_area::display_name($component, $itemtype);
            }
        }
        return $itemtypes;
    }
     ...
     *
     * @param stdClass $data data from form local_tags_collection_form
    ...
    public static function delete($tagcoll) {
        ...
        $allitemtypes = local_tags_area::get_areas($tagcoll->id);
        foreach ($allitemtypes as $it) {
            foreach ($it as $v) {
                local_tags_area::update($v, array('tagcollid' => $defaulttagcollid));
            }
        }

        // Delete tags from this tag_coll.
        local_tags_tag::delete_tags($DB->get_fieldset_select('tag', 'id', 'tagcollid = ?', array($tagcoll->id)));
    ...
    public static function cleanup_unused_tags($collections = null) {
        ...
        if ($unusedtags = $DB->get_fieldset_sql($sql, $params)) {
            local_tags_tag::delete_tags($unusedtags);
        }
    }
     ...
     * @return \local_tags\output\tagcloud
     */
    public static function get_tag_cloud($tagcollid, $isstandard = false, $limit = 150, $sort = 'name',
        ...
        $fromclause .= ' JOIN {local_tags} lt ON lt.tagid = tg.id ';
        if(is_siteadmin()){
            $whereclause .= " AND  1 = 1";
        } elseif(has_capability('local/costcenter:manage_ownorganization',$context)){
            $whereclause .= " AND  lt.open_costcenterid = ?";
            $params[] = $USER->open_costcenterid;
        } elseif(has_capability('local/costcenter:manage_owndepartments',$context)){
            $whereclause .= " AND  lt.open_departmentid = ?";
            $params[] = $USER->open_departmentid;
        } else {
            $whereclause .= " AND  lt.open_costcenterid = ? AND  (lt.open_departmentid = ? OR lt.open_departmentid = 0 OR lt.open_departmentid is null)";
            $params[] = $USER->open_costcenterid;
            $params[] = $USER->open_departmentid;
        }
        ...
        return new local_tags\output\tagcloud($tagsincloud, $tagscount, $fromctx, $ctx, $rec);
    }
