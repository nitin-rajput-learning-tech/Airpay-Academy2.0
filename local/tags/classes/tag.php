 ...
 *        local_tags_tag::set_item_tags('post', 'core', $blogpost->id, $context, $arrayoftags);
 *
 *  - to "remove all the tags on a blog post":
 *        local_tags_tag::remove_all_item_tags('post', 'core', $blogpost->id);
 *
...
class local_tags_tag {
     ...
     *
     * @param stdClass|local_tags_tag $tag record from db table tag, must contain properties name and rawname
     * @param bool $ashtml (default true) if true will return htmlspecialchars encoded string
     * @return string
     */
     ...
     * @return   local_tags_tag|false  tag object
     */
     ...
     *                        Default value is 'id,name,rawname,tagcollid',
     *                        specify '*' to include all fields.
     * @return   local_tags_tag[] array of retrieved tags
     */
     ...
     * @return array array of local_tags_tag instances
     */
    public static function guess_by_name($name, $returnfields='id, name, rawname, tagcollid') {
        ...
        $tagcolls = local_tags_collection::get_collections();
     ...
     * @return local_tags_tag[] array of tag objects indexed with lowercase normalised tag name
     */
    ...
    public static function make_url($tagcollid, $name, $exclusivemode = 0, $fromctx = 0, $ctx = 0, $rec = 1) {
        $coll = local_tags_collection::get_by_id($tagcollid);
        if (!empty($coll->customurl)) {
            $url = '/' . ltrim(trim($coll->customurl), '/');
        } else {
            $url = '/local/tags/index.php';
        }
    ...
    protected function ensure_fields_exist($list, $caller) {
        ...
        if ($missing) {
            debugging('local_tags_tag::' . $caller . '() must be called on fully retrieved tag object. Missing fields: '.
    ...
    /**
     * Get the array of local_tags_tag objects associated with a list of items.
     *
     * Use {@link local_tags_tag::get_item_tags_array()} if you wish to get the same data as simple array.
     *
     * @param string $component component responsible for tagging. For BC it can be empty but in this case the
     *               query will be slow because DB index will not be used.
     ...
     * @return local_tags_tag[] each object contains additional fields taginstanceid, taginstancecontextid and ordering
     */
    public static function get_items_tags($component, $itemtype, $itemids, $standardonly = self::BOTH_STANDARD_AND_NOT,
        ...
        // Note: if the fields in this query are changed, you need to do the same changes in local_tags_tag::get_correlated_tags().
    ...
    /**
     * Get the array of local_tags_tag objects associated with an item (instances).
     *
     * Use {@link local_tags_tag::get_item_tags_array()} if you wish to get the same data as simple array.
     *
     * @param string $component component responsible for tagging. For BC it can be empty but in this case the
     ...
     * @param int $tiuserid tag instance user id, only needed for tag areas with user tagging (such as core/course)
     */
    public static function set_item_tags($component, $itemtype, $itemid, context $context, $tagnames, $tiuserid = 0, $costcenterid = 0, $departmentid = 0) {
        global $DB;
        if ($itemtype === 'tag') {
            if ($tiuserid) {
        ...
        if ($tagnames) {
            $tagcollid = local_tags_area::get_collection($component, $itemtype);
            $tagobjects = static::create_if_missing($tagcollid, $tagnames);
        } else {
            $tagobjects = array();
        }

        $allowmultiplecontexts = local_tags_area::allows_tagging_in_multiple_contexts($component, $itemtype);
        ...
        foreach ($tagobjects as $name => $tag) {
            ...
            foreach ($currenttags as $currenttag) {
                ...
                if ($costcenterid != 0 OR $departmentid != 0) {
                    $localtag = $DB->get_record('local_tags', ['tagid'=>$currenttag->id,
                        'taginstanceid'=>$currenttag->taginstanceid]);
                    if (!empty($localtag)) {
                        $localtag->open_costcenterid = $costcenterid;
                        $localtag->open_departmentid = $departmentid;
                        $localtag->timemodified = time();
                        $DB->update_record('local_tags', $localtag);
                    } else {
                        $localtag = new stdclass();
                        $localtag->open_costcenterid = $costcenterid;
                        $localtag->open_departmentid = $departmentid;
                        $localtag->tagid = $currenttag->id;
                        $localtag->taginstanceid = $currenttag->taginstanceid;
                        $localtag->timemodified = time();
                        $DB->insert_record('local_tags', $localtag);
                    }
                }
            ...
            $taginstanceid = $tag->add_instance($component, $itemtype, $itemid, $context, $ordering, $tiuserid);
            if ($costcenterid != 0 OR $departmentid != 0) {
                $taginstance = $DB->get_record('tag_instance', ['id'=>$taginstanceid]);
                $localtag = new stdclass();
                $localtag->open_costcenterid = $costcenterid;
                $localtag->open_departmentid = $departmentid;
                $localtag->tagid = $taginstance->tagid;
                $localtag->taginstanceid = $taginstanceid;
                $localtag->timemodified = time();
                $DB->insert_record('local_tags', $localtag);
            }
        }
        ...
        if (array_key_exists('rawname', $data)) {
            ...
            if (!$name || $data['rawname'] === $this->rawname) {
                ...
                if ($existing->id != $this->id) {
                    throw new moodle_exception('namesalreadybeeingused', 'local_tags');
                }
            }
    ...
    public function set_related_tags($tagnames) {
        ...
        foreach ($currenttags as $currenttag) {
            if (!array_key_exists($currenttag->name, $tagobjects)) {
                ...
                $currenttag->delete_instance_as_record($taginstance, false);
                $this->delete_instance('local_tags', 'tag', $currenttag->id);
            }
        }
        ...
        foreach ($tagobjects as $name => $tag) {
            ...
            $this->add_instance('local_tags', 'tag', $tag->id, $context, 0);
            $tag->add_instance('local_tags', 'tag', $this->id, $context, 0);
            $currenttags[] = $tag;
        }
    }
     ...
     * @return local_tags_tag[] an array of tag objects
     */
    public function get_correlated_tags($keepduplicates = false) {
        ...
        // This is (and has to) return the same fields as the query in local_tags_tag::get_item_tags().
        $sql = "SELECT ti.id AS taginstanceid, tg.id, tg.isstandard, tg.name, tg.rawname, tg.flag,
                tg.tagcollid, ti.ordering, ti.contextid AS taginstancecontextid, ti.itemid
              FROM {tag} tg
     ...
     *
     * @return local_tags_tag[]
     */
    public function get_manual_related_tags() {
        return self::get_item_tags('local_tags', 'tag', $this->id);
    }
     ...
     *
     * Note that this is a static method and not a method of local_tags object because the tag might not exist yet,
    ...
    public static function is_enabled($component, $itemtype) {
        return local_tags_area::is_enabled($component, $itemtype);
    }
     ...
     */
    public function get_tag_index($tagarea, $exclusivemode, $fromctx, $ctx, $rec, $page = 0, $sort = '') {
        global $CFG;
        if (!empty($tagarea->callback)) {
            ...
            if ($callback == 'course_get_tagged_courses')
                return null;
            return call_user_func_array($callback, [$this, $exclusivemode, $fromctx, $ctx, $rec, $page,$sort]);
        }
        return null;
    }
    ...
    public function get_links() {
        ...
        if (static::is_enabled('core', 'user') && local_tags_area::get_collection('core', 'user') == $this->tagcollid) {
            if (static::is_item_tagged_with('core', 'user', $USER->id, $this->name)) {
                $url = new moodle_url('/local/tags/user.php', array('action' => 'removeinterest',
                ...
                $url = new moodle_url('/local/tags/user.php', array('action' => 'addinterest',
                    'sesskey' => sesskey(), 'tag' => $this->rawname));
        ...
        if (has_capability('moodle/tag:flag', $systemcontext)) {
            $url = new moodle_url('/local/tags/user.php', array('action' => 'flaginappropriate',
        ...
        if (has_capability('moodle/tag:edit', $systemcontext) ||
            ...
            $url = new moodle_url('/local/tags/edit.php', array('id' => $this->id));
    ...
    public static function delete_tags($tagids) {
        ...
        $select = 'WHERE tagid ' . $tagsql;
        $sql = "DELETE FROM {local_tags} $select";
        $DB->execute($sql, $tagparams);
     ...
     *
     * @param local_tags_tag[] $tags tags to combine into this one
     */
        ...
        foreach ($tagstocombine as $tag) {
            ...
            foreach ($records as $record) {
                if ($record->alreadyhasmaintag) {
                    ...
                    // remove instance in local mapped tags
                    $DB->delete_records('local_tags', array('taginstanceid'=>$record->id, 'tagid'=>$record->tagid));
                    $sql = "UPDATE {tag_instance} SET ordering = ordering - 1
                            WHERE itemtype = :itemtype
            ...
            // update tags in local_tags before default deletion 
            $localmappedtags = $DB->get_records('local_tags', ['tagid'=>$tag->id]);
            foreach ($localmappedtags as $localmappedtag) {
                $DB->update_record('local_tags', array('id' => $localmappedtag->id, 'tagid' => $maintag));
            }
        }
    ...
    public static function get_tags_by_area_in_contexts($component, $itemtype, array $contexts) {
        ...
        return array_map(function($record) {
            return new local_tags_tag($record);
