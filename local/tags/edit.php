...
if ($tagname) {
    ...
    if (!$tagcollid) {
        ...
        if (count($tags) > 1) {
            ...
            redirect(new moodle_url('/local/tags/index.php', array('tag' => $tagname)));
        } else if (count($tags) == 1) {
            $tag = reset($tags);
        }
    } else {
        if (!$tag = local_tags_tag::get_by_name($tagcollid, $tagname, '*')) {
            redirect(new moodle_url('/local/tags/index.php', array('tagcollid' => $tagcollid)));
        }
    }
} else if ($tagid) {
...
if (empty($tag)) {
    redirect(new moodle_url('/local/tags/index.php'));
}
...
$data = $tag->to_object();
$data->relatedtags = local_tags_tag::get_item_tags_array('local_tags', 'tag', $tag->id);
...
$tagform = new local_tags_edit_form(null, array('editoroptions' => $editoroptions, 'tag' => $tag));
...
navigation_node::override_active_url(new moodle_url('/local/tags/index.php'));
