...
//geography form fields
function local_costcenter_get_geography_fields_form($mform, $ajaxformdata, $customdata, $pluginname, $context, $multiple = false){
    global $DB, $USER;
    $depth = $USER->access['currentroleinfo']['depth'];
    if(is_siteadmin()){
        $depth = 0;
    ...
    $total_fields = 5;
    $fields = local_costcenter_get_geography_fields();
    $prev_element = '';
...
function local_costcenter_get_geography_data(&$data){
    $fields = local_costcenter_get_fields();
    $path = '';
    foreach($fields AS $field){
        if(isset($data->$field) && $data->$field > 0){
...
function local_costcenter_set_geography_data(&$data){
    $fields = local_costcenter_get_fields();
    if(isset($data['open_costcenterpath'])){
        $recordedpathids = explode('/', $data['open_costcenterpath']);
        foreach($fields as $levelid => $field){
...
function local_costcenter_get_geography_fields(){
