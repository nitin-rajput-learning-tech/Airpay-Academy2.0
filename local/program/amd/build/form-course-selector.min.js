 ...
 */
define("local_program/form-course-selector",["jquery","core/ajax","core/templates"],(function($,Ajax,Templates){return{processResults:function(selector,results){var courses=[];return $.each(results,(function(index,course){courses.push({value:course.id,label:course._label})})),courses},transport:function(selector,query,success,failure){contextid=parseInt($(selector).data("contextid"),10),includes=$(selector).data("includes"),Ajax.call([{methodname:"local_program_form_course_selector",args:{query:query,context:{contextid:contextid},includes:includes}}])[0].then((function(results){var promises=[],i=0;return $.each(results.courses,(function(index,course){promises.push(Templates.render("local_program/form-course-selector-suggestion",course))})),$.when.apply($.when,promises).then((function(){var args=arguments;$.each(results.courses,(function(index,course){course._label=args[i],i++})),success(results.courses)}))})).catch(failure)}}}));
