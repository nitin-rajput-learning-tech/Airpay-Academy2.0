...
$PAGE->set_heading(get_string('view_standard_log', 'local_biz_cart'));
$url = new moodle_url('/local/biz_cart/standard_report.php', []);
$PAGE->set_url($url);
$PAGE->navbar->add(get_string('manage_courses','local_courses'), new moodle_url("/local/courses/courses.php"));
$PAGE->navbar->add(get_string('view_standard_log','local_biz_cart'));
echo $OUTPUT->header();
...
$filterparams = $renderer->view_course_standard_log(true,$formattype);
...
$renderer = $PAGE->get_renderer('local_biz_cart');
echo $renderer->view_course_standard_log(false, $formattype);
echo $OUTPUT->footer();