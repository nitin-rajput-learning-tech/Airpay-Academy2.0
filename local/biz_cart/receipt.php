...
$filename = get_config('local_biz_cart', 'receiptimage');
$cfghtml = get_config('local_biz_cart', 'receipthtml');
$context = \context_system::instance();
$fs = get_file_storage();
$files = $fs->get_area_files($context->id, 'local_biz_cart', 'local_biz_cart_receiptimage');
...
$items = local_biz_cart\biz_cart_history::return_data_from_ledger_via_identifier($id);
...
foreach ($items as $item) {
    ...
    if ($item->area == "option" && class_exists('mod_booking\singleton_service')) {
        ...
        $optionsettings = \local_courses\singleton_service::get_instance_of_booking_option_settings($optionid);
