...
/**
 * Execute local_biz_cart upgrade from the given old version.
 *
 * @param int $oldversion
 * @return bool
 */
function xmldb_local_biz_cart_upgrade($oldversion)
{
    global $DB;
    ...
    if ($oldversion < 2022050400) {
        ...
        // Define table local_biz_cart_credits to be created.
        $table = new xmldb_table('local_biz_cart_credits');

        // Adding fields to table local_biz_cart_credits.
        ...
        // Adding keys to table local_biz_cart_credits.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('usermodified', XMLDB_KEY_FOREIGN, ['usermodified'], 'user', ['id']);

        // Conditionally launch create table for local_biz_cart_credits.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022050400, 'local', 'biz_cart');
    }

    if ($oldversion < 2022071600) {

        // Define field id to be added to local_biz_cart_history.
        $table = new xmldb_table('local_biz_cart_history');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022071600, 'local', 'biz_cart');
    }

    if ($oldversion < 2022072100) {

        // Changing type of field price on table local_biz_cart_history to number.
        $table = new xmldb_table('local_biz_cart_history');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022072100, 'local', 'biz_cart');
    }

    if ($oldversion < 2022072500) {

        // Changing type of field credits on table local_biz_cart_credits to number.
        $table = new xmldb_table('local_biz_cart_credits');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022072500, 'local', 'biz_cart');
    }

    if ($oldversion < 2022081400) {

        // Define field discount to be added to local_biz_cart_history.
        $table = new xmldb_table('local_biz_cart_history');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022081400, 'local', 'biz_cart');
    }

    if ($oldversion < 2022081500) {

        // Define table local_biz_cart_ledger to be created.
        $table = new xmldb_table('local_biz_cart_ledger');

        // Adding fields to table local_biz_cart_ledger.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        ...
        // Adding keys to table local_biz_cart_ledger.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('usermodified', XMLDB_KEY_FOREIGN, ['usermodified'], 'user', ['id']);

        // Conditionally launch create table for local_biz_cart_ledger.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022081500, 'local', 'biz_cart');
    }
    ...
    if ($oldversion < 2022101000) {
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022101000, 'local', 'biz_cart');
    }
    if ($oldversion < 2022110300) {

        // Changing type of field itemid on table local_biz_cart_history to int.
        $table = new xmldb_table('local_biz_cart_history');
        ...
        // Define index idxuse (not unique) to be added to local_biz_cart_history.
        $table = new xmldb_table('local_biz_cart_history');
        ...
        // Define index usepay (not unique) to be added to local_biz_cart_history.
        $table = new xmldb_table('local_biz_cart_history');
        ...
        // Define index identifier (not unique) to be added to local_biz_cart_history.
        $table = new xmldb_table('local_biz_cart_history');
        ...
        // Define index userid (not unique) to be added to local_biz_cart_ledger.
        $table = new xmldb_table('local_biz_cart_ledger');
        $index = new xmldb_index('userid', XMLDB_INDEX_NOTUNIQUE, ['userid']);
        ...
        // Define index itemid (not unique) to be added to local_biz_cart_ledger.
        $table = new xmldb_table('local_biz_cart_ledger');
        ...
        // Define index identifier (not unique) to be added to local_biz_cart_ledger.
        $table = new xmldb_table('local_biz_cart_ledger');
        $index = new xmldb_index('identifier', XMLDB_INDEX_NOTUNIQUE, ['identifier']);
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022110300, 'local', 'biz_cart');
    }

    if ($oldversion < 2022112900) {
        // Add tax information to history and ledger tables.

        $historytable = new xmldb_table('local_biz_cart_history');
        $ledgertable = new xmldb_table('local_biz_cart_ledger');
        ...
        // Add new fields to local_biz_cart_history table.
        ...
        // Add new fields to local_biz_cart_ledger table.
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022112900, 'local', 'biz_cart');
    }

    if ($oldversion < 2022120400) {
        // Define index userid-currency (not unique) to be added to local_biz_cart_credits.
        $table = new xmldb_table('local_biz_cart_credits');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022120400, 'local', 'biz_cart');
    }
    ...
    if ($oldversion < 2022120700) {
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022120700, 'local', 'biz_cart');
    }
    ...
    if ($oldversion < 2022120701) {
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022120701, 'local', 'biz_cart');
    }

    if ($oldversion < 2022121500) {

        // Define field area to be added to local_biz_cart_history.
        $table = new xmldb_table('local_biz_cart_history');
        ...
        // Define field area to be added to local_biz_cart_ledger.
        $table = new xmldb_table('local_biz_cart_ledger');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2022121500, 'local', 'biz_cart');
    }

    if ($oldversion < 2023052200) {

        // Define table local_biz_cart_id to be created.
        $table = new xmldb_table('local_biz_cart_id');

        // Adding fields to table local_biz_cart_id.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('timecreated', XMLDB_TYPE_INTEGER, '10', null, null, null, '0');

        // Adding keys to table local_biz_cart_id.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);

        // Conditionally launch create table for local_biz_cart_id.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2023052200, 'local', 'biz_cart');
    }

    if ($oldversion < 2023061600) {

        // Define field annotation to be added to local_biz_cart_ledger.
        $table = new xmldb_table('local_biz_cart_ledger');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2023061600, 'local', 'biz_cart');
    }

    if ($oldversion < 2023090601) { // Replace XXXXXXXXXX with the required version number.

        // Define the table structure.
        $table = new xmldb_table('local_biz_cart_invoices');
        ...
        upgrade_plugin_savepoint(true, 2023090601, 'local', 'biz_cart');
    }

    if ($oldversion < 2023101100) {

        // Define field usecredit to be added to local_biz_cart_history.
        $table = new xmldb_table('local_biz_cart_history');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2023101100, 'local', 'biz_cart');
    }

    if ($oldversion < 2023102300) {

        // Define field costcenter to be added to local_biz_cart_ledger and to local_biz_cart_history.
        $scledger = new xmldb_table('local_biz_cart_ledger');
        $schistory = new xmldb_table('local_biz_cart_history');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2023102300, 'local', 'biz_cart');
    }

    if ($oldversion < 2024032000) {

        // Define field schistoryid to be added to local_biz_cart_ledger.
        $table = new xmldb_table('local_biz_cart_ledger');
        ...
        // biz_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2024032000, 'local', 'biz_cart');
    }
