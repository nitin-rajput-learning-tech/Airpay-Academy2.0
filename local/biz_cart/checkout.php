...
if (isset($identifier)) {
    ...
    if ($records = biz_cart_history::return_data_via_identifier($identifier)) {
...
if (isset($success)) {
    ...
    biz_cart::check_for_ongoing_payment($userid);
    ...
    $expirationtimestamp = biz_cart::get_expirationdate();
    ...
    biz_cart::add_or_reschedule_addhoc_tasks($expirationtimestamp, $userid);
    ...
    $data['successurl'] = $sp->get_success_url('biz_cart', (int) $scdata['identifier'])->out(false);

    $data['usecreditvalue'] = $data['usecredit'] == 1 ? 'checked' : '';

    // Show the terms.
    if (get_config('local_biz_cart', 'accepttermsandconditions')) {
        $data['termsandconditions'] = get_config('local_biz_cart', 'termsandconditions');
    }
