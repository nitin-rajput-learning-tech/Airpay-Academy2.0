...
namespace local_biz_cart\invoice;
...
use local_biz_cart\interfaces\invoice;
use local_biz_cart\task\create_invoice_task;
...
class erpnext_invoice implements invoice
{
    ...
    public function __construct()
    {
        ...
        $this->baseurl = get_config('local_biz_cart', 'baseurl');
        $this->token = get_config('local_biz_cart', 'token');
        $this->headers = [
    ...
    public function create_invoice(int $identifier): bool
    {
        ...
        $this->invoiceitems = biz_cart_history::return_data_via_identifier($identifier);
        // Set user.
        ...
        if (!$response) {
            throw new moodle_exception('generalexceptionmessage', 'local_biz_cart', '',
        ...
        if ($success) {
            ...
            $DB->insert_record('local_biz_cart_invoices', $invoice);
            return true;
        } else {
            return false;
        }
    ...
    public function customer_exists(): bool
    {
        ...
        if (!$response) {
            throw new moodle_exception('serverconnection', 'local_biz_cart', '',
    ...
    public function create_customer(): bool
    {
        ...
        $countrycode = get_config('local_biz_cart', 'defaultcountry');
