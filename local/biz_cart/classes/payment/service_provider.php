...
/**
 * Payment subsystem callback implementation for local_biz_cart.
 *
...
namespace local_biz_cart\payment;
...

{
    ...
    public static function get_payable(string $paymentarea, int $cartidentifier): \core_payment\local\entities\payable
    {
        ...
        if (!$records = $DB->get_records('local_biz_cart_history', ['identifier' => $cartidentifier])) {
            throw new moodle_exception('identifierisnotindb', 'local_biz_cart');
        }
        ...
        if ($cachedeleted) {
            ...
            if (!empty($price) && !empty($usecredit)) {
                ...
                list($credit, $currency) = biz_cart_credits::get_balance($userid);
            ...
            $price = biz_cart_credits::get_price_from_shistorycart($shoppingcart);
            $currency = $shoppingcart->currency;

        }

        if (!$accountid = get_config('local_biz_cart', 'accountid')) {
            $accountid = 1;
        }
    ...
    public static function get_success_url(string $paymentarea, int $identifier): moodle_url
    {
        ...
        return new \moodle_url('/local/biz_cart/checkout.php', ['success' => 1, 'identifier' => $identifier]);
    }
    ...
    public static function deliver_order(string $paymentarea, int $identifier, int $paymentid, int $userid): bool
    {
        ...
        $data['items'] = biz_cart_history::return_data_via_identifier($identifier);

        if (count($data['items']) == 0) {
            return false;
        }

        biz_cart::confirm_payment($userid, LOCAL_BIZCART_PAYMENT_METHOD_ONLINE, $data);

        $success = biz_cart_history::set_success_in_db($data['items']);
        return $success;
    }
}
