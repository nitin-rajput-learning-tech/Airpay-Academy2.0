...
use local_biz_cart\event\item_added;
use local_biz_cart\event\item_bought;
use local_biz_cart\event\item_canceled;
use local_biz_cart\event\item_deleted;
use local_biz_cart\event\item_notbought;
use local_biz_cart\event\payment_rebooked;
use local_biz_cart\interfaces\interface_transaction_complete;
use local_biz_cart\payment\service_provider;
use local_biz_cart\task\delete_item_task;
...
class biz_cart
{
     ...
     *
     * Add Item to cart.
     * - First we check if we are below maxitems from the biz_cart isde.
     * - Then we check if the item is already in the cart and can be add it again the biz_cart side.
    ...
    public static function allow_add_item_to_cart(string $component, string $area, int $itemid, int $userid): array
    {
        ...
        $maxitems = get_config('local_biz_cart', 'maxitems');
        ...
        if (isset($maxitems) && isset($cachedrawdata['items']) && (count($cachedrawdata['items']) >= $maxitems)) {
            return [
                'success' => LOCAL_BIZCART_CARTPARAM_CARTISFULL,
                'itemname' => '',
            ];
        }
        ...
        if (isset($cachedrawdata['items'][$cacheitemkey])) {
            return [
                'success' => LOCAL_BIZCART_CARTPARAM_ALREADYINCART,
                'itemname' => '',
            ];
        }
        //  print_r($area);exit;
        if ($area == "option") {
            ...
            $cartitem['allow'] = true;
            //  if (get_config('local_biz_cart', 'samecostcenter')) {
                ...
                foreach ($cachedrawdata['items'] as $itemincart) {
                    if ($itemincart['area'] = 'bookingfee' || $itemincart['area'] = 'rebookingcredit') {
                        ...
                        if ($currentcostcenter != $costcenterincart) {
                            return [
                                'success' => LOCAL_BIZCART_CARTPARAM_COSTCENTER,
    ...
    public static function add_item_to_cart(string $component, string $area, int $itemid, int $userid): array
    {
        ...
        if ($cartparam == LOCAL_BIZCART_CARTPARAM_SUCCESS) {
            // If we have nothing in our cart and we are not about...
            // ... to add the booking fee...
            // ... we add the booking fee.
            if ((empty($cachedrawdata['items'])
                ...
                biz_cart_bookingfee::add_fee_to_cart($buyforuser ? -1 : $userid, $buyforuser ? $userid : 0);
                $cachedrawdata = $cache->get($cachekey);
            }
        }
        $expirationtimestamp = self::get_expirationdate();
        switch ($cartparam) {
            case LOCAL_BIZCART_CARTPARAM_SUCCESS:
                ...
                if (isset($cartitemarray['cartitem'])) {
                    ...
                    if (($itemdata['price'] == 0)
                        ...
                        $regexkey = '/^local_biz_cart-bookingfee-/';
                    ...
                    biz_cart_rebookingcredit::add_rebookingcredit($cachedrawdata, $area, $buyforuser ? -1 : $userid);

                    $itemdata['expirationdate'] = $expirationtimestamp;
                    $itemdata['success'] = LOCAL_BIZCART_CARTPARAM_SUCCESS;
                    ...
                    $itemdata = [];
                    $itemdata['success'] = LOCAL_BIZCART_CARTPARAM_ERROR;
                    $itemdata['expirationdate'] = 0;
                    $itemdata['price'] = 0;
                    $itemdata['buyforuser'] = $USER->id == $userid ? 0 : $userid;
                }
                break;
            case LOCAL_BIZCART_CARTPARAM_COSTCENTER:
                $itemdata = [];
                $itemdata['success'] = LOCAL_BIZCART_CARTPARAM_COSTCENTER;
                // Important. In JS we show the modal based on success 2.
                $itemdata['expirationdate'] = 0;
                $itemdata['buyforuser'] = $USER->id == $userid ? 0 : $userid;
                $itemdata['price'] = 0;
                break;
            case LOCAL_BIZCART_CARTPARAM_ALREADYINCART:
                ...
                $itemdata = [];
                $itemdata['success'] = LOCAL_BIZCART_CARTPARAM_ALREADYINCART;
                $itemdata['buyforuser'] = $USER->id == $userid ? 0 : $userid;
                $itemdata['expirationdate'] = $expirationtimestamp;
                $itemdata['price'] = 0;
                break;
            case LOCAL_BIZCART_CARTPARAM_CARTISFULL:
                $itemdata['success'] = LOCAL_BIZCART_CARTPARAM_CARTISFULL;
                $itemdata['buyforuser'] = $USER->id == $userid ? 0 : $userid;
                $itemdata['expirationdate'] = $expirationtimestamp;
                $itemdata['price'] = 0;
                break;
            case LOCAL_BIZCART_CARTPARAM_FULLYBOOKED:
                $itemdata['success'] = LOCAL_BIZCART_CARTPARAM_FULLYBOOKED;
                $itemdata['buyforuser'] = $USER->id == $userid ? 0 : $userid;
                $itemdata['expirationdate'] = $expirationtimestamp;
                $itemdata['price'] = 0;
                break;
            case LOCAL_BIZCART_CARTPARAM_ALREADYBOOKED:
                $itemdata['success'] = LOCAL_BIZCART_CARTPARAM_ALREADYBOOKED;
                $itemdata['buyforuser'] = $USER->id == $userid ? 0 : $userid;
                $itemdata['expirationdate'] = $expirationtimestamp;
                $itemdata['price'] = 0;
                break;
            case LOCAL_BIZCART_CARTPARAM_ERROR:
            default:
                $itemdata['success'] = LOCAL_BIZCART_CARTPARAM_ERROR;
                $itemdata['buyforuser'] = $USER->id == $userid ? 0 : $userid;
                $itemdata['expirationdate'] = $expirationtimestamp;
    ...
    {
        return time() + get_config('local_biz_cart', 'expirationtime') * 60;
    }
        ...
        if (!empty($cachedrawdata['items'])) {
            ...
            foreach ($cachedrawdata['items'] as $remainingitem) {
                if ($remainingitem['area'] === 'bookingfee' ||
                    ...
                    biz_cart_rebookingcredit::add_rebookingcredit($cachedrawdata, $area, $userid);
                }
            }
            ...
            if ($letsdelete) {
                foreach ($cachedrawdata['items'] as $item) {
                    if (($item['area'] == 'bookingfee' ||
                        ...
                        && $item['componentname'] == 'local_biz_cart') {
    ...
    {
        $providerclass = "$component\\biz_cart\\service_provider";
    ...
    public static function load_cartitem(string $component, string $area, int $itemid, int $userid): array
    {
        ...
        $cache = \cache::make('local_biz_cart', 'schistory');
        $result = $cache->get('schistorycache');

        if (!empty($result)) {
            $result = $result;
            $identifier = (int) $result['identifier'];
            $history = new biz_cart_history();
    ...
    public static function successful_checkout(string $component, string $area, int $itemid, int $userid): bool
    {
        ...
        return component_class_callback($providerclass, 'successful_checkout',
            [$area, $itemid, LOCAL_BIZCART_PAYMENT_METHOD_CASHIER, $userid]);
    }
    ...
    /**
     * Function local_biz_cart_get_cache_data
     ...
     * @return array
     */
    public static function local_biz_cart_get_cache_data(int $userid, bool $usecredit = null): array
    {
        global $USER, $CFG;
        ...
        $usecredit = biz_cart_credits::use_credit_fallback($usecredit, $userid);
        $taxesenabled = get_config('local_biz_cart', 'enabletax') == 1;
        if ($taxesenabled) {
            $taxcategories = taxcategories::from_raw_string(
                get_config('local_biz_cart', 'defaulttaxcategory'),
                get_config('local_biz_cart', 'taxcategories')
            );
        } else {
            $taxcategories = null;
        }
        ...
        $data['maxitems'] = get_config('local_biz_cart', 'maxitems');
        $data['items'] = [];
        $data['price'] = 0.00;
        ...
        $data['nowdate'] = time();
        $data['checkouturl'] = $CFG->wwwroot . "/local/biz_cart/checkout.php";

        if (!$cachedrawdata) {
            list($data['credit'], $data['currency']) = biz_cart_credits::get_balance($userid);
            $data['items'] = [];
        ...
        if ($cachedrawdata && count($data['items']) > 0) {
            ...
            biz_cart_credits::prepare_checkout($data, $userid, $usecredit);
    ...
    {
        $cache = \cache::make('local_biz_cart', 'cashier');
    ...
    public static function confirm_payment(int $userid, int $paymenttype, array $datafromhistory = null,
        ...
        if (!$data = $datafromhistory) {
            ...
            $data = self::local_biz_cart_get_cache_data($userid);
            ...
            if ($userid == $USER->id) {
                ...
                } else if (!has_capability('local/biz_cart:cashier', $context)) {
                    // The cashier could call this to pay for herself, therefore only for non cashiers, we return here.
                    return [
                        'status' => 0,
                        'error' => get_string('notenoughcredit', 'local_biz_cart'),
                ...
                if (!has_capability('local/biz_cart:cashier', $context)) {
                    return [
                        'status' => 0,
                        'error' => get_string('nopermission', 'local_biz_cart'),
                        'credit' => 0.0,
                        'identifier' => $identifier,
                    ];
                }
            ...
            $identifier = biz_cart_history::create_unique_cart_identifier($userid);
        ...
        if (empty($data['items'])) {
            return [
                'status' => 0,
                'error' => get_string('noitemsincart', 'local_biz_cart'),
                'credit' => 0.0,
                'identifier' => $identifier,
            ];
        }
        ...
        biz_cart_rebookingcredit::correct_item_price_for_rebooking($data);
        ...
        if (isset($data["usecredit"]) && $data["usecredit"] && isset($data["credit"]) && $data["credit"] > 0) {
            ...
            $ledgerrecord->componentname = 'local_biz_cart';
            $ledgerrecord->identifier = $identifier;
            $ledgerrecord->payment = $paymenttype;
            $ledgerrecord->paymentstatus = LOCAL_BIZCART_PAYMENT_SUCCESS;
            $ledgerrecord->usermodified = $USER->id;
            $ledgerrecord->timemodified = time();
            $ledgerrecord->timecreated = time();
            $ledgerrecord->itemname = get_string('creditsused', 'local_biz_cart');
            $ledgerrecord->annotation = get_string('creditsusedannotation', 'local_biz_cart');
        ...
        foreach ($data['items'] as $item) {
            ...
            if (($item['componentname'] === 'local_biz_cart')
                && ($item['area'] === 'rebookitem')) {

                biz_cart_rebookingcredit::checkout_rebooking_item(
                    $item['componentname'],
                    $item['area'],
                    $item['itemid'],
                    $userid,
                ...
                $success = false;
                $error[] = get_string('itemcouldntbebought', 'local_biz_cart', $item['itemname']);
            ...
            if ($success == true) {
                ...
                if (!$datafromhistory) {
                    ...
                    // In cash report we have to sum up cash sums, so we cannot use LOCAL_BIZCART_PAYMENT_METHOD_CREDITS.
                    // So do not do this anymore but use the actual payment method.
                    // phpcs:ignore Squiz.PHP.CommentedOutCode.Found
                    /* $paymentmethod = $data['price'] == 0 ? LOCAL_BIZCART_PAYMENT_METHOD_CREDITS :
                    LOCAL_BIZCART_PAYMENT_METHOD_CASHIER;
                    if ($paymentmethod === LOCAL_BIZCART_PAYMENT_METHOD_CASHIER) {
                    // We now need to specify the actual payment method (cash, debit or credit card).
                    switch ($paymenttype) {
                    case LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_CASH:
                    case LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_CREDITCARD:
                    case LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_DEBITCARD:
                    case LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_MANUAL:
                    ...
                    if (($item['componentname'] === 'local_biz_cart')
                        ...
                        $historyitem = biz_cart_history::return_item_from_history($item['itemid']);

                        $item['schistoryid'] = $item['itemid'];
                        $item['itemid'] = $historyitem->itemid;
                    }

                    biz_cart_history::create_entry_in_history(
                        $userid,
                        $item['itemid'],
                        $item['itemname'],
                        $item['price'],
        ...
        if ($success) {
            ...
            if ($paymenttype == LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_MANUAL) {
    ...
    public static function cancel_purchase(
        ...
        if (empty($historyid)) {
            $record = biz_cart_history::get_most_recent_historyitem(
                $componentname,
                $area,
                $itemid,
                $userid);
        ...
        // Check if cancelation is still within the allowed period set in biz_cart_history.
        ...
        if ($applytocomponent == 1) {
            if (!self::cancel_purchase_for_component($componentname, $area, $itemid, $userid)) {
                return [
                    'success' => 0,
                    'error' => get_string('canceldidntwork', 'local_biz_cart'),
                    'credit' => 0,
                ];
            }
        }
        ...
        list($success, $error, $credit, $currency, $record) = biz_cart_history::cancel_purchase($itemid,
        ...
        if ($success == 1) {
            ...
            if ($userid == $USER->id) {
                ...
                $consumption = get_config('local_biz_cart', 'calculateconsumation');
                if ($consumption == 1) {
                    $quota = self::get_quota_consumed($componentname, $area, $itemid, $userid, $historyid);
                    $customcredit = $quota['remainingvalue'];
                }

                if (($cancelationfee = get_config('local_biz_cart', 'cancelationfee'))
                    && $cancelationfee > 0) {
            ...
            $discountprecision = get_config('local_biz_cart', 'rounddiscounts') ? 0 : 2;
            ...
            list($newcredit) = biz_cart_credits::add_credit($userid, $customcredit, $currency);
    ...
    public static function credit_paid_back(int $userid,
        int $method = LOCAL_BIZCART_PAYMENT_METHOD_CREDITS_PAID_BACK_BY_CASH): array {
        ...
        if (!biz_cart_credits::credit_paid_back($userid, $method)) {
            return [
                'status' => 0,
    ...
    public static function allowed_to_cancel(int $historyid, int $itemid, string $area, int $userid): bool
    {
        ...
        if (!$item = biz_cart_history::return_item_from_history($historyid)) {
            return false;
        }

        // We can only cancel items that are successfully paid.
        if ($item->paymentstatus != LOCAL_BIZCART_PAYMENT_SUCCESS) {
            return false;
        }
    ...
    public static function allowed_to_cancel_for_item(stdClass $item, string $area)
    {
        ...
        $cancelationfee = get_config('local_biz_cart', 'cancelationfee');
    ...
    public static function add_discount_to_item(
        ...
        if (!isset($cachedrawdata['items'][$cacheitemkey])) {
            throw new moodle_exception('itemnotfound', 'local_biz_cart');
        }
        ...
        if (!empty($percent)) {
            ...
            if ($percent < 0 || $percent > 100) {
                throw new moodle_exception('absolutevalueinvalid', 'local_biz_cart');
            }
    ...
    /**
     * Helper function to add entries to local_biz_cart_ledger table.
    ...
    public static function add_record_to_ledger_table(stdClass $record)
    {
        ...
        switch ($record->paymentstatus) {
            case LOCAL_BIZCART_PAYMENT_SUCCESS:
                ...
                if (($record->itemid === 0) || (!$DB->get_record('local_biz_cart_ledger', [
                    ...
                    $id = $DB->insert_record('local_biz_cart_ledger', $record);
                    cache_helper::purge_by_event('setbackcachedcashreport');
                }
                break;
            case LOCAL_BIZCART_PAYMENT_CANCELED:
                $now = time();
                $record->price = null;
                ...
                $record->payment = LOCAL_BIZCART_PAYMENT_METHOD_CREDITS;
                $record->gateway = null;
                $record->orderid = null;
            ...
            case LOCAL_BIZCART_PAYMENT_ABORTED:
            case LOCAL_BIZCART_PAYMENT_PENDING:
            default:
                break;
        }
    ...
    public static function update_item_price_data(array $items, ?taxcategories $taxcategories): array
    {
        ...
        foreach ($items as $key => $item) {
            ...
            if ($items[$key]['nodelete'] === 1 &&
                has_capability('local/biz_cart:cashier', $context)) {
            ...
            if ($taxcategories) {
                ...
                if ($taxpercent >= 0) {
                    ...
                    $itemisnet = get_config('local_biz_cart', 'itempriceisnet');
                    if ($itemisnet) {
    ...
    public static function get_quota_consumed(string $component, string $area, int $itemid, int $userid, int $historyid): array
    {
        ...
        $item = biz_cart_history::return_item_from_history($historyid);
    ...
    public static function check_for_ongoing_payment(int $userid)
    {
        ...
        $params['paymentstatus'] = LOCAL_BIZCART_PAYMENT_PENDING;
        ...
        foreach ($accounts as $account) {
            ...
            foreach ($account->get_gateways() as $gateway) {
                ...
                $sql = "SELECT DISTINCT sch.identifier, sch.userid, oo.timecreated, COALESCE(oo.tid, '') as tid
                        FROM {local_biz_cart_history} sch
                        JOIN {" . $table . "} oo
    ...
    public static function add_quota_consumed_to_item(stdClass &$item, int $userid)
    {
        ...
        if (get_config('local_biz_cart', 'calculateconsumation')
            && get_config('local_biz_cart', 'calculateconsumationfixedpercentage') > 0) {

            // We also check if the setting to only apply fixed percentage within service period is turned on.
            if (get_config('local_biz_cart', 'fixedpercentageafterserviceperiodstart')) {
                if (time() >= $item->serviceperiodstart) {
                    $item->quotaconsumed = (float) 0.01 * get_config('local_biz_cart', 'calculateconsumationfixedpercentage');
                    return;
                } else {
    ...
    public static function get_daily_sums_data(string $date, string $selectorformoutput = ''): array
    {
        ...
        $dailysumsdata['currency'] = get_config('local_biz_cart', 'globalcurrency') ?? 'EUR';
        $dailysumsdata['title'] = get_string('titledailysums', 'local_biz_cart');

        // SQL to get daily sums.
        $dailysumssql = "SELECT payment, sum(price) dailysum
            FROM {local_biz_cart_ledger}
        ...
        $dailysumsparams = [
            ...
            'paymentsuccess' => LOCAL_BIZCART_PAYMENT_SUCCESS,
        ];
        ...
        foreach ($dailysumsfromdb as $dailysumrecord) {
            ...
            switch ($dailysumrecord->payment) {
                case LOCAL_BIZCART_PAYMENT_METHOD_ONLINE:
                    $total += (float) $dailysumrecord->dailysum;
                    $dailysumrecord->paymentmethod = get_string('paymentmethodonline', 'local_biz_cart');
                    $dailysumsdata['online'] = $dailysumrecord->dailysumformatted;
                    break;
                case LOCAL_BIZCART_PAYMENT_METHOD_CASHIER:
                    $total += (float) $dailysumrecord->dailysum;
                    $dailysumrecord->paymentmethod = get_string('paymentmethodcashier', 'local_biz_cart');
                    $dailysumsdata['cashier'] = $dailysumrecord->dailysumformatted;
                    break;
                case LOCAL_BIZCART_PAYMENT_METHOD_CREDITS_PAID_BACK_BY_CASH:
                    // Will be a negative number, so we can still use "+=".
                    $total += (float) $dailysumrecord->dailysum;
                    $dailysumrecord->paymentmethod = get_string('paymentmethodcreditspaidbackcash', 'local_biz_cart');
                    $dailysumsdata['creditspaidbackcash'] = $dailysumrecord->dailysumformatted;
                    break;
                case LOCAL_BIZCART_PAYMENT_METHOD_CREDITS_PAID_BACK_BY_TRANSFER:
                    // Will be a negative number, so we can still use "+=".
                    $total += (float) $dailysumrecord->dailysum;
                    $dailysumrecord->paymentmethod = get_string('paymentmethodcreditspaidbacktransfer', 'local_biz_cart');
                    $dailysumsdata['creditspaidbacktransfer'] = $dailysumrecord->dailysumformatted;
                    break;
                case LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_CASH:
                    $total += (float) $dailysumrecord->dailysum;
                    $dailysumrecord->paymentmethod = get_string('paymentmethodcashier:cash', 'local_biz_cart');
                    $dailysumsdata['cash'] = $dailysumrecord->dailysumformatted;
                    break;
                case LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_CREDITCARD:
                    $total += (float) $dailysumrecord->dailysum;
                    $dailysumrecord->paymentmethod = get_string('paymentmethodcashier:creditcard', 'local_biz_cart');
                    $dailysumsdata['creditcard'] = $dailysumrecord->dailysumformatted;
                    break;
                case LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_DEBITCARD:
                    $total += (float) $dailysumrecord->dailysum;
                    $dailysumrecord->paymentmethod = get_string('paymentmethodcashier:debitcard', 'local_biz_cart');
                    $dailysumsdata['debitcard'] = $dailysumrecord->dailysumformatted;
                    break;
                case LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_MANUAL:
                    $total += (float) $dailysumrecord->dailysum;
                    $dailysumrecord->paymentmethod = get_string('paymentmethodcashier:manual', 'local_biz_cart');
        ...
        if (get_config('local_biz_cart', 'showdailysumscurrentcashier')) {
        ...
        // Add download URL.
        if (!empty($date)) {
            $dailysumsdata['dailysumspdfurl'] = new moodle_url('/local/biz_cart/daily_sums_pdf.php', ['date' => $date]);
        }
