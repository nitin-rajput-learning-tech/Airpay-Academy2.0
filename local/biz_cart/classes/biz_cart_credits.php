...
/**
 * biz_cart_credits class for local shopping cart.
 *
...
class biz_cart_credits
{
    ...
    public static function get_balance(int $userid): array
    {
        ...
        if (empty($currencies)) {
            ...
            throw new moodle_exception('nomulticurrencysupportyet', 'local_biz_cart');
        }

        // Get the latest balance.
        if (!$balancerecord = $DB->get_record_sql("SELECT balance, currency
            FROM {local_biz_cart_credits}
            WHERE userid = :userid
            ORDER BY id DESC
        ...
        if ($CFG->debug == DEBUG_DEVELOPER) {
            ...
            if (round($sumofcredits, 2) != round($balance, 2)) {
                throw new moodle_exception(get_string('creditnotmatchbalance', 'local_biz_cart', strval($userid)));
            }
        }
    ...
    public static function prepare_checkout(array &$data, int $userid, $usecredit = null)
    {
        ...
        $pricebelowzero = biz_cart_rebookingcredit::correct_total_price_for_rebooking($data);
    ...
    public static function add_credit(int $userid, float $credit, string $currency): array
    {
        ...
        if ($data->balance >= 0) {
            $DB->insert_record('local_biz_cart_credits', $data);
        } else {
    ...
    public static function credit_paid_back(int $userid,
        int $method = LOCAL_BIZCART_PAYMENT_METHOD_CREDITS_PAID_BACK_BY_CASH) {
        global $USER;
    ...
    public static function use_credit_fallback($usecredit, int $userid): int
    {
        ...
        if ($usecredit === null) {
            $tempusecredit = biz_cart::get_saved_usecredit_state($userid);
