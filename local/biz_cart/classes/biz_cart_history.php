...
/**
 * biz_cart_history class for local shopping cart.
...
class biz_cart_history
{
    ...
    public static function get_history_list_for_user(int $userid): array
    {
        ...
        if (!empty($accountid)) {
            ...
            $errmsg = get_string('nopaymentaccounts', 'local_biz_cart');
            $errmsg .= ' ' . get_string('nopaymentaccountsdesc', 'local_biz_cart', $urlobject);
            echo $errmsg;
            exit();
        }
        ...
        $records = $DB->get_records_sql($sql, ['userid' => $userid, 'paymentstatus' => LOCAL_BIZCART_PAYMENT_SUCCESS]);
    ...
    public static function get_user_list_for_option(int $optionid, string $componentname, string $area)
    {
        ...
        $sql = "SELECT sch.id, u.id as userid, u.firstname, u.lastname, u.email, sch.itemid, sch.price, sch.currency
                FROM {user} u
                JOIN {local_biz_cart_history} sch
                ON u.id=sch.userid
    ...
    private static function write_to_db(stdClass $data): bool
    {
        ...
        if (isset($data->items)) {
            ...
            biz_cart::add_record_to_ledger_table($data);
        } else {
            if (!$DB->record_exists('local_biz_cart_history', [
                'userid' => $data->userid,
                'itemid' => $data->itemid,
                ...
                $data->usecredit = biz_cart_credits::use_credit_fallback(null, $data->userid);

                if ($id = $DB->insert_record('local_biz_cart_history', $data)) {
    ...
    /**
     * Add new entry to biz_cart_history.
    ...
    public static function create_entry_in_history(
        ...
        string $identifier,
        string $payment,
        int $paymentstatus = LOCAL_BIZCART_PAYMENT_PENDING,
    ...
    public static function cancel_purchase(int $itemid, int $userid, string $componentname, string $area, int $entryid = null,
        ...
        // Identify record.
        if ($entryid) {
            $record = $DB->get_record('local_biz_cart_history', [
                'id' => $entryid,
                'paymentstatus' => LOCAL_BIZCART_PAYMENT_SUCCESS,
            ]);
        } else {
            // Only return successful payments.
            // We only take the last record.
            $sql = "SELECT *
                    FROM {local_biz_cart_history}
                    WHERE itemid=:itemid
                    AND  userid=:userid
                    AND componentname=:componentname
                    AND area=:area
                    AND paymentstatus = " . LOCAL_BIZCART_PAYMENT_SUCCESS . "
        ...
        $record->paymentstatus = LOCAL_BIZCART_PAYMENT_CANCELED;
        $record->timemodified = time();

        try {
            $DB->update_record('local_biz_cart_history', $record);

            // NOTE: Ledger entry will be inserted in biz_cart::cancel_purchase function!
    ...
    {
        global $DB;
        if ($data = $DB->get_records('local_biz_cart_history', ['identifier' => $identifier])) {

            // If there is an error registered, we return null.
            foreach ($data as $record) {
                $aborted = false;
                // Status LOCAL_BIZCART_PAYMENT_ABORTED is 1. Fails in adhoc task if constant is used. Weird.
    ...
    {
        global $DB;
        if ($data = $DB->get_records('local_biz_cart_ledger', ['identifier' => $identifier])) {
    ...
    public static function error_occured_for_identifier(int $identifier, int $userid): bool
    {
        ...
        foreach ($records as $record) {
            ...
            if ($record->paymentstatus == LOCAL_BIZCART_PAYMENT_PENDING) {
                $record->paymentstatus = LOCAL_BIZCART_PAYMENT_ABORTED;
                $record->timemodified = time();
                $DB->update_record('local_biz_cart_history', $record);
            }
        }
    ...
    public static function set_success_in_db(array $records): bool
    {
        ...
        foreach ($records as $record) {
            ...
            $record->paymentstatus = LOCAL_BIZCART_PAYMENT_SUCCESS;
            $record->timemodified = $now;

            if ($record->componentname === 'local_biz_cart'
            ...
            if (!$DB->update_record('local_biz_cart_history', $record)) {
                $success = false;
            } else {
                ...
                biz_cart::add_record_to_ledger_table($record);
            }
        }
    ...
    public function prepare_data_from_cache(int $userid, int $identifier = 0): array
    {
        ...
        $items = biz_cart::update_item_price_data(array_values($cachedrawdata['items']), $taxcategories);
        foreach ($items as $item) {
            ...
            $data['payment'] = LOCAL_BIZCART_PAYMENT_METHOD_ONLINE; // This function is only used for online payment.
            $data['paymentstatus'] = LOCAL_BIZCART_PAYMENT_PENDING;
        ...
        if (!empty($items)) {
            $dataarr['price'] = biz_cart::calculate_total_price($dataarr["items"]);
            if ($taxesenabled) {
                $dataarr['price_net'] = biz_cart::calculate_total_price($dataarr["items"], true);
            }
        } else {
            $dataarr['price'] = 0.00;
        }
    ...
    public function fetch_data_from_schistory_cache(string $identifier)
    {
        ...
        if (isset($shoppingcart['identifier']) && ($shoppingcart['identifier'] != $identifier)) {
            throw new moodle_exception('wrongidentifier', 'local_biz_cart');
        }
        ...
        if (isset($shoppingcart['identifier']) && !isset($shoppingcart['storedinhistory'])) {
            ...
            throw new moodle_exception('noidentifierfound', 'local_biz_cart');
        }
    ...
    public static function create_unique_cart_identifier(int $userid): int
    {
        ...
        $uid = $DB->insert_record('local_biz_cart_id', [
            'userid' => $userid,
            'timecreated' => time(),
        ]);

        $basevalue = (int) get_config('local_biz_cart', 'uniqueidentifier') ?? 0;
        ...
        if ((!empty($basevalue) && (($uid / $basevalue) > 10))) {
            throw new moodle_exception('uidistoobig', 'local_biz_cart');
        }
    ...
    public static function return_item_from_history(int $historyid)
    {
        ...
        return $DB->get_record('local_biz_cart_history',
            ['id' => $historyid]);
    }
    ...
    public static function return_items_from_history(
        ...
        return $DB->get_records('local_biz_cart_history',
    ...
    public static function toggle_mark_for_rebooking(int $historyid, int $userid, bool $remove = null): array
    {
        ...
        $userid = biz_cart::set_user($userid);

        $cachekey = 'rebook_userid_' . $userid;
        $cache = \cache::make('local_biz_cart', 'cacherebooking');
        ...
        if ((($itemstorebook = $cache->get($cachekey))
            ...
            if (in_array($historyid, $itemstorebook) || $remove) {
                ...
                $marked = 0;
                biz_cart::delete_item_from_cart('local_biz_cart', 'rebookitem', $historyid, $userid);
            } else {
        ...
        if (!empty($marked) && empty($remove)) {
            ...
            biz_cart_rebookingcredit::delete_booking_fee($userid);

            biz_cart::add_item_to_cart('local_biz_cart', 'rebookitem', $historyid, $userid);
        }
