...
class biz_cart_bookingfee
{
    ...
    public static function add_fee_to_cart(int $userid, int $buyforuserid = 0): bool
    {
        ...
        if (get_config('local_biz_cart', 'bookingfee') <= 0) {
            return false;
        }

        // Which kind of fee?
        if (get_config('local_biz_cart', 'bookingfeeonlyonce')) {
        ...
        if (($buyforuserid == 0) && biz_cart_rebookingcredit::is_rebooking($userid)) {
            return false;
        }

        biz_cart::add_item_to_cart('local_biz_cart', 'bookingfee', $itemid, $userid);
    ...
    private static function user_has_paid_fee(int $userid, int $bookingfeetype = LOCAL_BIZCART_BOOKINGFEE_ANY)
    {
        ...
        if ($bookingfeetype === LOCAL_BIZCART_BOOKINGFEE_ANY) {
            ...
            $records = biz_cart_history::return_items_from_history(
                $bookingfeetype,
                'local_biz_cart',
                'bookingfee',
                $userid);
