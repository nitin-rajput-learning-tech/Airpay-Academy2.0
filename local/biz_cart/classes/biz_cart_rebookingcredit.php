 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class biz_cart_rebookingcredit
{
        ...
        if (!empty($canceledrecords)
            ...
            if ($itemcount <= count($canceledrecords)) {
                ...
                if (!empty($rebookingcreditrecords)) {
                    ...
                    biz_cart::delete_item_from_cart('local_biz_cart', 'rebookingcredit',
    ...
    private static function get_records_canceled_with_future_canceluntil(int $userid): array
    {
        ...
        $canceledrecords = $DB->get_records_select('local_biz_cart_history',
    ...
    {
        global $DB;
        return $DB->record_exists_select('local_biz_cart_ledger',
    ...
    public static function add_rebookingcredit_item_to_cart(int $userid, int $itemcount = 1): bool
    {
        ...
        if (!get_config('local_biz_cart', 'allowrebookingcredit')
            || self::rebookingcredit_already_used($userid) // If it was already used, we do not add.
        ) {
            return false;
        }

        biz_cart::add_item_to_cart('local_biz_cart', 'rebookingcredit', $itemcount, $userid);
    ...
    public static function delete_booking_fee(int $userid)
    {
        ...
        foreach ($items as $item) {
            if (($item['area'] === 'bookingfee')
                ...
                biz_cart::delete_item_from_cart(
                    'local_biz_cart',
                    'bookingfee',
                    $item['itemid'],
                    $userid,
     ...
     * This is partly the same as the biz_cart::cancel_purchase function.
     * It also calls the componentcallback and biz_cart_history::cancel_purchase.
    ...
    public static function checkout_rebooking_item($component, $area, $itemid, $userid)
    {
        ...
        if (($component === 'local_biz_cart')
            ...
            $historyitem = biz_cart_history::return_item_from_history($itemid);
            ...
            $providerclass = biz_cart::get_service_provider_classname($historyitem->componentname);
            component_class_callback($providerclass, 'cancel_purchase',
                [$historyitem->area, $historyitem->itemid, $userid]);

            list($success, $error, $credit, $currency, $record) = biz_cart_history::cancel_purchase(
                $historyitem->itemid,
                $userid,
