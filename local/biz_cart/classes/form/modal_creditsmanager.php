...
use local_biz_cart\biz_cart_credits;
...
class modal_creditsmanager extends dynamic_form
{
    ...
    public function definition()
    {
        ...
        if (!has_capability('local/biz_cart:cashier', $context)) {
            $mform->addElement('html', get_string('nopermissiontoaccesspage', 'local_biz_cart'));
            return;
        }
        ...
        $mform->addElement('html', '<div class="alert alert-info">' .
            get_string('creditsmanager:infotext', 'local_biz_cart', $a) . '</div>');

        $modes = [
            0 => get_string('choose...', 'local_biz_cart'),
            1 => get_string('creditsmanager:correctcredits', 'local_biz_cart'),
            2 => get_string('creditsmanager:payback', 'local_biz_cart'),
        ];
        $mform->addElement('select', 'creditsmanagermode',
            get_string('creditsmanagermode', 'local_biz_cart'), $modes);
        $mform->setDefault('creditsmanagermode', 0);

        $mform->addElement('float', 'creditsmanagercredits',
            get_string('creditsmanagercredits', 'local_biz_cart'));
        $mform->setDefault('creditsmanagercredits', 0.00);
        $mform->addHelpButton('creditsmanagercredits', 'creditsmanagercredits', 'local_biz_cart');
        ...
        $mform->addElement('select', 'creditsmanagerpaymentmethod',
            get_string('paymentmethod', 'local_biz_cart'), $paymentmethods);
        ...
        $mform->addElement('text', 'creditsmanagerreason',
            get_string('creditsmanagerreason', 'local_biz_cart'));
    ...
    {
        require_capability('local/biz_cart:cashier', $this->get_context_for_dynamic_submission());
    }
    ...
    public function creditsmanager_correct_credits(stdClass $data)
    {
        ...
        $currency = get_config('local_biz_cart', 'globalcurrency') ?? 'EUR';

        // Add credits.
        try {
            biz_cart_credits::add_credit($data->userid, $data->creditsmanagercredits, $currency);
            ...
            $ledgerrecord->payment = LOCAL_BIZCART_PAYMENT_METHOD_CREDITS_CORRECTION;
            ...
            biz_cart::add_record_to_ledger_table($ledgerrecord);
    ...
    public static function creditsmanager_payback_credits(stdClass $data): bool
    {
        ...
        list($currentbalance, $currency) = biz_cart_credits::get_balance($userid);
        ...
        if (!$DB->insert_record('local_biz_cart_credits', $creditrecord)) {
            return false;
        }
    ...
    public function validation($data, $files)
    {
        ...
        if (empty($data['creditsmanagermode'])) {
            $errors['creditsmanagermode'] = get_string('error:choosevalue', 'local_biz_cart');
        }
        if (empty($data['creditsmanagercredits']) ||
            ($data['creditsmanagermode'] == 2 && $data['creditsmanagercredits'] <= 0)) {
            $errors['creditsmanagercredits'] = get_string('error:notpositive', 'local_biz_cart');
        }
        if (isset($data['creditsmanagermode']) && $data['creditsmanagermode'] == 2 && empty($data['creditsmanagerpaymentmethod'])) {
            $errors['creditsmanagerpaymentmethod'] = get_string('error:choosevalue', 'local_biz_cart');
        }
        if (empty($data['creditsmanagerreason'])) {
            $errors['creditsmanagerreason'] = get_string('error:noreason', 'local_biz_cart');
        }
