...
class modal_add_discount_to_item extends dynamic_form
{
    ...
    public function definition()
    {
        ...
        if ($this->_ajaxformdata["userid"] == -1) {
            $userid = biz_cart::return_buy_for_userid();
        ...
        $mform->addElement('static', 'bodytext', '', get_string('adddiscounttoitem', 'local_biz_cart'));
        ...
        $mform->addElement('float', 'discountpercent', get_string('discountpercent', 'local_biz_cart'));
        $mform->addHelpButton('discountpercent', 'discountpercent', 'local_biz_cart');
        $mform->setType('discountpercent', PARAM_FLOAT);
        $mform->setDefault('discountpercent', 0);
        $mform->addRule('discountpercent', get_string('floatonly', 'local_biz_cart'), 'numeric', null, 'client');

        $mform->addElement('float', 'discountabsolute', get_string('discountabsolute', 'local_biz_cart'));
        $mform->addHelpButton('discountabsolute', 'discountabsolute', 'local_biz_cart');
        $mform->setType('discountabsolute', PARAM_FLOAT);
        $mform->setDefault('discountabsolute', 0);
        $mform->addRule('discountabsolute', get_string('floatonly', 'local_biz_cart'), 'numeric', null, 'client');
    }
    ...
    public function validation($data, $files)
    {
        ...
        if (!empty($data['discountpercent']) && !empty($data['discountabsolute'])) {
            $errors['discountpercent'] = get_string('onlyonevaluecanbeset', 'local_biz_cart');
            $errors['discountabsolute'] = get_string('onlyonevaluecanbeset', 'local_biz_cart');
        }
