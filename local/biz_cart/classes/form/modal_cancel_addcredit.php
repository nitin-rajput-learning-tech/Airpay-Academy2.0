...
class modal_cancel_addcredit extends dynamic_form
{
    ...
    public function definition()
    {
        ...
        if (get_config('local_biz_cart', 'calculateconsumation')) {
            $consumed = (object) biz_cart::get_quota_consumed(
        ...
        if (empty($consumed->quota)) {
            ...
            $mform->addElement('static', 'bodytext', '',
                get_string('confirmcancelbody', 'local_biz_cart', $consumed));
        } else if ($consumed->quota == 1) {
            $remainingvalue = 0;
            $cancelationfee = 0;
            $mform->addElement('static', 'bodytext', '',
                get_string('confirmcancelbodynocredit', 'local_biz_cart', $consumed));
        } else {
            $remainingvalue = $consumed->remainingvalue;
            $mform->addElement('static', 'bodytext', '',
                get_string('confirmcancelbodyconsumption', 'local_biz_cart', $consumed));
        }

        $mform->addElement('float', 'credittopayback', get_string('credittopayback', 'local_biz_cart'));
        $mform->addElement('float', 'cancelationfee', get_string('cancelationfee', 'local_biz_cart'));

        $mform->addElement('advcheckbox', 'applytocomponent', get_string('applytocomponent', 'local_biz_cart'),
            get_string('applytocomponent_desc', 'local_biz_cart'));
    ...
    public function process_dynamic_submission()
    {
        ...
        biz_cart::cancel_purchase($data->itemid, $data->area, $data->userid, $data->componentname, $data->historyid,
    ...
    public function validation($data, $files)
    {
        ...
        if (isset($data["credittopayback"]) && $data["credittopayback"] < 0) {
            $errors["credittopayback"] = get_string('error:negativevaluenotallowed', 'local_biz_cart');
        }

        if (isset($data["cancelationfee"]) && $data["cancelationfee"] < 0) {
            $errors["cancelationfee"] = get_string('error:negativevaluenotallowed', 'local_biz_cart');
        }

        if (isset($data["credittopayback"]) && isset($data["cancelationfee"])
            && $data["cancelationfee"] > $data["credittopayback"]) {
            $errors["cancelationfee"] = get_string('error:cancelationfeetoohigh', 'local_biz_cart');
        }
