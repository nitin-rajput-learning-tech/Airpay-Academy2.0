...
namespace local_biz_cart\biz_cart;

use context_system;
use local_biz_cart\local\entities\cartitem;

/**
 * biz_cart subsystem callback implementation for local_biz_cart, for testing, does not have any use for production.
 *
 * @package    local_biz_cart
 * @copyright  2024 Moodle India <info@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class service_provider implements \local_biz_cart\local\callback\service_provider

{
    ...
    public static function load_cartitem(string $area, int $itemid, int $userid = 0): array
    {
        ...
        switch ($area) {
            case 'bookingfee':
                $imageurl = new \moodle_url('/local/biz_cart/pix/coins.png');
                $cartitem = new cartitem(
                    $itemid,
                    get_string('bookingfee', 'local_biz_cart'),
                    get_config('local_biz_cart', 'bookingfee'),
            ...
            case 'rebookingcredit':
                ...
                if (get_config('local_biz_cart', 'cancelationfee') > 0) {
                    $price = -1 * (
                        (float) get_config('local_biz_cart', 'bookingfee') +
                        (float) $itemid * (float) get_config('local_biz_cart', 'cancelationfee')
                    );
                } else {
                    $price = -1 * (
                        (float) get_config('local_biz_cart', 'bookingfee')
                ...
                $imageurl = new \moodle_url('/local/biz_cart/pix/rebook.png');
                $cartitem = new cartitem(
                    $itemid,
                    get_string('rebookingcredit', 'local_biz_cart'),
                    $price,
                    get_config('local_biz_cart', 'globalcurrency') ?? 'EUR',
                    'local_biz_cart',
                    'rebookingcredit',
                ...
                return ['cartitem' => $cartitem];
            case 'rebookitem':
                $record = $DB->get_record('local_biz_cart_history', ['id' => $itemid]);
                ...
                $cartitem = new cartitem(
                    $itemid,
                    get_string('rebooking', 'local_biz_cart') . ': ' . $record->itemname,
        ...
        $imageurl = new \moodle_url('/local/biz_cart/pix/edu.png');
