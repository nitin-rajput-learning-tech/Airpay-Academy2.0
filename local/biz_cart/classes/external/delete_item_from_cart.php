...
use local_biz_cart\biz_cart_bookingfee;
...
class delete_item_from_cart extends external_api
{
    ...
    public static function execute(string $component, string $area, int $itemid, int $userid)
    {
        ...
        if (!has_capability('local/biz_cart:canbuy', $context)) {
        ...
        if (biz_cart_bookingfee::is_fee($params['component'], $params['area'])
            && !has_capability('local/biz_cart:cashier', $context)) {
            return false;
        }

        // We can't delete the rebookingcredit item via webservice.
        if (biz_cart::is_rebookingcredit($params['component'], $params['area'])
            && !has_capability('local/biz_cart:cashier', $context)) {
            return false;
        }

        if ($params['component'] == 'local_biz_cart' && $params['area'] == 'rebookitem') {
            biz_cart_history::toggle_mark_for_rebooking($params['itemid'], $userid, true);
            biz_cart_bookingfee::add_fee_to_cart($userid);
            return ['success' => 1];
        }

        // This treats the cache side.
        if (biz_cart::delete_item_from_cart($params['component'], $params['area'], $params['itemid'], $userid)) {
            return ['success' => 1];
        }
        return ['success' => 0];
    }
