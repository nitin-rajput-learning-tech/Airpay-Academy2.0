...
import {updateTotalPrice, buttoninit} from 'local_biz_cart/cart';
...
import {
    ...
    get_strings as getStrings
        ...
        from 'core/str';
import {showNotification} from 'local_biz_cart/notifications';
import ModalFactory from 'core/modal_factory';
import ModalEvents from 'core/modal_events';
import ModalForm from 'core_form/modalform';

const SELECTORS = {
    CANCELBUTTON: '.cashier-history-items .biz_cart_history_cancel_button',
    PAIDBACKBUTTON: 'button.biz_cart_history_paidback_button',
    CREDITSMANAGER: 'button.biz_cart_history_creditsmanager',
    REBOOKBUTTON: '.biz_cart_history_rebook_button',
};
...
(async() => {
    creditsmanagersuccess = await getString('creditsmanagersuccess', 'local_biz_cart');
    notenoughcredits = await getString('notenoughcredits', 'local_biz_cart');
})();
...
export function cancelPurchase(itemid, area, userid, componentname, historyid, currency, price, credit, button) {
    ...
    Ajax.call([{
        methodname: "local_biz_cart_cancel_purchase",
        args: {
            'itemid': itemid,
            ...
            if (data.success == 1) {
                ...
                getString('cancelsuccess', 'local_biz_cart').then(message => {
                ...
                if (!addtocartbutton) {
                    ...
                    Templates.renderForPromise('local_biz_cart/addtocartdb', data).then(({html}) => {
                ...
                getString('canceldidntwork', 'local_biz_cart').then(message => {
...
function setButtonToCanceled(button) {
    ...
    getString('canceled', 'local_biz_cart').then(result => {
...
function showCredit(credit, currency, userid) {
    ...
    let creditelement = document.querySelector('li.biz_cart_history_paidback');
    ...
    if (creditelement) {
        ...
        Templates.renderForPromise('local_biz_cart/credit_item', data).then(({html}) => {
...
function confirmPaidBack(element) {
    ...
    Ajax.call([{
        methodname: "local_biz_cart_credit_paid_back",
        args: {
            userid,
            method
        },
            ...
            let licreditelements = document.querySelectorAll('.biz_cart_history_paidback');

            licreditelements.forEach(licreditelement => licreditelement.classList.add('hidden'));

            getString('creditpaidback', 'local_biz_cart').then(message => {
...
export async function confirmCancelModal(button, cancelationFee) {
    ...
    if (!button.dataset.hasOwnProperty('price')) {
        await new Promise(function(resolve, reject) {
            Ajax.call([{
                methodname: 'local_biz_cart_get_history_item',
                args: {
    ...
    getStrings([
            {key: 'confirmcanceltitle', component: 'local_biz_cart'},
            {key: bodystring, component: 'local_biz_cart', param: params},
            {key: 'cancelpurchase', component: 'local_biz_cart'}
        ]
    ).then(strings => {
...
function confirmCancelAndSetCreditModal(button) {
    ...
    const modalForm = new ModalForm({
        ...
        formClass: "local_biz_cart\\form\\modal_cancel_addcredit",
        ...
        modalConfig: {title: getString('confirmcanceltitle', 'local_biz_cart')},
...
function confirmPaidBackModal(element) {
    ...
    getStrings([
        {key: 'confirmpaidbacktitle', component: 'local_biz_cart'},
        {key: 'confirmpaidbackbody', component: 'local_biz_cart'},
        {key: 'confirmpaidback', component: 'local_biz_cart'}
    ]
    ).then(strings => {
        // eslint-disable-next-line promise/no-nesting
        ModalFactory.create({type: ModalFactory.types.SAVE_CANCEL}).then(modal => {
...
function openCreditsManagerModal(button) {
    ...
    const modalForm = new ModalForm({
        // Name of the class where form is defined (must extend \core_form\dynamic_form):
        formClass: "local_biz_cart\\form\\modal_creditsmanager",
        // Add as many arguments as you need, they will be passed to the form:
        args: {
            userid: button.dataset.userid
        },
        // Pass any configuration settings to the modal dialogue, for example, the title:
        modalConfig: {title: getString('creditsmanager', 'local_biz_cart')},
        // DOM element that should get the focus after the modal dialogue is closed:
        returnFocus: button
    });
    // Listen to events if you want to execute something on form submit.
...
function markforrebooking(button) {
    ...
    Ajax.call([{
        methodname: 'local_biz_cart_mark_item_for_rebooking',
        args: {
            historyid,
            userid
        },
        ...
        fail: ex => {
            // eslint-disable-next-line no-console
            console.log("local_biz_cart_mark_item_for_rebooking failed: " + JSON.stringify(ex));
        },
    }]);
}