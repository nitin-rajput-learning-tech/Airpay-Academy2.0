...
import {reinit} from 'local_biz_cart/cart';
import {deleteAllItems} from 'local_biz_cart/cart';
...
export const init = (userid = 0) => {
    ...
    if (cartcancelbtn) {
        cartcancelbtn.addEventListener('click', () => {
            ...
            const newurl = Url.relativeUrl("/local/biz_cart/cashier.php", [], false);
...
export const confirmPayment = (userid, paymenttype, annotation = '') => {
    ...
    Ajax.call([{
        methodname: "local_biz_cart_confirm_cash_payment",
        args: {
            'userid': userid,
            ...
            if (data.status === 1) {
                ...
                if (oncashier < 1) {
                    ...
                    const newurl = Url.relativeUrl("/local/biz_cart/checkout.php", params, false);
...
export function discountModal(event) {
    ...
    const modalForm = new ModalForm({
        ...
        formClass: "local_biz_cart\\form\\modal_add_discount_to_item",
        ...
        modalConfig: {title: getString('applydiscount', 'local_biz_cart')},
...
function displayPaymentMessage(message, success = true) {
    ...
    if (displaymessage) {
        getString(message, 'local_biz_cart').then(localizedmessage => {
...
export function rebookOrderidModal(userid, identifier) {
    ...
    const modalForm = new ModalForm({
        ...
        // Name of the class where form is defined (must extend \core_form\dynamic_form):
        formClass: "local_biz_cart\\form\\modal_cashier_manual_rebook",
        // Add as many arguments as you need, they will be passed to the form:
        args: {
            'userid': userid,
            'identifier': identifier,
        ...
        // Pass any configuration settings to the modal dialogue, for example, the title:
        modalConfig: {title: getString('annotation_rebook_desc', 'local_biz_cart')},
        // DOM element that should get the focus after the modal dialogue is closed:
        // returnFocus: button
    });
