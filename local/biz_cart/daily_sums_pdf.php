 ...
 * @copyright   2024 2024 Moodle India <info@moodle.com>
 * @author      Moodle India
...
$data = biz_cart::get_daily_sums_data($date);
$html = get_config('local_biz_cart', 'dailysumspdfhtml');
if (empty($html)) {
    // No template defined, so use default mustache template.
    $html = $OUTPUT->render_from_template('local_biz_cart/report_daily_sums_pdf', $data);
} else {
