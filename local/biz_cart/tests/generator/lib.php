...
/**
 * Class local_biz_cart_generator for generation of dummy data
 *
 ...
 */
class local_biz_cart_generator extends testing_module_generator
{
    ...
    public function create_plugin_setup($record = null)
    {
        ...
        foreach ($record as $label => $value) {
            if (!set_config($label, $value, 'local_biz_cart')) {
    ...
    public function create_user_credit($record = null)
    {
        ...
        $record->id = $DB->insert_record('local_biz_cart_credits', $record);
    ...
    {
        // Clean cart.
        biz_cart::delete_all_items_from_cart($record['userid']);
        // Set user to buy in behalf of.
        biz_cart::buy_for_user($record['userid']);
        // Get cached data or setup defaults.
        biz_cart::local_biz_cart_get_cache_data($record['userid']);
        // Put in a test item with given ID (or default if ID > 4).
        biz_cart::add_item_to_cart('local_biz_cart', 'main', $record['testitemid'], -1);
        // Confirm cash payment.
        $res = biz_cart::confirm_payment($record['userid'], LOCAL_BIZCART_PAYMENT_METHOD_CASHIER_CASH);
        return $res;
    }
