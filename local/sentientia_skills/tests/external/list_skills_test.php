 ...
 *
 * - Capability gate: callers without local/sentientia_skills:manage are rejected
...
final class list_skills_test extends \advanced_testcase {
    ...
    private function seed_skill(string $name, int $categoryid = 0, int $sortorder = 0): int {
        ...
        if (!$DB->get_manager()->table_exists('local_sentientia_skills')) {
            $this->markTestSkipped('local_sentientia_skills table not present.');
        }
        $now = time();
    ...
    private function wipe_skills(): void {
        global $DB;
        if ($DB->get_manager()->table_exists('local_sentientia_skills')) {
            $DB->delete_records('local_sentientia_skills');
        }
    }

    /**
     * Create a user with local/sentientia_skills:manage capability.
     */
    private function user_with_manage(): \stdClass {
        $u = $this->getDataGenerator()->create_user();
        $sysctx = \context_system::instance();
        $roleid = $this->getDataGenerator()->create_role();
        role_change_permission($roleid, $sysctx, 'local/sentientia_skills:manage', CAP_ALLOW);
