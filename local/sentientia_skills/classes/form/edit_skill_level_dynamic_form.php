...
class edit_skill_level_dynamic_form extends dynamic_form {
    ...
    protected function definition(): void {
        ...
        if ($skillid > 0) {
            $skill = $DB->get_record('local_sentientia_skills', ['id' => $skillid]);
            if ($skill) {
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        $level   = $this->optional_param('level',   1, PARAM_INT);
        if ($skillid > 0) {
            $existing = $DB->get_record('local_sentientia_skill_levels',
    ...
    protected function get_page_url_for_dynamic_submission(): moodle_url {
        return new moodle_url('/local/sentientia_skills/level_definitions.php',
            ['skillid' => $this->optional_param('skillid', 0, PARAM_INT)]);
    }
}
