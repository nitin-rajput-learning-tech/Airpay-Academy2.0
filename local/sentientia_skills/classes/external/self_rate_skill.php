 ...
 *                  'local/sentientia_skills:manage' capability — admins
 *                  can backfill levels for a learner during onboarding
 *                  with their consent. Learners can only set their
 *                  own level.
 *
 * Returns:
 *   rowid        — The local_sentientia_user_skills row id (after upsert).
...
class self_rate_skill extends external_api {
    ...
    public static function execute(int $skillid, int $level, int $userid = 0): array {
        ...
        if ($target_userid === 0 || $target_userid === (int) $USER->id) {
            ...
            require_capability('local/sentientia_skills:self_rate', $context);
        ...
        $previous_level = (int) ($DB->get_field('local_sentientia_user_skills',
            'current_level',
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            'rowid'          => new external_value(PARAM_INT,
                'local_sentientia_user_skills.id (after upsert)'),
