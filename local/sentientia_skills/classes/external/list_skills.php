...
class list_skills extends external_api {
    ...
    public static function execute(string $search = '', string $sort = 'name', string $sortdir = 'asc',
        ...
        if (strlen($params['filters']) > 4096) throw new \moodle_exception('filterstoolong', 'local_sentientia_skills');
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_sentientia_skills} s WHERE $wheresql", $sqlparams);
        ...
        foreach ($records as $s) {
            $rows[] = [
                ...
                'actions'   => '<a href="' . s((new \moodle_url('/local/sentientia_skills/view.php', ['id' => (int) $s->id]))->out(false)) . '" class="btn btn-sm btn-link p-1" title="View detail"><i class="fa fa-eye"></i></a>'
                    . ' <a href="' . s((new \moodle_url('/local/sentientia_skills/level_definitions.php', ['skillid' => (int) $s->id]))->out(false)) . '" class="btn btn-sm btn-link text-muted p-1" title="Level definitions"><i class="fa fa-list-ol"></i></a>'
