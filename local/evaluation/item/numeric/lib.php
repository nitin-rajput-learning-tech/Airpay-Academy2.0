...
class evaluation_item_numeric extends evaluation_item_base {
    ...
    public function print_analysed($item, $itemnr = '', $groupid = false, $courseid = false) {
        ...
        if (isset($values->data) AND is_array($values->data)) {
            ...
            foreach ($values->data as $value) {
                $out .= '<tr><td colspan="2" class="singlevalue">';
                $out .= $this->format_float($value);
                $out .= '</td></tr>';
            }
            ...
            $out .= get_string('average', 'local_evaluation').': '.$avg;
            $out .= '</b></td></tr>';
            $out .= '</table>';
            $out .= '</div>';
        }
    ...
    protected function get_boundaries_for_display($item) {
        ...
        if (is_null($rangefrom) && is_numeric($rangeto)) {
            return ' (' . get_string('maximal', 'local_evaluation') .
                        ': ' . $this->format_float($rangeto) . ')';
        }
        if (is_numeric($rangefrom) && is_null($rangeto)) {
            return ' (' . get_string('minimal', 'local_evaluation') .
    ...
    public function complete_form_element($item, $form) {
        ...
        $form->add_validation_rule(function($values, $files) use ($item) {
            ...
            if ((is_numeric($rangefrom) && $value < floatval($rangefrom)) ||
                ...
                return array($inputname => get_string('numberoutofrange', 'local_evaluation'));
            }
            return true;
        });
    }
