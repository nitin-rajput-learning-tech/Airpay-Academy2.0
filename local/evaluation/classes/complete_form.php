...
/**
 * Class local_evaluation_complete_form
 *
 * @package   local_evaluation
 * @author Sreenivas <sreenivasula@eabyas.in>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class local_evaluation_complete_form extends moodleform {
    ...
    /** @var int */
    protected $mode;
    /** @var mod_evaluation_structure|mod_evaluation_completion */
    protected $structure;
    /** @var mod_evaluation_completion */
     ...
     *
     * @param int $mode
     * @param mod_evaluation_structure $structure
     * @param string $formid CSS id attribute of the form
     * @param array $customdata
     */
    public function __construct($mode, local_evaluation_structure $structure, $formid, $customdata = null) {
        $this->mode = $mode;
        $this->structure = $structure;
        $this->evalid = isset($customdata['evalid'])? $customdata['evalid'] : 0;
        $this->gopage = isset($customdata['gopage']) ? $customdata['gopage'] : 0;
        $this->teamuserid = isset($customdata['teamuserid']) ? $customdata['teamuserid'] : 0;
        $isanonymous = /*$this->structure->is_anonymous() ? */' ianonymous'/* : ''*/;
        parent::__construct(null, $customdata, 'POST', '',
                array('id' => $formid, 'class' => 'evaluation_form' . $isanonymous), true);
    }
    ...
    public function definition() {
        global $DB;
        $mform = $this->_form;
        $mform->addElement('hidden', 'id', $this->evalid);
        ...
        $mform->addElement('hidden', 'teamuserid', $this->teamuserid);
        $mform->setType('teamuserid', PARAM_INT);
        ...
        if (isloggedin() && !isguestuser() && $this->mode != self::MODE_EDIT && $this->mode != self::MODE_VIEW_TEMPLATE &&
            ...
            if ($this->structure->is_anonymous()) {
                $anonymousmodeinfo = get_string('anonymous', 'local_evaluation');
            } else {
                $anonymousmodeinfo = get_string('non_anonymous', 'local_evaluation');
            }
            $element = $mform->addElement('static', 'anonymousmode', '',
                    get_string('mode', 'local_evaluation') . ': ' . $anonymousmodeinfo);
            $element->setAttributes($element->getAttributes() + ['class' => 'evaluation_mode']);
        }
        ...
        if($this->evalid > 0){
            ...
            $record = $DB->get_record('local_evaluations', array('id' => $this->evalid));
            $description = file_prepare_draft_area($draftitemid, $systemcontext->id,
                                      'local_evaluation', 'intro', false,
                                      $editoroptions,
                                      $record->intro);
            if($this->mode !== self::MODE_VIEW_TEMPLATE){
                $descelement = $mform->addElement('static', 'description', '',
                    get_string('description', 'local_evaluation') . ': ' . $description);
                $descelement->setAttributes($descelement->getAttributes() + ['class' => 'evaluation_mode']);
            }
        }

        // Add buttons to go to previous/next pages and submit the evaluation.
        if ($this->mode == self::MODE_COMPLETE) {
            $buttonarray = array();
            $buttonarray[] = &$mform->createElement('submit', 'gopreviouspage', get_string('previous_page', 'local_evaluation'));
            $buttonarray[] = &$mform->createElement('submit', 'gonextpage', get_string('next_page', 'local_evaluation'),
                    array('class' => 'form-submit'));
            $buttonarray[] = &$mform->createElement('submit', 'savevalues', get_string('save_entries', 'local_evaluation'),
                    array('class' => 'form-submit'));
     ...
     *
     * This will add only items from a current page to the evaluation and adjust the buttons
     */
    protected function definition_complete() {
        if (!$this->structure instanceof local_evaluation_completion) {
    ...
    protected function enhance_name_for_edit($item, $element) {
        ...
        $menu->set_alignment(action_menu::TR, action_menu::BL);
        $menu->set_menu_trigger(get_string('edit'));
        $menu->prioritise = true;

        $itemobj = evaluation_get_item_class($item->typ);
        $actions = $itemobj->edit_actions($item, $this->get_evaluation());
        ...
        $name = html_writer::span('', 'itemdd', array('id' => 'evaluation_item_box_' . $item->id)) .
                html_writer::span($editmenu, 'itemactions', array('class'=>'pull-left pr-15')).
    ...
    public function display() {
        ...
        if ($this->mode == self::MODE_EDIT) {
            $PAGE->requires->js_call_amd('local_evaluation/edit', 'setup');
        }
    }
}
