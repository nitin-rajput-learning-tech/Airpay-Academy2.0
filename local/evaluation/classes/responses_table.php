...
/**
 * Contains class local_evaluation_responses_table
 *
...
/**
 * Class local_evaluation_responses_table
 *
 * @package   local_evaluation
 * @author    2019 eabyas
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class local_evaluation_responses_table extends table_sql {

    /**
     * Maximum number of evaluation questions to display in the "Show responses" table
     */
    const PREVIEWCOLUMNSLIMIT = 10;

    /**
     * Maximum number of evaluation questions answers to retrieve in one SQL query.
    ...
    /** @var local_evaluation_structure */
     ...
     * Constructor
     *
     * @param local_evaluation_structure $evaluationstructure
     * @param int $group retrieve only users from this group (optional)
     */
    public function __construct(local_evaluation_structure $evaluationstructure, $group = 0) {
        $this->evaluationstructure = $evaluationstructure;

        parent::__construct('evaluation-showentry-list-' . $evaluationstructure->get_evaluation()->id);

        $this->showall = optional_param($this->showallparamname, 0, PARAM_BOOL);
        $this->define_baseurl(new moodle_url('/local/evaluation/show_entries.php',
            ['id' => $this->evaluationstructure->get_evaluation()->id]));
        if ($this->showall) {
            $this->baseurl->param($this->showallparamname, $this->showall);
        }

        $name = format_string($evaluationstructure->get_evaluation()->name);
        $this->is_downloadable(true);
        $this->is_downloading(optional_param($this->downloadparamname, 0, PARAM_ALPHA),
                $name, get_string('responses', 'local_evaluation'));
    ...
    protected function init($group = 0) {
        ...
        $tablecolumns = array('employee_fullname');
        $tableheaders = array(get_string('le_employeename', 'local_evaluation'));

        $extrafields = get_extra_user_fields($this->get_context());
        $ufields = user_picture::fields('u', $extrafields, $this->useridfield);
        $fields = 'c.id, c.timemodified as completed_timemodified, c.evaluatedby as customcompleted_evaluateduser, concat(u.firstname,\' \',u.lastname) AS employee_fullname, '.$ufields;
        ...
        if ($this->is_downloading()) {
            ...
            /*uerpic is removed from the displaying list*/
        ...
        $tablecolumns[] = 'customcompleted_evaluateduser';
        $tableheaders[] = get_string('evaluateduser','local_evaluation');
    ...
    public function col_userpic($row) {
        ...
        return $OUTPUT->user_picture($user, array('courseid' => 1));
    }
    ...
    public function col_deleteentry($row) {
        ...
        $deleteaction = new confirm_action(get_string('confirmdeleteentry', 'local_evaluation'));
        return $OUTPUT->action_icon($deleteentryurl,
            new pix_icon('t/delete', get_string('delete_entry', 'local_evaluation')), $deleteaction);
    }
     ...
     * @return string
     */
    public function col_customcompleted_evaluateduser($student) {
             global $DB;

            return $DB->get_field('user','firstname',  array('id'=>$student->customcompleted_evaluateduser));

    }
    public function col_employee_fullname($student){
    ...
    public function col_courseid($row) {
        $courses = $this->evaluationstructure->get_completed_courses();
        $name = '';
    ...
    protected function add_all_values_to_output() {
        ...
        $items = $this->evaluationstructure->get_items(true);
        if (!$this->is_downloading()) {
        ...
        // Add evaluation response values.
        foreach ($items as $nr => $item) {
            if ($columnscount++ < self::TABLEJOINLIMIT) {
                ...
                $this->sql->from .= " LEFT OUTER JOIN {local_evaluation_value} v{$nr} " .
    ...
    public function display() {
        global $OUTPUT;
        //groups_print_activity_menu($this->evaluationstructure->get_evaluation(), $this->baseurl->out());
        ...
        if (count($this->evaluationstructure->get_items(true)) > self::PREVIEWCOLUMNSLIMIT) {
            echo $OUTPUT->notification(get_string('questionslimited', 'local_evaluation', self::PREVIEWCOLUMNSLIMIT), 'info');
        }
    ...
    public function get_reponse_navigation_links($record) {
        ...
        while ($this->rawdata->valid()) {
            ...
            if ($row->id == $record->id) {
                $page = $this->showall ? 0 : floor($counter / EVALUATION_DEFAULT_PAGE_COUNT);
                $thisrow = $row;
                $this->rawdata->next();
    ...
    protected function build_table_chunk(&$rows, &$columnsgroups) {
        ...
        foreach ($columnsgroups as $columnsgroup) {
            ...
            foreach ($columnsgroup as $nr => $item) {
                ...
                $from .= " LEFT OUTER JOIN {local_evaluation_value} v{$nr} " .
                    "ON v{$nr}.completed = c.id AND v{$nr}.item = :itemid{$nr}";
