...
/**
 * Contains class local_evaluation_templates_table
 *
...
/**
 * Class local_evaluation_templates_table
 *
 * @package   local_evaluation
 * @author Sreenivas <sreenivasula@eabyas.in>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class local_evaluation_templates_table extends flexible_table {
    ...
    public function __construct($uniqueid, $baseurl) {
        ...
        $tableheaders = array(get_string('template', 'local_evaluation'), '');
    ...
    public function display($templates) {
        global $OUTPUT;
        if (empty($templates)) {
            echo $OUTPUT->box(get_string('no_templates_available_yet', 'local_evaluation'),
        ...
        foreach ($templates as $template) {
            ...
            $deleteaction = new confirm_action(get_string('confirmdeletetemplate', 'local_evaluation'));
            $data[] = $OUTPUT->action_icon($url, new pix_icon('t/delete', $strdeleteevaluation), $deleteaction);
            $this->add_data($data);
        }
        $this->finish_output();
    }
