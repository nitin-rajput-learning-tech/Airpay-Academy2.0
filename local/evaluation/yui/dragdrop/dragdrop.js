YUI.add('moodle-local_evaluation-dragdrop', function(Y) {
    var DRAGDROPNAME = 'local_evaluation_dragdrop';
    var CSS = {
        DRAGAREA: '#evaluation_dragarea',
        DRAGITEMCLASS: 'evaluation_itemlist',
        DRAGITEM: 'div.evaluation_itemlist',
        DRAGLIST: '#evaluation_dragarea form',
    ...
    Y.extend(DRAGDROP, M.core.dragdrop, {
        ...
        initializer : function(params) {
            ...
            var groups = ['evaluationitem'];

            var handletitle = M.util.get_string('move_item', 'local_evaluation');
            ...
            listitems = basenode.all(CSS.DRAGITEM).each(function(v) {
                var item_id = this.get_node_id(v.get('id')); //Get the id of the evaluation item.
        ...
        drag_drophit_handler : function(e) {
            ...
            if (!drop.hasClass(CSS.DRAGITEMCLASS)) {
                ...
                drop.all(CSS.DRAGITEM).each(function(v) {
                    childElement = v.one('.felement').one('[id^="evaluation_item_"]');
                    if (childElement) {
        ...
        save_item_order : function(cmid, itemorder, spinner) {
            ...
            Y.io(M.cfg.wwwroot + '/local/evaluation/ajax.php', {
         ...
         *
         * @param id The dom id, f.g.: evaluation_item_22
         * @return int
         */
        get_node_id : function(id) {
            return Number(id.replace(/^.*evaluation_item_/i, ''));
        }
    ...
    M.local_evaluation = M.local_evaluation || {};
    M.local_evaluation.init_dragdrop = function(params) {
