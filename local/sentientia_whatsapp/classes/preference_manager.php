...
class preference_manager {
    ...
    public static function set(
        ...
        if (isset($values['mobile_number']) && $values['mobile_number'] !== '') {
            ...
            if (!self::is_valid_mobile($values['mobile_number'])) {
                throw new \moodle_exception('mobile_invalid', 'local_sentientia_whatsapp');
            }
        }
        ...
        $now = time();
        $existing = $DB->get_record('local_sentientia_user_channel_prefs',
            ['userid' => $userid]);
        ...
        try {
            if ($existing) {
                ...
                if (!empty($audit_pairs)) {
                    ...
                    $DB->update_record('local_sentientia_user_channel_prefs', $existing);
                ...
                $row->id = $DB->insert_record('local_sentientia_user_channel_prefs', $row);
    ...
    private static function write_audit(
        ...
        foreach ($pairs as $field => [$old, $new]) {
            $DB->insert_record('local_sentientia_user_channel_audit', (object) [
                'userid'      => $userid,
