...
/**
 * @covers \local_sentientia_whatsapp\preference_manager
 *
...
class preference_manager_test extends \advanced_testcase {
    ...
    public function test_set_creates_row_on_first_save(): void {
        ...
        $row = $DB->get_record('local_sentientia_user_channel_prefs',
    ...
    public function test_set_writes_audit_row_per_changed_field(): void {
        ...
        $before = $DB->count_records('local_sentientia_user_channel_audit',
            ['userid' => $user->id]);
        ...
        $after = $DB->count_records('local_sentientia_user_channel_audit',
            ['userid' => $user->id]);
        $this->assertGreaterThan($before, $after);

        // Verify the audit captured at least the mobile_number transition.
        $rows = $DB->get_records('local_sentientia_user_channel_audit',
    ...
    public function test_idempotent_set_no_extra_audit_rows(): void {
        ...
        $first = $DB->count_records('local_sentientia_user_channel_audit',
            ['userid' => $user->id]);
        ...
        $second = $DB->count_records('local_sentientia_user_channel_audit',
            ['userid' => $user->id]);
    ...
    public function test_delete_user_data_clears_both_tables(): void {
        ...
        $this->assertTrue($DB->record_exists('local_sentientia_user_channel_prefs',
            ['userid' => $user->id]));
        $this->assertNotEmpty($DB->get_records('local_sentientia_user_channel_audit',
            ['userid' => $user->id]));

        preference_manager::delete_user_data($user->id);

        $this->assertFalse($DB->record_exists('local_sentientia_user_channel_prefs',
            ['userid' => $user->id]));
        $this->assertEmpty($DB->get_records('local_sentientia_user_channel_audit',
            ['userid' => $user->id]));
    }
