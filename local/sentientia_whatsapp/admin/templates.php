...
$PAGE->set_url('/local/sentientia_whatsapp/admin/templates.php');
$PAGE->set_context($context);
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('templates_pagetitle', 'local_sentientia_whatsapp'));
$PAGE->set_heading(get_string('templates_heading', 'local_sentientia_whatsapp'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($new_status === 'approved' && $dlt_id !== '') {
            global $DB;
            $DB->set_field('local_sentientia_dlt_templates', 'dlt_id', $dlt_id,
                ['id' => $template_id]);
        }
        ...
        redirect(
            ...
            get_string('template_status_updated', 'local_sentientia_whatsapp'),
            2,
...
$templates = \local_sentientia_whatsapp\dlt_template_registry::list_all();
...
<div class="container-fluid">
    <p class="text-muted">
        <?= get_string('templates_intro', 'local_sentientia_whatsapp') ?>
    </p>

    <table class="generaltable" style="width: 100%;">
        <thead>
            <tr>
                <th><?= get_string('th_template', 'local_sentientia_whatsapp') ?></th>
                <th><?= get_string('th_channel', 'local_sentientia_whatsapp') ?></th>
                <th><?= get_string('th_status', 'local_sentientia_whatsapp') ?></th>
                <th><?= get_string('th_dlt_id', 'local_sentientia_whatsapp') ?></th>
                <th><?= get_string('th_body', 'local_sentientia_whatsapp') ?></th>
                <th><?= get_string('th_actions', 'local_sentientia_whatsapp') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($templates as $t): ?>
                ...
                <tr>
                    ...
                    <td>
                        <details>
                            <summary><?= get_string('show_body', 'local_sentientia_whatsapp') ?></summary>
                        ...
                        <form method="post" style="display: inline-flex; gap: 4px; flex-wrap: wrap; align-items: center;">
                            ...
                            <?php if ($t->status === 'pending'): ?>
                                <button type="submit" name="new_status" value="submitted" class="btn btn-sm btn-info">
                                    <?= get_string('btn_submit', 'local_sentientia_whatsapp') ?>
                                </button>
                            <?php elseif ($t->status === 'submitted'): ?>
                                ...
                                <button type="submit" name="new_status" value="approved" class="btn btn-sm btn-success">
                                    <?= get_string('btn_approve', 'local_sentientia_whatsapp') ?>
                                </button>
                                <button type="submit" name="new_status" value="rejected" class="btn btn-sm btn-danger">
                                    <?= get_string('btn_reject', 'local_sentientia_whatsapp') ?>
                                </button>
                            <?php elseif ($t->status === 'rejected'): ?>
                                <button type="submit" name="new_status" value="pending" class="btn btn-sm btn-warning">
                                    <?= get_string('btn_redraft', 'local_sentientia_whatsapp') ?>
                                </button>
                            <?php elseif ($t->status === 'approved'): ?>
                                <span class="text-muted small"><?= get_string('approved_ready', 'local_sentientia_whatsapp') ?></span>
                            <?php endif; ?>
                        </form>
                    </td>
                </tr>
