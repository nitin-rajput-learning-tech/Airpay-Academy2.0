...
class aws_verifier implements identity_verifier_interface {
    ...
    public function verify(string $id_photo_bytes, string $selfie_bytes): array {
        ...
        $key    = (string) get_config('local_sentientia_proctoring', 'aws_key');
        $secret = (string) get_config('local_sentientia_proctoring', 'aws_secret');
        $threshold = (int) (get_config('local_sentientia_proctoring', 'match_threshold') ?: 85);
