...
/**
 * End-to-end smoke for sentientia_proctoring.
 *
...
set_config('provider', 'mock', 'local_sentientia_proctoring');
set_config('default_reviewer', 2, 'local_sentientia_proctoring');

echo "=== sentientia_proctoring smoke test ===\n\n";
...
// === 2. Consent ===
echo "\n=== 2. Consent ===\n";
$session = \local_sentientia_proctoring\session_manager::record_consent(
...
$session = $DB->get_record('local_sentientia_proctor_sessions', ['id' => $session->id]);
...
$session2 = \local_sentientia_proctoring\session_manager::start_session(
    (int) $user->id, (int) $quiz->id);
$id_row2 = \local_sentientia_proctoring\session_manager::submit_identity(
...
// === 4. Record events ===
echo "\n=== 4. Record events ===\n";
\local_sentientia_proctoring\session_manager::record_event(
...
$ev_count = $DB->count_records('local_sentientia_proctor_events',
...
\local_sentientia_proctoring\session_manager::register_chunk(
    (int) $session->id, 'webcam', 0, "sess{$session->id}/cam_0.webm",
    1024 * 512, 30000);
\local_sentientia_proctoring\session_manager::register_chunk(
    (int) $session->id, 'screen', 0, "sess{$session->id}/scr_0.webm",
    1024 * 1024, 30000);
$chunks = $DB->count_records('local_sentientia_proctor_recordings',
    ['sessionid' => $session->id]);
$check('Chunks registered', $chunks === 2);
$retain = $DB->get_field('local_sentientia_proctor_recordings', 'retain_until',
...
echo "\n=== 6. Finalize ===\n";
$session = \local_sentientia_proctoring\session_manager::finalize((int) $session->id);
...
$session = \local_sentientia_proctoring\session_manager::submit_review(
...
$review_count = $DB->count_records('local_sentientia_proctor_reviews',
    ['sessionid' => $session->id]);
...
$session3 = \local_sentientia_proctoring\session_manager::start_session(
    (int) $user->id, (int) $quiz->id);
\local_sentientia_proctoring\session_manager::submit_identity(
    (int) $session3->id, (int) $user->id, 'ID', 'SELFIE');
for ($i = 0; $i < 3; $i++) {
    \local_sentientia_proctoring\session_manager::record_event(
        (int) $session3->id, 'multiple_faces', 'critical', []);
}
$session3 = \local_sentientia_proctoring\session_manager::finalize((int) $session3->id);
// 3 × 25 = 75 → fail
...
if (!empty($session_ids)) {
    ...
    $DB->delete_records_select('local_sentientia_proctor_events',
        "sessionid $insql", $inparams);
    $DB->delete_records_select('local_sentientia_proctor_recordings',
        "sessionid $insql", $inparams);
    $DB->delete_records_select('local_sentientia_proctor_reviews',
        "sessionid $insql", $inparams);
}
$DB->delete_records('local_sentientia_proctor_identity', ['userid' => $user->id]);
$DB->delete_records('local_sentientia_proctor_sessions', ['userid' => $user->id]);
