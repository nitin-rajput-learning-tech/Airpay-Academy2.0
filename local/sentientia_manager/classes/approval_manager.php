...
class approval_manager {
    ...
    public static function create_request(int $userid, int $courseid,
        ...
        if ($DB->record_exists_select('local_sentientia_mgr_requests',
                "userid = :uid AND courseid = :cid AND status = :st",
                ['uid' => $userid, 'cid' => $courseid, 'st' => self::STATUS_PENDING])) {
            throw new \moodle_exception('duplicaterequest', 'local_sentientia_manager');
        }

        $now = time();
        return (int) $DB->insert_record('local_sentientia_mgr_requests', (object) [
            'userid'      => $userid,
            'courseid'    => $courseid,
            'managerid'   => $managerid,
    ...
    public static function list_requests(int $managerid, string $status = 'pending',
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_sentientia_mgr_requests} r
    ...
    public static function decide_request(int $requestid, string $decision,
        ...
        $request = $DB->get_record('local_sentientia_mgr_requests',
            ['id' => $requestid], '*', MUST_EXIST);
        if ($request->status !== self::STATUS_PENDING) {
            throw new \moodle_exception('alreadydecided', 'local_sentientia_manager');
        }
        ...
        try {
            ...
            $DB->update_record('local_sentientia_mgr_requests', $request);
            $tx->allow_commit();
        } catch (\Throwable $e) {
            $tx->rollback($e);
        }
    ...
    public static function notify_requester_of_decision(\stdClass $request,
        ...
        $eventdata->component         = 'local_sentientia_manager';
    ...
    public static function enrol_user_in_course(int $userid, int $courseid,
        ...
        if (!$enrolinstance) {
            throw new \moodle_exception('manualenrolnotavailable', 'local_sentientia_manager');
        }
    ...
    public static function list_allocations(int $managerid, string $status = 'all',
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_sentientia_mgr_allocations} a
    ...
    public static function create_allocation(int $managerid, int $userid, int $courseid,
        ...
        $tx = $DB->start_delegated_transaction();
        try {
            $id = (int) $DB->insert_record('local_sentientia_mgr_allocations', (object) [
                'managerid'    => $managerid,
                'userid'       => $userid,
    ...
    //   3. insert one row into local_sentientia_mgr_allocations with the new
    ...
    private static function allocation_exists(int $userid, string $item_type,
                                                int $itemid): bool {
        global $DB;
        return $DB->record_exists('local_sentientia_mgr_allocations', [
            'userid'    => $userid,
            'item_type' => $item_type,
            'itemid'    => $itemid,
        ]);
    ...
    private static function insert_allocation_row(int $managerid, int $userid,
        ...
        global $DB;
        $now = time();
        return (int) $DB->insert_record('local_sentientia_mgr_allocations', (object) [
            'managerid'    => $managerid,
            'userid'       => $userid,
            'item_type'    => $item_type,
            'itemid'       => $itemid,
    ...
    private static function notify_assignee_of_typed_allocation(int $managerid,
        ...
        try {
            ...
            $msg->component         = 'local_sentientia_manager';
    ...
    public static function delete_allocation(int $id): void {
        global $DB;
        $DB->delete_records('local_sentientia_mgr_allocations', ['id' => $id]);
    }
    ...
    public static function pending_request_count(int $managerid): int {
        global $DB;
        return (int) $DB->count_records('local_sentientia_mgr_requests',
