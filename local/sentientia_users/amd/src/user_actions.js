 ...
 *
 * @module     local_sentientia_users/user_actions
...
const openUserForm = async (userid, returnFocus) => {
    ...
    const title = await getString(titleKey, 'local_sentientia_users');

    const modalForm = new ModalForm({
        formClass: 'local_sentientia_users\\form\\edit_user',
        args: {userid: userid},
...
const confirmSuspend = async (userid, username, suspend, returnFocus) => {
    ...
    const [title, message, successMsg] = await Promise.all([
        getString(titleKey, 'local_sentientia_users'),
        getString(msgKey, 'local_sentientia_users', username),
        getString(successKey, 'local_sentientia_users'),
    ]);
    ...
    Notification.confirm(
        ...
        async () => {
            try {
                await Ajax.call([{
                    methodname: 'local_sentientia_users_suspend_user',
...
const confirmDelete = async (userid, username, returnFocus) => {
    const [title, message, deleteLabel, successMsg] = await Promise.all([
        getString('deleteuser', 'local_sentientia_users'),
        getString('confirmdelete', 'local_sentientia_users', username),
        getString('delete', 'core'),
        getString('userdeleted', 'local_sentientia_users'),
    ]);
    ...
    Notification.deleteCancelPromise(title, message, deleteLabel, returnFocus)
        .then(async () => {
            try {
                await Ajax.call([{
                    methodname: 'local_sentientia_users_delete_user',
                    args: {userid: userid},
                }])[0];
                Notification.addNotification({message: successMsg, type: 'success'});
                window.location.reload();
