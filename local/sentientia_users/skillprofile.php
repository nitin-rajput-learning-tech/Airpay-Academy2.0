...
if (!empty($user->open_designation)) {
    $expectations = $DB->get_records('local_sentientia_role_skills',
...
foreach ($all_skill_ids as $sid) {
    $skill = $DB->get_record('local_sentientia_skills', ['id' => $sid]);
    if (!$skill) continue;
    $current = 0;
    ...
    $skill_rows[] = [
        ...
        'view_url' => (new moodle_url('/local/sentientia_skills/view.php',
            ['id' => $sid]))->out(false),
    ];
...
$data = [
    ...
    'grade_url'        => $grade_url,
    'profile_url'      => (new moodle_url('/local/sentientia_users/profile.php',
        ['id' => $userid]))->out(false),
    'is_self'          => $is_self,
];

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_sentientia_users/skillprofile', $data);
echo $OUTPUT->footer();
