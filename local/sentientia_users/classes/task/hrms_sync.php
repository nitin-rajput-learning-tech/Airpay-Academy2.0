...
namespace local_sentientia_users\task;
 ...
 *
 * Closes audit item #4 from parity-audit-2026-05-15/sentientia_users.md
...
class hrms_sync extends \core\task\scheduled_task {
    ...
    public function get_name(): string {
        return get_string('task_hrms_sync', 'local_sentientia_users');
    }

    public function execute(): void {
        $mode = (string) get_config('local_sentientia_users', 'hrms_sync_mode');
        if ($mode === '' || $mode === 'disabled') {
            mtrace('local_sentientia_users hrms_sync: disabled (no source configured)');
            return;
        }

        mtrace('local_sentientia_users hrms_sync: starting (mode=' . $mode . ')');

        try {
            $csv = $this->fetch_csv($mode);
        } catch (\Throwable $e) {
            mtrace('local_sentientia_users hrms_sync: fetch failed: ' . $e->getMessage());
            return;
        }

        if ($csv === '' || $csv === false) {
            mtrace('local_sentientia_users hrms_sync: empty CSV — nothing to do');
            return;
        }
        ...
        $runner_userid = (int) (get_config('local_sentientia_users', 'hrms_sync_user_id')
            ?: 2);
        ...
        set_config('hrms_sync_last_run_id', $run_id,  'local_sentientia_users');

        global $DB;
        $run = $DB->get_record('local_sentientia_users_sync_runs', ['id' => $run_id]);
        if ($run) {
            mtrace(sprintf(
                'local_sentientia_users hrms_sync: run #%d finished. '
    ...
    private function fetch_url(): string {
        ...
        $url = (string) get_config('local_sentientia_users', 'hrms_sync_url');
        if ($url === '') {
            throw new \moodle_exception('hrms_sync_url_empty', 'local_sentientia_users');
        }

        $auth_header = (string) get_config('local_sentientia_users',
        ...
        if ($http_code < 200 || $http_code >= 300) {
            throw new \moodle_exception('hrms_sync_url_http_error',
                'local_sentientia_users', '', "HTTP $http_code from $url");
        }
    ...
    private function fetch_filesystem(): string {
        $path = (string) get_config('local_sentientia_users', 'hrms_sync_path');
        if ($path === '') {
            throw new \moodle_exception('hrms_sync_path_empty', 'local_sentientia_users');
        }
