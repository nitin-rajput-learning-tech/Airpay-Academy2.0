...
class user_manager {
    ...
    public static function get_role_names(int $userid): string {
        ...
        return get_string('employee', 'local_sentientia_users');
    }
    ...
    public static function build_profile_context(int $userid): array {
        ...
        if (file_exists($CFG->dirroot . '/local/sentientia_skills/classes/skills_manager.php')) {
            try {
                ...
                $gap_courses = \local_sentientia_skills\skills_manager::get_gap_courses($userid, 5);
        ...
        foreach ($pluginlist as $pluginname => $pluginurl) {
            ...
            if (class_exists($userclass) && method_exists($userclass, 'user_profile_content')) {
                ...
                if ($pluginname !== 'users' && $pluginname !== 'sentientia_users') {
    ...
    public static function create(object $data): int {
        ...
        if (empty($data->username) || empty($data->email) ||
            ...
            throw new \moodle_exception('missingrequiredfields', 'local_sentientia_users');
        }

        // Check uniqueness.
        if ($DB->record_exists('user', ['username' => strtolower($data->username),
                                         'mnethostid' => $CFG->mnet_localhost_id])) {
            throw new \moodle_exception('usernametaken', 'local_sentientia_users');
        }
        if ($DB->record_exists('user', ['email' => $data->email, 'deleted' => 0])) {
            throw new \moodle_exception('emailtaken', 'local_sentientia_users');
        }
        ...
        if (!empty($data->emailwelcome) && $password) {
            try {
                ...
                debugging('local_sentientia_users: welcome email failed for user '
    ...
    public static function delete(int $userid): bool {
        ...
        global $USER;
        if ($userid == $USER->id) {
            throw new \moodle_exception('cannotdeleteself', 'local_sentientia_users');
        }
        if ($userid <= 2) {
            throw new \moodle_exception('cannotdeletesystemuser', 'local_sentientia_users');
        }
