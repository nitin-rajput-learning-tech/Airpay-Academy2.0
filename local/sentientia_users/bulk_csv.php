...
require_capability('local/sentientia_users:bulkstatuschange', $context);

$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/local/sentientia_users/bulk_csv.php'));
...
class local_sentientia_users_bulkcsv_form extends moodleform {
...
$form = new local_sentientia_users_bulkcsv_form();
$summary = null;
...
if ($data = $form->get_data()) {
    ...
    if ($content === false || $content === '') {
        ...
        try {
            $summary = \local_sentientia_users\bulk_csv_processor::process(
...
if ($summary) {
    ...
    echo $OUTPUT->render_from_template('local_sentientia_users/bulk_csv_summary',
        $table_data);
}
