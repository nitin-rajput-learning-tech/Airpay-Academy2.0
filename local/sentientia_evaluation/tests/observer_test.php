...
final class observer_test extends \advanced_testcase {
    ...
    private function seed_evaluation(string $trigger_event, int $days_after = 0,
        ...
        return (int) $DB->insert_record('local_sentientia_evaluation', $rec);
    }
    ...
    public function test_course_completed_queues_trigger(): void {
        ...
        $row = $DB->get_record('local_sentientia_evaluation_triggers', [
            'evaluationid' => $eid,
            'userid'       => $u->id,
            'itemid'       => $course->id,
        ]);
    ...
    public function test_archived_form_does_not_queue(): void {
        ...
        $DB->set_field('local_sentientia_evaluation', 'status',
    ...
    public function test_process_due_triggers_fires_past_rows(): void {
        ...
        $this->assertEquals(1, $DB->count_records('local_sentientia_evaluation_responses', [
            'evaluationid' => $eid,
            'userid'       => $u->id,
            'courseid'     => $course->id,
        ]));
