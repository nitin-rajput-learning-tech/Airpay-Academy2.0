...
const openEvalForm = async (evaluationid, returnFocus) => {
    ...
    const title = await getString(titleKey, 'local_sentientia_evaluation');
...
const confirmStatus = async (evaluationid, name, newStatus, msgKey, returnFocus) => {
    ...
    const [title, message, success] = await Promise.all([
        getString(titleKey, 'local_sentientia_evaluation'),
        getString(msgKey, 'local_sentientia_evaluation', name),
        getString('evaluationstatuschanged', 'local_sentientia_evaluation'),
    ]);
...
const confirmDelete = async (evaluationid, name, returnFocus) => {
    const [title, message, label, success] = await Promise.all([
        getString('deleteevaluation', 'local_sentientia_evaluation'),
        getString('confirmdelete', 'local_sentientia_evaluation', name),
        getString('delete', 'core'),
        getString('evaluationdeleted', 'local_sentientia_evaluation'),
    ]);
