...
class learningplan_courses_created extends \core\event\base {
    /**
     * Init method.
     *
     * @return void
    ...
    public function get_description() {
        ...
        // $courseid=$DB->get_field_sql("SELECT courseid FROM {local_learningplan_courses} where id=$this->objectid");
        if($courseid){
            ...
            return "The user with id '$firstname ($this->userid)' has added course with id '$learningplan_courses_name ($courseid)' in the Learning Path with id '$lpname ($learningplan_id) '.";
        }else{
            return "The user with id '$firstname ($this->userid)' has added course with id '$this->objectid' in the Learning Path.";
        }
    ...
    public static function get_name() {
        return get_string('eventlearningplancourses_created', 'local_learningplan');
    }
    ...
    public function get_url() {
        ...
        $learningplan_id=$DB->get_field_sql("SELECT planid FROM {local_learningplan_courses} where id=$this->objectid");
        if($learningplan_id){
            $url = new \moodle_url('/local/learningplan/plan_view.php',array('id'=>$learningplan_id));
        }else{
