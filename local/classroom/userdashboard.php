...
if ($formattype == 'card') {
    ...
    $display_icon = get_string('listicon','local_classroom');
} else {
    $formattype_url = 'card';
...
$PAGE->requires->js_call_amd('block_userdashboard/userdashboard', 'makeActive',array('identifier' => 'classroom_'.$tab));
...
$filterparams = $renderer->get_userdashboard_classroom($tab, true,$formattype);
echo $OUTPUT->header();
 $display_url = new moodle_url('/local/classroom/userdashboard.php',array('tab' => $tab ,'formattype' => $formattype_url));
  ...
  $displaytype_div .= '</a>';
  $displaytype_div .= '</div>';
  // $displaytype_div = '<div class="col-12 d-inline-block"><a class="pull-right btn btn-outline-secondary cardlist_view" id="card_list_view_detailed" data-displaytype="table">
  //                      <i class="icon fa fa-fw fa-bars"></i><span>List</span> </a></div>';
  echo $displaytype_div;

echo $OUTPUT->render_from_template('local_classroom/userdashboard_inner_tab', array());
echo $OUTPUT->render_from_template('local_costcenter/global_filter', $filterparams);
$content = $renderer->get_userdashboard_classroom($tab,false,$formattype);
