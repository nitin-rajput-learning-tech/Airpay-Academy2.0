...
/**
 * Smoke test for sentientia_recompletion — rule lifecycle + reset.
 *
...
$rule->id = $DB->insert_record('local_sentientia_recompletion_rules', $rule);
...
$rule = $DB->get_record('local_sentientia_recompletion_rules', ['id' => $rule->id]);
echo "  (rule reloaded — trigger_type=$rule->trigger_type courseid=$rule->courseid)\n";
$r_dry = \local_sentientia_recompletion\recompletion_engine::run_rule($rule, true);
...
$dry_history = $DB->count_records('local_sentientia_recompletion_history',
    ['ruleid' => $rule->id, 'dryrun' => 1]);
$check('Dryrun history rows recorded', $dry_history >= 1);

// === 4. Run rule for real ===
echo "\n=== 4. Run rule (real) ===\n";
$r = \local_sentientia_recompletion\recompletion_engine::run_rule($rule, false);
...
$real_history = $DB->count_records('local_sentientia_recompletion_history',
...
echo "\n=== 5. Idempotent re-run ===\n";
$r2 = \local_sentientia_recompletion\recompletion_engine::run_rule($rule, false);
...
$bulk = \local_sentientia_recompletion\recompletion_engine::bulk_reset(
    (int) $course->id, [(int) $user->id], 2, 'bulk', true, true);
$check('Bulk reset returns reset=1', $bulk['reset'] === 1);
$check('Audit row marks reason=bulk',
    $DB->record_exists('local_sentientia_recompletion_history',
        ['ruleid' => 0, 'userid' => $user->id, 'reason' => 'bulk']));

// === 7. Engine run_all ===
echo "\n=== 7. run_all() with multiple rules ===\n";
$totals = \local_sentientia_recompletion\recompletion_engine::run_all(true);
...
// Cleanup.
$DB->delete_records('local_sentientia_recompletion_rules', ['id' => $rule->id]);
