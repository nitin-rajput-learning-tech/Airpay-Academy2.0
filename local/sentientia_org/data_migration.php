...
/**
 * Data migration — copy local_costcenter → local_sentientia_org.
 *
 * Run via CLI:
 *   php local/sentientia_org/data_migration.php
 *
...
if (!$dbman->table_exists('local_costcenter')) {
    cli_writeln(get_string('sourcetablemissing', 'local_sentientia_org'));
    exit(0);
}

// Check target table exists.
if (!$dbman->table_exists('local_sentientia_org')) {
    cli_error('Target table local_sentientia_org does not exist. Run Site Admin → Notifications first.');
}

// Skip if target already has data.
$existing = $DB->count_records('local_sentientia_org');
if ($existing > 0) {
    cli_writeln(get_string('migrationskipped', 'local_sentientia_org'));
...
cli_writeln("Migrating {$sourcecount} records from local_costcenter → local_sentientia_org ...");
...
try {
    foreach ($sources as $src) {
        ...
        $DB->import_record('local_sentientia_org', $record);
        $migrated++;
    }

    // Reset the auto-increment sequence to continue after highest ID.
    $maxid = $DB->get_field_sql("SELECT MAX(id) FROM {local_sentientia_org}");
    if ($maxid) {
    ...
    cli_writeln(get_string('migrationcomplete', 'local_sentientia_org'));
    cli_writeln("  Migrated: {$migrated}/{$sourcecount} records.");

    // Verify counts match.
    $finalcount = $DB->count_records('local_sentientia_org');
