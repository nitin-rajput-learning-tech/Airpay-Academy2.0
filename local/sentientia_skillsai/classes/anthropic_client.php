...
class anthropic_client {
    ...
    public static function extract(string $sourcetext, int $maxskills, string $model = self::DEFAULT_MODEL, ?array $promptctx = null): array {
        ...
        // (sentientia.skillsai.enabled / .live_api / per-call [CONFIRM] —
        // all still enforced here or in the UI). Our mock is passed down
        // as a callable so mock fidelity (incl. v2-hindi output) stays
        // byte-identical. The skillsai live_api flag above remains
        // authoritative for THIS plugin's live intent.
        ...
        if ($routegateway) {
            $mockfn = function (array $req) use ($sourcetext, $maxskills, $promptctx): string {
                return self::call_mock($sourcetext, $maxskills, $promptctx)['body'];
            };
            if (!$islive) {
                ...
                $result = \local_sentientia_ai\client::complete([
                    'component'        => 'local_sentientia_skillsai',
                    'purpose'          => 'skill_extraction',
                    'usertext'         => '[mock-intent] ' . mb_substr($sourcetext, 0, 200),
                    'mock'             => $mockfn,
                    'legacy_component' => 'local_sentientia_skillsai',
                ]);
            ...
            // the UI persists a retriable failed job. 'mock' here means the
            // gateway's live flags are still OFF (Addendum-A gating) —
