...
function xmldb_local_sentientia_request_install(): void {
    global $DB;
    ...
    // ('local_sentientia_request'), NOT the slash form ('local/sentientia_request').
    // Slash form silently returns 0 caps registered — caught by Phase 7
    // multi-role UAT (commit 3b117b664+1).
    update_capabilities('local_sentientia_request');

    $context = \context_system::instance();
    $rolemap = [
        'employee' => [
            ['local/sentientia_request:request', CAP_ALLOW],
        ],
        'administrator' => [
            ['local/sentientia_request:request',        CAP_ALLOW],
            ['local/sentientia_request:approve',        CAP_ALLOW],
            ['local/sentientia_request:viewall',        CAP_ALLOW],
            ['local/sentientia_request:overrideroute',  CAP_ALLOW],
        ],
    ];
