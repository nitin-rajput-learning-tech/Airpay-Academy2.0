...
/**
 * CRUD tests for sentientia_notifications.
 *
...
final class crud_test extends \advanced_testcase {
    ...
    public function test_toggle_enabled_persists(): void {
        ...
        $this->assertEquals(0, (int) $DB->get_field('local_sentientia_notif_rules', 'enabled', ['id' => $rid]));

        rule_manager::toggle_enabled($rid, true);
        $this->assertEquals(1, (int) $DB->get_field('local_sentientia_notif_rules', 'enabled', ['id' => $rid]));
    }
    ...
    public function test_delete_removes_rule(): void {
        ...
        $this->assertFalse($DB->record_exists('local_sentientia_notif_rules', ['id' => $rid]));
    }
