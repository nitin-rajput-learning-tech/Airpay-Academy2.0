 ...
 *
 * - Capability gate: callers without local/sentientia_notifications:manage are rejected
...
final class list_rules_test extends \advanced_testcase {
    ...
    private function seed_rule(string $name, int $enabled = 1, string $rule_type = 'deadline_warning'): int {
        global $DB;
        if (!$DB->get_manager()->table_exists('local_sentientia_notif_rules')) {
            $this->markTestSkipped('local_sentientia_notif_rules table not present.');
        }
        $now = time();
    ...
    private function wipe_rules(): void {
        global $DB;
        if ($DB->get_manager()->table_exists('local_sentientia_notif_rules')) {
            $DB->delete_records('local_sentientia_notif_rules');
        }
    }

    /**
     * Create a user with local/sentientia_notifications:manage capability.
     */
    private function user_with_manage(): \stdClass {
        $u = $this->getDataGenerator()->create_user();
        $sysctx = \context_system::instance();
        $roleid = $this->getDataGenerator()->create_role();
        role_change_permission($roleid, $sysctx, 'local/sentientia_notifications:manage', CAP_ALLOW);
