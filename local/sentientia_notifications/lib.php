 ...
 */
function local_sentientia_notifications_get_for_navbar(int $userid, int $limit = 10): array {
    global $DB;
    ...
    foreach ($records as $r) {
        ...
        $type = 'system';
        $rule = $DB->get_record('local_sentientia_notif_rules', ['id' => $r->ruleid], 'rule_type');
        if ($rule) {
            $typemap = [
 ...
 */
function local_sentientia_notifications_mark_read(int $notifid, int $userid): bool {
    global $DB;
    return $DB->set_field_select('local_sentientia_notif_log',
        'status', 'read',
 ...
 */
function local_sentientia_notifications_mark_all_read(int $userid): void {
    global $DB;
    $DB->execute(
        "UPDATE {local_sentientia_notif_log} SET status = 'read', timeread = :now
