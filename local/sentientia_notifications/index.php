...
$PAGE->set_url(new moodle_url('/local/sentientia_notifications/index.php'));
$PAGE->set_title('Notification Rules');
$PAGE->set_heading('Notification Rules');
$PAGE->set_pagelayout('standard');
$PAGE->set_secondary_navigation(false);

$can_manage = is_siteadmin() || has_capability('local/sentientia_notifications:manage', $context);

$dbman = $DB->get_manager();
$total = $enabled = $disabled = 0;
if ($dbman->table_exists('local_sentientia_notif_rules')) {
    $total    = (int) $DB->count_records('local_sentientia_notif_rules');
    $enabled  = (int) $DB->count_records('local_sentientia_notif_rules', ['enabled' => 1]);
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_sentientia_notifications/manage', $data);
echo $OUTPUT->footer();
