...
class rule_engine {
    ...
    public static function process_all(): array {
        ...
        $rules = $DB->get_records('local_sentientia_notif_rules', ['enabled' => 1]);
    ...
    private static function rule_peer_completion_celebration(\stdClass $rule): array {
        global $DB;
        $result = ['sent' => 0, 'skipped' => 0];
        $batchlimit = (int) (get_config('local_sentientia_notifications', 'batch_limit') ?: 100);
        $day_ago = time() - 86400;
    ...
    private static function send(\stdClass $rule, int $userid, ?int $courseid,
        ...
        try {
            $exists = $DB->record_exists_select('local_sentientia_notif_log',
            ...
            $logid = $DB->insert_record('local_sentientia_notif_log', $logrecord);
            $transaction->allow_commit();
        } catch (\Throwable $e) {
        ...
        if (!empty($templatekey) && class_exists('\\local_sentientia_emails\\email_renderer')) {
            ...
            if ($user) {
                ...
                try {
                    $html = \local_sentientia_emails\email_renderer::render(
                        'local_sentientia_emails/' . $templatekey, $tplcontext, $userid
                    );
                } catch (\Exception $e) {
