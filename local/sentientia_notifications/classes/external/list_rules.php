...
class list_rules extends external_api {
    ...
    public static function execute(string $search = '', string $sort = 'name', string $sortdir = 'asc',
        ...
        if (strlen($params['filters']) > 4096) throw new \moodle_exception('filterstoolong', 'local_sentientia_notifications');
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_sentientia_notif_rules} r WHERE $wheresql", $sqlparams);

        $records = [];
        if ($total > 0) {
            $records = $DB->get_records_sql(
                "SELECT r.* FROM {local_sentientia_notif_rules} r
                  WHERE $wheresql
