...
class waitlist_manager {
    ...
    public static function join(int $classroomid, int $userid): \stdClass {
        ...
        // Already enrolled?
        if ($DB->record_exists('local_sentientia_classroom_users',
            ['classroomid' => $classroomid, 'userid' => $userid])) {
            throw new \moodle_exception('alreadyenrolled', 'local_sentientia_classroom');
        }

        // Already in queue?
        $existing = $DB->get_record('local_sentientia_classroom_waitlist',
        ...
        $max_pos = (int) $DB->get_field_sql(
            "SELECT MAX(position) FROM {local_sentientia_classroom_waitlist}
        ...
        $row->id = $DB->insert_record('local_sentientia_classroom_waitlist', $row);
        return $row;
    }
    ...
    public static function leave(int $waitlistid, int $userid): bool {
        global $DB;
        $row = $DB->get_record('local_sentientia_classroom_waitlist',
        ...
        $DB->update_record('local_sentientia_classroom_waitlist', $row);
    ...
    public static function auto_promote(int $classroomid): int {
        ...
        try {
            ...
            $DB->update_record('local_sentientia_classroom_waitlist', $head);
    ...
    private static function renumber_positions(int $classroomid): void {
        ...
        foreach ($rows as $r) {
            if ((int) $r->position !== $pos) {
                $r->position = $pos;
                $r->timemodified = time();
                $DB->update_record('local_sentientia_classroom_waitlist', $r);
            }
            $pos++;
        }
    }
    ...
    private static function notify_promotion(int $classroomid, int $userid): void {
        ...
        $msg->component         = 'local_sentientia_classroom';
