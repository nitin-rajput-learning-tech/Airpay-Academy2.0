...
class edit_session extends \core_form\dynamic_form {
    ...
    protected function definition() {
        ...
        $mform->addElement('text', 'title',
            get_string('session_title', 'local_sentientia_classroom'),
        ...
        $mform->addElement('date_time_selector', 'starttime',
            get_string('session_starttime', 'local_sentientia_classroom'),
            ['step' => 5, 'optional' => false]);

        $mform->addElement('date_time_selector', 'endtime',
            get_string('session_endtime', 'local_sentientia_classroom'),
            ['step' => 5, 'optional' => false]);

        $mform->addElement('text', 'location',
            get_string('session_location', 'local_sentientia_classroom'),
        ...
        $mform->addElement('autocomplete', 'trainerid',
            get_string('session_trainer', 'local_sentientia_classroom'), [], $trainer_options);
        $mform->setType('trainerid', PARAM_INT);

        $mform->addElement('textarea', 'notes',
            get_string('session_notes', 'local_sentientia_classroom'),
            ['rows' => 3, 'cols' => 50]);
        ...
        $mform->addElement('header', 'virtualheader',
            get_string('session_virtual_header', 'local_sentientia_classroom'));
        $mform->setExpanded('virtualheader', false);

        $mform->addElement('url', 'meeting_url',
            get_string('session_meeting_url', 'local_sentientia_classroom'),
            ['size' => 60, 'placeholder' => 'https://airpay.zoom.us/j/123456789'],
            ['usefilepicker' => false]);
        $mform->setType('meeting_url', PARAM_URL);
        $mform->addHelpButton('meeting_url', 'session_meeting_url', 'local_sentientia_classroom');

        $mform->addElement('url', 'recording_url',
            get_string('session_recording_url', 'local_sentientia_classroom'),
            ['size' => 60, 'placeholder' => 'https://airpay.zoom.us/rec/share/...'],
            ['usefilepicker' => false]);
        $mform->setType('recording_url', PARAM_URL);
        $mform->addHelpButton('recording_url', 'session_recording_url', 'local_sentientia_classroom');
    }
    ...
    public function validation($data, $files) {
        ...
        if ($start <= 0) {
            $errors['starttime'] = get_string('invalidsessiontime', 'local_sentientia_classroom');
        }
        if ($end <= 0) {
            $errors['endtime'] = get_string('invalidsessiontime', 'local_sentientia_classroom');
        }
    ...
    public function process_dynamic_submission() {
        ...
        if ($sessionid === 0) {
            $newid = \local_sentientia_classroom\session_manager::create_session($classroomid, $data);
            return [
                'classroomid' => $classroomid,
                'sessionid'   => $newid,
                'message'     => get_string('session_created', 'local_sentientia_classroom'),
            ];
        }

        \local_sentientia_classroom\session_manager::update_session($sessionid, $data);
        return [
            'classroomid' => $classroomid,
            'sessionid'   => $sessionid,
            'message'     => get_string('session_updated', 'local_sentientia_classroom'),
        ];
    }
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        $s = $DB->get_record('local_sentientia_classroom_sessions',
