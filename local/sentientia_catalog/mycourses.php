...
$PAGE->set_url(new moodle_url('/local/sentientia_catalog/mycourses.php'));
...
foreach ($enrolledcourses as $course) {
    ...
    try {
        if (!empty($course->open_categoryid)) {
            $categoryname = \local_sentientia_catalog\category_manager::get_name((int)$course->open_categoryid);
        }
...
for ($i = 0; $i < $totalpages; $i++) {
    $pages[] = [
        ...
        'label' => $i + 1,
        'active' => ($i === $page),
        'url' => (new moodle_url('/local/sentientia_catalog/mycourses.php',
...
$data = [
    ...
    'baseurl'       => (new moodle_url('/local/sentientia_catalog/mycourses.php'))->out(false),
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_sentientia_catalog/mycourses', $data);
echo $OUTPUT->footer();
