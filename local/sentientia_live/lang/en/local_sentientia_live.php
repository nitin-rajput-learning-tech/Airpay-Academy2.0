...
$string['setting_sse_max_connections_desc']     = 'Hard cap on how many Server-Sent-Events streams may be open at once across the whole site. Once reached, new connection attempts get an immediate HTTP 503 with Retry-After instead of a stream. Default 4. Every open stream holds one PHP worker for the whole stream lifetime, so keep this BELOW the PHP-FPM pool size (pm.max_children) or Apache MaxRequestWorkers, leaving headroom for ordinary page requests (UAT: pm.max_children = 5, cap = 3). A fixed cap of 2 concurrent streams per trainer or per audience participant also applies and is not configurable here.';
