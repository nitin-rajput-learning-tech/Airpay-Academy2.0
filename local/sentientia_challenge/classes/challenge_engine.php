...
class challenge_engine {
    ...
    public static function create_challenge(array $data): int {
        ...
        return (int) $DB->insert_record('local_sentientia_challenge_challenges', $row);
    }
    ...
    public static function update_challenge(int $id, array $data): void {
        ...
        if (!empty($data['shortname']) && $data['shortname'] !== $existing->shortname
                && $DB->record_exists_select('local_sentientia_challenge_challenges',
        ...
        $DB->update_record('local_sentientia_challenge_challenges', $upd);
    }
    ...
    public static function delete_challenge(int $id): void {
        ...
        try {
            $DB->delete_records('local_sentientia_challenge_attempts',     ['challengeid' => $id]);
            $DB->delete_records('local_sentientia_challenge_leaderboard',  ['challengeid' => $id]);
            $DB->delete_records('local_sentientia_challenge_challenges',   ['id' => $id]);
            $tx->allow_commit();
        } catch (\Throwable $e) {
            $tx->rollback($e);
        }
    ...
    public static function join(int $challengeid, int $userid): int {
        ...
        if (!empty($challenge->cohortid)) {
            ...
            if (!$member) {
                throw new \moodle_exception('err_outside_cohort', 'local_sentientia_challenge');
            }
        }

        // Idempotency: don't double-join.
        if ($existing = $DB->get_record('local_sentientia_challenge_attempts',
                ['challengeid' => $challengeid, 'userid' => $userid])) {
            throw new \moodle_exception('err_already_joined', 'local_sentientia_challenge');
        }
        ...
        $attemptid = (int) $DB->insert_record('local_sentientia_challenge_attempts', $row);
    ...
    public static function leave(int $challengeid, int $userid): void {
        global $DB;
        if (!$DB->record_exists('local_sentientia_challenge_attempts',
                ['challengeid' => $challengeid, 'userid' => $userid])) {
            throw new \moodle_exception('err_not_joined', 'local_sentientia_challenge');
        }
        $DB->delete_records('local_sentientia_challenge_attempts',
            ['challengeid' => $challengeid, 'userid' => $userid]);
    }
    ...
    public static function evaluate_attempt(int $attemptid): void {
        ...
        if (!$challenge) {
            ...
            $DB->set_field('local_sentientia_challenge_attempts', 'status',
        ...
        $DB->update_record('local_sentientia_challenge_attempts', $upd);
    }
    ...
    public static function expire_overdue_attempts(): int {
        ...
        $rows = $DB->get_records_sql("
            SELECT a.id, a.challengeid
              FROM {local_sentientia_challenge_attempts} a
              JOIN {local_sentientia_challenge_challenges} c ON c.id = a.challengeid
    ...
    public static function validate_definition(array $data): void {
        ...
        if (isset($data['type']) && !in_array($data['type'], self::valid_types(), true)) {
            throw new \invalid_parameter_exception(
                get_string('err_invalid_type', 'local_sentientia_challenge'));
        }
        if (isset($data['status']) && !in_array((int) $data['status'], self::valid_statuses(), true)) {
            throw new \invalid_parameter_exception(
                get_string('err_invalid_status', 'local_sentientia_challenge'));
        }
        if (isset($data['targetcount']) && (int) $data['targetcount'] < 1) {
            throw new \invalid_parameter_exception(
                get_string('err_targetcount_min', 'local_sentientia_challenge'));
        }
        if (isset($data['pointsreward']) && (int) $data['pointsreward'] < 0) {
            throw new \invalid_parameter_exception(
                get_string('err_pointsreward_min', 'local_sentientia_challenge'));
        }
    ...
    public static function list_challenges(int $tenant = 0, string $statusfilter = 'active',
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_sentientia_challenge_challenges} c WHERE $wheresql",
            $params);

        $records = $DB->get_records_sql("
            SELECT c.*,
                   (SELECT COUNT(*) FROM {local_sentientia_challenge_attempts} a
                     WHERE a.challengeid = c.id) AS participants,
                   (SELECT COUNT(*) FROM {local_sentientia_challenge_attempts} a
                     WHERE a.challengeid = c.id AND a.status = '" . self::ATTEMPT_COMPLETED . "') AS completed
              FROM {local_sentientia_challenge_challenges} c
             WHERE $wheresql
