...
namespace local_sentientia_challenge\form;
...
class edit_challenge_dynamic_form extends dynamic_form {
    ...
    protected function definition(): void {
        ...
        // Name + shortname.
        $mform->addElement('text', 'name', get_string('form_name', 'local_sentientia_challenge'),
            ['size' => 60, 'maxlength' => 255]);
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', null, 'required', null, 'client');

        $mform->addElement('text', 'shortname', get_string('form_shortname', 'local_sentientia_challenge'),
            ['size' => 30, 'maxlength' => 100]);
        $mform->setType('shortname', PARAM_ALPHANUMEXT);
        $mform->addHelpButton('shortname', 'form_shortname', 'local_sentientia_challenge');

        // Description.
        $mform->addElement('textarea', 'description',
            get_string('form_description', 'local_sentientia_challenge'),
            ['rows' => 4, 'cols' => 60]);
        $mform->setType('description', PARAM_RAW);

        // Type.
        $types = [
            challenge_engine::TYPE_COURSE_COMPLETION => get_string('type_course_completion', 'local_sentientia_challenge'),
            challenge_engine::TYPE_CUSTOM            => get_string('type_custom', 'local_sentientia_challenge'),
        ];
        $mform->addElement('select', 'type', get_string('form_type', 'local_sentientia_challenge'), $types);
        $mform->setDefault('type', challenge_engine::TYPE_COURSE_COMPLETION);

        // Target count.
        $mform->addElement('text', 'targetcount',
            get_string('form_targetcount', 'local_sentientia_challenge'), ['size' => 5]);
        $mform->setType('targetcount', PARAM_INT);
        $mform->setDefault('targetcount', 1);
        $mform->addHelpButton('targetcount', 'form_targetcount', 'local_sentientia_challenge');

        // Points reward.
        $mform->addElement('text', 'pointsreward',
            get_string('form_pointsreward', 'local_sentientia_challenge'), ['size' => 5]);
        $mform->setType('pointsreward', PARAM_INT);
        $mform->setDefault('pointsreward', 100);

        // Status.
        $statuses = [
            challenge_engine::STATUS_DRAFT    => get_string('status_draft',    'local_sentientia_challenge'),
            challenge_engine::STATUS_ACTIVE   => get_string('status_active',   'local_sentientia_challenge'),
            challenge_engine::STATUS_ARCHIVED => get_string('status_archived', 'local_sentientia_challenge'),
        ];
        $mform->addElement('select', 'status',
            get_string('form_status', 'local_sentientia_challenge'), $statuses);
        $mform->setDefault('status', challenge_engine::STATUS_DRAFT);

        // Date window (optional).
        $mform->addElement('date_time_selector', 'startdate',
            get_string('form_startdate', 'local_sentientia_challenge'),
            ['optional' => true]);
        $mform->addElement('date_time_selector', 'enddate',
            get_string('form_enddate', 'local_sentientia_challenge'),
            ['optional' => true]);
    }
    ...
    protected function get_page_url_for_dynamic_submission(): moodle_url {
        return new moodle_url('/local/sentientia_challenge/index.php');
    }
}
