...
final class challenge_engine_phase_2_test extends \advanced_testcase {
    ...
    public function test_streak_progress_zero_on_no_lastaccess(): void {
        ...
        global $DB;
        $progress = (int) $DB->get_field('local_sentientia_challenge_attempts', 'progress',
            ['id' => $aid]);
    ...
    public function test_streak_progress_counts_consecutive_days(): void {
        ...
        $row = $DB->get_record('local_sentientia_challenge_attempts', ['id' => $aid]);
    ...
    public function test_expire_overdue_attempts_marks_status(): void {
        ...
        $u = $this->getDataGenerator()->create_user();
        $now = time();
        $DB->insert_record('local_sentientia_challenge_attempts', (object) [
            'challengeid' => $cid,
            'userid' => (int) $u->id,
        ...
        $count = $DB->count_records('local_sentientia_challenge_attempts',
    ...
    public function test_expire_does_not_touch_completed_attempts(): void {
        ...
        $row = $DB->get_record('local_sentientia_challenge_attempts',
