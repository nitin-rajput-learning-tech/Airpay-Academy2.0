...
*/
class local_crequest_new_course {
 ...
 * 
 */
function local_crequest_create_new_course_by_record_id($mid, $sendMail) {
    ...
    $new_course = new local_crequest_new_course();

    $new_course->format = get_config('moodlecourse', 'format');
    //Get the default timestamp for new courses
    $timestamp_startdate = $DB->get_field('local_crequest_config', 'value', array('varname'=>'startdate'), IGNORE_MULTIPLE);
    ...
    $mode = $DB->get_field('local_crequest_config', 'value', array('varname'=>'page1_field3status'));
    // what naming mode is operating
    $naming = $DB->get_field('local_crequest_config', 'value', array('varname'=>'naming'), IGNORE_MULTIPLE);
    	//what short naming format is operating
    $snaming = $DB->get_field('local_crequest_config', 'value', array('varname'=>'snaming'), IGNORE_MULTIPLE);
    //get the record for the request
    $rec =  $DB->get_record('local_crequest_records', array('id'=>$mid));
	...
	$DB->update_record('local_crequest_records', $updatedRecord);
	...
	// enrollment key.
	$autoKey = $DB->get_field_select('local_crequest_config', 'value', "varname = 'autoKey'");
	...
	if ($sendMail == true) {
        local_crequest_send_emails($nid, $new_course->shortname, $new_course->fullname, $modkey, $mid);
	}
 ...
 * 
 */
function local_crequest_send_emails($courseid, $modcode, $modname, $modkey, $mid){
	...
	global $USER, $CFG, $DB;
		...
		$currentRecord =  $DB->get_record('local_crequest_records', array('id'=>$mid));
		$user_ids = '';
	    ...
	    $replaceValues['[req_link]'] = $CFG->wwwroot .'/blocks/request/view_summary.php?id=' . $courseid;
	    
		//mail the user
		local_crequest_new_course_approved_mail_user($user_ids, $replaceValues);
		//mail the admin
		local_crequest_new_course_approved_mail_admin($replaceValues);
