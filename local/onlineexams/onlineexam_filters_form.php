...
class onlineexam_filters_form extends moodleform {
    public $thisform;
    function definition() {
        global $CFG;
       ...
       // $submitid =  $this->_customdata['submitid'] ? $this->_customdata['submitid'] : 'filteringform';
        ...
        foreach ($filterlist as $key => $value) {
            if($value === 'categories' || $value === 'elearning' || $value === 'status' || $value === 'coursetype'){
                ...
                local_costcenter_get_hierarchy_fields($mform, $this->_ajaxformdata, $this->_customdata,null, false, 'local_onlineexams', $categorycontext, $multiple = true, $prefix = 'filter');
                continue;
            } else if($value === 'geographyfields'){
                require_once($CFG->dirroot.'/local/users/lib.php');
                local_users_get_userprofile_fields($mform, $this->_ajaxformdata, $this->_customdata, $allenable = false, 'local_onlineexams', $categorycontext, $multiple = true, $prefix = 'filter');
                continue;
            ...
            $onlineexams_plugin_exist = $core_component::get_plugin_directory('local', $filter);
            if ($onlineexams_plugin_exist) {
