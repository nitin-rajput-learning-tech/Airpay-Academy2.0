...
function local_onlineexams_output_fragment_custom_onlineexams_form($args){
    ...
    if ($courseid) {
        ...
        if($get_coursedetails->format == 'singleactivity'){
            $moduleinfoSql = "SELECT q.id, q.attempts ,q.grade,q.timeopen,q.timeclose
                FROM {quiz} as q               
            ...
            $formdata['timeopen'] = $moduleinfo->timeopen; 
            $formdata['timeclose'] = $moduleinfo->timeclose;
        // }else{
        }
    } else {
...
function get_listof_onlineexams($stable, $filterdata) {
    ...
    if(!empty($courses)){
        ...
        foreach ($courses as $key => $course) {
            ...
            $courseslist[$count]["grade_view"] = ((has_capability('local/onlineexams:grade_view',
            $context) || is_siteadmin())&&has_capability('local/onlineexams:manage', $context)) ? true : false;
        ...
        $courseslist[$count]=array_merge($courseslist[$count],array(
                                    "actions"=>(((has_capability('local/onlineexams:enrol',
                                $maincheckcontext)|| has_capability('local/onlineexams:update',
                                $context)||has_capability('local/onlineexams:delete',
                                $context) || has_capability('local/onlineexams:grade_view',
                                $context)|| has_capability('local/courses:report_view',
                                $context)) || is_siteadmin())&&has_capability('local/onlineexams:manage', $maincheckcontext)) ? true : false,
                                "enrol"=>((has_capability('local/onlineexams:enrol',
                                $maincheckcontext)  || is_siteadmin())&&has_capability('local/onlineexams:manage', $maincheckcontext)) ? true : false,
                                "update"=>((has_capability('local/onlineexams:update',
                                $context) || is_siteadmin())&&has_capability('local/onlineexams:manage', $context)&&has_capability('moodle/course:update', $context)) ? true : false,
                                "delete"=>((has_capability('local/onlineexams:delete',
                                $context) || is_siteadmin())&&has_capability('local/onlineexams:manage', $context)&&has_capability('moodle/course:delete', $context)) ? true : false,
