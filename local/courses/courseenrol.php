...
$canenrol = has_capability('local/courses:enrol', $categorycontext);
//$canunenrol = has_capability('local/courses:unenrol', $context);
// Note: manage capability not used here because it is used for editing
// of existing enrolments which is not possible here.
// if (!$canenrol) {
// No need to invent new error strings here...
require_capability('local/courses:enrol', $categorycontext);
require_capability('local/courses:unenrol', $categorycontext);
...
$PAGE->set_url('/local/courses/courseenrol.php', array('id' => $course_id, 'enrolid' => $instance->id));
...
if ($course) {
  ...
  <label>' . get_string('enrolled_users', 'local_courses', $select_from_users_total) . '</label>' . $select_all_not_enrolled_users;
...
if ($course) {
  ...
  $( document ).ready(function() {
    ...
    jQuery(
        function($)
        {
          $('.dual_select').on('scroll', function()
            {
...
$backurl = new moodle_url('/local/courses/courses.php');
