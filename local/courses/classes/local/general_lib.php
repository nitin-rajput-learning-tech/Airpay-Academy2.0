...
class general_lib{
    ...
    public static function inprogress_coursenames($filter_text='', $offset = 0, $limit = 10,$source = '') {
        ...
        $sql .= " WHERE ue.userid = {$USER->id} AND course.id <> 1 AND course.visible=1 AND (course.open_coursetype = 0 OR course.open_coursetype IS NULL) ";
        if($source == 'mobile'){
    ...
    public static function inprogress_coursenames_count($filter_text = '', $source = ''){
        ...
        $sql = "SELECT COUNT(DISTINCT(course.id))  FROM {course} AS course
            ...
            AND course.id NOT IN(SELECT course FROM {course_completions} WHERE course = course.id AND userid = {$USER->id} 
            AND timecompleted IS NOT NULL) AND (course.open_coursetype = 0 OR course.open_coursetype IS NULL) ";
        if($source == 'mobile'){
            $sql .= " AND course.open_securecourse != 1 ";
        }
        $params = [];
    ...
    public static function completed_coursenames($filter_text='', $offset = 0, $limit = 10, $source = '') {
        ...
        $sql = " FROM {course_completions} cc
                ...
                JOIN {user_enrolments} ue ON e.id = ue.enrolid
                WHERE ue.userid = {$USER->id}
                AND cc.timecompleted IS NOT NULL AND c.visible = 1 AND c.id > 1 AND (c.open_coursetype = 0 OR c.open_coursetype IS NULL) ";
        if($source == 'mobile'){
    ...
    public static function completed_coursenames_count($filter_text = '', $source = ''){
        ...
        $sql = "SELECT COUNT(DISTINCT(c.id))
                ...
                AND cc.timecompleted IS NOT NULL AND (c.open_coursetype = 0 OR c.open_coursetype IS NULL) ";
    ...
    public function get_courses_having_completion_criteria($courseid, $query = '', $offset = 0, $limit = 0){
        ...
        $coursesSql = "SELECT DISTINCT c.id, c.fullname
            ...
            AND c.open_path = (SELECT open_path FROM {course} WHERE id = :thiscourseid) AND (c.open_coursetype = 0 OR c.open_coursetype IS NULL) ";
    ...
    public static function enrolled_coursenames_count($filter_text='', $source = ''){
        ...
        $sql = "SELECT COUNT(DISTINCT(course.id))  FROM {course} AS course
            ...
            WHERE ue.userid = {$USER->id}
            AND course.id <> 1 AND course.visible = 1 AND u.id > 2 AND u.suspended = 0 AND u.deleted = 0 AND (course.open_coursetype = 0 OR course.open_coursetype IS NULL) ";
        if($source == 'mobile'){
            $sql .= " AND course.open_securecourse != 1 ";
        }
        $params = [];
