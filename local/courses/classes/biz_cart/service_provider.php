...
/**
 * biz_cart subsystem callback implementation for mod_booking.
 *
...
namespace local_courses\biz_cart;

use local_biz_cart\biz_cart;
use local_biz_cart\local\entities\cartitem;
use local_courses\booking;
use local_courses\booking_bookit;
use local_courses\booking_option;
use local_courses\event\booking_failed;
use local_courses\singleton_service;
...
class service_provider implements \local_biz_cart\local\callback\service_provider
{
    ...
    public static function load_cartitem(string $area, int $itemid, int $userid = 0): array
    {
        ...
        $course->imageurl = course_thumbimage($course);
    ...
    public static function unload_cartitem(string $area, int $itemid, int $userid = 0): array
    {
        ...
        if ($area === 'option') {
            ...
            booking_bookit::answer_booking_option($area, $itemid, LOCAL_COURSES_STATUSPARAM_NOTBOOKED, $userid);
    ...
    public static function successful_checkout(string $area, int $itemid, int $paymentid, int $userid): bool
    {
        ...
        if ($area === 'option') {
            ...
            subbookings_info::save_response($area, $itemid, LOCAL_COURSES_BOOKING_STATUSPARAM_BOOKED, $userid);
    ...
    public static function cancel_purchase(string $area, int $itemid, int $userid = 0): bool
    {
        ...
        if ($area === 'option') {
            booking_bookit::answer_booking_option($area, $itemid, LOCAL_COURSES_STATUSPARAM_DELETED, $userid);
            return true;
            ...
            // We actually book this subbooking option.
            subbookings_info::save_response($area, $itemid, LOCAL_COURSES_STATUSPARAM_DELETED, $userid);
    ...
    private static function unload_subbooking(string $area, int $itemid, int $userid = 0): array
    {
        ...
        subbookings_info::save_response($area, $itemid, LOCAL_COURSES_STATUSPARAM_NOTBOOKED, $userid);
