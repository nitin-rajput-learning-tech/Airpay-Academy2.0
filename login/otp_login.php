...
$PAGE->set_url("$CFG->wwwroot/login/otp_login.php");
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if ($user) {
        ...
        if (empty($user->confirmed)) {       // This account was never confirmed
            ...
            $PAGE->set_heading($site->fullname);
            echo $OUTPUT->header();
            echo '<div class="row text-center"><div class="col-md-6 offset-md-3">';
            echo '<div class="confirmyouraccount">';
            ...
            $resendconfirmurl = new moodle_url('/login/otp_login.php',
                [
...
if (empty($SESSION->wantsurl)) {
    ...
    if ($referer &&
            ...
            strpos($referer, $CFG->wwwroot . '/login/otp_login.php') !== 0) { // There might be some extra params such as ?lang=.
...
if (!empty($SESSION->loginerrormsg)) {
    ...
    redirect(new moodle_url('/login/otp_login.php'));
}
...
if (isloggedin() and !isguestuser()) {
    ...
    $loginform = new \core_auth\output\otplogin($authsequence, $frm->username);
