...
/**
 * Add zipped fonts action.
 *
 * File         addzippedfont.php
 * Encoding     UTF-8
 *
...
/**
 * tool_tcpdffonts\output\component\addzippedfont
 *
 * @package     tool_tcpdffonts
 *
 * @copyright   2021 RvD
 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class addzippedfont implements renderable, templatable {
    /**
     * Base url
     *
    ...
    public function __construct($baseurl, $actionstring = null) {
        ...
        if (empty($actionstring)) {
            $this->actionstring = get_string('action:font:addzippedfont', 'tool_tcpdffonts');
        } else {
        ...
        if ($this->renderasicon) {
            return (object) [
                'action' => $output->action_icon(
                    new moodle_url($this->baseurl, ['action' => 'addzippedfont']),
