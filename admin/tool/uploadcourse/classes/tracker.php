...
class tool_uploadcourse_tracker {
    ...
    public function output($line, $outcome, $status, $data) {
        ...
        if ($this->outputmode == self::OUTPUT_PLAIN) {
            ...
            echo html_writer::tag('td', isset($data['category_code']) ? $data['category_code'] : '', array('class' => 'c' . $ci++));
    ...
    public function start() {
        ...
        if ($this->outputmode == self::OUTPUT_PLAIN) {
            ...
            echo html_writer::tag('th', get_string('categorycode', 'tool_uploadcourse'), array('class' => 'c' . $ci++, 'scope' => 'col'));
