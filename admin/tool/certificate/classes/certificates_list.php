...
//            'coursecategory' => get_string('coursecategory'),
            'costcenter' => get_string('orgname', 'tool_certificate'),
        ];
    ...
    public function col_costcenter($row) {
        global $PAGE;
        return $row->ccname;
    }
    /**
...
//            mallikarjun added start
			...
			if(is_siteadmin() || has_capability('local/certificates:delete', $systemcontext)){
				...
				foreach($learningmodules AS $pluginname){
					switch($pluginname){
		            	case 'courses':
		            	$exists[] = $DB->record_exists('course' , array('open_certificateid'=>$template->get_id()));
		            	break;
		            	case 'classroom':
                                $exists[] = $DB->record_exists('local_classroom' , array('certificateid'=>$template->get_id()));
		            	break;
		            	case 'program':
		            	$exists[] = $DB->record_exists('local_program', array('certificateid'=>$template->get_id()));
		            	break;
		            	case 'learningplan':
		            	$exists[] = $DB->record_exists('local_learningplan', array('certificateid'=>$template->get_id()));
		            	break;
		            	case 'onlinetests':
		            	$exists[] = $DB->record_exists('local_onlinetests', array('certificateid'=>$template->get_id()));
		            	break;
		            	case 'certification':
		            	$exists[] = $DB->record_exists('local_certification', array('certificateid'=>$template->get_id()));
					}
				}
            ...
            $link = new \moodle_url('#');
            $icon = new \pix_icon('i/trash', get_string('cantdelete_certificate','tool_certificate'), 'core');
            $attributes = [
                'data-action' => '',
                'data-id' => $template->get_id(),
                'data-name' => $template->get_formatted_name(),
