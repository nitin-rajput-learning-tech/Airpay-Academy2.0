 ...
 * because that savepoint inserts into `quizaccess_sentientia_proctor`
 ...
 *   1. Drop the quizaccess_sentientia_proctor table (mirrors prod state at v2026051120).
 *   2. Seed a `mdl_config_plugins` row keyed `quizaccess_sentientia_proctoring`
 *      / `quiz_42_enabled` = 1 (the legacy config-row-per-quiz pattern).
 *   3. Call xmldb_quizaccess_sentientia_proctoring_upgrade($oldversion=2026051200).
 ...
 *
 * @covers ::xmldb_quizaccess_sentientia_proctoring_upgrade
 */
final class upgrade_test extends \advanced_testcase {
    ...
    public function test_upgrade_creates_table_when_missing_and_migrates_legacy_config(): void {
        ...
        $DB->insert_record('config_plugins', (object) [
            'plugin' => 'quizaccess_sentientia_proctoring',
        ...
        $row42 = $DB->get_record('quizaccess_sentientia_proctor', ['quizid' => 42]);
        $this->assertNotFalse($row42, 'quiz 42 should have been migrated');
        $this->assertEquals(1, (int) $row42->enabled);

        $row43 = $DB->get_record('quizaccess_sentientia_proctor', ['quizid' => 43]);
    ...
    public function test_upgrade_is_idempotent_on_fresh_install_with_no_legacy_rows(): void {
        ...
        $this->assertEquals($before,
            $DB->count_records('quizaccess_sentientia_proctor'),
    ...
    public function test_upgrade_skips_migration_for_already_migrated_version(): void {
        ...
        $this->assertFalse($DB->record_exists('quizaccess_sentientia_proctor', ['quizid' => 99]));
    }
    ...
    public function test_upgrade_ignores_unrelated_config_keys(): void {
        ...
        $this->assertTrue($result);
        // 77 was migrated.
        $this->assertTrue($DB->record_exists('quizaccess_sentientia_proctor', ['quizid' => 77]));
