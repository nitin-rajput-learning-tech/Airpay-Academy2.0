# quizaccess_sentientia_proctoring
...
| Field | Value |
|---|---|
| Component | `quizaccess_sentientia_proctoring` |
| Plugin type | `quizaccess` (sub-plugin of `mod_quiz`) |
| Path | `mod/quiz/accessrule/sentientia_proctoring/` |
| Depends on | `local_sentientia_proctoring` |
...
1. **Pre-attempt**: requires the candidate's proctoring session to be in
   ...
   redirects to the consent/identity flow under `local_sentientia_proctoring`.
2. **During attempt**: blocks resume if the session expired or was
   flagged with a critical event.
3. **Post-attempt**: triggers `local_sentientia_proctoring\session_manager::finalize()`
...
3. **Expected**: redirected to consent screen at
   `/local/sentientia_proctoring/consent.php?quizid=X`.
...
in `local_sentientia_proctoring` tables (which has its own GDPR provider).
...
`local_sentientia_proctoring`, but the access-rule pattern is more
