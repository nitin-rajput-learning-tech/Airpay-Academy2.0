...
class schedule {
	...
	public function get_userajax($enrolid) {
		global $DB, $CFG, $PAGE;
		if ($enrolid == -1) {
			$sql = "select u.id,u.firstname,u.lastname,u.email from {user}  as u JOIN {role_assignments} as ra where u.id=ra.userid";
			$userdetails = $DB->get_records_sql($sql);
		} else {
			$sql = "select u.id,u.firstname,u.lastname,u.email from {user}  as u JOIN {role_assignments} as ra where u.id=ra.userid and ra.roleid IN($enrolid)";
			$userdetails = $DB->get_records_sql($sql);
		}
		return $userdetails;
	}
	public function add_userajax($userid) {
		global $DB, $CFG, $PAGE;
		$sql = "select u.id,u.firstname,u.lastname from {user}  as u JOIN {role_assignments} as ra where u.id=ra.userid and ra.userid IN($userid)";
		$userdetails = $DB->get_records_sql($sql);
		return $userdetails;
	}
	public function viewusers($userids) {
		global $DB, $CFG, $PAGE;
		$sql = "select u.id,u.firstname,u.lastname,u.email,ra.roleid,r.shortname from {user}  as u JOIN {role_assignments} as ra JOIN {role} r ON u.id=ra.userid where r.id=ra.roleid and ra.userid IN($userids)";
		//$sql="select id,firstname,email,lastname from {user}  where id IN($userids)";
		$userdetails = $DB->get_records_sql($sql);
		return $userdetails;
	}

	public function dataetimemonthly($frequency) {
		global $DB, $CFG, $PAGE;
		if ($frequency == 1) {
			...
			$updatefrequency_list = $times;

		} elseif ($frequency == 2) {

			$weeks = array('--Select--', 'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat');

			$updatefrequency_list = $weeks;
			...
			$updatefrequency_list = $months;

		}

		if ($frequency) {
			$data = json_encode(['data' => $updatefrequency_list]);
		}
		return $data;
		///sarath ended this code
	}
	...
	///////sarath wrote this function end here //////
	/**
		...
		case self::WEEKLY:
			$calendardays = calendar_get_days();
			 if ((($calendardays[$schedule]->fullname == strtolower(strftime('%A', time()))) && (!$is_cron))) {
				...
				$nextschedule = mktime(9, 0, 0, $timemonth, $timeday + 7, $timeyear);
			}
			break;
		case self::MONTHLY:
	...
	/**
	 * @method is_time Check if it's time to run event
	 *
	 * @return bool
	 */
	public function is_time($nextschedule) {
	...
	function scheduledreport_send_scheduled_report($schedule) {
		...
		foreach ($sendinguserids as $sendinguserid) {
			...
			if ($schedule->exporttofilesystem != REPORT_EXPORT) {
				...
				$messagedetails->exporttype = get_string($schedule->exportformat . 'format', 'block_learnerscript', null, $user->lang);
				...
				$subject = $schedule->name . ' ' . get_string('report', 'block_learnerscript', null, $user->lang);
				...
				$message = get_string('scheduledreportmessage', 'block_learnerscript', $messagedetails, $user->lang);
	...
	function scheduledreport_create_attachment($schedule, $user) {
		...
		if ($schedule->exporttofilesystem == REPORT_EMAIL) {
			if ($reportdata[1] == false) {
				return false;
			}
			if (empty($reportdata[0]->table->data)) {
				return false;
			}
		}
		switch ($format) {
		case 'ods':
			$filename = $this->export_ods($reportdata[0], $CFG->dataroot . '/' . $reportfilepathname);
			$reportfilepathname = $reportfilepathname . '.ods';
			break;
		case 'xls':
			$filename = $this->export_xls($reportdata[0], $CFG->dataroot . '/' . $reportfilepathname);
			$reportfilepathname = $reportfilepathname . '.xls';
			break;
		case 'csv':
			$filename = $this->export_csv($reportdata[0], $CFG->dataroot . '/' . $reportfilepathname);
			$reportfilepathname = $reportfilepathname . '.csv';
			break;
		case 'pdf':
			$filename = $this->export_pdf($reportdata[0], $CFG->dataroot . '/' . $reportfilepathname);
	...
	*/
	...
	public function rolewiseusers($roles, $orgid, $deptid, $subdeptid, $search = '', $page = 0, $reportid = 0) {
      ...
      if (is_siteadmin()) {
		...
		$reportclass->role = $DB->get_field('role', 'shortname', array('id' => $roles));
	 ...
	 * @return Object List of users with id and fullname
	 */
	public function schroleusers($reportid, $scheduleid, $type, $roles, $search = '', $bullkselectedusers = '') {
		global $DB, $USER;
		...
		case 'add':
			...
			$sql = " SELECT u.id,
                     ...
                     WHERE u.id = ra.userid $search_sql $escselsql AND u.confirmed = 1 AND u.suspended = 0 AND u.deleted = 0";
		...
		case 'remove':
			$userslistsql = "SELECT u.id, CONCAT(u.firstname, ' ', u.lastname) as fullname
					FROM {user} as u
					JOIN {block_ls_schedule} as bcs ON FIND_IN_SET(u.id, bcs.sendinguserid)
	...
	public function viewschusers($reportid, $scheduleid, $schuserslist, $stable) {
		...
		if ($stable->table) {
			$schuserscountsql = "SELECT COUNT(u.id) as count
				   FROM {user} as u
				WHERE FIND_IN_SET(u.id, '$schuserslist') AND u.confirmed = 1 AND u.suspended = 0 AND u.deleted = 0";
		} else {
			$schuserscountsql = "SELECT COUNT(u.id) as count
				   FROM {user} as u
				WHERE FIND_IN_SET(u.id, '$schuserslist') AND u.confirmed = 1 AND u.suspended = 0 AND u.deleted = 0";
	...
	public function schedulereports($reportid, $table = true, $start = 0, $length = 5, $search = '') {
		...
		$fields .= " LIKE '%" . $search . "%' ";
		if (!$table) {
			$schreportssql = "SELECT * 
								FROM (SELECT bcs.id, bcs.exportformat, bcs.frequency, 
								bcs.schedule, bcr.name,
								CASE
								    WHEN bcs.roleid > 0 THEN r.shortname
					    	   ...
					    	   WHERE bcs.reportid = $reportid AND bcs.frequency > 0 ) sr WHERE 1=1 ";
		}
		$totalschreportssql = "SELECT * 
								FROM (SELECT COUNT(bcs.id) as totalrecords,
								CASE
								    WHEN bcs.roleid > 0 THEN r.shortname
								ELSE 'admin' END as role,
								bcs.exportformat
		...
		if (!$table) {
			$schreportssql .= ' ORDER BY sr.id ASC';
	...
	public function reportroles($selectedroleid = '', $reportid = 0) {
    	...
    	$managereportsroles = $DB->get_records_sql_menu($sql, $params);
    	if($managereportsroles){
    		$roles = $roles + $managereportsroles;
    	}
        ...
        if (!empty($permissions['elements'])) {
            foreach ($permissions['elements'] as $p) {
            	if($p['pluginname'] == 'sitelevelroles') {
			        ...
			        $rolename = $DB->get_field_sql($sql, array('roleid'=>$p['formdata']->roleid));
            		$roles[$p['formdata']->roleid] = $rolename;
            	}
            }
        }
		...
		foreach ($roles as $key => $value) {
			...
			$roles_list[] = array('key' => $key, 'value' => $value, 'selected' => $selected);
		}
