...
class report_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        $catarray = array(null =>'Select Category');
        ...
        foreach($local_pluginlist as $key => $local_pluginname){
            if(file_exists($CFG->dirroot.'/local/'.$key.'/lib.php')){
                ...
                $functionname = 'learnerscript_'.$key.'_list';
        ...
        $catarray['enduser'] = 'Enduser';
        $mform->addElement('select', 'category', get_string('category'), $catarray);
        $mform->addHelpButton('category', 'category', 'block_learnerscript');
        $mform->addRule('category', null, 'required', null, 'client');
        // for Category field -- ended here
