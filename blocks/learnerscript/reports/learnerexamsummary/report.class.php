...
class report_learnerexamsummary extends reportbase implements report {
    /**
    ...
    public function __construct($report, $reportproperties) {
        ...
        $this->parent = true;
        $this->columns = array('learnerexamsummarycolumns' => array('learner', 'course', 'progress', 'completed', 'deadline','upcomingexpiry','upcomingendoflife'));
        $this->components = array('columns', 'filters', 'permissions', 'plot');
        $this->courselevel = true;
        $this->basicparams = array(['name' => 'organization'], ['name' => 'departments'], ['name'=>'subdepartments']);
        $this->filters = array('user');
        $this->orderable = array('learner', 'course', 'progress', 'completed', 'deadline','upcomingexpiry','upcomingendoflife');
    ...
    public function select() {
        $this->sql = " SELECT DISTINCT ue.id, u.id, CONCAT(u.firstname, ' ', u.lastname) AS learner, c.id AS courseid, c.fullname AS course, ue.id, ue.userid, cc.timecompleted AS completed, ue.completiondate AS deadline ";
        parent::select();
    }
    ...
    public function where() { 
        $this->sql .= "  WHERE 1 = 1 AND CONCAT(',',c.open_identifiedas,',') LIKE CONCAT('%,',3,',%') ";
    ...
    public function filters() { 
        ...
        if ($this->params['filter_user'] > 0) {
            $this->sql .= " AND u.id = ".$this->params['filter_user'];
            $subqueryfilter .= " AND u.id = ".$this->params['filter_user'];
        }
        ...
        if (isset($this->params['filter_status'])) {
            if($this->params['filter_status'] == 'completed') {
                ...
                $this->sql .= "  AND cff.name = 'EOL' AND cfd.intvalue !=0 AND CONCAT(',',c.open_identifiedas,',') LIKE CONCAT('%,',3,',%') ";
            }
        }
    }
