...
class report_learnerattendance extends reportbase implements report {
  /**
   ...
   */
    public function __construct($report, $reportproperties) {
        ...
        $columns = array('fullname','date','timeloggedin','timeloggedout','totaltimespent','activitycompleted');
        $columnsarray = array('userfield'=>['userfield'], 'learnerattendance' => $columns);
    ...
    function select() {
         $this->sql = "SELECT DISTINCT lsl.id, u.id as userid, CONCAT(u.firstname, ' ', u.lastname) AS fullname, lp.id AS programid, lsl.action, lsl.timecreated, lsl.id" ;
        parent::select();
    }
    ...
    function joins() {
        $this->sql .=" JOIN {local_program_users} lpu ON lpu.userid = u.id 
                    ...
                    JOIN {logstore_standard_log} lsl on lpu.userid = lsl.userid";
        parent::joins();
    }

    function where() {
        global $USER, $DB;
        $this->sql .= " WHERE u.deleted = 0 AND u.suspended = 0 AND u.confirmed = 1 AND lsl.action = 'loggedout'";
