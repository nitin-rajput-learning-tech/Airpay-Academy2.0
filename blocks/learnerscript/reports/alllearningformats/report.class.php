...
class report_alllearningformats extends reportbase implements report {
    /**
    ...
    public function __construct($report, $reportproperties) {
        ...
        $this->parent = true;
        $this->columns = array('alllearningformats' => array('learner', 'course', 'enrolmentdate', 'progress', 'dateofcompletion', 'completiondeadline', 'upcomingdeadline', 'overduedeadline'));
        ...
        if (!$this->scheduling) {
            if(is_siteadmin() || has_capability('local/costcenter:manage_multiorganizations', $systemcontext)){ 
                if ($this->params['filter_organization']) {
                    ...
                    $subqueryfilter .= " AND u.open_costcenterid = " .$this->params['filter_organization'];
                }
                if ($this->params['filter_departments'] > 0) {
                    $this->sql .= " AND u.open_departmentid = ".$this->params['filter_departments'];
                    $subqueryfilter .= " AND u.open_departmentid = ".$this->params['filter_departments'];
                }

            } else if(!is_siteadmin() && has_capability('local/costcenter:manage_ownorganization', $systemcontext) && $ohs) { 
                $this->sql .= " AND u.open_costcenterid = " .$USER->open_costcenterid;
                $subqueryfilter .= " AND u.open_costcenterid = " .$USER->open_costcenterid;             
                if ($this->params['filter_departments'] > 0) {
                    $this->sql .= " AND u.open_departmentid = ".$this->params['filter_departments'];
                    $subqueryfilter .= " AND u.open_departmentid = ".$this->params['filter_departments'];
                }
            }else if(has_capability('local/costcenter:manage_owndepartments', $systemcontext) && $dhs){ 
                  $this->sql .= " AND c.open_costcenterid = " .$USER->open_costcenterid . " AND u.open_departmentid = ". $USER->open_departmentid ;
                $subqueryfilter .= " AND c.open_costcenterid = " .$USER->open_costcenterid . " AND u.open_departmentid = ". $USER->open_departmentid ; 
            } else { 
                $this->sql .= " AND c.open_costcenterid = " .$USER->open_costcenterid . " AND u.open_departmentid = ". $USER->open_departmentid . " AND u.open_subdepartment = ". $USER->open_subdepartment ;
                $subqueryfilter .= " AND c.open_costcenterid = " .$USER->open_costcenterid . " AND u.open_departmentid = ". $USER->open_departmentid . " AND u.open_subdepartment = ". $USER->open_subdepartment ;                 
            } 

            if($this->params['filter_subdepartments'] > 0){
                $this->sql .= " AND u.open_subdepartment = ".$this->params['filter_subdepartments'];
                $subqueryfilter .= " AND u.open_subdepartment = ".$this->params['filter_subdepartments'];
            }
        }
