    ...
    public function filters() { 
        ...
        if (!$this->scheduling) {
           // if ($this->loggedinuserrole != 'dh') {
                ...
                }else if(has_capability('local/costcenter:manage_owndepartmentsge', $systemcontext) && $dhs){ 
        ...
        switch ($columnname) {
            ...
            case 'submittedusers':
                ...
                $query = "SELECT COUNT(asb.id) AS submittedusers  
                            ...
                            WHERE asb.status = 'submitted' AND asb.userid in ($learnersql) $where $concatsql $departmentsql";
            break;
            case 'completedusers':
                $identy = 'cmo.instance';
                $courseid = 'cmo.course';
                $query = "SELECT COUNT(DISTINCT cmc.userid) AS completedusers  
                           ...
                           WHERE cmc.userid in ($learnersql) AND cmc.completionstate > 0 
                             AND cmo.module = 1 AND cmo.visible = 1 $where $concatsql $departmentsql";
            break;
            case 'needgrading':
                $identy = 'asb.assignment';
                $query = "SELECT COUNT(asb.id) AS needgrading  
                            ...
                            WHERE asb.status = 'submitted' AND asb.userid > 2 AND g.finalgrade IS NULL AND gi.iteminstance = asb.assignment
                                AND gi.itemmodule = 'assign' $where $concatsql $departmentsql";
            break;
            case 'avggrade':
                $identy = 'gi.iteminstance';
             ...
             case 'numviews':
                ...
                if($this->reporttype == 'table'){
                                 ...
                                 WHERE lsl.crud = 'r' AND lsl.contextlevel = 70  AND lsl.anonymous = 0 AND u.id IN ($learnersql)
                                  ...
                                  AS c1,
                                   (SELECT COUNT('X') as numviews 
                                     ...
                                     JOIN {assign} q ON q.id = cm.instance 
                                     JOIN {modules} m ON m.id = cm.module
                                     WHERE  lsl.crud = 'r' AND lsl.contextlevel = 70 AND lsl.userid > 2 AND u.id IN ($learnersql) AND lsl.anonymous = 0 AND u.confirmed = 1 AND u.deleted = 0 AND m.name = 'assign' $where $concatsql $departmentsql) AS c2)";
                }else{
                    $query = "  SELECT COUNT('X') as numviews 
                                 ...
                                 JOIN {assign} q ON q.id = cm.instance 
                                 JOIN {modules} m ON m.id = cm.module
                                 WHERE  lsl.crud = 'r' AND lsl.contextlevel = 70 AND lsl.userid > 2 AND u.id IN ($learnersql) AND lsl.anonymous = 0 AND u.confirmed = 1 AND u.deleted = 0 AND m.name = 'assign' $where $concatsql $departmentsql ";
                } 
            break;   
            default:
                return false;
