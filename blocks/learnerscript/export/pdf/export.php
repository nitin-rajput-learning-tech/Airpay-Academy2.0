...
function export_report($reportclass, $id) {
	...
	foreach ($filters as $k => $filter) {
		if ($k == 'courses' && !empty($filter)) {
			$filterdata[$k] = "<b>" . get_string('course') . " </b>" . $DB->get_field('course', 'fullname', array('id' => $filter));
		}
		if ($k == 'categories' && !empty($filter)) {
			$filterdata[$k] = "<b>$k </b>" . $DB->get_field('course_categories', 'name', array('id' => $filter));
		}

	}
	$finalfilterdata = implode("&nbsp;&nbsp;&nbsp;&nbsp;", $filterdata);
	...
	$doc->writeHTMLCell($w = 100, $h = 10, $x = '80', $y = '25', $finalfilterdata, $border = 0, $ln = 1, $fill = 0, $reseth = true, $align = '', $autopadding = true);
