...
function formatdata_validation($reportid, $data, $linenum, &$formatteddata, $reportclass) {
	...
	//  if ($id) {
	//      $checkmail = $DB->get_record_sql('select userid from {role_assignments}  where userid=' . $id . ' AND roleid=' . $data->roleid);
	//  } else {
	//      $checkmail = '';
	//  }if ($checkmail->userid == '') {
	//      echo '<h6 style= "color:red;">this user is not available in this role "' . $data->roleid . '" in line no "' . $linenum . '".</h6>';
	//  } else {
	...
	// if($userid) {
	// 	$userroleexists = $DB->record_exists_sql("SELECT ra.id FROM {role_assignments} ra JOIN {role} r ON r.id = ra.roleid WHERE ra.userid = $userid AND ra.roleid = $data->roleid");
	// 	if (!$userroleexists) {
	// 		$mfields[] = get_string('nouserrole', 'block_learnerscript');
	// 		$errors[] = '<div class="alert alert-error" role="alert">Selected User Not Available for the role which entered in line no. "' . $linenum . '" of uploaded sheet.</div>';
	// 	}
	// }
...
/*	if(!$exportformats) {
		$errors[] = '<div class="alert alert-error" role="alert">Export Format not available for this report.</div>';
	} else*/ if (!in_array($data->exportformat, array_keys($REPORT_EXPPORT_FORMATS))) {
