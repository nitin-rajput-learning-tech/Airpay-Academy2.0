...
class plugin_graphlearnerenrolmentscolumns extends pluginbase {
    public function init() {
        $this->fullname = get_string('graphlearnerenrolments', 'block_learnerscript');
        $this->type = 'undefined';
        $this->form = true;
        $this->reporttypes = array('graphlearnerenrolments');
    }
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        if (!$this->scheduling) {
            ...
            }else if(has_capability('local/costcenter:manage_owndepartments', $context) && $ohs){
                $parentstatus = " AND lcc.parentid > 0 ";
                $costcenter = " AND u.open_costcenterid = " .$USER->open_costcenterid . " AND c.open_departmentid = ". $USER->open_departmentid ; 
            }else{
                $parentstatus = " AND lcc.parentid > 0 ";
                $costcenter = " AND u.open_costcenterid = " .$USER->open_costcenterid . " AND c.open_departmentid = ". $USER->open_departmentid. " AND c.open_subdepartment = ".$USER->open_subdepartment ; 
            }
        ...
        switch ($data->column) {
            ...
            case 'enrolments': 
                if(!isset($row->enrolments) && isset($data->subquery)){
                    ...
                    $sql = "SELECT COUNT(ue.id) AS 'enrolments'
                            ...
                            WHERE c.id = ctx.instanceid AND u.confirmed=1 AND u.deleted=0 AND DATE_FORMAT(from_unixtime(ue.timecreated), '%M') = '{$row->month}'  AND CONCAT(',',c.open_identifiedas,',') LIKE CONCAT('%,',3,',%') {$parentstatus} {$costcenter} {$dept} {$subdept} {$timefilter} ";
                    $enrolments = $DB->get_field_sql($sql);
                }
