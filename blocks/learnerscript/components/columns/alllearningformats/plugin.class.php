...
class plugin_alllearningformats extends pluginbase{
	public function init(){
		$this->fullname = get_string('alllearningformats','block_learnerscript');
		$this->type = 'undefined';
		$this->form = true;
		$this->reporttypes = array('alllearningformats');
	}
    ...
    $learning = " AND c.open_learningformat IN (1, 3, 4, 7) ";
            ...
            case 'progress': 
              ...
              if(!isset($row->upcomingdeadline) && isset($data->subquery)){
                ...
                $sql = " SELECT ue.completiondate 
                        ...
                        WHERE CONCAT(',',c.open_identifiedas,',') LIKE CONCAT('%,',3,',%') AND ue.id = $row->id {$learning} AND  ue.completiondate !=0 AND ue.completiondate > UNIX_TIMESTAMP() AND ue.id NOT IN (SELECT DISTINCT ue.id 
                        FROM {user_enrolments} ue
                        JOIN {enrol} e ON e.id = ue.enrolid 
                        JOIN {role_assignments} ra ON ra.userid = ue.userid
                        JOIN {context} ct ON ct.id = ra.contextid AND ct.contextlevel = 50
                        ...
                        JOIN {course} c ON c.id = e.courseid AND c.id = ct.instanceid   
                        WHERE CONCAT(',',c.open_identifiedas,',') LIKE CONCAT('%,',3,',%') AND ue.id = $row->id {$learning} {$costcenter} {$dept} {$subdept} {$timefilter} ) {$costcenter} {$dept} {$subdept} {$timefilter} ";
