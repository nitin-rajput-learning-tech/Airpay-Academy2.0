...
class plugin_learnercoursesoverview extends pluginbase{
	public function init(){
		$this->fullname = get_string('learnercoursesoverview','block_learnerscript');
		$this->type = 'undefined';
		$this->form = true;
		$this->reporttypes = array('learnercoursesoverview');
	}
	...
	public function execute($data,$row,$user,$courseid,$starttime=0,$endtime=0,$reporttype){
        ...
        if (empty($reportid) || empty($courseprofilepermissions)) {
            ...
            $url = new moodle_url('/blocks/learnerscript/viewreport.php', array('id' => $reportid,'filter_course' => $row->id));
        }
        ...
        $allactivityurl = new moodle_url('/blocks/learnerscript/viewreport.php', 
                        array('id' => $activityinfoid, 'filter_course' => $row->id, 
                               'filter_modules' => $this->reportfilterparams['filter_modules'], 'filter_users' => $this->reportfilterparams['filter_users'], 'filter_organization' => $this->reportfilterparams['filter_organization'], 'filter_departments' => $this->reportfilterparams['filter_departments']));
        $inprogressactivityurl = new moodle_url('/blocks/learnerscript/viewreport.php', array('id' => $activityinfoid, 'filter_course' => $row->id, 'filter_status' => 'notcompleted', 'filter_modules' => $this->reportfilterparams['filter_modules'], 'filter_users' => $this->reportfilterparams['filter_users'], 'filter_organization' => $this->reportfilterparams['filter_organization'], 'filter_departments' => $this->reportfilterparams['filter_departments']));
        $completedactivityurl = new moodle_url('/blocks/learnerscript/viewreport.php', array('id' => $activityinfoid, 'filter_course' => $row->id, 'filter_status' => 'completed', 'filter_modules' => $this->reportfilterparams['filter_modules'], 'filter_users' => $this->reportfilterparams['filter_users'], 'filter_organization' => $this->reportfilterparams['filter_organization'], 'filter_departments' => $this->reportfilterparams['filter_departments']));
        switch ($data->column) {
            case 'coursename':
