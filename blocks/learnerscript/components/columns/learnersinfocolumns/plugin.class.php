...
class plugin_learnersinfocolumns extends pluginbase{
    ...
    if (!$this->scheduling) {
        if(is_siteadmin() || has_capability('local/costcenter:manage_multiorganizations', $context)){ 
            ...
            if ($this->reportfilterparams['filter_departments'] > 0) {
             ...
             $costcenter = " AND u.open_costcenterid = ".$USER->open_costcenterid;
            $dept = " AND u.open_departmentid = ".$USER->open_departmentid ;
        } else {
            $costcenter = " AND u.open_costcenterid = ".$USER->open_costcenterid;
            $dept = " AND u.open_departmentid = ".$USER->open_departmentid ;
            $subdept = " AND u.open_subdepartment = ".$USER->open_subdepartment ;
        }
          ...
          case 'upcomingdeadline':
              if(!isset($row->upcomingdeadline) && isset($data->subquery)){
                  ...
                  $sql = " SELECT ue.completiondate
                            ...
                            WHERE ue.id = {$row->id} AND ue.completiondate > UNIX_TIMESTAMP() AND ue.id NOT IN (SELECT DISTINCT ue.id 
                        ...
                        WHERE 1 = 1 {$costcenter} {$dept} {$subdept} {$timefilter} )  ";
