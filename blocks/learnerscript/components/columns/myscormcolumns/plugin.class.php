        ...
        switch ($data->column) {
            case 'course':
                ...
                if(empty($reportid) || empty($checkpermissions)){
                    ...
                    $row->{$data->column} = '<a href="'.$CFG->wwwroot.'/blocks/learnerscript/viewreport.php?id='.$reportid.'&filter_course='.$row->courseid.'&filter_organization='.$this->reportfilterparams['filter_organization'].'&filter_departments='.$this->reportfilterparams['filter_departments'].'" />'.$row->{$data->column}.'</a>';
                }
            break;
            case 'scormname':
            ...
            case 'activitystate':
                ...
                $scormattemptstatus = $DB->get_field_sql("SELECT value FROM {scorm_scoes_track}
                                                        WHERE scormid = $row->id AND userid = $row->userid
                                                        ORDER BY id DESC LIMIT 1 ");
                $scormcomppletion = $DB->get_field_sql("SELECT id FROM {course_modules_completion}
                                                        WHERE coursemoduleid = $row->cmid AND userid = $row->userid
