...
class plugin_scormfield extends pluginbase {
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        if (isset($scormrecord->{$data->column})) {
            switch ($data->column) {
                case 'name':
                    $scormmoduleid = $DB->get_field_sql("SELECT cm.id FROM {course_modules} cm JOIN {modules} m ON m.id = cm.module JOIN {scorm} s ON s.id = cm.instance WHERE s.id = $row->id AND m.name = 'scorm'");
                ...
                case 'course':
                    ...
                    if(empty($reportid) || empty($checkpermissions)){
                        ...
                        $scormrecord->{$data->column} = '<a href="'.$CFG->wwwroot.'/blocks/learnerscript/viewreport.php?id='.$reportid.'&filter_course='.$scormrecord->course.'" />'.$coursename.'</a>';
                    }
                break;
                case 'timeopen':
                case 'timeclose':
