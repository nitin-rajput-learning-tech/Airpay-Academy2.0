    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
       ...
       switch ($data->column) {
            case 'achievedgrade':
                    ...
                    $gradegrades = $DB->get_record_sql($sql, array('quizid'=>$row->testid, 'itemmodule'=>'quiz', 'courseid'=>1,'userid' => $USER->id));
                    $row->{$data->column} = !is_null($gradegrades->finalgrade) ? round($gradegrades->finalgrade, 2) : '--';
                break;
            case 'completionstatus':
                $row->{$data->column} = ($row->status == 1) ? 'Completed' : 'Not Completed';
                break;
            case 'completiondate':
                if($row->status == 1){
                    $sql = "SELECT gg.timemodified 
                            FROM {grade_grades} as gg 
                            JOIN {grade_items} as gi ON gi.id =gg.itemid 
                            WHERE gi.itemmodule = :itemmodule AND gi.iteminstance = :quizid
                            AND gi.courseid =:courseid AND gg.userid = :userid ";
