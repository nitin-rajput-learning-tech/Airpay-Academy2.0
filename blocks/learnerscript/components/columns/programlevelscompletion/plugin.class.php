...
class plugin_programlevelscompletion extends pluginbase {

    public function init() {
        $this->fullname = get_string('programlevelscompletion', 'block_learnerscript');
        $this->type = 'undefined';
        $this->form = true;
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        switch ($data->column) {
         /*    case 'completionstatus':
                ...
                break; */
            case 'completionstatus':
                $row->{$data->column} =  !empty($row->{$data->column}) ? $row->{$data->column} : 'Not Completed';
                break; 
            case 'completiondate':
