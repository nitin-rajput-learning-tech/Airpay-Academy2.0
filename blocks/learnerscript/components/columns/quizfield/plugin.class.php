...
class plugin_quizfield extends pluginbase {
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        if (isset($quizrecord->{$data->column})) {
            switch ($data->column) {
                case 'name':
                    $module = $DB->get_field_sql('SELECT name FROM {modules} AS m JOIN {course_modules} AS cm ON m.id = cm.module WHERE cm.id = '.$row->activityid.'');
                ...
                case 'course':
                    ...
                    if(empty($reportid) || empty($checkpermissions)){
                        ...
                        $quizrecord->{$data->column} = '<a href="'.$CFG->wwwroot.'/blocks/learnerscript/viewreport.php?id='.$reportid.'&filter_course='.$quizrecord->course.'" target="_blank" class="edit">'.$coursename.'</a>';
                   }
                break;
                case 'timecreated':
                case 'timemodified':
                case 'timeclose':
                case 'timeopen':
                    $quizrecord->{$data->column} = userdate($quizrecord->{$data->column});
                break;
                case 'timelimit':
                    $quizrecord->{$data->column} = gmdate("H:i:s", $quizrecord->{$data->column});
                break;
            }
        }
       return (isset($quizrecord->{$data->column})) ? $quizrecord->{$data->column} : '';
    }
