	...
	public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0, $reporttype) {
		...
		switch($data->column){
            ...
            case 'averagegrade':
                ...
                if($reporttype == 'table'){
                    $row->{$data->column} = !empty($grade) ? $grade : '--';
                }else{
                    $row->{$data->column} = !empty($grade) ? $grade : 0;
                }
            break;
            ...
            case 'grades';
                ...
                if(empty($gradesReportID) || empty($checkpermissions)){
                    ...
                    $row->{$data->column} =  html_writer::link("$CFG->wwwroot/blocks/learnerscript/viewreport.php?id=$gradesReportID&filter_course=$row->course&filter_activities=$row->id&filter_organization=".$this->reportfilterparams['filter_organization']."&filter_departments=".$this->reportfilterparams['filter_departments']."&filter_subdepartments=".$this->reportfilterparams['filter_subdepartments']."", 'Grades');
                }
            break;
            case 'totaltimespent':
                if(!isset($row->{$data->column})){
            ...
            case 'numviews':
                ...
                $reportid = $DB->get_field('block_learnerscript', 'id', array('type' => 'noofviews'), IGNORE_MULTIPLE);
                    return html_writer::link("$CFG->wwwroot/blocks/learnerscript/viewreport.php?id=$reportid&filter_course=$row->course&filter_activities=$row->id&filter_organization=".$this->reportfilterparams['filter_organization']."&filter_departments=".$this->reportfilterparams['filter_departments']."&filter_subdepartments=".$this->reportfilterparams['filter_subdepartments']."", get_string('numviews', 'report_outline', $numviews), array("target" => "_blank"));
            break;
            case 'description':
