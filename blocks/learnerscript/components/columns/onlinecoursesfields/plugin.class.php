...
class plugin_onlinecoursesfields extends pluginbase{
	public function init(){
		$this->fullname = get_string('onlinecourses','block_learnerscript');
		$this->type = 'undefined';
		$this->form = true;
		$this->reporttypes = array('onlinecourses');
	}
    ...
    if (!$this->scheduling) {
        if(is_siteadmin() || has_capability('local/costcenter:manage_multiorganizations', $context)){ 
            ...
            $costcenter = " AND bll.costcenterid IN (" .$USER->open_costcenterid .", 0 ) AND bll.costcenterid =". $USER->open_costcenterid ;
            if ($this->reportfilterparams['filter_departments'] > 0) {
                $dept = " AND bll.departmentid IN (".$this->reportfilterparams['filter_departments'].", 0) AND bll.user_departmentid=".$this->reportfilterparams['filter_departments'] ;
            }
		...
		switch($data->column){ 
            ...
            case 'progress': 
              ...
              if(!isset($row->upcomingdeadline) && isset($data->subquery)){
                ...
                WHERE bll.upcomingdeadline > UNIX_TIMESTAMP() AND bll.moduleid IN (1,7) AND bll.completiondate = 0
                AND bll.id = $row->id {$learning} {$costcenter} {$dept} {$subdept} {$timefilter} ";
          ...
          case 'overduedeadline': 
            if(!isset($row->overduedeadline) && isset($data->subquery)){
                ...
                FROM {block_ls_learningformats} bll 
                WHERE bll.upcomingdeadline < UNIX_TIMESTAMP() AND bll.moduleid IN (1,7) AND bll.completiondate = 0 AND bll.upcomingdeadline != 0 AND bll.id = $row->id {$learning} {$costcenter} {$dept} {$subdept} {$timefilter} ";
