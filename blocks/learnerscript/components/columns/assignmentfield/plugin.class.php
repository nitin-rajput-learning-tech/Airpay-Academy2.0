...
class plugin_assignmentfield extends pluginbase {
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        $assignmentrecord = $DB->get_record('assign', array('id' => $row->id));
        ...
        switch($data->column){
            ...
            case 'course':
                ...
                if(empty($reportid) || empty($checkpermissions)){
                    ...
                    $assignmentrecord->{$data->column} = '<a href="'.$CFG->wwwroot.'/blocks/learnerscript/viewreport.php?id='.$reportid.'&filter_course='.$assignmentrecord->course.'" />'.$coursename.'</a>';
                }
            break;
            case 'duedate':
