...
class plugin_customcategories extends pluginbase {

    public function init() {
        $this->form = false;
        $this->unique = true;
        $this->fullname = get_string('filtercustomcategories', 'block_learnerscript');
        $this->reporttypes = array('courses');
        $this->filtertype = 'custom';
        if (!empty($this->reportclass->basicparams)) {
            foreach ($this->reportclass->basicparams as $basicparam) {
                if ($basicparam['name'] == 'customcategories') {
    ...
    public function summary($data) {
        return get_string('filtercustomcategories_summary', 'block_learnerscript');
    }

    public function execute($finalelements, $data) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/course/lib.php");
        $category = optional_param('filter_customcategories', 0, PARAM_INT);
        if (!$category) {
    ...
    public function filter_data($selectoption = true){
        ...
        $filtercategories = optional_param('filter_customcategories', 0, PARAM_INT);
        ...
        if(!is_siteadmin($this->reportclass->userid) && !(new ls)->is_manager($this->reportclass->userid, $this->reportclass->contextlevel, $this->reportclass->role)){
            if(!empty($this->reportclass->rolewisecourses)){
                ...
                $categories = $DB->get_records_sql_menu("SELECT distinct(cat.id),cat.fullname from {local_custom_category} as cat 
                                                           JOIN {course} as c ON cat.id = c.open_categoryid 
    ...
    public function print_filter(&$mform) {
        ...
        $select = $mform->addElement('select', 'filter_customcategories', get_string('category'), $displaylist, array('data-select2'=>1));
        $select->setHiddenLabel(true);
        $mform->setDefault('filter_customcategories', 0);
        $mform->setType('filter_customcategories', PARAM_INT);
    }
