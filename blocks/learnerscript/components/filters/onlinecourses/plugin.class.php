...
class plugin_onlinecourses extends pluginbase {

    public function init() {
        $this->form = false;
        $this->unique = true;
        $this->fullname = get_string('onlinecourses', 'block_learnerscript');
        $this->reporttypes = array(); 
        if (!empty($this->reportclass->basicparams)) {
            foreach ($this->reportclass->basicparams as $basicparam) {
                if ($basicparam['name'] == 'onlinecourses') {
    ...
    public function summary($data) {
        return get_string('onlinecourses_summary', 'block_learnerscript');
    }

    public function execute($finalelements, $data) {
        $filteronlinecourses = optional_param('filter_onlinecourses', 0, PARAM_INT);
        if (!$filteronlinecourses) {
            return $finalelements;
        }

        if ($this->report->type != 'sql') {
            return array($filteronlinecourses);
        } else {
            if (preg_match("/%%FILTER_ONLINECOURSES:([^%]+)%%/i", $finalelements, $output)) {
                $replace = ' AND ' . $output[1] . ' = ' . $filteronlinecourses;
                return str_replace('%%FILTER_ONLINECOURSES:' . $output[1] . '%%', $replace, $finalelements);
            }
        }
        return $finalelements;
    }
     public function filter_data($selectoption = true, $request){
        ...
        $selectoption = array();
        $selectoption[0] = get_string('selectonlinecourse', 'block_learnerscript');

        $onlinecourseslist = $selectoption + $onlinecourses;
    ...
    public function print_filter(&$mform) { 
        ...
        $onlinecourseslist = $this->filter_data(false, $request);
        $array = array('data-select2' => 1,'data-maximum-selection-length' => $this->maxlength);
        $select = $mform->addElement('select', 'filter_onlinecourses', get_string('onlinecourses'),  $onlinecourseslist, $array);

        $mform->setType('filter_onlinecourses', PARAM_INT);
    }
}
