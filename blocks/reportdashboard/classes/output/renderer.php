...
class block_reportdashboard_renderer extends plugin_renderer_base {
    ...
    public function compliances($complianceid, $costcenter=NULL, $department=NULL, $subdepartment=NULL) {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/blocks/reportdashboard/lib.php');
        $complicanes = compliances($costcenter, $department,$subdepartment);
        $overcomplicanes = overcompliances($complianceid, $costcenter, $department, $subdepartment);
        $sections = sections($complianceid, $costcenter, $department, $subdepartment);
        $compliancetracking = compliancetracking($complianceid);
        echo $this->render_from_template('block_reportdashboard/compliance', array('overcompliances' => $overcomplicanes));
        echo $this->render_from_template('block_reportdashboard/compliance_tabs', array('compliances' => $complicanes, 'section' => $sections, 'compliancetracking' => $compliancetracking));
    }    
    /**
