...
class widgetheader implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        //     }
        // }
        if (!empty($this->schreportform)) {
                $data['action'][] = ["schreportform" => $this->schreportform,
                                   "icon" => $OUTPUT->image_url($this->schreportform, "block_reportdashboard"),
                                   "title" => get_string($this->schreportform, 'block_reportdashboard')
                                  ];
        }
        if (!empty($this->sendreportemail)) {
                $data['action'][] = ["sendreportemail" => $this->sendreportemail,
                                   "icon" => $OUTPUT->image_url($this->sendreportemail, "block_reportdashboard"),
                                   "title" => get_string($this->sendreportemail, 'block_reportdashboard')
                                  ];
        }
        if (!empty($this->reportfilter)) {
                $data['action'][] = ["reportfilter" => $this->reportfilter,
                                   "icon" => $OUTPUT->image_url($this->reportfilter, "block_reportdashboard"),
                                   "title" => get_string($this->reportfilter, 'block_reportdashboard')
                                  ];
        }
        ...
        if ($data['dashboardurl'] == 'Maindashboard') {
            $data['exploredashboard'] = 1;
        } else {
            $data['exploredashboard'] = 0;
        } 
        if ($data['reporttitle'] == 'Learning overview') {
           $data['titleexplore'] = 'Learning';
           $data['explorelink'] = 'Learnerdashboard';
        } else if ($data['reporttitle'] == 'Learning by Learner') {
            $data['titleexplore'] = 'Learning';
            $data['explorelink'] = 'Learnerdashboard';
        } else if ($data['reporttitle'] == 'Exams By Vendor') {
            $data['titleexplore'] = 'Exams';
            $data['explorelink'] = 'Examdashboard';
        } else if ($data['reporttitle'] == 'Certifications By Vendor') {
            $data['titleexplore'] = 'Certifications'; 
            $data['explorelink'] = 'Certification';
        }
        return $data;
    }
}
