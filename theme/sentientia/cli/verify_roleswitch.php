...
/**
 * CLI QA tool — verify the sentientia sidebar role-switcher options builder.
 *
 ...
 *   php ../theme/sentientia/cli/verify_roleswitch.php [--userid=142]
 *
 * Exit codes:
...
if ($options['help']) {
    cli_writeln("Verify sentientia role-switcher options builder.\n  php ../theme/sentientia/cli/verify_roleswitch.php [--userid=142]");
    exit(0);
}
...
// Bring up the sentientia renderer in CLI.
...
// (the factory forces it), which lacks our trait. Instantiate the sentientia
// renderer directly — get_role_switch_options() needs only $USER / $DB /
// accesslib / role_detector / sesskey, no web-output context.
$rendererclass = '\\theme_sentientia\\output\\core_renderer';
if (!class_exists($rendererclass)) {
    cli_error("$rendererclass not autoloadable — is the sentientia theme deployed?", 2);
}
...
if ($empid) {
    ...
    $assert('B: switched to Employee — expect Employee active',
        get_string('employee', 'theme_sentientia'));
}
