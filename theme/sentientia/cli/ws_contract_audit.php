 ...
 * theme_sentientia/datatable is a shared AMD client used by 9+ WS endpoints
 ...
 *
 * This CLI tool wraps theme_sentientia\ws_contract_scanner so any dev can
 * run the audit on demand without writing PHP. Same logic + assertions
 * as theme/sentientia/tests/ws_contract_test.php — the PHPUnit gate that
 * runs in CI.
 *
 * Usage:
 *   cd <moodle-public-root>
 *   php ../theme/sentientia/cli/ws_contract_audit.php [--verbose] [--json]
 *
 * Options:
...
require_once($CFG->dirroot . '/theme/sentientia/classes/ws_contract_scanner.php');
...
theme_sentientia WS contract audit
================================
...
Usage:
    php theme/sentientia/cli/ws_contract_audit.php [options]
...
Background: this tool wraps theme_sentientia\\ws_contract_scanner, the
...
$result = \theme_sentientia\ws_contract_scanner::audit();
...
cli_writeln('');
cli_writeln('=== theme_sentientia WS contract audit ===');
cli_writeln('');
