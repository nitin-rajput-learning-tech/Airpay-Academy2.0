...
global $DB;
// SENTIENTIA ADR-018 (2026-06-09): open_supervisorid is a BizLMS-injected column,
// absent on a vanilla / non-Airpay install. Guard it (same pattern as role_detector);
// when missing, derive supervisor status from the native Moodle capability so the
// dashboard renders on any install. On Airpay (column present) behaviour is unchanged.
$is_supervisor = false;
try {
    ...
    if ($dbman->field_exists(new \xmldb_table('user'), new \xmldb_field('open_supervisorid'))) {
        ...
        $is_supervisor = has_capability('moodle/site:viewreports', \context_system::instance(), $USER->id);
    }
} catch (\Throwable $e) {
    $is_supervisor = false;
}
...
$sidebarnav = new \theme_sentientia\sidebar_navigation($PAGE);
