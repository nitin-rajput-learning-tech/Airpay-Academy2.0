 ...
 * theme_sentientia_user_get_user_navigation_info() (in lib.php) which is
 ...
 *   - $this->theme_sentientia_user_get_user_navigation_info(...)
...
trait user_menu {
    ...
    public function user_menu($user = null, $withlinks = null) {
        ...
        $opts = $this->theme_sentientia_user_get_user_navigation_info($user, $this->page, array('avatarsize' => 35));
        ...
        if (is_array($roles) && (count($roles) > 0)) {
            ...
            if(!empty($learnerroleid)){
                if($learnerroleid->id == $current_roleid){
                 ...
                 $switchrole->title = get_string('switchroleas','theme_sentientia').$rolename;
     ...
     * live in a top-right navbar. The sentientia dashboard "shell" layout
     ...
     * no logout) so the sidebar can paint a native sentientia control. It is a
    ...
    public function get_role_switch_options(): array {
        ...
        if (!empty($learnerrole)) {
            $work[] = [
                'label'      => get_string('employee', 'theme_sentientia'),
                'icon'       => 'fa-user',
        ...
        if (!$anyactive && !empty($work) && class_exists('\\theme_sentientia\\role_detector')) {
            $tier = \theme_sentientia\role_detector::detect();
            $marked = false;
