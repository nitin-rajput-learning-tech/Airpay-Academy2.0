...
vendor/bin/phpunit.bat --group role_detector ^
    public/theme/sentientia/tests/role_detector_test.php

# ws_contract — 1 test (walks every datatable consumer), ~1 sec
vendor/bin/phpunit.bat --group ws_contract ^
    public/theme/sentientia/tests/ws_contract_test.php
...
### Expected results
  ...
  - `role_detector_test`: **8 tests, 17 assertions, 4 skipped**.
    ...
    schema (e.g., `local/sentientia_courses:manage` capability,
...
```powershell
cd C:\xampp\htdocs\moodle5
php public/theme/sentientia/cli/ws_contract_audit.php          # human-readable
php public/theme/sentientia/cli/ws_contract_audit.php --json   # CI-pipeable
php public/theme/sentientia/cli/ws_contract_audit.php --help   # usage
```

The CLI consumes the same `\theme_sentientia\ws_contract_scanner` utility
...
## Adding new tests
  ...
  - Use `@group theme_sentientia` plus a specific group annotation
...
adoption decision for IT is:
  ...
  3. Optional: run the full `theme_sentientia` group nightly to catch
     deeper regressions.
