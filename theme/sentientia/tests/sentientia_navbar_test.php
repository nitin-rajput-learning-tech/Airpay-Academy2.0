...
/**
 * Test the sentientia_navbar file
 *
 * @package    theme_sentientia
 * @covers     \theme_sentientia\sentientia_navbar
 * @copyright  2021 Peter Dias
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sentientia_navbar_test extends \advanced_testcase {
    /**
    ...
    public function test_remove_no_link_items(array $setup, bool $removesectionnodes, array $expected) {
        ...
        $sentientia_navbar = $this->getMockBuilder(sentientia_navbar::class)
            ->disableOriginalConstructor()
            ->onlyMethods([])
            ->getMock();

        $rc = new \ReflectionClass(sentientia_navbar::class);
        $rcp = $rc->getProperty('items');
        $rcp->setAccessible(true);
        $rcp->setValue($sentientia_navbar, $page->navbar->get_items());

        // Make the call to the function.
        $rcm = $rc->getMethod('remove_no_link_items');
        $rcm->setAccessible(true);
        $rcm->invoke($sentientia_navbar, $removesectionnodes);

        // Get the value for the class variable that the function modifies.
        $values = $rcp->getValue($sentientia_navbar);
        $actual = [];
    ...
    public function test_remove_items_that_exist_in_navigation(string $navmenu, array $navmenunodes, array $navbarnodes,
        ...
        $rcm->setAccessible(true);
        $rcm->invoke($sentientia_navbar, $navigationmenu);
