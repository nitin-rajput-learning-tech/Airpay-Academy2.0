 ...
 */
function theme_sentientia_css_tree_post_processor($tree, $theme) {
    error_log('theme_sentientia_css_tree_post_processor() is deprecated. Required' .
        'prefixes for Bootstrap are now in theme/epsilon/scss/moodle/prefixes.scss');
    $prefixer = new theme_sentientia\autoprefixer($tree);
    $prefixer->prefix();
}
 ...
 * @return string
 */
function theme_sentientia_get_extra_scss($theme) {
    $content = '';
 ...
 * @return bool
 */
function theme_sentientia_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM && ($filearea === 'logo' || $filearea === 'backgroundimage'
        ...
        // SENTIENTIA independence (eAbyas/epsilon audit 2026-06-02): serve sentientia's OWN
        // setting files. The 7 setting files were migrated theme_epsilon -> theme_sentientia.
        $theme = theme_config::load('sentientia');
 ...
 * @return string
 */
// function theme_sentientia_get_main_scss_content($theme) {
//     global $CFG;
...
//         $scss .= file_get_contents($CFG->dirroot . '/theme/sentientia/scss/preset/default.scss');
//     } else if ($filename == 'plain.scss') {
//         $scss .= file_get_contents($CFG->dirroot . '/theme/sentientia/scss/preset/plain.scss');
//     } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_sentientia', 'preset', 0, '/', $filename))) {
...
//     if($theme_scheme){
//         if(file_exists($CFG->dirroot . '/theme/sentientia/scss/schemes/'.$theme_scheme.'.scss')){
//             $scheme_scss = file_get_contents($CFG->dirroot . '/theme/sentientia/scss/schemes/'.$theme_scheme.'.scss');
//         }
//     }
 ...
 */
function theme_sentientia_get_precompiled_css() {
    global $CFG;
    return file_get_contents($CFG->dirroot . '/theme/sentientia/style/moodle.css');
}
 ...
 * @return array
 */
function theme_sentientia_get_pre_scss($theme) {
    global $CFG;
 ...
 * @return url
 */
function theme_sentientia_get_css_for_costcenter_scss($costcenter_scheme = false){
    global $CFG;
    ...
    if($costcenter_scheme){
        if(file_exists($CFG->dirroot . '/theme/sentientia/scss/schemes/'.$costcenter_scheme.'.scss')){
            $www_file = $CFG->wwwroot.'/theme/sentientia/style/'.$costcenter_scheme.'.css';
            $dir_file = $CFG->dirroot.'/theme/sentientia/style/'.$costcenter_scheme.'.css';
            ...
            $scheme_scss = file_get_contents($CFG->dirroot . '/theme/sentientia/scss/schemes/'.$costcenter_scheme.'.scss');
            $scss = $scheme_scss;
...
// `\theme_sentientia\hook_callbacks::before_standard_top_of_body_html_generation()`
...
if (!class_exists('\\core\\hook\\output\\before_standard_top_of_body_html_generation')) {
     ...
     *                blob consumed by AMD theme_sentientia/user_status_badge.
     *                Empty string when no badges are needed.
     */
    function theme_sentientia_before_standard_top_of_body_html(): string {
        // Delegate to the same builder the hook class uses — single
        // source of truth across 5.1 and 5.2.
        if (class_exists('\\theme_sentientia\\hook_callbacks')) {
            return \theme_sentientia\hook_callbacks::build_user_status_html();
        }
        return '';
    }
}
...
/**
 * Register the theme's writable user preferences.
 *
 * Moodle 5.1 removed /lib/ajax/setuserpref.php. The modern persistence path
 * is the core_user/repository AMD module's setUserPreference(), which POSTs
 * to core_user -> current/preferences/{name} and validates each preference
 * against the list returned by <component>_user_preferences() callbacks.
 * Without this registration the sidebar-collapse toggle's persistence call
 * is rejected, so the collapsed state would not survive a page reload.
 *
 * Mirrors core theme_boost_user_preferences() (theme/boost/lib.php).
 *
 * @return array Preference definitions keyed by full preference name.
 */
function theme_sentientia_user_preferences(): array {
    return [
        'theme_sentientia_sidebar_collapsed' => [
            'type' => PARAM_BOOL,
