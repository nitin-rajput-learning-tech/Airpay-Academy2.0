...
/**
 * Privacy Subsystem implementation for theme_epsilon.
 *
...
namespace theme_epsilon\privacy;
...
class provider implements
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if (isset($draweropennavpref)) {
            $preferencestring = get_string('privacy:drawernavclosed', 'theme_epsilon');
            if ($draweropennavpref == 'true') {
                $preferencestring = get_string('privacy:drawernavopen', 'theme_epsilon');
            }
        ...
        if (isset($draweropenindexpref)) {
            $preferencestring = get_string('privacy:drawerindexclosed', 'theme_epsilon');
            if ($draweropenindexpref == 1) {
                $preferencestring = get_string('privacy:drawerindexopen', 'theme_epsilon');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_epsilon',
        ...
        if (isset($draweropenblockpref)) {
            $preferencestring = get_string('privacy:drawerblockclosed', 'theme_epsilon');
            if ($draweropenblockpref == 1) {
                $preferencestring = get_string('privacy:drawerblockopen', 'theme_epsilon');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_epsilon',
