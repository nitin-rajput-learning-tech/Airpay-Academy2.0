...
// use core_component; // Removed — non-compound name has no effect in PHP 8.2+
$value = '';
...
if(!is_siteadmin()){
    // Allow users to view their own learning paths even without manage capability.
    // Only require manage capability for admin actions (create/edit/delete).
    $hasmanage = has_capability('local/learningplan:manage', $categorycontext);
    $hasview = has_capability('local/learningplan:view', $categorycontext, null, false);
    if (!$hasmanage && !$hasview) {
        // Check if user has any learning plan enrollments — if yes, they can view.
        $hasenrollments = $DB->record_exists('local_learningplan_user', ['userid' => $USER->id]);
        if (!$hasenrollments) {
