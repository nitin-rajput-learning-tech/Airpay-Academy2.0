...
const openRuleForm = async (ruleid, returnFocus) => {
    ...
    const title = await getString(titleKey, 'local_airpay_notifications');
...
const confirmToggle = async (ruleid, name, makeEnabled, returnFocus) => {
    ...
    const [title, message, success] = await Promise.all([
        getString(titleKey, 'local_airpay_notifications'),
        getString(msgKey, 'local_airpay_notifications', name),
        getString('rulestatuschanged', 'local_airpay_notifications'),
    ]);
...
const confirmDelete = async (ruleid, name, returnFocus) => {
    const [title, message, label, success] = await Promise.all([
        getString('deleterule', 'local_airpay_notifications'),
        getString('confirmdelete', 'local_airpay_notifications', name),
        getString('delete', 'core'),
        getString('ruledeleted', 'local_airpay_notifications'),
    ]);
