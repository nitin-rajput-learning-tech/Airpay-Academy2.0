...
//
// Run: php public/local/airpay_notifications/cli/smoke_preview_send.php
//
...
// 1. Seed a rule.
$rule = $DB->insert_record('local_airpay_notif_rules', (object) [
...
// 2. preview_rule.
$result = \local_airpay_notifications\external\preview_rule::execute($rule, 0);
...
$result = \local_airpay_notifications\external\test_send::execute($rule, 0);
if (!$result['ok']) {
...
$logged = $DB->record_exists_select('local_airpay_notif_log',
...
// 5. Cleanup.
$DB->delete_records('local_airpay_notif_log', ['ruleid' => $rule]);
$DB->delete_records('local_airpay_notif_rules', ['id' => $rule]);
echo "Cleanup ✓\n";
