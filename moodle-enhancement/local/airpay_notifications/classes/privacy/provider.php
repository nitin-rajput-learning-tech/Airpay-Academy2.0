...
//
// Phase Z.1 (2026-05-08) — privacy provider for airpay_notifications.

namespace local_airpay_notifications\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table('local_airpay_notif_log',
            [
        ...
        $collection->add_database_table('local_airpay_notif_prefs',
            [
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        if ($DB->record_exists('local_airpay_notif_log', ['userid' => $userid])
            || $DB->record_exists('local_airpay_notif_prefs', ['userid' => $userid])) {
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        $log = $DB->get_records('local_airpay_notif_log',
        ...
        \core_privacy\local\request\writer::with_context(
            ...
            ->export_data(['airpay_notifications'],
                (object) [
                    'log_count' => count($log),
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        $DB->delete_records('local_airpay_notif_log');
        $DB->delete_records('local_airpay_notif_prefs');
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        $DB->delete_records('local_airpay_notif_log', ['userid' => $uid]);
        $DB->delete_records('local_airpay_notif_prefs', ['userid' => $uid]);
    }

    public static function get_users_in_context(userlist $userlist) {
        global $DB;
        if ($userlist->get_context()->contextlevel !== CONTEXT_SYSTEM) return;
        $log_users = $DB->get_fieldset_select('local_airpay_notif_log',
            'DISTINCT userid', 'userid > 0');
        $pref_users = $DB->get_fieldset_select('local_airpay_notif_prefs',
            'DISTINCT userid', 'userid > 0');
    ...
    public static function delete_data_for_users(
        ...
        $DB->delete_records_select('local_airpay_notif_log', "userid $insql", $inparams);
        $DB->delete_records_select('local_airpay_notif_prefs', "userid $insql", $inparams);
    }
