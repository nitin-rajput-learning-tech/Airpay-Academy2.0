...
namespace local_airpay_org\form;
...
class edit_org extends \core_form\dynamic_form {
    ...
    protected function definition() {
        ...
        // ── Identity ──────────────────────────────────────────────────
        $mform->addElement('header', 'hdr_basic', get_string('heading_basic', 'local_airpay_org'));

        $mform->addElement('text', 'fullname',
            get_string('org_fullname', 'local_airpay_org'),
            ['size' => 50, 'maxlength' => 254, 'placeholder' => 'e.g. Airpay Payment Services']);
        $mform->setType('fullname', PARAM_TEXT);
        $mform->addRule('fullname', null, 'required', null, 'client');

        $mform->addElement('text', 'shortname',
            get_string('org_shortname', 'local_airpay_org'),
            ['size' => 30, 'maxlength' => 100, 'placeholder' => 'e.g. AirPay_Acquiring']);
        $mform->setType('shortname', PARAM_TEXT);
        $mform->addHelpButton('shortname', 'shortname', 'local_airpay_org');

        $mform->addElement('textarea', 'description',
            get_string('description', 'local_airpay_org'),
            ['rows' => 2, 'cols' => 50]);
        $mform->setType('description', PARAM_TEXT);

        // ── Hierarchy ─────────────────────────────────────────────────
        $mform->addElement('header', 'hdr_hierarchy', get_string('heading_hierarchy', 'local_airpay_org'));

        if ($orgid === 0) {
            // Creating new org — pick parent.
            $parents = $this->get_parent_options();
            $mform->addElement('select', 'parentid',
                get_string('parent_org', 'local_airpay_org'), $parents);
            $mform->setType('parentid', PARAM_INT);
            $mform->setDefault('parentid', $parentid);
            $mform->addHelpButton('parentid', 'parent_org', 'local_airpay_org');
        } else {
            ...
            $parent_label = $existing && (int) $existing->parentid > 0
                ? \local_airpay_org\org_manager::get_name((int) $existing->parentid)
                : get_string('top_level_tenant', 'local_airpay_org');
            $mform->addElement('static', 'parent_static',
                get_string('parent_org', 'local_airpay_org'),
        ...
        $mform->addElement('select', 'theme_scheme',
            get_string('theme_scheme', 'local_airpay_org'), $themes);
        $mform->setType('theme_scheme', PARAM_ALPHA);

        // ── Visibility ────────────────────────────────────────────────
        $mform->addElement('header', 'hdr_visibility', get_string('heading_visibility', 'local_airpay_org'));

        $visibility_options = [
            1 => get_string('visible_yes', 'local_airpay_org'),
            0 => get_string('visible_no', 'local_airpay_org'),
        ];
        $mform->addElement('select', 'visible',
            get_string('visible', 'local_airpay_org'), $visibility_options);
        $mform->setType('visible', PARAM_INT);
        $mform->setDefault('visible', 1);

        $mform->addElement('text', 'sortorder',
            get_string('sortorder', 'local_airpay_org'),
            ['size' => 5, 'maxlength' => 5]);
    ...
    public function process_dynamic_submission() {
        ...
        if ($orgid === 0) {
            ...
            return [
                'orgid'   => $newid,
                'message' => get_string('org_created', 'local_airpay_org'),
            ];
        } else {
            org_manager::update($orgid, $data);
            return [
                'orgid'   => $orgid,
                'message' => get_string('org_updated', 'local_airpay_org'),
            ];
        }
    }
    ...
    protected function get_page_url_for_dynamic_submission(): \moodle_url {
        return new \moodle_url('/local/airpay_org/admin.php');
    }

    protected function check_access_for_dynamic_submission(): void {
        require_capability('local/airpay_org:manage', $this->get_context_for_dynamic_submission());
    }
    ...
    private function get_parent_options(): array {
        global $DB;
        $options = [0 => get_string('top_level_tenant', 'local_airpay_org')];
        $orgs = $DB->get_records_select('local_airpay_org',
    ...
    private function get_existing_record(int $orgid) {
        global $DB;
        return $DB->get_record('local_airpay_org', ['id' => $orgid], '*', MUST_EXIST);
    }
}
