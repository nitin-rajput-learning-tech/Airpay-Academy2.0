...
namespace local_airpay_org\task;

defined('MOODLE_INTERNAL') || die();

/**
 * Daily cron — sync local_airpay_org tree into Moodle core cohorts.
 *
...
class sync_cohorts extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        $org_nodes = $DB->get_records('local_airpay_org', null, 'depth ASC, path ASC',
        ...
        foreach ($org_nodes as $org) {
            ...
            if ($existing) {
                ...
                $c->idnumber      = $idnumber;
                $c->description   = "Auto-synced from local_airpay_org id={$org->id} (path={$org->path}). "
                    . "Managed by local_airpay_org sync_cohorts task — do not edit manually.";
                $c->descriptionformat = FORMAT_PLAIN;
                $c->visible       = 1;
                $c->component     = 'local_airpay_org';  // marks as plugin-managed
        ...
        foreach ($users as $u) {
            ...
            foreach ($parts as $part) {
                $current_path .= '/' . $part;
                $org = $DB->get_record('local_airpay_org', ['path' => $current_path], 'id');
                if (!$org) continue;
        ...
        mtrace(sprintf("airpay_org sync_cohorts: created=%d updated=%d "
