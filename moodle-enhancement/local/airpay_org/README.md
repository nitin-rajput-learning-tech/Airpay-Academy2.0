...
|---|---|---|
| `local/airpay_org:manage` | write | siteadmin |
| `local/airpay_org:manage_multiorganizations` | write | siteadmin |
| `local/airpay_org:manage_ownorganization` | write | tenant admin |
| `local/airpay_org:manage_owndepartments` | write | OH role |

## Tables

`local_airpay_org` — node hierarchy with `path`, `depth`, `costcenterid`.

## Scheduled tasks

`\local_airpay_org\task\sync_cohorts` — daily 02:47 IST. Production
...
```php
php -r "require '/path/to/moodle/config.php'; echo \\local_airpay_org\\manager::resolve_tenant_root(\$USER), PHP_EOL;"
