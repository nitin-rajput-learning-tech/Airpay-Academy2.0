...
if (!$tenantid) {
    ...
    $tenantid = \local_airpay_org\tenant_manager::get_tenant_id($USER);
}
if ($tenantid <= 0) {
    throw new \moodle_exception('invalidtenant', 'local_airpay_org');
}

$tenant = $DB->get_record('local_airpay_org', ['id' => $tenantid], '*', MUST_EXIST);

$ctx = context_system::instance();
$PAGE->set_context($ctx);
$PAGE->set_url(new moodle_url('/local/airpay_org/tenant_settings.php', ['id' => $tenantid]));
...
// - has local/airpay_org:managetenant cap → own tenant only
require_capability('local/airpay_org:view', $ctx);
$is_admin = is_siteadmin();
$can_manage = $is_admin
    || has_capability('local/airpay_org:managetenant', $ctx);
...
if (!$is_admin) {
    $user_tenant = \local_airpay_org\tenant_manager::get_tenant_id($USER);
...
class tenant_settings_form extends moodleform {
    protected function definition() {
        ...
        $mform->addHelpButton('org_logo', 'org_logo', 'local_airpay_org');
        ...
        $mform->addHelpButton('theme_scheme', 'theme_scheme', 'local_airpay_org');
...
file_prepare_draft_area($draftid_logo, $ctx->id, 'local_airpay_org',
    'org_logo', (int) ($tenant->org_logo ?? 0), ['subdirs' => 0, 'maxfiles' => 1]);

$draftid_favicon = file_get_submitted_draft_itemid('favicon');
file_prepare_draft_area($draftid_favicon, $ctx->id, 'local_airpay_org',
...
if ($form->is_cancelled()) {
    redirect(new moodle_url('/local/airpay_org/admin.php'));
}
...
if ($data = $form->get_data()) {
    ...
    $DB->update_record('local_airpay_org', $update);

    // Persist filemanager uploads (logo + favicon).
    file_save_draft_area_files((int) $data->org_logo, $ctx->id,
        'local_airpay_org', 'org_logo', $tenant->id,
        ['subdirs' => 0, 'maxfiles' => 1]);
    $DB->set_field('local_airpay_org', 'org_logo', (int) $data->org_logo,
        ['id' => $tenant->id]);

    file_save_draft_area_files((int) $data->favicon, $ctx->id,
        'local_airpay_org', 'favicon', $tenant->id,
        ['subdirs' => 0, 'maxfiles' => 1]);
    $DB->set_field('local_airpay_org', 'favicon', (int) $data->favicon,
        ['id' => $tenant->id]);
    ...
    redirect(new moodle_url('/local/airpay_org/tenant_settings.php',
        ['id' => $tenant->id]));
}
