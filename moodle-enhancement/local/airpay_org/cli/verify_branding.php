 ...
 *
 * Usage: php local/airpay_org/cli/verify_branding.php
 *
...
// ═══════════════════════════════════════════════════════
$check = 'local_airpay_org plugin installed';
$pluginman = \core_plugin_manager::instance();
$plugininfo = $pluginman->get_plugin_info('local_airpay_org');
if ($plugininfo) {
...
// ═══════════════════════════════════════════════════════
// 7. Migration completed (airpay_org table has data)
// ═══════════════════════════════════════════════════════
$check = 'local_airpay_org table has data';
$dbman = $DB->get_manager();
if ($dbman->table_exists('local_airpay_org')) {
    $count = $DB->count_records('local_airpay_org');
    if ($count > 0) {
        cli_pass($check, "{$count} records");
        $pass++;
    } else {
        cli_warn($check, 'empty — run php local/airpay_org/cli/migrate_all.php');
        $warn++;
    }
} else {
