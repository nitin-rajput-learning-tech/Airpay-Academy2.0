...
//
// Run: php public/local/airpay_org/cli/uat_l_messaging.php
//
...
echo "=== UAT-L3.1: Manager decision (request_decided) ===\n";
{
    ...
    echo "  Pre-test local_airpay_manager count: $pre\n";
    ...
    try {
        \local_airpay_manager\approval_manager::notify_requester_of_decision(
    ...
    $msg = $DB->get_record_sql(
        "SELECT * FROM {notifications}
          WHERE useridfrom <> :uid AND component = 'local_airpay_manager'
       ORDER BY id DESC LIMIT 1",
        ['uid' => $recipient->id]);
    if ($msg) {
        pass('UAT-L3.1.c Found a local_airpay_manager message');
        echo "    subject: '$msg->subject'\n";
        echo "    component: '$msg->component', eventtype: '$msg->eventtype'\n";
    } else {
        fail('UAT-L3.1.c No local_airpay_manager message in mdl_notifications', '');
    }
}
...
echo "\n=== UAT-L3.2: Manager allocation (allocation_assigned) ===\n";
{
    ...
    try {
        ...
        \local_airpay_manager\approval_manager::notify_assignee_of_allocation(
    ...
    $msg = $DB->get_record_sql(
        "SELECT * FROM {notifications}
          WHERE component = 'local_airpay_manager'
...
echo "\n=== UAT-L3.3: Notification test_send (smart_alert) ===\n";
{
    ...
    $ruleid = $DB->insert_record('local_airpay_notif_rules', (object) [
    ...
    try {
        $result = \local_airpay_notifications\external\test_send::execute(
    ...
    $msg = $DB->get_record_sql(
        "SELECT * FROM {notifications}
          WHERE component = 'local_airpay_notifications'
       ORDER BY id DESC LIMIT 1");
    if ($msg) {
    ...
    // Cleanup the rule.
    $DB->delete_records('local_airpay_notif_rules', ['id' => $ruleid]);
    $DB->delete_records('local_airpay_notif_log', ['ruleid' => $ruleid]);
}
...
echo "\n=== UAT-L3.4: Message providers registration ===\n";
{
    $providers = $DB->get_records('message_providers',
        ['component' => 'local_airpay_manager']);
    pass('UAT-L3.4.a local_airpay_manager providers: ' . count($providers));
    ...
    pass('UAT-L3.4.b local_airpay_notifications providers: ' . count($providers2));
