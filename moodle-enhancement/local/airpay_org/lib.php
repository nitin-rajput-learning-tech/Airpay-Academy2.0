 ...
 * Can be called as airpay_org_logo($itemid) from core_renderer.
 *
 * @param int $itemid  File item ID stored in org_logo / costcenter_logo field
 * @return string  Logo URL or empty string
 */
function airpay_org_logo(int $itemid): string {
    return \local_airpay_org\branding_manager::get_logo_url($itemid);
}
 ...
 * @return bool
 */
function local_airpay_org_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    ...
    $fs = get_file_storage();
    $file = $fs->get_file($context->id, 'local_airpay_org', $filearea, $itemid, $filepath, $filename);
