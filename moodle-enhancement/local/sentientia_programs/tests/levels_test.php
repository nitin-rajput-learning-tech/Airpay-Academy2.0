...
final class levels_test extends \advanced_testcase {
    ...
    private function seed_program(string $name = 'Test Program'): int {
        ...
        if (!$DB->get_manager()->table_exists('local_sentientia_programs')) {
            $this->markTestSkipped('local_sentientia_programs not present.');
        }
        $now = time();
    ...
    public function test_create_level_persists_with_auto_sortorder(): void {
        ...
        global $DB;
        $a = $DB->get_record('local_sentientia_programs_levels', ['id' => $first], '*', MUST_EXIST);
        $b = $DB->get_record('local_sentientia_programs_levels', ['id' => $second], '*', MUST_EXIST);
    ...
    public function test_update_level_persists(): void {
        ...
        $row = $DB->get_record('local_sentientia_programs_levels', ['id' => $lid], '*', MUST_EXIST);
    ...
    public function test_delete_level_cascades_courses(): void {
        ...
        $this->assertFalse($DB->record_exists('local_sentientia_programs_levels', ['id' => $lid]));
        $this->assertFalse($DB->record_exists('local_sentientia_programs_courses', ['levelid' => $lid]));
    }
    ...
    public function test_delete_level_reflows_sortorder(): void {
        ...
        $this->assertEquals(0, (int) $DB->get_field('local_sentientia_programs_levels', 'sortorder', ['id' => $a]));
        $this->assertEquals(1, (int) $DB->get_field('local_sentientia_programs_levels', 'sortorder', ['id' => $c]));
    }
    ...
    public function test_reorder_levels_swaps_positions(): void {
        ...
        $this->assertEquals(0, (int) $DB->get_field('local_sentientia_programs_levels', 'sortorder', ['id' => $c]));
        $this->assertEquals(1, (int) $DB->get_field('local_sentientia_programs_levels', 'sortorder', ['id' => $a]));
        $this->assertEquals(2, (int) $DB->get_field('local_sentientia_programs_levels', 'sortorder', ['id' => $b]));
    }
    ...
    public function test_reorder_levels_skips_outsider_ids(): void {
        ...
        $this->assertEquals(0, (int) $DB->get_field('local_sentientia_programs_levels', 'sortorder', ['id' => $b]));
        $this->assertEquals(1, (int) $DB->get_field('local_sentientia_programs_levels', 'sortorder', ['id' => $a]));
    }
    ...
    public function test_unassign_course_from_level(): void {
        ...
        $this->assertTrue($DB->record_exists('local_sentientia_programs_courses',
            ['levelid' => $lid, 'courseid' => $c->id]));

        program_manager::unassign_course_from_level($lid, (int) $c->id);

        $this->assertFalse($DB->record_exists('local_sentientia_programs_courses',
            ['levelid' => $lid, 'courseid' => $c->id]));
    }
    ...
    public function test_delete_program_cascades_through_all_tables(): void {
        ...
        $this->assertTrue($DB->record_exists('local_sentientia_programs_levels', ['programid' => $pid]));
        $this->assertTrue($DB->record_exists('local_sentientia_programs_courses', ['levelid' => $lid]));
        $this->assertTrue($DB->record_exists('local_sentientia_programs_users', ['programid' => $pid]));

        program_manager::delete($pid);

        $this->assertFalse($DB->record_exists('local_sentientia_programs', ['id' => $pid]));
        $this->assertFalse($DB->record_exists('local_sentientia_programs_levels', ['programid' => $pid]));
        $this->assertFalse($DB->record_exists('local_sentientia_programs_courses', ['levelid' => $lid]));
        $this->assertFalse($DB->record_exists('local_sentientia_programs_users', ['programid' => $pid]));
    }
}
