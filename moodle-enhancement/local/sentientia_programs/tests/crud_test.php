...
/**
 * CRUD tests for sentientia_programs (delete + change_status + level/course cascade).
 *
...
final class crud_test extends \advanced_testcase {
    ...
    private function seed_level(int $programid): int {
        global $DB;
        if (!$DB->get_manager()->table_exists('local_sentientia_programs_levels')) {
            return 0;
        }
        $now = time();
    ...
    public function test_change_status_persists(): void {
        ...
        $this->assertEquals(program_manager::STATUS_ARCHIVED,
            (int) $DB->get_field('local_sentientia_programs', 'status', ['id' => $pid]));
    }
    ...
    public function test_delete_cascades_to_levels(): void {
        ...
        $this->assertTrue($DB->record_exists('local_sentientia_programs_levels', ['id' => $lid]));
