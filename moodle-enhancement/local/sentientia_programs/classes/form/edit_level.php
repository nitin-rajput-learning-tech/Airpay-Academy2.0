...
class edit_level extends \core_form\dynamic_form {
    ...
    protected function definition() {
        ...
        $mform->addElement('text', 'name',
            get_string('level_name', 'local_sentientia_programs'),
        ...
        $mform->addElement('textarea', 'description',
            get_string('level_description', 'local_sentientia_programs'),
            ['rows' => 3, 'cols' => 50]);
        $mform->setType('description', PARAM_TEXT);

        $mform->addElement('select', 'completion_required',
            get_string('level_completion', 'local_sentientia_programs'),
            [
                1 => get_string('level_required', 'local_sentientia_programs'),
                0 => get_string('level_optional', 'local_sentientia_programs'),
            ]);
    ...
    public function process_dynamic_submission() {
        ...
        if ($levelid === 0) {
            $newid = \local_sentientia_programs\program_manager::create_level($programid, $data);
            return [
                'programid' => $programid,
                'levelid'   => $newid,
                'message'   => get_string('levelcreated', 'local_sentientia_programs'),
            ];
        }

        \local_sentientia_programs\program_manager::update_level($levelid, $data);
        return [
            'programid' => $programid,
            'levelid'   => $levelid,
            'message'   => get_string('levelupdated', 'local_sentientia_programs'),
        ];
    }
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        $l = $DB->get_record('local_sentientia_programs_levels',
