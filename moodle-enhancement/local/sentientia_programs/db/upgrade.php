...
/**
 * Upgrade script for local_sentientia_programs.
 *
...
function xmldb_local_sentientia_programs_upgrade(int $oldversion): bool {
    global $DB;
    ...
    if ($oldversion < 2026041906) {
        ...
        $table = new xmldb_table('local_sentientia_programs');
        ...
        // ── local_sentientia_programs_levels ──────────────────────────────
        ...
        if (!$dbman->table_exists($table)) {
            ...
            $table->add_key('fk_program',  XMLDB_KEY_FOREIGN, ['programid'],
                            'local_sentientia_programs', ['id']);
            $table->add_index('idx_program_sort', XMLDB_INDEX_NOTUNIQUE, ['programid', 'sortorder']);
            $dbman->create_table($table);
        }

        // ── local_sentientia_programs_courses ─────────────────────────────
        $table = new xmldb_table('local_sentientia_programs_courses');
        if (!$dbman->table_exists($table)) {
            $table->add_field('id',          XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
            ...
            $table->add_field('timecreated', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
            $table->add_key('primary',   XMLDB_KEY_PRIMARY, ['id']);
            $table->add_key('fk_level',  XMLDB_KEY_FOREIGN, ['levelid'], 'local_sentientia_programs_levels', ['id']);
        ...
        // ── local_sentientia_programs_users ───────────────────────────────
        ...
        if (!$dbman->table_exists($table)) {
            ...
            $table->add_key('primary',    XMLDB_KEY_PRIMARY, ['id']);
            $table->add_key('fk_program', XMLDB_KEY_FOREIGN, ['programid'], 'local_sentientia_programs', ['id']);
        ...
        upgrade_plugin_savepoint(true, 2026041906, 'local', 'sentientia_programs');
    }

    // 2026051600 — P1 #9: enrolment-window dates + rich-text description.
    // Mirrors the sentientia_learningpath W2 #2 commit (8df39b36f).
