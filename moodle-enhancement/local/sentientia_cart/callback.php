...
$iplist = trim((string) get_config('local_sentientia_cart', 'airpay_callback_iplist'));
if ($iplist !== '') {
    ...
    foreach (explode(',', $iplist) as $cidr) {
        $cidr = trim($cidr);
        if ($cidr === '') continue;
        if (\local_sentientia_cart\ip_check::ip_in_cidr($remote, $cidr)) {
            $allowed = true;
            break;
        }
    }
    if (!$allowed) {
        ...
        $dedupe_cache = \cache::make('local_sentientia_cart', 'callback_drop_dedupe');
        $dedupe_key = 'drop:' . $remote . ':h:' . floor(time() / 3600);
        if (!$dedupe_cache->get($dedupe_key)) {
            \local_sentientia_cart\callback_logger::log('ip_blocked',
...
\local_sentientia_cart\callback_logger::log($gateway_name, $payload, $raw);
...
try {
    $gateway = \local_sentientia_cart\gateway\gateway_factory::get($gateway_name);
    ...
    if ($gateway->is_success($payload)) {
        ...
        if ($paid_raw === null) {
            \local_sentientia_cart\callback_logger::log('amount_missing',
                ['orderid' => $orderid], $raw);
            http_response_code(400);
            echo 'Amount missing';
            exit;
        ...
        if (abs($paid_amount - $expected) > 0.01) {
            \local_sentientia_cart\callback_logger::log('amount_mismatch',
        ...
        if ($paid_currency !== '' && $paid_currency !== strtoupper((string) $cart->currency)) {
            \local_sentientia_cart\callback_logger::log('currency_mismatch',
        ...
        \local_sentientia_cart\cart_manager::mark_paid((int) $cart->id, $ref, $payload);
        http_response_code(200);
        echo 'OK';
    } else {
        \local_sentientia_cart\cart_manager::mark_failed((int) $cart->id,
    ...
    echo 'Error';
    debugging('sentientia_cart callback error: ' . $e->getMessage(), DEBUG_DEVELOPER);
    \local_sentientia_cart\callback_logger::log('exception',
