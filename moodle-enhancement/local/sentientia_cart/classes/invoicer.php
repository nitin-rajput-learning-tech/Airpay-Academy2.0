...
class invoicer {
    ...
    public static function issue_for_order(\stdClass $cart): \stdClass {
        ...
        $invoice->id = $DB->insert_record('local_sentientia_cart_invoices', $invoice);
        return $invoice;
    }
    ...
    private static function reserve_invoice_number(string $prefix, int $year): string {
        ...
        $count = (int) $DB->get_field_sql(
            "SELECT COUNT(*) FROM {local_sentientia_cart_invoices}
        ...
        for ($attempt = 1; $attempt <= 5; $attempt++) {
            ...
            if (!$DB->record_exists('local_sentientia_cart_invoices',
    ...
    public static function render_html(\stdClass $invoice): string {
        ...
        $data = [
            ...
            'company_name'    => (string) get_config('local_sentientia_cart', 'company_name'),
            // Pre-built HTML wrapper — template uses {{{ }}}.
            'company_address' => $wrap_multiline(
                (string) get_config('local_sentientia_cart', 'company_address')),
            'company_gstn'    => (string) get_config('local_sentientia_cart', 'our_gstn'),
