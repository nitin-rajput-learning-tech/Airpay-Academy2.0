...
/**
 * Privacy provider lock-in tests for local_sentientia_roles.
 *
...
final class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function test_get_metadata(): void {
        ...
        $this->assertSame('local_sentientia_roles_auditlog', $items[0]->get_name());
    }
    ...
    public function test_export_user_data_includes_their_audit_rows(): void {
        ...
        $contextlist = new approved_contextlist($u, 'local_sentientia_roles', [$sysctx->id]);
    ...
    public function test_delete_data_for_user_redacts_changedby(): void {
        ...
        $rows_before = $DB->count_records('local_sentientia_roles_auditlog',
            ['changedby' => $u->id]);
        ...
        // (audit retention).
        $rows_after_changedby = $DB->count_records('local_sentientia_roles_auditlog',
            ['changedby' => $u->id]);
        $this->assertSame(0, $rows_after_changedby,
            'changedby references must be redacted to 0');

        // Audit row still exists with changedby=0.
        $redacted = $DB->count_records('local_sentientia_roles_auditlog',
            ['changedby' => 0]);
    ...
    public function test_delete_data_for_user_redacts_targetuserid(): void {
        ...
        $rows_after = $DB->count_records('local_sentientia_roles_auditlog',
            ['targetuserid' => $u->id]);
