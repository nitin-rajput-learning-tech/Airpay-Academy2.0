...
class list_courses extends external_api {
    ...
    public static function execute(string $search = '', string $sort = 'fullname', string $sortdir = 'asc',
        ...
        $can_edit   = has_capability('local/sentientia_courses:update', $context);
        $can_delete = has_capability('local/sentientia_courses:delete', $context);
        $can_visibility = has_capability('local/sentientia_courses:visibility', $context);
        ...
        // icon in each row that links to /local/sentientia_courses/share.php.
        //
        ...
        if (strlen($params['filters']) > 4096) {
            throw new \moodle_exception('filterstoolong', 'local_sentientia_courses');
        }
        ...
        // Same shape as sentientia_users — deepest non-zero level wins,
        ...
        foreach ($records as $c) {
            ...
            $enrolledurl = (new \moodle_url('/local/sentientia_courses/enrolledusers.php',
            ...
            // deep-link). Opens in-page via local_sentientia_courses/enrol_modal.
            ...
            if ($can_share) {
                // Sprint C: link to /local/sentientia_courses/share.php as a
                // full page (not a modal) — the share form has enough
                // state to warrant its own URL.
                $shareurl = (new \moodle_url('/local/sentientia_courses/share.php',
                    ['id' => (int) $c->id]))->out(false);
