 ...
 * --------------------
 * The `local_sentientia_courses_tenant_share` table maps courses to the
 ...
 *     Fires \local_sentientia_courses\event\course_share_created per new
...
class sharing_manager {
    ...
    public static function share_course(int $courseid, array $tenant_ids,
        ...
        foreach ($tenant_ids as $tid) {
            ...
            $existing = $DB->get_record('local_sentientia_courses_tenant_share',
            ...
            if ($existing) {
                ...
                $DB->update_record('local_sentientia_courses_tenant_share', $existing);
                $out['reactivated'][] = $tid;
            } else {
    ...
    public static function list_course_shares(int $courseid): array {
        global $DB;
        $rows = $DB->get_records('local_sentientia_courses_tenant_share',
    ...
    public static function is_course_shared_to(int $courseid, int $tenant_id): bool {
        ...
        return $DB->record_exists('local_sentientia_courses_tenant_share', [
            'courseid'  => $courseid,
            'tenant_id' => $tenant_id,
            'status'    => self::STATUS_ACTIVE,
        ]);
     ...
     *     OR EXISTS (SELECT 1 FROM {local_sentientia_courses_tenant_share} csh
    ...
    public static function build_catalog_filter_sql(string $alias,
        ...
        $sql = "($col_open_path = :share_orgexact
                  ...
                  OR EXISTS (
                       SELECT 1 FROM {local_sentientia_courses_tenant_share} csh
