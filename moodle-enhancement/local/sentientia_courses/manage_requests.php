 ...
 *
 * Access: local/sentientia_courses:approve_request (siteadmin only by default).
 *
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve') {
        \local_sentientia_courses\request_manager::approve_request($requestid);
        $msg = get_string('request_approved', 'local_sentientia_courses');
    } else if ($action === 'reject') {
        $reason = optional_param('reason', '', PARAM_TEXT);
        \local_sentientia_courses\request_manager::reject_request($requestid, $reason);
        $msg = get_string('request_rejected', 'local_sentientia_courses');
    } else {
        throw new \moodle_exception('invalidparameter', 'local_sentientia_courses');
    }

    redirect(
        new moodle_url('/local/sentientia_courses/manage_requests.php'),
        $msg,
        2,
...
$PAGE->set_url(new moodle_url('/local/sentientia_courses/manage_requests.php'));
...
$data = [
    ...
    'count'      => count($rows),
    'sesskey'    => sesskey(),
    'post_url'   => (new moodle_url('/local/sentientia_courses/manage_requests.php'))->out(false),
];

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_sentientia_courses/manage_requests', $data);
echo $OUTPUT->footer();
