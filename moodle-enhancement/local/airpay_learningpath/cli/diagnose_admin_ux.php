 ...
 * `local_airpay_learningpath` does have the full UX — view.php,
 ...
 *   2. The user's role doesn't have `local/airpay_learningpath:update`
 ...
 *
 *   php local/airpay_learningpath/cli/diagnose_admin_ux.php --user=academy@airpay.co.in
 ...
 *
 *   php local/airpay_learningpath/cli/diagnose_admin_ux.php --json
...
$installed_version = $DB->get_field('config_plugins', 'value', [
    'plugin' => 'local_airpay_learningpath',
    'name'   => 'version',
]);
...
if ($installed_version) {
    record_check(
        ...
        true,
        "local_airpay_learningpath version $installed_version"
    );
} else {
    record_check(
        'Plugin installed in DB',
        false,
        'No row in {config_plugins} for local_airpay_learningpath — '
...
$needed_tables = [
    ...
    'local_airpay_learningpath_users',
];
$missing_tables = [];
...
$plugin_root = $CFG->dirroot . '/local/airpay_learningpath';
$needed_files = [
...
$caps = [
    'local/airpay_learningpath:view',
    'local/airpay_learningpath:create',
    'local/airpay_learningpath:update',
    'local/airpay_learningpath:enrol',
    'local/airpay_learningpath:manage',
    'local/airpay_learningpath:delete',
];
$missing_caps = [];
foreach ($caps as $cap) {
...
if ($manager_role) {
    foreach (['local/airpay_learningpath:update', 'local/airpay_learningpath:enrol',
              'local/airpay_learningpath:manage', 'local/airpay_learningpath:create'] as $cap) {
...
if ($options['user']) {
    ...
    if (!$target) {
        ...
        $can_update = has_capability(
            'local/airpay_learningpath:update', $systemctx, $target);
        $can_manage = has_capability(
            'local/airpay_learningpath:manage', $systemctx, $target);
        $can_enrol  = has_capability(
            'local/airpay_learningpath:enrol',  $systemctx, $target);
        ...
        if ($can_see_button) {
            ...
            record_check(
                ...
                "Either: (a) make them a site admin, or (b) assign them the "
                    ...
                    . "local/airpay_learningpath:update directly to their role. "
...
if ($options['json']) {
    ...
    if (empty($any_fail)) {
        ...
        cli_writeln('  visit:  ' . $CFG->wwwroot
            . '/local/airpay_learningpath/index.php');
