...
class toggle_status extends external_api {
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'active' => new external_value(PARAM_BOOL, 'true=active, false=archived'),
        ]);
    }

    public static function execute(int $pathid, bool $active): array {
        $params = self::validate_parameters(self::execute_parameters(),
            ['pathid' => $pathid, 'active' => $active]);
        $context = \context_system::instance();
        self::validate_context($context);
        require_capability('local/airpay_learningpath:update', $context);

        $newstatus = \local_airpay_learningpath\path_manager::toggle_status(
            $params['pathid'], $params['active']);
        return ['pathid' => $params['pathid'], 'active' => $newstatus];
    }
