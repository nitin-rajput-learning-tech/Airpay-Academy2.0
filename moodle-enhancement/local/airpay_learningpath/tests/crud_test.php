...
/**
 * CRUD tests for airpay_learningpath.
 *
...
final class crud_test extends \advanced_testcase {
    ...
    private function seed_path(string $name = 'Test Path'): int {
        ...
        if (!$DB->get_manager()->table_exists('local_airpay_learningpath')) {
            $this->markTestSkipped('local_airpay_learningpath table not present.');
        }
        $now = time();
    ...
    public function test_toggle_status_persists(): void {
        ...
        $status = (int) $DB->get_field('local_airpay_learningpath', 'status', ['id' => $pid]);
    ...
    public function test_delete_removes_path(): void {
        ...
        $this->assertFalse($DB->record_exists('local_airpay_learningpath', ['id' => $pid]));
    }
