...
$PAGE->set_url('/local/airpay_learningpath/exportcsv.php');
require_capability('local/airpay_learningpath:view', $ctx);

$mode = optional_param('mode', 'paths', PARAM_ALPHA);
$pathid = optional_param('id', 0, PARAM_INT);

$filename = 'airpay_learningpath_' . $mode
          . ($pathid ? "_$pathid" : '')
...
if ($mode === 'paths') {
    ...
    $rows = $DB->get_records_sql("
        SELECT lp.id, lp.name, lp.status, lp.timecreated,
               (SELECT COUNT(*) FROM {local_airpay_learningpath_courses}
                 WHERE pathid = lp.id) AS course_count,
               (SELECT COUNT(*) FROM {local_airpay_learningpath_users}
