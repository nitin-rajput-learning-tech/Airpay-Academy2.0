...
$path = $DB->get_record('local_airpay_learningpath', ['id' => $pathid], '*', MUST_EXIST);

// Page chrome.
$PAGE->set_url(new moodle_url('/local/airpay_learningpath/view.php',
    ['id' => $pathid, 'tab' => $tab]));
...
$can_enrol = is_siteadmin()
    || has_capability('local/airpay_learningpath:enrol', $context)
    || has_capability('local/airpay_learningpath:manage', $context);
...
$course_count = (int) $DB->count_records('local_airpay_learningpath_courses', ['pathid' => $pathid]);
$user_count   = (int) $DB->count_records('local_airpay_learningpath_users',   ['pathid' => $pathid]);
...
$data = [
    ...
    'back_url'       => (new moodle_url('/local/airpay_learningpath/index.php'))->out(false),
    'tab_overview_active' => ($tab === 'overview'),
    'tab_courses_active'  => ($tab === 'courses'),
    'tab_users_active'    => ($tab === 'users'),
    'tab_overview_url'    => (new moodle_url('/local/airpay_learningpath/view.php',
        ['id' => $pathid, 'tab' => 'overview']))->out(false),
    'tab_courses_url'     => (new moodle_url('/local/airpay_learningpath/view.php',
        ['id' => $pathid, 'tab' => 'courses']))->out(false),
    'tab_users_url'       => (new moodle_url('/local/airpay_learningpath/view.php',
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_learningpath/view', $data);
echo $OUTPUT->footer();
