...
class compliance_engine {
        ...
        foreach ($users as $user) {
            ...
            foreach ($mandatory as $mc) {
                ...
                if (!$enrol) {
                    ...
                    $auto_enrol = get_config('local_sentientia_compliance_report', 'auto_enrol');
                    if ($auto_enrol) {
    ...
    private static function send_escalation(\stdClass $user, \stdClass $mc,
        ...
        if ($templatekey && class_exists('\\local_sentientia_emails\\email_renderer')) {
            try {
                $html = \local_sentientia_emails\email_renderer::render(
                    'local_sentientia_emails/' . $templatekey, $context, $target_userid
                );
            } catch (\Exception $e) {
        ...
        $message->component         = 'local_sentientia_compliance_report';
