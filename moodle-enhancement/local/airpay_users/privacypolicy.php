...
// `local_airpay_users/custom_privacy_policy_html`. Falls back to a
...
$PAGE->set_url(new moodle_url('/local/airpay_users/privacypolicy.php'));
$PAGE->set_context(\context_system::instance());
$PAGE->set_pagelayout('login');  // no nav clutter
$PAGE->set_title(get_string('privacy_pagetitle', 'local_airpay_users'));
...
$custom = (string) get_config('local_airpay_users', 'custom_privacy_policy_html');
...
if (trim($custom) !== '') {
    ...
    echo html_writer::tag('h2',
        get_string('privacy_heading', 'local_airpay_users'),
        ['class' => 'mb-3']);
