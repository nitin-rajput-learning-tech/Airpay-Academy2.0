...
$PAGE->set_url(new moodle_url('/local/airpay_users/bulk_hrms.php'));
$PAGE->set_pagelayout('standard');
$PAGE->set_title(get_string('hrms_pagetitle', 'local_airpay_users'));
$PAGE->set_heading(get_string('hrms_pageheading', 'local_airpay_users'));
$PAGE->navbar->add(get_string('manage_users', 'local_airpay_users'),
    new moodle_url('/local/airpay_users/index.php'));
$PAGE->navbar->add(get_string('hrms_breadcrumb', 'local_airpay_users'));

$mform = new \local_airpay_users\form\bulk_hrms_form();

if ($mform->is_cancelled()) {
    redirect(new moodle_url('/local/airpay_users/index.php'));
}
...
if ($data = $mform->get_data()) {
    ...
    if ($content === false || $content === '') {
        ...
        echo $OUTPUT->notification(
            get_string('hrms_empty_csv', 'local_airpay_users'), 'error');
        $mform->display();
        echo $OUTPUT->footer();
        exit;
    }
    ...
    redirect(
        new moodle_url('/local/airpay_users/sync_run_detail.php',
            ['id' => $run_id]),
        get_string('hrms_import_done', 'local_airpay_users'),
        null,
...
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('hrms_pageheading', 'local_airpay_users'));
...
echo html_writer::link(
    ...
    get_string('hrms_view_history', 'local_airpay_users'),
