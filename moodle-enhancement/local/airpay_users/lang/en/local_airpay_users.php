...
$string['hrms_sync_auth_header_help'] = 'Pass the full header line, e.g. <code>Authorization: Bearer eyJhbGci...</code> or <code>X-Api-Key: ...</code>. Leave empty for unauthenticated endpoints. Stored unencrypted in the Sentientia LMS config table — rotate the token if you suspect a leak.';
$string['hrms_sync_path']             = 'CSV filesystem path';
$string['hrms_sync_path_help']        = 'Used when source mode is <em>Filesystem</em>. Must be an absolute server-local path (Unix: <code>/var/airpay/exports/hrms.csv</code> or Windows: <code>C:\airpay\hrms.csv</code>). The Sentientia LMS web-server user must have read permission. The file is re-read each run.';
$string['hrms_sync_user_id']          = 'Runner user ID';
$string['hrms_sync_user_id_help']     = 'Numeric user ID under which the import runs. Defaults to <code>2</code> (the site admin on a stock Sentientia LMS). Choose a user with the <code>local/airpay_users:edit</code> capability AND a tenant scope that covers every row in the CSV — otherwise rows will be rejected as cross-tenant.';
