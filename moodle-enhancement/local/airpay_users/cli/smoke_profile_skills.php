...
//
// Run: php public/local/airpay_users/cli/smoke_profile_skills.php
//
...
$skills = $DB->get_records_sql(
    "SELECT id, max_level FROM {local_airpay_skills} ORDER BY id ASC LIMIT 2");
if (count($skills) < 2) {
...
foreach ($skills as $s) {
    \local_airpay_skills\skills_manager::save_designation_skill(
...
$ctx = \local_airpay_users\user_manager::build_profile_context($userid);
...
$DB->delete_records('local_airpay_user_skills',
    ['userid' => $userid, 'skillid' => $skill_ids[0]]);
$DB->delete_records('local_airpay_role_skills', ['designation' => $designation]);
echo "Cleanup ✓\n";
