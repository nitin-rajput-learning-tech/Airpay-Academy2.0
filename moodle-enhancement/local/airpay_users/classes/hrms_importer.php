...
class hrms_importer {
    ...
    public static function import_csv(string $csv_content, int $caller_userid,
        ...
        $now = time();
        $run_id = (int) $DB->insert_record('local_airpay_users_sync_runs', (object) [
        ...
        try {
            ...
            $DB->set_field('local_airpay_users_sync_runs', 'totalrows',
                count($rows), ['id' => $run_id]);
        } catch (\Throwable $e) {
            // CSV itself is broken — fail the whole run.
            $DB->update_record('local_airpay_users_sync_runs', (object) [
                'id'            => $run_id,
                'status'        => 'failed',
    ...
    private static function parse_csv(string $content): array {
        ...
        if (!empty($missing)) {
            throw new \moodle_exception('error_csv_header_missing',
                'local_airpay_users', '', implode(', ', $missing));
        }
    ...
    /**
     * Write one row into local_airpay_users_sync_errors.
     */
    private static function write_log_row(int $run_id, int $line_num, array $identity,
        ...
        global $DB;
        $DB->insert_record('local_airpay_users_sync_errors', (object) [
            'runid'            => $run_id,
            'csv_line_number'  => $line_num,
