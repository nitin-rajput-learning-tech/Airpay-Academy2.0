...
TEMPLATE;
    ...
    public static function send(int $userid, string $plain_password): bool {
        ...
        try {
            ...
            $msg->component         = 'local_airpay_users';
            ...
            debugging('local_airpay_users welcome_mailer failed for user '
    ...
    private static function load_templates(int $tenantid): array {
        $tenant_subj = (string) get_config('local_airpay_users',
            self::CFG_TENANT_SUBJECT_PREFIX . $tenantid);
        $tenant_body = (string) get_config('local_airpay_users',
            self::CFG_TENANT_BODY_PREFIX . $tenantid);

        $subject = trim($tenant_subj) !== ''
            ? $tenant_subj
            : (string) get_config('local_airpay_users', self::CFG_DEFAULT_SUBJECT);
        if (trim($subject) === '') {
        ...
        $body = trim($tenant_body) !== ''
            ? $tenant_body
            : (string) get_config('local_airpay_users', self::CFG_DEFAULT_BODY);
        if (trim($body) === '') {
