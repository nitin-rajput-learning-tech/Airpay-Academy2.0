 ...
 * (configurable via local_airpay_users/signup_tenant_path). Open registration
...
class signup_service {
    ...
    public static function is_enabled(): bool {
        return (bool) get_config('local_airpay_users', self::SETTING_ENABLED);
    }
    ...
    public static function register(object $data): int {
        ...
        if (!self::is_enabled()) {
            throw new \moodle_exception('signup_disabled', 'local_airpay_users');
        }
        ...
        if (!empty($errors)) {
            throw new \moodle_exception('signup_validation_failed',
                'local_airpay_users', '', implode('; ', $errors));
        }
    ...
    private static function get_tenant_path(): string {
        $raw = (string) get_config('local_airpay_users', self::SETTING_TENANT_PATH);
        $raw = trim($raw);
        if ($raw === '') {
