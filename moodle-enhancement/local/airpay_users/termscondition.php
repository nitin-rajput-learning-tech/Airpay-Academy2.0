...
// `local_airpay_users/custom_tos_html`. Falls back to a default ToS text
...
$PAGE->set_url(new moodle_url('/local/airpay_users/termscondition.php'));
$PAGE->set_context(\context_system::instance());
$PAGE->set_pagelayout('login');
$PAGE->set_title(get_string('tos_pagetitle', 'local_airpay_users'));
$PAGE->set_heading(format_string($CFG->fullname ?? 'Airpay Academy'));

$custom = (string) get_config('local_airpay_users', 'custom_tos_html');
...
if (trim($custom) !== '') {
    ...
    echo html_writer::tag('h2',
        get_string('tos_heading', 'local_airpay_users'),
        ['class' => 'mb-3']);
    ...
    echo '<p>Your personal data is handled in line with our <a href="' . s(new \moodle_url('/local/airpay_users/privacypolicy.php')) . '">Privacy Policy</a>. By using the platform you also agree to that policy.</p>';
