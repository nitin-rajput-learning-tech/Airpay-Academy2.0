...
namespace local_airpay_reports\form;

defined('MOODLE_INTERNAL') || die();

use local_airpay_reports\report_manager;
...
class edit_report extends \core_form\dynamic_form {
    ...
    protected function definition() {
        ...
        // ── Identity ──────────────────────────────────────────────────
        $mform->addElement('header', 'hdr_basic', get_string('heading_basic', 'local_airpay_reports'));

        $mform->addElement('text', 'name', get_string('report_name', 'local_airpay_reports'),
            ['size' => 50, 'maxlength' => 254, 'placeholder' => 'e.g. Monthly Compliance Snapshot']);
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', null, 'required', null, 'client');

        $mform->addElement('textarea', 'description',
            get_string('description', 'local_airpay_reports'),
            ['rows' => 3, 'cols' => 50, 'placeholder' => 'Optional. What does this report show?']);
        $mform->setType('description', PARAM_TEXT);

        // ── Report type ───────────────────────────────────────────────
        $mform->addElement('header', 'hdr_type', get_string('heading_type', 'local_airpay_reports'));

        $mform->addElement('select', 'report_type',
            get_string('report_type', 'local_airpay_reports'),
            report_manager::REPORT_TYPES);
        $mform->setType('report_type', PARAM_ALPHAEXT);
        $mform->setDefault('report_type', 'course_completion');
        $mform->addHelpButton('report_type', 'report_type', 'local_airpay_reports');

        // ── Organisation scope ────────────────────────────────────────
        $mform->addElement('header', 'hdr_scope', get_string('heading_scope', 'local_airpay_reports'));

        $orgs = $this->get_org_options();
        $mform->addElement('select', 'costcenterid',
            get_string('organisation', 'local_airpay_reports'), $orgs);
        $mform->setType('costcenterid', PARAM_INT);
        $mform->addHelpButton('costcenterid', 'organisation', 'local_airpay_reports');

        // ── Status ────────────────────────────────────────────────────
        $statusoptions = [
            report_manager::STATUS_ACTIVE   => get_string('status_active', 'local_airpay_reports'),
            report_manager::STATUS_ARCHIVED => get_string('status_archived', 'local_airpay_reports'),
        ];
        $mform->addElement('select', 'status',
            get_string('status', 'local_airpay_reports'), $statusoptions);
    ...
    public function validation($data, $files) {
        ...
        if (empty(trim($data['name'] ?? ''))) {
            $errors['name'] = get_string('name_required', 'local_airpay_reports');
        }
        if (!array_key_exists($data['report_type'] ?? '', report_manager::REPORT_TYPES)) {
            $errors['report_type'] = get_string('invalid_report_type', 'local_airpay_reports');
        }
        return $errors;
    }
    ...
    public function process_dynamic_submission() {
        ...
        if ($reportid === 0) {
            ...
            return [
                'reportid' => $newid,
                'message'  => get_string('report_created', 'local_airpay_reports'),
            ];
        } else {
            report_manager::update($reportid, $data);
            return [
                'reportid' => $reportid,
                'message'  => get_string('report_updated', 'local_airpay_reports'),
            ];
        }
    }
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        $r = $DB->get_record('local_airpay_reports', ['id' => $reportid], '*', MUST_EXIST);
        $this->set_data((object) [
            'reportid'     => $r->id,
            'name'         => $r->name,
    ...
    protected function get_page_url_for_dynamic_submission(): \moodle_url {
        return new \moodle_url('/local/airpay_reports/index.php');
    }

    protected function check_access_for_dynamic_submission(): void {
        require_capability('local/airpay_reports:manage', $this->get_context_for_dynamic_submission());
    }
