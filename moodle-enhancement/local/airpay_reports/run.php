...
$PAGE->set_url(new moodle_url('/local/airpay_reports/run.php', ['id' => $reportid]));
$PAGE->set_title('Report — ' . format_string($report->name));
$PAGE->set_heading('Report — ' . format_string($report->name));
$PAGE->set_pagelayout('standard');
$PAGE->set_secondary_navigation(false);
$PAGE->navbar->add(get_string('pluginname', 'local_airpay_reports'),
    new moodle_url('/local/airpay_reports/index.php'));
...
$data = [
    ...
    'backurl'      => (new moodle_url('/local/airpay_reports/index.php'))->out(false),
    'can_export'   => $can_export,
];

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_reports/run', $data);
echo $OUTPUT->footer();
