...
// @module     local_airpay_reports/report_actions
...
const openReportForm = async (reportid, returnFocus) => {
    ...
    const title = await getString(titleKey, 'local_airpay_reports');
    const modalForm = new ModalForm({
        formClass: 'local_airpay_reports\\form\\edit_report',
...
const confirmToggle = async (reportid, name, isActive, returnFocus) => {
    ...
    const [title, message, success] = await Promise.all([
        getString(titleKey, 'local_airpay_reports'),
        getString(msgKey, 'local_airpay_reports', name),
        getString('reportstatuschanged', 'local_airpay_reports'),
    ]);
    Notification.confirm(title, message, title, null, () => {
        Ajax.call([{methodname: 'local_airpay_reports_toggle_status',
...
const confirmDelete = async (reportid, name, returnFocus) => {
    const [title, message, label, success] = await Promise.all([
        getString('deletereport', 'local_airpay_reports'),
        getString('confirmdelete', 'local_airpay_reports', name),
        getString('delete', 'core'),
        getString('reportdeleted', 'local_airpay_reports'),
    ]);
    Notification.deleteCancelPromise(title, message, label, returnFocus).then(() => {
        Ajax.call([{methodname: 'local_airpay_reports_delete_report',
                    args: {reportid: reportid}}])[0]
            .then(() => {
                Notification.addNotification({message: success, type: 'success'});
                window.location.reload();
