...
$program = $DB->get_record('local_airpay_programs', ['id' => $programid], '*', MUST_EXIST);
...
$can_enrol  = is_siteadmin() || has_capability('local/airpay_programs:enrol', $context)
    || has_capability('local/airpay_programs:manage', $context);

$page_url = new moodle_url('/local/airpay_programs/view.php',
    ['id' => $programid, 'tab' => $tab]);
$PAGE->set_context($context);
$PAGE->set_url($page_url);
$PAGE->set_title(get_string('view_program_title', 'local_airpay_programs', $program->name));
$PAGE->set_heading(format_string($program->name));
$PAGE->set_pagelayout('standard');
$PAGE->set_secondary_navigation(false);

$level_count    = \local_airpay_programs\program_manager::count_levels($programid);
$enrolled_count = \local_airpay_programs\program_manager::count_enrolled($programid);

$status_map = [
    \local_airpay_programs\program_manager::STATUS_DRAFT    => 'Draft',
    \local_airpay_programs\program_manager::STATUS_ACTIVE   => 'Active',
    \local_airpay_programs\program_manager::STATUS_ARCHIVED => 'Archived',
];
$status_css_map = [
    \local_airpay_programs\program_manager::STATUS_DRAFT    => 'badge-secondary',
...
$data = [
    ...
    'tab_overview_url'    => (new moodle_url('/local/airpay_programs/view.php',
        ['id' => $programid, 'tab' => 'overview']))->out(false),
    'tab_levels_url'      => (new moodle_url('/local/airpay_programs/view.php',
        ['id' => $programid, 'tab' => 'levels']))->out(false),
    'tab_users_url'       => (new moodle_url('/local/airpay_programs/view.php',
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_programs/view', $data);
echo $OUTPUT->footer();
