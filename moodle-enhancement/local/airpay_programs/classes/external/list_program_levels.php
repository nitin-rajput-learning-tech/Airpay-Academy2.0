...
class list_program_levels extends external_api {
    ...
    public static function execute(int $programid, string $search = '', string $sort = 'sortorder',
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_airpay_programs_levels} l WHERE $wheresql",
            $sqlparams);
        ...
        if ($total > 0) {
            $records = $DB->get_records_sql(
                "SELECT l.*,
                        (SELECT COUNT(*) FROM {local_airpay_programs_courses} lc
                          WHERE lc.levelid = l.id) AS course_count
                   FROM {local_airpay_programs_levels} l
                  WHERE $wheresql
        ...
        foreach ($records as $l) {
            $coursesurl = (new \moodle_url('/local/airpay_programs/levelcourses.php',
