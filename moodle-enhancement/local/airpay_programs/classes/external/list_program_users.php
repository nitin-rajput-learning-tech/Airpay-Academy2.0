...
class list_program_users extends external_api {
    ...
    public static function execute(int $programid, string $search = '', string $sort = 'lastname',
        ...
        $can_update = has_capability('local/airpay_programs:update', $context)
            || has_capability('local/airpay_programs:manage', $context);
        $can_enrol = has_capability('local/airpay_programs:enrol', $context)
            || has_capability('local/airpay_programs:manage', $context);

        $DB->get_record('local_airpay_programs', ['id' => $params['programid']],
            'id', MUST_EXIST);

        $total = \local_airpay_programs\program_manager::count_enrolled_filtered(
            $params['programid'], $params['search']);

        $rows = [];
        if ($total > 0) {
            $records = \local_airpay_programs\program_manager::get_enrolled_users(
                $params['programid'], $params['search'],
                $params['sort'], $params['sortdir'],
                $params['page'] * $params['perpage'], $params['perpage']);

            $statusmap = [
                \local_airpay_programs\program_manager::ENROL_NEW         => 'Enrolled',
                \local_airpay_programs\program_manager::ENROL_INPROGRESS  => 'In progress',
                \local_airpay_programs\program_manager::ENROL_COMPLETED   => 'Completed',
            ];
