 ...
 *
 * Mirrors local_airpay_learningpath\form\assign_courses_form (plain
...
class assign_level_courses extends \core_form\dynamic_form {
    ...
    protected function definition() {
        ...
        $assigned = $DB->get_fieldset_select('local_airpay_programs_courses',
        ...
        $mform->addElement('select', 'courseids',
            get_string('add_courses', 'local_airpay_programs'),
            $options,
    ...
    public function process_dynamic_submission() {
        ...
        $count = \local_airpay_programs\program_manager::assign_courses_to_level($levelid, $courseids);

        return [
            'levelid'  => $levelid,
            'inserted' => $count,
            'message'  => get_string('courses_assigned_count', 'local_airpay_programs', $count),
        ];
    }
    ...
    protected function get_page_url_for_dynamic_submission(): \moodle_url {
        $levelid = (int) $this->optional_param('levelid', 0, PARAM_INT);
        return new \moodle_url('/local/airpay_programs/levelcourses.php',
            ['levelid' => $levelid]);
    }
}
