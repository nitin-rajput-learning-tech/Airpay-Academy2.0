...
| Depends on | `local_airpay_org`, `local_sentientia_privacy`, `local_airpay_core` |
...
|---|---|---|
| `local/airpay_proctoring:attempt` | student | take a proctored quiz |
| `local/airpay_proctoring:review` | manager | review flagged sessions |
| `local/airpay_proctoring:viewattempts` | manager, editingteacher | list/view attempts |
| `local/airpay_proctoring:configure` | _(siteadmin)_ | configure providers/retention |

## Tables (5)

| Table | Purpose |
|---|---|
| `local_airpay_proctor_sessions` | One row per quiz attempt |
| `local_airpay_proctor_identity` | ID match score (photos NEVER stored, only the score) |
| `local_airpay_proctor_events` | Per-attempt event log (tab_switch, face_lost, etc.) |
| `local_airpay_proctor_recordings` | S3 object keys + retention metadata (no bytes server-side) |
...
| Function | Purpose |
|---|---|
| `local_airpay_proctoring_start_session` | Create a session row on quiz start |
| `local_airpay_proctoring_give_consent` | Candidate accepts recording terms |
| `local_airpay_proctoring_submit_identity` | Base64 ID+selfie → AWS verify (rate-limited, MIME-sniffed) ← Phase 8.1 B7 |
| `local_airpay_proctoring_upload_chunk` | Register a chunk (after browser direct-uploaded to S3) |
| `local_airpay_proctoring_report_event` | Add to per-session event log (owner-only) ← Phase 8.1 B3 |
| `local_airpay_proctoring_finalize_session` | Trigger AI analysis + decision |
| `local_airpay_proctoring_list_attempts` | Admin attempt list (tenant-scoped) |
| `local_airpay_proctoring_get_attempt` | Fetch single attempt (tenant-scoped) |
| `local_airpay_proctoring_flag_session` | Manual flag-for-review |
| `local_airpay_proctoring_list_review_queue` | Reviewer queue (tenant-scoped) |
| `local_airpay_proctoring_submit_review` | Reviewer decision (tenant-scoped) |
| `local_airpay_proctoring_compliance_report` | Daily aggregate (tenant-scoped) |
...
|---|---|---|
| `\local_airpay_proctoring\task\purge_old_recordings` | 03:30 daily | Mark expired recordings for S3 delete (stub in current build; production version performs the actual S3 delete) |
...
php "C:/xampp/htdocs/moodle5/public/local/airpay_proctoring/cli/smoke_proctoring.php"
