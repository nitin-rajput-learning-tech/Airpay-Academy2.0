...
namespace local_airpay_proctoring\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table('local_airpay_proctor_sessions', [], 'privacy:metadata:local_airpay_proctor_sessions');
        $collection->add_database_table('local_airpay_proctor_identity', [], 'privacy:metadata:local_airpay_proctor_identity');
        $collection->add_database_table('local_airpay_proctor_events',   [], 'privacy:metadata:local_airpay_proctor_events');
        $collection->add_database_table('local_airpay_proctor_recordings', [], 'privacy:metadata:local_airpay_proctor_recordings');
        $collection->add_database_table('local_airpay_proctor_reviews',  [], 'privacy:metadata:local_airpay_proctor_reviews');
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        global $DB;
        if ($DB->record_exists('local_airpay_proctor_sessions', ['userid' => $userid])) {
    ...
    public static function get_users_in_context(userlist $userlist): void {
        ...
        $ids = $DB->get_fieldset_sql(
            "SELECT DISTINCT userid FROM {local_airpay_proctor_sessions}");
    ...
    public static function export_user_data(approved_contextlist $contextlist): void {
        ...
        $sessions = $DB->get_records('local_airpay_proctor_sessions',
    ...
    private static function delete_for_user(int $userid): void {
        ...
        $DB->delete_records_select('local_airpay_proctor_events',     "sessionid $insql", $inparams);
        $DB->delete_records_select('local_airpay_proctor_recordings', "sessionid $insql", $inparams);
        $DB->delete_records_select('local_airpay_proctor_identity',   "sessionid $insql", $inparams);
        $DB->delete_records_select('local_airpay_proctor_reviews',    "sessionid $insql", $inparams);
        $DB->delete_records_select('local_airpay_proctor_sessions',   "id $insql",        $inparams);
    }
    ...
    private static function delete_all(): void {
        ...
        $DB->delete_records('local_airpay_proctor_identity');
        $DB->delete_records('local_airpay_proctor_reviews');
        $DB->delete_records('local_airpay_proctor_sessions');
    }
}
