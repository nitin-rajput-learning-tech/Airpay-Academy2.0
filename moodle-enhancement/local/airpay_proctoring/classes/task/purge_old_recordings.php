...
class purge_old_recordings extends \core\task\scheduled_task {
    ...
    private function delete_s3_object(string $key): bool {
        ...
        $access_key = (string) get_config('local_airpay_proctoring', 'aws_access_key');
        $secret_key = (string) get_config('local_airpay_proctoring', 'aws_secret_key');
        ...
        if (empty($bucket) || empty($key) || empty($access_key) || empty($secret_key)) {
            ...
            mtrace("airpay_proctoring: S3 delete skipped (config incomplete) for key=$key");
            return false;
        }
        ...
        mtrace("airpay_proctoring: S3 delete failed http=$http key=$key err="
