...
$PAGE->set_url(new moodle_url('/local/airpay_proctoring/attempt.php', ['id' => $id]));
...
$identity = $session->identity_id
    ? $DB->get_record('local_airpay_proctor_identity', ['id' => $session->identity_id])
    : null;
$user = $DB->get_record('user', ['id' => $session->userid], 'firstname, lastname, email');
$quiz = $DB->get_record('quiz', ['id' => $session->quizid], 'name');

$can_review = has_capability('local/airpay_proctoring:review', $ctx);
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_proctoring/attempt_detail', $data);
echo $OUTPUT->footer();
