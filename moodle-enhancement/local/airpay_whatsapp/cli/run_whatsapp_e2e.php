 ...
 *      cron makes)
 *   5. Verify a row landed in local_airpay_send_log with status='mocked'
 ...
 *
 * Usage:
 *   php local/airpay_whatsapp/cli/run_whatsapp_e2e.php --userid=N
 *
...
$prior_prefs = \local_airpay_whatsapp\preference_manager::get($userid);
...
$tpl = $DB->get_record('local_airpay_dlt_templates', [
...
try {
    ...
    \local_airpay_whatsapp\preference_manager::set($userid, [
...
$refreshed = \local_airpay_whatsapp\preference_manager::get($userid);
...
$approved = \local_airpay_whatsapp\dlt_template_registry::get_approved(
...
$before_count = $DB->count_records('local_airpay_send_log',
...
$after_count = $DB->count_records('local_airpay_send_log',
    ['userid' => $userid, 'channel' => 'whatsapp']);
...
$row = $DB->get_record_sql(
    "SELECT * FROM {local_airpay_send_log}
...
if ($row) {
    $DB->delete_records('local_airpay_send_log', ['id' => $row->id]);
