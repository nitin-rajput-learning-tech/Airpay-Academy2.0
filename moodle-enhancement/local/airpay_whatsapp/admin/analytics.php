 ...
 *
 * Reads from local_airpay_send_log + the analytics class to render:
...
$PAGE->set_url('/local/airpay_whatsapp/admin/analytics.php');
$PAGE->set_context($context);
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('analytics_pagetitle', 'local_airpay_whatsapp'));
$PAGE->set_heading(get_string('analytics_heading', 'local_airpay_whatsapp'));

// Date range — last 30 days by default.
$since_days = optional_param('since', 30, PARAM_INT);
$since_ts = time() - ($since_days * 86400);

$mix = \local_airpay_whatsapp\analytics::channel_mix($since_ts);
$cost = \local_airpay_whatsapp\analytics::cost_summary($since_ts);
$recent = \local_airpay_whatsapp\analytics::recent_log(20);
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_whatsapp/analytics', $data);
echo $OUTPUT->footer();
