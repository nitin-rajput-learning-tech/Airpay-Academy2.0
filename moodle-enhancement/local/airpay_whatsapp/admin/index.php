 ...
 * surfaced that `local_airpay_whatsapp` had two admin pages
...
admin_externalpage_setup('local_airpay_whatsapp_index',
    '', null, '', ['pagelayout' => 'admin']);

global $DB, $OUTPUT, $PAGE;
$PAGE->set_url('/local/airpay_whatsapp/admin/index.php');
$PAGE->set_title(get_string('admin_index_title', 'local_airpay_whatsapp'));
$PAGE->set_heading(get_string('admin_index_title', 'local_airpay_whatsapp'));

// ── Stats snapshot ─────────────────────────────────────────────────
$week_ago = time() - (7 * 24 * 60 * 60);
$day_ago  = time() - (24 * 60 * 60);

$sent_week     = $DB->record_exists('local_airpay_send_log', [])
    ? $DB->count_records_select('local_airpay_send_log',
        'channel = :channel AND timecreated >= :since',
        ['channel' => 'whatsapp', 'since' => $week_ago])
    : 0;

$active_templates = 0;
if ($DB->get_manager()->table_exists('local_airpay_whatsapp_templates')) {
    $active_templates = $DB->count_records('local_airpay_whatsapp_templates',
        ['status' => 'approved']);
}

$failures_24h = $DB->record_exists('local_airpay_send_log', [])
    ? $DB->count_records_select('local_airpay_send_log',
...
echo html_writer::tag('p',
    get_string('admin_index_intro', 'local_airpay_whatsapp'),
    ['class' => 'lead']);
...
$stats = [
    [
        'icon'   => 'fa-paper-plane',
        'value'  => $sent_week,
        'label'  => get_string('stats_sent_week', 'local_airpay_whatsapp'),
        'class'  => 'text-primary',
    ],
    [
        'icon'   => 'fa-file-text-o',
        'value'  => $active_templates,
        'label'  => get_string('stats_active_templates', 'local_airpay_whatsapp'),
        'class'  => 'text-success',
    ],
    [
        'icon'   => 'fa-exclamation-triangle',
        'value'  => $failures_24h,
        'label'  => get_string('stats_failures_24h', 'local_airpay_whatsapp'),
        'class'  => $failures_24h > 0 ? 'text-danger' : 'text-secondary',
    ],
    [
        'icon'   => $wa_enabled ? 'fa-toggle-on' : 'fa-toggle-off',
        'value'  => $wa_enabled
            ? get_string('stats_flag_on',  'local_airpay_whatsapp')
            : get_string('stats_flag_off', 'local_airpay_whatsapp'),
        'label'  => get_string('stats_flag_label', 'local_airpay_whatsapp'),
...
echo html_writer::tag('h4',
    get_string('admin_index_quicknav', 'local_airpay_whatsapp'),
    ['class' => 'mt-5']);

echo html_writer::start_div('list-group');

// Templates link
$tmpl_url = new moodle_url('/local/airpay_whatsapp/admin/templates.php');
echo html_writer::link($tmpl_url,
    html_writer::tag('i', '', ['class' => 'fa fa-file-text-o me-2 fa-fw'])
    . html_writer::tag('strong',
        get_string('admin_index_link_templates', 'local_airpay_whatsapp'))
    . html_writer::tag('p',
        get_string('admin_index_link_templates_desc', 'local_airpay_whatsapp'),
        ['class' => 'text-muted small mb-0']),
    ['class' => 'list-group-item list-group-item-action']);

// Analytics link
$ana_url = new moodle_url('/local/airpay_whatsapp/admin/analytics.php');
echo html_writer::link($ana_url,
    html_writer::tag('i', '', ['class' => 'fa fa-line-chart me-2 fa-fw'])
    . html_writer::tag('strong',
        get_string('admin_index_link_analytics', 'local_airpay_whatsapp'))
    . html_writer::tag('p',
        get_string('admin_index_link_analytics_desc', 'local_airpay_whatsapp'),
        ['class' => 'text-muted small mb-0']),
    ['class' => 'list-group-item list-group-item-action']);

// Settings link (admin/settings.php?section=local_airpay_whatsapp)
$set_url = new moodle_url('/admin/settings.php',
    ['section' => 'local_airpay_whatsapp']);
echo html_writer::link($set_url,
    html_writer::tag('i', '', ['class' => 'fa fa-cog me-2 fa-fw'])
    . html_writer::tag('strong',
        get_string('admin_index_link_settings', 'local_airpay_whatsapp'))
    . html_writer::tag('p',
        get_string('admin_index_link_settings_desc', 'local_airpay_whatsapp'),
        ['class' => 'text-muted small mb-0']),
    ['class' => 'list-group-item list-group-item-action']);
