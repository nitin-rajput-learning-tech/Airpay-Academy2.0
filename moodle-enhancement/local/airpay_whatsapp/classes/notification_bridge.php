 ...
 * They share the master `airpay_whatsapp_content_notifications` flag —
...
class notification_bridge {
     ...
     */
    public const CONTENT_FLAG = 'airpay_whatsapp_content_notifications';
     ...
     *
     * Wired from \local_airpay_whatsapp\observer::course_visibility_changed
     ...
     *
     * Wired from \local_airpay_whatsapp\observer::certificate_issued which
     ...
     *
     * Wired from \local_airpay_whatsapp\observer::course_completed which
    ...
    private static function can_throttle_pass(int $userid, string $template_key,
        ...
        return !$DB->record_exists_select('local_airpay_send_log', $select, $params);
    }
    ...
    private static function dispatch(\stdClass $user, string $template_key,
        ...
        try {
            ...
            if (!empty($result['log_id'])) {
                $row = $DB->get_record('local_airpay_send_log',
                ...
                if ($row) {
                    ...
                    $DB->update_record('local_airpay_send_log', $row);
                }
            }
            ...
            debugging(
                '[airpay_whatsapp] content dispatch failed for user '
                . ($user->id ?? 0) . ' / ' . $template_key . ': '
                . $e->getMessage(),
                DEBUG_DEVELOPER
            );
