...
$PAGE->set_url(new moodle_url('/local/airpay_whatsapp/preferences.php'));
$PAGE->set_context($context);
$PAGE->set_title(get_string('preferences_pagetitle', 'local_airpay_whatsapp'));
$PAGE->set_heading(get_string('preferences_heading', 'local_airpay_whatsapp'));
...
$prefs = \local_airpay_whatsapp\preference_manager::get($USER->id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($prefer, \local_airpay_whatsapp\preference_manager::VALID_CHANNELS, true)) {
        $prefer = 'email';
    }
    ...
    try {
        \local_airpay_whatsapp\preference_manager::set(
            $USER->id,
            $values,
            $USER->id,
        ...
        redirect(
            new moodle_url('/local/airpay_whatsapp/preferences.php'),
            get_string('preferences_saved', 'local_airpay_whatsapp'),
            2,
...
$data = [
    'sesskey'           => sesskey(),
    'action_url'        => (new moodle_url('/local/airpay_whatsapp/preferences.php'))->out(false),
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_whatsapp/preferences', $data);
echo $OUTPUT->footer();
