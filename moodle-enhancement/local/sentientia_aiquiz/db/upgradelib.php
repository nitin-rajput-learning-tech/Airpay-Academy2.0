...
/**
 * Upgrade helpers for local_sentientia_aiquiz.
 *
...
/**
 * ADR-031: take local/sentientia_aiquiz:manage_all back from every role that
 * holds it at system context.
 *
 ...
 */
function local_sentientia_aiquiz_revoke_manage_all(): int {
    global $DB;
    $syscontext = \context_system::instance();
    $roleids = $DB->get_fieldset_select('role_capabilities', 'DISTINCT roleid',
        'capability = :cap AND contextid = :ctx',
        ['cap' => 'local/sentientia_aiquiz:manage_all', 'ctx' => $syscontext->id]);
    foreach ($roleids as $roleid) {
        unassign_capability('local/sentientia_aiquiz:manage_all', (int) $roleid, $syscontext->id);
    }
