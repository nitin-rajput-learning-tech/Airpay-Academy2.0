 ...
 * map (admin setting local_airpay_pages | cert_template_tenant_map)
...
admin_externalpage_setup('local_airpay_pages_cert_templates',
    '', null, '', ['pagelayout' => 'admin']);

global $DB, $OUTPUT, $PAGE, $USER, $CFG;
$PAGE->set_url('/local/airpay_pages/certificate_templates.php');
$PAGE->set_title(get_string('cert_templates_title', 'local_airpay_pages'));
$PAGE->set_heading(get_string('cert_templates_title', 'local_airpay_pages'));

// ── Known tenants (BizLMS roots) ───────────────────────────────────
$tenant_labels = [
    0   => get_string('cert_tenant_global', 'local_airpay_pages'),
    1   => 'Airpay',
    77  => 'Public',
    177 => 'ZEEA',
];
...
$map_raw = (string) get_config('local_airpay_pages', 'cert_template_tenant_map');
$map = [];
if (trim($map_raw) !== '') {
...
echo html_writer::tag('p',
    get_string('cert_templates_intro', 'local_airpay_pages'),
    ['class' => 'lead']);
...
if (!$scope_on) {
    echo html_writer::div(
        html_writer::tag('i', '', ['class' => 'fa fa-info-circle me-2',
            'aria-hidden' => 'true'])
        . get_string('cert_scope_off_notice', 'local_airpay_pages'),
        'alert alert-info');
} else if (!$is_admin) {
    echo html_writer::div(
        html_writer::tag('i', '', ['class' => 'fa fa-filter me-2',
            'aria-hidden' => 'true'])
        . get_string('cert_scope_filtered_notice', 'local_airpay_pages',
...
$table->head = [
    get_string('cert_col_template', 'local_airpay_pages'),
    get_string('cert_col_scope', 'local_airpay_pages'),
    get_string('cert_col_issued', 'local_airpay_pages'),
    get_string('cert_col_actions', 'local_airpay_pages'),
];
...
foreach ($templates as $t) {
    ...
    $table->data[] = [
        ...
        html_writer::link($edit_url,
            get_string('cert_action_edit', 'local_airpay_pages'),
...
if ($shown === 0) {
    echo html_writer::tag('p',
        get_string('cert_templates_empty', 'local_airpay_pages'),
...
if ($is_admin) {
    $settings_url = new moodle_url('/admin/settings.php',
        ['section' => 'local_airpay_pages']);
    echo html_writer::div(
        get_string('cert_map_edit_hint', 'local_airpay_pages')
        . ' ' . html_writer::link($settings_url,
            get_string('cert_map_edit_link', 'local_airpay_pages')),
        'text-muted small mt-3');
} else if ($scope_on && $hidden > 0) {
    echo html_writer::tag('p',
        get_string('cert_hidden_count', 'local_airpay_pages', $hidden),
