...
namespace local_airpay_core\task;
 ...
 * the read-side helper `\local_airpay_core\cron_health` and emits the
 * result through three channels:
 *
 *   1. Structured log (`\local_airpay_core\structured_logger`).
 *   2. Moodle event `\local_airpay_core\event\cron_task_stuck` when
 *      anything is stuck.
...
class publish_cron_health extends \core\task\scheduled_task {
    ...
    public function get_name(): string {
        return get_string('task_publish_cron_health', 'local_airpay_core');
    }
    ...
    public function execute() {
        ...
        \local_airpay_core\structured_logger::info('core',
            'cron_health_summary',
            [
        ...
        if ($summary['stuck_airpay'] > 0 || $summary['in_backoff'] > 0) {
            ...
            \local_airpay_core\structured_logger::warn('core',
                'cron_tasks_stuck',
                [
    ...
    private static function raise_site_notification(array $summary, array $stuck): void {
        ...
        $cache = \cache::make('local_airpay_core', 'cron_health_banner');
        if ($cache->get($key)) {
