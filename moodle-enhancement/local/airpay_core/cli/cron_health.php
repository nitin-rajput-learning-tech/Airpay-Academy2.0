...
/**
 * CLI front-end to `\local_airpay_core\cron_health`.
 *
 ...
 *
 * USAGE
 *   php local/airpay_core/cli/cron_health.php
 *     -- print the summary tuple (Airpay stuck / other stuck / in backoff)
 *
 *   php local/airpay_core/cli/cron_health.php --stuck
 *     -- print full list of stuck Airpay tasks with overdue duration
 *
 *   php local/airpay_core/cli/cron_health.php --stuck-other
 *     -- print full list of stuck non-Airpay tasks (Moodle core, others)
 *
 *   php local/airpay_core/cli/cron_health.php --backoff
 *     -- print tasks in Moodle's faildelay exponential-backoff
 *
 *   php local/airpay_core/cli/cron_health.php --all
 *     -- print all three sections
 *
 *   php local/airpay_core/cli/cron_health.php --json
...
if ($options['help']) {
    fwrite(STDOUT,
        "Usage: php local/airpay_core/cli/cron_health.php [options]\n"
        . "\n"
        . "Options:\n"
...
if ($options['json']) {
    ...
    if ($want_stuck) {
        $payload['stuck_airpay'] = array_map(
            ...
            \local_airpay_core\cron_health::get_stuck_airpay_tasks()
        );
    }
    if ($want_stuck_other) {
        $payload['stuck_other'] = array_map(
            fn($t) => json_decode(json_encode($t), true),
            \local_airpay_core\cron_health::get_stuck_other_tasks()
        );
    }
    if ($want_backoff) {
        $payload['in_backoff'] = array_map(
            fn($t) => json_decode(json_encode($t), true),
            \local_airpay_core\cron_health::get_tasks_in_failure_backoff()
        );
    }
...
if ($want_stuck) {
    $rows = \local_airpay_core\cron_health::get_stuck_airpay_tasks();
...
if ($want_stuck_other) {
    $rows = \local_airpay_core\cron_health::get_stuck_other_tasks();
...
if ($want_backoff) {
    $rows = \local_airpay_core\cron_health::get_tasks_in_failure_backoff();
