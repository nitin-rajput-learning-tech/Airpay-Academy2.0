...
A single static helper class `\local_sentientia_platform\tenant` that every other
...
./vendor/bin/phpunit --testdox public/local/sentientia_platform/tests/tenant_test.php
```

## How to verify after install

```php
php -r "require '/path/to/moodle/config.php'; var_dump(class_exists('\local_sentientia_platform\tenant'));"
# → bool(true)
```

If false, the plugin didn't install (check `mdl_config_plugins` for the
version row + `local/sentientia_platform/` directory exists in moodle docroot).
