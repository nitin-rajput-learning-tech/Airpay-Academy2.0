...
class tenant {
     ...
     * inline check if local_sentientia_core is absent (standalone sentientia_platform).
     */
    public static function assert_valid(int $tenantid): void {
        ...
        if (!in_array($tenantid, self::VALID_TENANTS, true)) {
            throw new \moodle_exception('error_invalidtenant', 'local_sentientia_platform');
        }
    }
     ...
     *     \local_sentientia_platform\tenant::require_access((int)$cart->costcenterid);
     *     // ... safe to proceed
     */
     ...
     *
     *     [$tenantsql, $tenantargs] = \local_sentientia_platform\tenant::sql_filter('h');
