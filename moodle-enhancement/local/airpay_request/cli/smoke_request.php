...
echo "=== airpay_request smoke test ===\n\n";
...
try {
    \local_airpay_request\request_manager::submit($user->id, $course->id, 'too short');
...
// 1b. Valid submit
$req = \local_airpay_request\request_manager::submit($user->id, $course->id,
...
try {
    \local_airpay_request\request_manager::submit($user->id, $course->id,
...
// === 2. Decide — approve ===
echo "\n=== 2. Decide: approve ===\n";
$rec = \local_airpay_request\request_manager::decide(
...
try {
    \local_airpay_request\request_manager::decide(
...
// === 3. Decide — reject ===
echo "\n=== 3. Decide: reject ===\n";
$req2 = \local_airpay_request\request_manager::submit($user->id, $course->id,
...
// 3b. Reject with note
$rec2 = \local_airpay_request\request_manager::decide(
...
// === 4. Cancel ===
echo "\n=== 4. Cancel ===\n";
$req3 = \local_airpay_request\request_manager::submit($user->id, $course->id,
    'Third attempt — will cancel before decision is made.');
$ok = \local_airpay_request\request_manager::cancel((int) $req3->id, (int) $user->id);
$check('Cancel by requester returns true', $ok);
$rec3 = $DB->get_record('local_airpay_request', ['id' => $req3->id]);
$check('Status = cancelled', $rec3->status === 'cancelled');

// 4b. Someone else can't cancel
$req4 = \local_airpay_request\request_manager::submit($user->id, $course->id,
    'Fourth attempt — testing other-user cancel rejection.');
try {
    \local_airpay_request\request_manager::cancel((int) $req4->id, (int) $admin->id);
...
$pending = \local_airpay_request\request_manager::pending_count_for_approver($approver_id);
...
$DB->delete_records('local_airpay_request',
