...
| Depends on | `local_airpay_org`, `local_airpay_manager`, `local_airpay_core` |
...
|---|---|---|
| `local/airpay_request:request` | student, user | submit a request |
| `local/airpay_request:approve` | manager | approve/reject requests routed to you |
| `local/airpay_request:overrideroute` | _(none by default)_ | bypass routing — but **only within own tenant** ← Phase 8.1 B10 |
| `local/airpay_request:view` | manager | view all-requests list (tenant-scoped) |

## Tables (1)

| Table | Purpose |
|---|---|
| `local_airpay_request` | One row per request: userid, courseid, reason, status, route, approver_userid, timedue, decision_note |

## Web services (6)

| Function | Purpose |
|---|---|
| `local_airpay_request_submit` | Submit a new request |
| `local_airpay_request_decide` | Approve or reject (tenant-scoped) |
| `local_airpay_request_cancel` | Requester cancels own pending |
| `local_airpay_request_list_mine` | List own requests |
| `local_airpay_request_list_pending` | List requests assigned to me as approver |
| `local_airpay_request_pending_count` | Badge count for nav |
...
|---|---|---|
| `\local_airpay_request\task\escalate_overdue` | every 15 min | Re-route past-SLA requests |
| `\local_airpay_request\task\auto_expire` | daily | Mark expired pending requests |
...
```powershell
# 1. CLI smoke:
php "C:/xampp/htdocs/moodle5/public/local/airpay_request/cli/smoke_request.php"
# Expected: 23/23 cases pass

# 2. Manual escalation cron:
php "C:/xampp/htdocs/moodle5/admin/cli/scheduled_task.php" \
    --execute=\\local_airpay_request\\task\\escalate_overdue
```
