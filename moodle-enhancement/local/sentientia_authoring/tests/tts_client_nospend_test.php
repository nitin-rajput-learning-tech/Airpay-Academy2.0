 ...
 *
 * No-spend guard: this client talks to ElevenLabs over raw PHP cURL, which
 ...
 */
final class tts_client_nospend_test extends \advanced_testcase {

    public function test_live_call_is_refused_under_phpunit_even_with_a_key_set(): void {
        $this->resetAfterTest();
        set_config('elevenlabs_api_key', 'sk-test-not-a-real-key', 'local_sentientia_authoring');

        $result = tts_client::call_live('Narration text.', 'en', 'voice-id');

        $this->assertIsArray($result);
        $this->assertArrayHasKey('error', $result);
        $this->assertSame('live_blocked_in_tests', $result['error'],
            'the live ElevenLabs path must refuse to run under PHPUNIT_TEST; '
    ...
    public function test_the_guard_precedes_the_api_key_check(): void {
        ...
        unset_config('elevenlabs_api_key', 'local_sentientia_authoring');
