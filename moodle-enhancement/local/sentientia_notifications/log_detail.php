...
$PAGE->set_url(new moodle_url('/local/sentientia_notifications/log_detail.php', ['id' => $id]));
...
if ($log->userid && $log->ruleid) {
    ...
    $events = $DB->get_records_sql(
        "SELECT * FROM {local_sentientia_notif_log}
...
$data = [
    ...
    'back_url'    => (new moodle_url('/local/sentientia_notifications/logs.php'))->out(false),
];

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_sentientia_notifications/log_detail', $data);
echo $OUTPUT->footer();
