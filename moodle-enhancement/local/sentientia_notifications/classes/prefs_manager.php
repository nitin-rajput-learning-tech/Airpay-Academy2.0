...
class prefs_manager {
    ...
    public static function get_for_user(int $userid): object {
        global $DB;
        $row = $DB->get_record('local_sentientia_notif_prefs', ['userid' => $userid]);
        if (!$row) {
            return (object) [
                'id'                  => null,
                'userid'              => $userid,
    ...
    public static function save_for_user(int $userid,
        ...
        $now = time();
        $existing = $DB->get_record('local_sentientia_notif_prefs', ['userid' => $userid]);
        $payload = (object) [
            'userid'              => $userid,
        ...
        if ($existing) {
            ...
            $DB->update_record('local_sentientia_notif_prefs', $payload);
            return (int) $existing->id;
        }
        return (int) $DB->insert_record('local_sentientia_notif_prefs', $payload);
    }
}
