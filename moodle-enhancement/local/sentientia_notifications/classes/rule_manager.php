...
class rule_manager {
    ...
    private const TABLE = 'local_sentientia_notif_rules';
    ...
    public static function create(object $data): int {
        ...
        if (empty($data->name) || empty($data->rule_type)) {
            throw new \moodle_exception('missingrequiredfields', 'local_sentientia_notifications');
        }

        // Validate enums.
        if (!array_key_exists($data->rule_type, self::RULE_TYPES)) {
            throw new \moodle_exception('invalidruletype', 'local_sentientia_notifications');
        }
        $channel = $data->channel ?? 'inapp';
        if (!array_key_exists($channel, self::CHANNELS)) {
            throw new \moodle_exception('invalidchannel', 'local_sentientia_notifications');
        }
        $audience = $data->audience ?? 'learner';
        if (!array_key_exists($audience, self::AUDIENCES)) {
            throw new \moodle_exception('invalidaudience', 'local_sentientia_notifications');
        }
