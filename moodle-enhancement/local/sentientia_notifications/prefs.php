...
$PAGE->set_url(new moodle_url('/local/sentientia_notifications/prefs.php'));
...
$PAGE->navbar->add('My notifications',
    new moodle_url('/local/sentientia_notifications/nudge.php'));
$PAGE->navbar->add('Preferences');

$prefs = \local_sentientia_notifications\prefs_manager::get_for_user((int) $USER->id);

// Build list of rule types as toggle rows, with current opt-out state.
$ruletype_options = [];
foreach (\local_sentientia_notifications\prefs_manager::RULE_TYPES as $key => $label) {
    $ruletype_options[] = [
        'key'      => s($key),
        'label'    => s($label),
...
$digest_options = [];
foreach (\local_sentientia_notifications\prefs_manager::DIGEST_FREQUENCIES as $f) {
    $digest_options[] = [
        'value'    => $f,
        'label'    => ucfirst($f),
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_sentientia_notifications/prefs', $data);
$PAGE->requires->js_call_amd('local_sentientia_notifications/prefs', 'init', []);
echo $OUTPUT->footer();
