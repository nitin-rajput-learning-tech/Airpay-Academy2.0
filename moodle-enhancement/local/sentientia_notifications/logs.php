...
$PAGE->set_url(new moodle_url('/local/sentientia_notifications/logs.php'));
...
$total = (int) $DB->get_field_sql(
  ...
  LEFT JOIN {local_sentientia_notif_rules} r ON r.id = l.ruleid
...
foreach ($rows as $r) {
    $shape_rows[] = [
        ...
        'detail_url' => (new moodle_url('/local/sentientia_notifications/log_detail.php',
            ['id' => $r->id]))->out(false),
    ];
}
...
$data = [
    ...
    'prev_url' => (new moodle_url('/local/sentientia_notifications/logs.php',
        array_filter(['p' => max(0, $page - 1), 'status' => $status,
                      'channel' => $channel, 'ruletype' => $ruletype,
                      'from' => $from, 'to' => $to, 'q' => $search])))->out(false),
    'next_url' => (new moodle_url('/local/sentientia_notifications/logs.php',
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_sentientia_notifications/logs', $data);
echo $OUTPUT->footer();
