...
class edit_skill extends \core_form\dynamic_form {
    ...
    protected function definition() {
        ...
        // ── Category ──────────────────────────────────────────────────
        $cats = \local_sentientia_skills\skills_manager::get_categories_options();
        $mform->addElement('select', 'categoryid', get_string('category', 'local_sentientia_skills'), $cats);
        $mform->setType('categoryid', PARAM_INT);
        $mform->addRule('categoryid', null, 'required', null, 'client');

        // ── Levels ────────────────────────────────────────────────────
        $mform->addElement('header', 'hdr_levels', get_string('heading_levels', 'local_sentientia_skills'));
        ...
        $mform->addElement('select', 'max_level', get_string('max_level', 'local_sentientia_skills'),
            $level_options);
        $mform->setType('max_level', PARAM_INT);
        $mform->setDefault('max_level', 5);
        $mform->addHelpButton('max_level', 'max_level', 'local_sentientia_skills');
    ...
    public function process_dynamic_submission() {
        ...
        if ($skillid === 0) {
            $newid = \local_sentientia_skills\skills_manager::create_skill($data);
            return ['skillid' => $newid, 'message' => get_string('skillcreated', 'local_sentientia_skills')];
        } else {
            \local_sentientia_skills\skills_manager::update_skill($skillid, $data);
            return ['skillid' => $skillid, 'message' => get_string('skillupdated', 'local_sentientia_skills')];
        }
    }
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        $s = $DB->get_record('local_sentientia_skills', ['id' => $skillid], '*', MUST_EXIST);
    ...
    protected function check_access_for_dynamic_submission(): void {
        // ADR-031: skills are catalogue-wide - :manage AND cross-tenant.
