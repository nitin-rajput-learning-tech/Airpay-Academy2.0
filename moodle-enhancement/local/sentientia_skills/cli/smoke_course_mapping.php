...
// Run: php public/local/sentientia_skills/cli/smoke_course_mapping.php
//
...
$skill = $DB->get_record_sql(
    "SELECT id, name, max_level FROM {local_sentientia_skills} ORDER BY id ASC LIMIT 1");
if (!$skill) {
    fwrite(STDERR, "FAIL: No skill found in local_sentientia_skills.\n");
    exit(1);
}
...
$pre_count = count(\local_sentientia_skills\skills_manager::list_course_skills((int) $course->id));
echo "Pre-list count: $pre_count\n";

// 2. Save: upsert mapping at level 2.
$id1 = \local_sentientia_skills\skills_manager::save_course_skill(
    (int) $course->id, (int) $skill->id, 2);
echo "Save level=2 → row id $id1\n";

// 3. Verify it's in the list at level 2.
$list = \local_sentientia_skills\skills_manager::list_course_skills((int) $course->id);
$match = null;
foreach ($list as $r) {
...
$id2 = \local_sentientia_skills\skills_manager::save_course_skill(
...
// 5. Delete.
$ok = \local_sentientia_skills\skills_manager::delete_course_skill((int) $id2);
if (!$ok) {
...
$results = \local_sentientia_skills\skills_manager::search_courses(
