...
function xmldb_local_sentientia_emails_upgrade(int $oldversion): bool {
    ...
    if ($oldversion < 2026041200) {
        ...
        // --- Table: local_sentientia_email_overrides ---
        $table = new xmldb_table('local_sentientia_email_overrides');
        ...
        // --- Table: local_sentientia_email_rules ---
        $table = new xmldb_table('local_sentientia_email_rules');
        if (!$dbman->table_exists($table)) {
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
        ...
        // --- Table: local_sentientia_email_log ---
        ...
        // --- Table: local_sentientia_email_prefs ---
        $table = new xmldb_table('local_sentientia_email_prefs');
        if (!$dbman->table_exists($table)) {
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
        ...
        foreach ($defaults as $rule) {
            ...
            $DB->insert_record('local_sentientia_email_rules', (object)$rule);
        }

        upgrade_plugin_savepoint(true, 2026041200, 'local', 'sentientia_emails');
    }

    // ── Sprint B (2026-05-13): course-completion email + ramping reminders ──
    // Adds 2 columns to local_sentientia_email_log (attachment_filename,
    // certificate_issue_id) and 3 columns to local_sentientia_email_rules
    ...
    if ($oldversion < 2026051301) {
        ...
        // --- local_sentientia_email_log: attachment columns ---
        $table = new xmldb_table('local_sentientia_email_log');
        ...
        // --- local_sentientia_email_rules: cadence + cap + auto-stop columns ---
        $table = new xmldb_table('local_sentientia_email_rules');
        ...
        foreach ($sprintb_rules as $rule) {
            if ($DB->record_exists('local_sentientia_email_rules',
        ...
        upgrade_plugin_savepoint(true, 2026051301, 'local', 'sentientia_emails');
    }
