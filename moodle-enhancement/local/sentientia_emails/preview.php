 ...
 *
 * URL: /local/sentientia_emails/preview.php?template=compliance/deadline_warning&tenant=1
 *
...
$PAGE->set_url(new moodle_url('/local/sentientia_emails/preview.php'));
$PAGE->set_context($context);
$PAGE->set_title(get_string('emailpreview', 'local_sentientia_emails'));
$PAGE->set_heading(get_string('emailpreview', 'local_sentientia_emails'));
$PAGE->set_pagelayout('standard');

// Get template categories.
$categories = \local_sentientia_emails\email_renderer::get_template_list();
$tenants = [
    ['id' => 1,   'name' => get_string('tenant_airpay', 'local_sentientia_emails'),  'selected' => ($tenantid == 1)],
    ['id' => 77,  'name' => get_string('tenant_public', 'local_sentientia_emails'),   'selected' => ($tenantid == 77)],
    ['id' => 177, 'name' => get_string('tenant_zeea', 'local_sentientia_emails'),     'selected' => ($tenantid == 177)],
];
...
foreach ($categories as &$cat) {
    foreach ($cat['templates'] as &$tpl) {
        ...
        $tpl['url'] = (new moodle_url('/local/sentientia_emails/preview.php', [
            'template' => $tpl['key'],
            'tenant'   => $tenantid,
            'view'     => $viewmode,
        ]))->out(false);
...
<div class="ap-email-preview-wrap">
    ...
    <div class="ap-email-main">
        <?php if (!empty($templatekey) && !empty($previewhtml)): ?>
            ...
            <p class="ap-email-subtitle">Template: <code>local_sentientia_emails/<?php echo s($templatekey); ?></code></p>

            <div class="ap-email-controls">
                <label style="font-size:13px; color:#5a6070; font-weight:500;">Tenant:</label>
                <select onchange="window.location='<?php echo (new moodle_url('/local/sentientia_emails/preview.php', ['template' => $templatekey]))->out(false); ?>&tenant='+this.value">
        ...
        <?php else: ?>
            <div class="ap-email-empty">
                <?php echo get_string('no_template_selected', 'local_sentientia_emails'); ?>
            </div>
        <?php endif; ?>
    </div>
</div>
