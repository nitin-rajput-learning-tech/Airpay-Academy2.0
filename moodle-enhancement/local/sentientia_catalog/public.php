...
$PAGE->set_url('/local/sentientia_catalog/public.php');
$PAGE->set_title(get_string('catalog', 'local_sentientia_catalog') . ' — airpay academy');
$PAGE->set_heading(get_string('catalog', 'local_sentientia_catalog'));
...
$results = \local_sentientia_catalog\commerce::get_public_catalog($search, $sort, $page, 12);
$cart_count = \local_sentientia_catalog\commerce::get_cart_count();
...
$viewer_oneclick = \local_sentientia_catalog\enrolment::should_offer_oneclick($USER, ['is_free' => true]);
...
if ($lxp_on && $search === '' && $page === 0) {
    $rail = \local_sentientia_catalog\commerce::get_public_catalog('', 'popular', 0, 8)['courses'];
}
...
if ($lxp_on) {
    ...
    // ════════════════════════════════════════════════════════════════
    $carturl = (new moodle_url('/local/sentientia_catalog/cart.php'))->out(false);
    $baseurl = (new moodle_url('/local/sentientia_catalog/public.php'))->out(false);
    ...
    $render_card = function (array $course) use ($viewer_oneclick): string {
        ...
        $addurl = (new moodle_url('/local/sentientia_catalog/course.php', [
            'id'      => $course['id'],
            'action'  => $oneclick ? 'enrolnow' : 'addtocart',
            'sesskey' => sesskey(),
        ]))->out(false);
        $cta = $oneclick
            ? get_string('enrol_now_free', 'local_sentientia_catalog')
            : ($isfree
                ? get_string('public_enrolfree', 'local_sentientia_catalog')
                : get_string('public_addtocart', 'local_sentientia_catalog'));
        $pricecls = $isfree
        ...
        if ($isfree) {
            $h .= '<span class="airpay-catalog__badge airpay-catalog__badge--new">'
                . s(get_string('public_free', 'local_sentientia_catalog')) . '</span>';
        }
        $h .= '<span class="airpay-catalog__card-type" aria-hidden="true">'
            . s(get_string('catalog', 'local_sentientia_catalog')) . '</span>';
        $h .= '</div></a>';
    ...
    echo '<h2 class="airpay-catalog__pubtitle">'
        ...
        . s(get_string('catalog', 'local_sentientia_catalog')) . '</h2>';
    echo '<p class="airpay-catalog__pubcount">'
        . s(get_string('public_coursesavailable', 'local_sentientia_catalog', $results['total']))
        . '</p>';
    echo '</div>';
    if ($cart_count > 0) {
        echo '<a class="airpay-catalog__pubcart" href="' . s($carturl) . '">'
            . '<i class="fa fa-shopping-cart" aria-hidden="true"></i> '
            . s(get_string('public_cart', 'local_sentientia_catalog', $cart_count)) . '</a>';
    }
    echo '</div>';
    ...
    echo '<input type="text" name="q" value="' . s($search) . '" '
        . 'placeholder="' . s(get_string('search', 'local_sentientia_catalog')) . '" '
        . 'aria-label="' . s(get_string('search', 'local_sentientia_catalog')) . '">';
    echo '</form>';
    ...
    foreach ([
        ...
        echo '<a class="airpay-catalog__pubsort-pill' . $active . '" href="' . s($url) . '">'
            . s(get_string($strkey, 'local_sentientia_catalog')) . '</a>';
    }
    echo '</div>';
    echo '</div>';
    ...
    if (!empty($rail)) {
        ...
        echo '<h3 class="airpay-catalog__section-title"><i class="fa fa-fire" style="color:#d97706;" aria-hidden="true"></i> '
            . s(get_string('public_popularpicks', 'local_sentientia_catalog')) . '</h3>';
        echo '<button class="airpay-catalog__carousel-nav airpay-catalog__carousel-nav--left" data-dir="left" '
            . 'aria-label="' . s(get_string('public_scrollleft', 'local_sentientia_catalog')) . '">'
        ...
        echo '<button class="airpay-catalog__carousel-nav airpay-catalog__carousel-nav--right" data-dir="right" '
            . 'aria-label="' . s(get_string('public_scrollright', 'local_sentientia_catalog')) . '">'
    ...
    if (!empty($results['courses'])) {
        if (!empty($rail)) {
            echo '<h3 class="airpay-catalog__section-title">'
                . s(get_string('public_browseall', 'local_sentientia_catalog')) . '</h3>';
        }
        ...
        echo '<h4 class="ap-empty-state__title">' . s(get_string('public_nocourses', 'local_sentientia_catalog')) . '</h4>';
        echo '<p class="ap-empty-state__message">' . s(get_string('public_nocourses_hint', 'local_sentientia_catalog')) . '</p>';
        if ($search !== '') {
            echo '<a class="airpay-catalog__btn airpay-catalog__btn--enroll" href="' . s($baseurl) . '">'
                . s(get_string('public_clearsearch', 'local_sentientia_catalog')) . '</a>';
        }
        echo '</div>';
    }
...
<div class="ap-public-catalog" style="max-width:1200px; margin:0 auto; padding:0 20px;">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
        ...
        <?php if ($cart_count > 0): ?>
        <a href="<?php echo (new moodle_url('/local/sentientia_catalog/cart.php'))->out(); ?>"
    ...
    <div style="display:flex; gap:12px; margin-bottom:24px; flex-wrap:wrap;">
        <form action="<?php echo (new moodle_url('/local/sentientia_catalog/public.php'))->out(false); ?>" method="get" style="flex:1; min-width:200px;">
            ...
            <div style="padding:20px;">
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding-top:12px;
                    ...
                    <div style="display:flex; gap:6px;">
                        ...
                        ?>
                        <a href="<?php echo s((new moodle_url('/local/sentientia_catalog/course.php', ['id' => $course['id'], 'action' => $lg_oneclick ? 'enrolnow' : 'addtocart', 'sesskey' => sesskey()]))->out(false)); ?>"
                           style="padding:6px 14px; border-radius:8px; font-size:13px; font-weight:600;
