...
class request_manager {
    ...
    public static function submit(int $userid, int $courseid, string $reason): \stdClass {
        ...
        if (strlen(trim($reason)) < 20) {
            throw new \moodle_exception('error_reasonshort', 'local_sentientia_request');
        }
        ...
        $sla_hours = (int) (get_config('local_sentientia_request', 'sla_hours') ?: 48);
        $now = time();
        ...
        $rec->id = $DB->insert_record('local_sentientia_request', $rec);
        ...
        try {
            \local_sentientia_request\event\request_submitted::create([
            ...
            debugging('local_sentientia_request: failed to emit request_submitted: '
    ...
    public static function submit_path(int $userid, int $pathid, string $reason): \stdClass {
        ...
        try {
            ...
            debugging('local_sentientia_request: failed to emit path request_submitted: '
                . $e->getMessage(), DEBUG_DEVELOPER);
        }
    ...
    public static function decide(int $requestid, int $deciderid,
        ...
        if ((int) $rec->approver_userid !== $deciderid
            ...
            $has_override = has_capability('local/sentientia_request:overrideroute',
        ...
        //
        // Solution: keep the txn scope to the actual `local_sentientia_request`
        ...
        try {
            ...
            $DB->update_record('local_sentientia_request', $rec);
        ...
        if ($decision === 'approved') {
            try {
                ...
                debugging('local_sentientia_request: enrolment failed for request '
        ...
        try {
            ...
            debugging('local_sentientia_request: notifier failed for request '
                . $rec->id . ': ' . $e->getMessage(), DEBUG_DEVELOPER);
        }
        ...
        try {
            ...
            if ($decision === 'approved') {
                \local_sentientia_request\event\request_approved::create([
                    'context'       => \context_system::instance(),
                    'objectid'      => (int) $rec->id,
                    'userid'        => $deciderid,
                    'relateduserid' => (int) $rec->userid,
                ...
                \local_sentientia_request\event\request_rejected::create([
                    'context'       => \context_system::instance(),
                    'objectid'      => (int) $rec->id,
                    'userid'        => $deciderid,
                    'relateduserid' => (int) $rec->userid,
            ...
            debugging('local_sentientia_request: failed to emit decide event: '
                . $e->getMessage(), DEBUG_DEVELOPER);
        }
    ...
    public static function cancel(int $requestid, int $userid): bool {
        global $DB;
        $rec = $DB->get_record('local_sentientia_request',
    ...
    public static function escalate_overdue(): int {
        ...
        foreach ($rows as $r) {
            ...
            if ($r->route === 'manager' || $r->route === 'courseowner') {
                $admin = (int) (get_config('local_sentientia_request', 'default_approver') ?: 2);
                $r->approver_userid = $admin;
                $r->route = 'admin';
                $r->timeescalated = $now;
                $r->timedue = $now + ((int) (get_config('local_sentientia_request', 'sla_hours') ?: 48) * 3600);
                $r->timemodified = $now;
                $DB->update_record('local_sentientia_request', $r);
    ...
    public static function auto_expire(): int {
        global $DB;
        $days = (int) (get_config('local_sentientia_request', 'auto_expire_days') ?: 30);
        if ($days <= 0) return 0;
