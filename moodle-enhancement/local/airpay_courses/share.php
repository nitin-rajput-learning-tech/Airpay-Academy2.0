 ...
 *
 * Access: requires `local/airpay_courses:share_to_tenant` (siteadmin
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $current = \local_airpay_courses\sharing_manager::list_course_shares($courseid);
    $current_active_ids = [];
    foreach ($current as $tenant_id => $row) {
        if ($row->status === \local_airpay_courses\sharing_manager::STATUS_ACTIVE) {
    ...
    if (!empty($to_add)) {
        \local_airpay_courses\sharing_manager::share_course($courseid,
            array_values($to_add));
    }
    foreach ($to_remove as $tid) {
        \local_airpay_courses\sharing_manager::unshare_course($courseid,
            (int) $tid);
    }
    ...
    redirect(
        ...
        get_string('share_saved', 'local_airpay_courses'),
        2,
...
// Page chrome.
$PAGE->set_url(new moodle_url('/local/airpay_courses/share.php', ['id' => $courseid]));
...
foreach ($known_tenants as $t) {
    ...
    $is_active = $share_row
        && $share_row->status === \local_airpay_courses\sharing_manager::STATUS_ACTIVE;
    $tenant_rows[] = [
        'id'         => $tid,
        'name'       => $t->name,
...
$data = [
    ...
    'save_url'    => (new moodle_url('/local/airpay_courses/share.php',
        ['id' => $courseid]))->out(false),
];

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_courses/share_page', $data);
echo $OUTPUT->footer();
