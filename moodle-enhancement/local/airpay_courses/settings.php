...
if ($hassiteconfig) {
    ...
    $settings->add(new admin_setting_heading(
        'local_airpay_courses/overdue_heading',
        get_string('overdue_settings_heading', 'local_airpay_courses'),
        get_string('overdue_settings_intro', 'local_airpay_courses')));

    $settings->add(new admin_setting_configcheckbox(
        'local_airpay_courses/overdue_enabled',
        get_string('overdue_enabled', 'local_airpay_courses'),
        get_string('overdue_enabled_help', 'local_airpay_courses'),
        0));

    $settings->add(new admin_setting_configtext(
        'local_airpay_courses/overdue_days_after',
        get_string('overdue_days_after', 'local_airpay_courses'),
        get_string('overdue_days_after_help', 'local_airpay_courses'),
        '1,7,14', PARAM_TEXT));

    $settings->add(new admin_setting_configtext(
        'local_airpay_courses/overdue_max_per_run',
        get_string('overdue_max_per_run', 'local_airpay_courses'),
        get_string('overdue_max_per_run_help', 'local_airpay_courses'),
        '500', PARAM_INT));

    $lastrun_o  = (int) get_config('local_airpay_courses', 'overdue_last_run');
    $lastsent_o = (int) get_config('local_airpay_courses', 'overdue_last_sent');
    if ($lastrun_o > 0) {
        $body_o = get_string('overdue_last_run_value', 'local_airpay_courses',
            (object) ['time' => userdate($lastrun_o), 'count' => $lastsent_o]);
    } else {
        $body_o = get_string('overdue_last_run_never', 'local_airpay_courses');
    }
    $settings->add(new admin_setting_heading(
        'local_airpay_courses/overdue_status',
        get_string('overdue_status', 'local_airpay_courses'),
        $body_o));
