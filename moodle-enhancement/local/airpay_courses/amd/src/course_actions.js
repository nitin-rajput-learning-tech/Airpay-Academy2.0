...
const openCourseForm = async (courseid, returnFocus) => {
    ...
    const title = await getString(titleKey, 'local_airpay_courses');
...
const confirmVisibility = async (courseid, coursename, makeVisible, returnFocus) => {
    ...
    const [title, message, successMsg] = await Promise.all([
        getString(titleKey, 'local_airpay_courses'),
        getString(msgKey, 'local_airpay_courses', coursename),
        getString(successKey, 'local_airpay_courses'),
    ]);
...
const confirmDelete = async (courseid, coursename, returnFocus) => {
    const [title, message, deleteLabel, successMsg] = await Promise.all([
        getString('deletecourse', 'local_airpay_courses'),
        getString('confirmdelete', 'local_airpay_courses', coursename),
        getString('delete', 'core'),
        getString('coursedeleted', 'local_airpay_courses'),
    ]);
