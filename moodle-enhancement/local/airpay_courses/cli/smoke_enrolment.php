...
if ($user2) {
    $r_emp = \local_airpay_courses\external\enrol_single::execute(
...
echo "\n=== 2. list_course_enrolments ===\n";
$r = \local_airpay_courses\external\list_course_enrolments::execute(
...
// Search filter
$r_search = \local_airpay_courses\external\list_course_enrolments::execute(
    $user->email, 'lastname', 'asc', 0, 50, json_encode(['courseid' => $course->id]));
$check('Search by email returns >= 1', $r_search['total'] >= 1);

// === 3. unenrol_single ===
echo "\n=== 3. unenrol_single ===\n";
$r = \local_airpay_courses\external\unenrol_single::execute(
...
// Idempotent un-enrol
$r2 = \local_airpay_courses\external\unenrol_single::execute(
    (int) $course->id, (int) $user->id);
