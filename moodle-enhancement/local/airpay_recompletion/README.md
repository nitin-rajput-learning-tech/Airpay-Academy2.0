...
|---|---|---|
| `local/airpay_recompletion:view` | manager | view history + receive messages |
| `local/airpay_recompletion:manage` | manager | create/edit/delete rules |
| `local/airpay_recompletion:reset` | manager | run bulk-reset UI |

## Tables (2)

| Table | Purpose |
|---|---|
| `local_airpay_recompletion_rules` | Rule definitions (one per course or wildcard) |
| `local_airpay_recompletion_history` | Audit log of every reset event (cron + bulk) |
...
|---|---|---|
| `\local_airpay_recompletion\task\run_rules` | 02:47 daily | Evaluate every enabled rule |
...
```powershell
# 1. CLI smoke:
php "C:/xampp/htdocs/moodle5/public/local/airpay_recompletion/cli/smoke_recompletion.php"
...
The engine writes a row to `local_airpay_recompletion_history` for every
