 ...
 * `\local_sentientia_evaluation\observer`. When one of these events fires, the
 * observer scans `local_sentientia_evaluation` for ACTIVE rows whose
 * `trigger_event` matches and the user is in scope, then enqueues a row in
 * `local_sentientia_evaluation_triggers` with `fire_after = event_time + (days_after * 86400)`.
...
$observers = [
    ...
    [
        ...
        'callback'  => '\\local_sentientia_evaluation\\observer::course_completed',
        'internal'  => true,
        'priority'  => 9000,
    ],
