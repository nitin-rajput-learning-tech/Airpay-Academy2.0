...
function xmldb_local_sentientia_evaluation_upgrade(int $oldversion): bool {
    global $DB;
    ...
    if ($oldversion < 2026041910) {
        ...
        // ── local_sentientia_evaluation_questions ─────────────────────────
        $table = new xmldb_table('local_sentientia_evaluation_questions');
        if (!$dbman->table_exists($table)) {
            $table->add_field('id',           XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
            ...
            $table->add_key('fk_evaluation', XMLDB_KEY_FOREIGN, ['evaluationid'],
                            'local_sentientia_evaluation', ['id']);
        ...
        upgrade_plugin_savepoint(true, 2026041910, 'local', 'sentientia_evaluation');
    }
    ...
    //
    // Adds the local_sentientia_evaluation_triggers table so the event observer
    ...
    if ($oldversion < 2026051500) {
        $table = new xmldb_table('local_sentientia_evaluation_triggers');
        if (!$dbman->table_exists($table)) {
        ...
        upgrade_plugin_savepoint(true, 2026051500, 'local', 'sentientia_evaluation');
    }

    // 2026051901 — P1 #17: time-bounded availability + multiple-submit.
    //
    // Three new columns on local_sentientia_evaluation:
    ...
    if ($oldversion < 2026051901) {
        ...
        upgrade_plugin_savepoint(true, 2026051901, 'local', 'sentientia_evaluation');
    }

    // 2026051903 — P1 #19: email-on-response admin notification flag.
    //
    // Adds one column to local_sentientia_evaluation:
    ...
    if ($oldversion < 2026051903) {
        ...
        upgrade_plugin_savepoint(true, 2026051903, 'local', 'sentientia_evaluation');
    }

    // 2026052010 — P1 #30: conditional question display.
    //
    // Adds two columns to `local_sentientia_evaluation_questions`:
    ...
    // parity-audit-2026-05-15/sentientia_evaluation.md. Enables branching
    ...
    if ($oldversion < 2026052020) {
        $table = new xmldb_table('local_sentientia_evaluation_assign');
        if (!$dbman->table_exists($table)) {
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null,
                XMLDB_NOTNULL, XMLDB_SEQUENCE);
            $table->add_field('evaluationid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL);
    ...
    if ($oldversion < 2026052030) {
        $table = new xmldb_table('local_sentientia_evaluation_template');
        if (!$dbman->table_exists($table)) {
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null,
                XMLDB_NOTNULL, XMLDB_SEQUENCE);
            $table->add_field('name', XMLDB_TYPE_CHAR, '254', null, XMLDB_NOTNULL);
