...
class list_reports extends external_api {
    ...
    public static function execute(string $search = '', string $sort = 'name', string $sortdir = 'asc',
        ...
        $can_manage = has_capability('local/sentientia_reports:manage', $context);
        $can_export = has_capability('local/sentientia_reports:export', $context);

        if (strlen($params['filters']) > 4096) throw new \moodle_exception('filterstoolong', 'local_sentientia_reports');
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_sentientia_reports} r WHERE $wheresql", $sqlparams);

        $records = [];
        if ($total > 0) {
            $records = $DB->get_records_sql(
                "SELECT r.* FROM {local_sentientia_reports} r
                  WHERE $wheresql
