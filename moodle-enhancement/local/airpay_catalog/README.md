...
- Tiled course view at `/local/airpay_catalog/index.php`.
- Search + category filter + tag filter.
- Tenant-scoped: each visitor sees only the courses their tenant has
  access to.
- Public-tenant variant at `/local/airpay_catalog/public.php` accessible
...
Navigate to `/local/airpay_catalog/index.php`. Twelve to twenty course
