...
$PAGE->set_url('/local/airpay_catalog/cart.php');
...
if ($action === 'remove') {
    ...
    require_sesskey();
    \local_airpay_catalog\commerce::remove_from_cart($removeid);
    redirect(new moodle_url('/local/airpay_catalog/cart.php'), 'Removed from cart.');
}

// Handle clear cart.
if ($action === 'clear') {
    require_sesskey();
    \local_airpay_catalog\commerce::clear_cart();
    redirect(new moodle_url('/local/airpay_catalog/cart.php'), 'Cart cleared.');
}
...
$cart = \local_airpay_catalog\commerce::get_cart();
$totals = \local_airpay_catalog\commerce::get_cart_total();
...
<div class="ap-cart" style="max-width:700px; margin:0 auto;">
    ...
    <div style="display:flex; flex-direction:column; gap:12px; margin-bottom:20px;">
        ...
        <div style="display:flex; align-items:center; gap:16px; padding:16px 20px;
            ...
            <a href="<?php echo (new moodle_url('/local/airpay_catalog/cart.php', ['action' => 'remove', 'courseid' => $item['courseid'], 'sesskey' => sesskey()]))->out(); ?>"
    ...
    <div style="background:var(--ap-surface,#fff); border:1px solid var(--ap-border,#e3eaf3); border-radius:12px; padding:20px;">
        ...
        <a href="<?php echo (new moodle_url('/login/index.php', ['wantsurl' => $CFG->wwwroot . '/local/airpay_catalog/cart.php']))->out(); ?>"
        ...
        <p style="text-align:center; font-size:12px; color:var(--ap-text-muted); margin:8px 0 0;">
            Don't have an account? <a href="<?php echo (new moodle_url('/local/users/signup.php'))->out(); ?>" style="color:var(--ap-primary);">Register here</a>
        </p>

        <?php elseif ($totals['all_free']): ?>
        <!-- All free: enroll directly -->
        <form method="post" action="<?php echo (new moodle_url('/local/airpay_catalog/cart.php'))->out(false); ?>">
        ...
        <div style="text-align:center; margin-top:12px;">
            <a href="<?php echo (new moodle_url('/local/airpay_catalog/cart.php', ['action' => 'clear', 'sesskey' => sesskey()]))->out(); ?>"
