...
/**
 * CRUD tests for sentientia_classroom (delete + change_status + cascade).
 *
 * Locks in:
...
final class crud_test extends \advanced_testcase {
    ...
    private function seed_session(int $classroomid): int {
        ...
        $now = time();
        return (int) $DB->insert_record('local_sentientia_classroom_sessions', (object) [
    ...
    public function test_change_status_persists(): void {
        ...
        $this->assertEquals(session_manager::STATUS_COMPLETED,
            (int) $DB->get_field('local_sentientia_classroom', 'status', ['id' => $cid]));
    }
    ...
    public function test_delete_removes_classroom(): void {
        ...
        global $DB;
        $cid = $this->seed_classroom();
        $this->assertTrue($DB->record_exists('local_sentientia_classroom', ['id' => $cid]));

        $ok = session_manager::delete($cid);
        $this->assertTrue($ok);
        $this->assertFalse($DB->record_exists('local_sentientia_classroom', ['id' => $cid]));
    }
