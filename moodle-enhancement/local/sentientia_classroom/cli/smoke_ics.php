...
//
// Run: php public/local/sentientia_classroom/cli/smoke_ics.php
//
...
use local_sentientia_classroom\ics_builder;

// Find any existing classroom + session, or seed test data.
$session = $DB->get_record_sql(
    "SELECT * FROM {local_sentientia_classroom_sessions} ORDER BY id DESC LIMIT 1");
$classroom = null;
...
if (!$session) {
    // Seed.
    $cid = $DB->insert_record('local_sentientia_classroom', (object) [
    ...
    $classroom = $DB->get_record('local_sentientia_classroom', ['id' => $cid]);
    $sid = $DB->insert_record('local_sentientia_classroom_sessions', (object) [
        'classroomid' => $cid,
    ...
    $session = $DB->get_record('local_sentientia_classroom_sessions', ['id' => $sid]);
...
if ($cleanup_session_id > 0) {
    $DB->delete_records('local_sentientia_classroom_sessions',
        ['id' => $cleanup_session_id]);
    $DB->delete_records('local_sentientia_classroom',
