...
class edit_classroom extends \core_form\dynamic_form {
    ...
    protected function definition() {
        ...
        // ── Basic info ────────────────────────────────────────────────
        $mform->addElement('header', 'hdr_basic', get_string('heading_basic', 'local_sentientia_classroom'));

        $mform->addElement('text', 'name', get_string('name', 'local_sentientia_classroom'),
            ['size' => 50, 'maxlength' => 254]);
        $mform->setType('name', PARAM_TEXT);
        $mform->addRule('name', null, 'required', null, 'client');

        $mform->addElement('textarea', 'description', get_string('description', 'local_sentientia_classroom'),
            ['rows' => 3, 'cols' => 50]);
        $mform->setType('description', PARAM_TEXT);

        // ── Logistics ─────────────────────────────────────────────────
        $mform->addElement('header', 'hdr_logistics', get_string('heading_logistics', 'local_sentientia_classroom'));

        $mform->addElement('text', 'location', get_string('location', 'local_sentientia_classroom'),
            ['size' => 40, 'maxlength' => 254, 'placeholder' => 'Mumbai HQ - Training Room A']);
        $mform->setType('location', PARAM_TEXT);

        $mform->addElement('text', 'capacity', get_string('capacity', 'local_sentientia_classroom'),
        ...
        $mform->addElement('autocomplete', 'trainerid',
            get_string('trainer', 'local_sentientia_classroom'), [], $trainer_options);
        $mform->setType('trainerid', PARAM_INT);

        // ── Organisation ──────────────────────────────────────────────
        $mform->addElement('header', 'hdr_org', get_string('heading_org', 'local_sentientia_classroom'));

        $orgs = $this->get_org_options();
        $mform->addElement('select', 'costcenterid', get_string('organisation', 'local_sentientia_classroom'), $orgs);
        ...
        $mform->addElement('header', 'hdr_window',
            get_string('heading_window', 'local_sentientia_classroom'));

        $mform->addElement('date_selector', 'startdate',
            get_string('startdate', 'local_sentientia_classroom'),
            ['optional' => true]);
        $mform->setType('startdate', PARAM_INT);
        $mform->addHelpButton('startdate', 'startdate', 'local_sentientia_classroom');

        $mform->addElement('date_selector', 'enddate',
            get_string('enddate', 'local_sentientia_classroom'),
            ['optional' => true]);
        $mform->setType('enddate', PARAM_INT);
        $mform->addHelpButton('enddate', 'enddate', 'local_sentientia_classroom');

        // ── Status ────────────────────────────────────────────────────
        if (!$iscreate) {
            $mform->addElement('header', 'hdr_status', get_string('heading_status', 'local_sentientia_classroom'));
            $statusoptions = [
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        $cr = $DB->get_record('local_sentientia_classroom', ['id' => $classroomid], '*', MUST_EXIST);
    ...
    protected function get_page_url_for_dynamic_submission(): \moodle_url {
        return new \moodle_url('/local/sentientia_classroom/index.php');
    }
