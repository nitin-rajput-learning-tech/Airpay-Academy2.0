...
class list_classroom_sessions extends external_api {
    ...
    public static function execute(int $classroomid, string $search = '', string $sort = 'sessiondate',
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_sentientia_classroom_sessions} s WHERE $wheresql",
            $sqlparams);

        $records = [];
        if ($total > 0) {
            $records = $DB->get_records_sql(
                "SELECT s.* FROM {local_sentientia_classroom_sessions} s
                  WHERE $wheresql
        ...
        foreach ($records as $s) {
            ...
            $atturl = (new \moodle_url('/local/sentientia_classroom/attendance.php',
            ...
            $icsurl = (new \moodle_url('/local/sentientia_classroom/ics.php',
                ['sessionid' => (int) $s->id]))->out(false);
