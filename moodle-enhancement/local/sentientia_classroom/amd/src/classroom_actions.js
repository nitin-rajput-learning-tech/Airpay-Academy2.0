 ...
 *
 * @module     local_sentientia_classroom/classroom_actions
...
const openClassroomForm = async (classroomid, returnFocus) => {
    ...
    const title = await getString(titleKey, 'local_sentientia_classroom');
    const modalForm = new ModalForm({
        formClass: 'local_sentientia_classroom\\form\\edit_classroom',
...
const confirmStatusChange = async (classroomid, name, newStatus, msgKey, returnFocus) => {
    const successMsg = await getString('classroomstatuschanged', 'local_sentientia_classroom');
    const [title, message] = await Promise.all([
        getString(msgKey === 'confirmcancel' ? 'cancelclassroom' :
                  msgKey === 'confirmcomplete' ? 'completeclassroom' :
                  'reopenclassroom', 'local_sentientia_classroom'),
        getString(msgKey, 'local_sentientia_classroom', name),
    ]);
    Notification.confirm(title, message, title, null, () => {
        Ajax.call([{
            methodname: 'local_sentientia_classroom_change_status',
...
const confirmDelete = async (classroomid, name, returnFocus) => {
    const [title, message, deleteLabel, successMsg] = await Promise.all([
        getString('deleteclassroom', 'local_sentientia_classroom'),
        getString('confirmdelete', 'local_sentientia_classroom', name),
        getString('delete', 'core'),
        getString('classroomdeleted', 'local_sentientia_classroom'),
    ]);
    Notification.deleteCancelPromise(title, message, deleteLabel, returnFocus).then(() => {
        Ajax.call([{
            methodname: 'local_sentientia_classroom_delete_classroom',
            args: {classroomid: classroomid},
        }])[0].then(() => {
            Notification.addNotification({message: successMsg, type: 'success'});
            window.location.reload();
...
const openSessionForm = async (classroomid, sessionid, returnFocus) => {
    ...
    const title = await getString(titleKey, 'local_sentientia_classroom');
    const modalForm = new ModalForm({
        formClass: 'local_sentientia_classroom\\form\\edit_session',
...
const openEnrolUsersForm = async (classroomid, returnFocus) => {
    const title = await getString('enrol_users', 'local_sentientia_classroom');
    const modalForm = new ModalForm({
        formClass: 'local_sentientia_classroom\\form\\enrol_classroom_users',
        args: {classroomid: classroomid},
        modalConfig: {title: title, large: true},
        returnFocus: returnFocus,
    });
...
const openBulkEnrolAudienceForm = async (classroomid, returnFocus) => {
    ...
    const modalForm = new ModalForm({
        formClass: 'local_sentientia_classroom\\form\\bulk_enrol_audience_form',
        args: {classroomid: classroomid},
        modalConfig: {title: title, large: true},
        returnFocus: returnFocus,
    });
    modalForm.addEventListener(modalForm.events.LOADED, async () => {
        ...
        if (modalBody) {
            const helper = await import(
                'local_sentientia_classroom/audience_form_helper');
...
const confirmDeleteSession = async (sessionid, title, returnFocus) => {
    const [confirmTitle, message, deleteLabel, successMsg] = await Promise.all([
        getString('delete_session', 'local_sentientia_classroom'),
        getString('confirm_delete_session', 'local_sentientia_classroom', title),
        getString('delete', 'core'),
        getString('sessiondeleted', 'local_sentientia_classroom'),
    ]);
    Notification.deleteCancelPromise(confirmTitle, message, deleteLabel, returnFocus).then(() => {
        Ajax.call([{
            methodname: 'local_sentientia_classroom_delete_session',
...
const confirmUnenrolUser = async (classroomid, userid, name, returnFocus) => {
    const [confirmTitle, message, removeLabel, successMsg] = await Promise.all([
        getString('unenrol_user', 'local_sentientia_classroom'),
        getString('confirm_unenrol_user', 'local_sentientia_classroom', name),
        getString('remove', 'core'),
        getString('userunenrolled', 'local_sentientia_classroom'),
    ]);
    Notification.deleteCancelPromise(confirmTitle, message, removeLabel, returnFocus).then(() => {
        Ajax.call([{
            methodname: 'local_sentientia_classroom_unenrol_user',
