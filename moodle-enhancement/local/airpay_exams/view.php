...
$exam = $DB->get_record('local_airpay_exams', ['id' => $id], '*', MUST_EXIST);
$quiz = $exam->quizid
...
$PAGE->set_url(new moodle_url('/local/airpay_exams/view.php',
    ['id' => $id, 'tab' => $tab]));
$PAGE->set_pagelayout('admin');
$PAGE->set_title('Exam: ' . format_string($exam->name));
$PAGE->set_heading('Exam: ' . format_string($exam->name));
require_capability('local/airpay_exams:view', $ctx);

$can_edit  = has_capability('local/airpay_exams:update', $ctx);
$can_enrol = has_capability('local/airpay_exams:enrol', $ctx);
...
$data = [
    ...
    'tab_overview_url'    => (new moodle_url('/local/airpay_exams/view.php', ['id' => $id, 'tab' => 'overview']))->out(false),
    'tab_attempts_url'    => (new moodle_url('/local/airpay_exams/view.php', ['id' => $id, 'tab' => 'attempts']))->out(false),
    'tab_roster_url'      => (new moodle_url('/local/airpay_exams/view.php', ['id' => $id, 'tab' => 'roster']))->out(false),
    'tab_analytics_url'   => (new moodle_url('/local/airpay_exams/view.php', ['id' => $id, 'tab' => 'analytics']))->out(false),

    'can_edit'  => $can_edit,
    'can_enrol' => $can_enrol,
    'back_url'  => (new moodle_url('/local/airpay_exams/index.php'))->out(false),
];
$data = array_merge($data, $tab_data);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_exams/view', $data);
echo $OUTPUT->footer();
