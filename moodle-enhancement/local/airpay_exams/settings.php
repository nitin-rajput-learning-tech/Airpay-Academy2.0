...
if ($hassiteconfig) {
    ...
    $settings->add(new admin_setting_heading(
        'local_airpay_exams/overdue_heading',
        get_string('overdue_settings_heading', 'local_airpay_exams'),
        get_string('overdue_settings_intro', 'local_airpay_exams')));

    $settings->add(new admin_setting_configcheckbox(
        'local_airpay_exams/overdue_enabled',
        get_string('overdue_enabled', 'local_airpay_exams'),
        get_string('overdue_enabled_help', 'local_airpay_exams'),
        0));

    $settings->add(new admin_setting_configtext(
        'local_airpay_exams/overdue_days_after',
        get_string('overdue_days_after', 'local_airpay_exams'),
        get_string('overdue_days_after_help', 'local_airpay_exams'),
        '1,7,14', PARAM_TEXT));

    $settings->add(new admin_setting_configtext(
        'local_airpay_exams/overdue_max_per_run',
        get_string('overdue_max_per_run', 'local_airpay_exams'),
        get_string('overdue_max_per_run_help', 'local_airpay_exams'),
        '500', PARAM_INT));

    $lastrun_o  = (int) get_config('local_airpay_exams', 'overdue_last_run');
    $lastsent_o = (int) get_config('local_airpay_exams', 'overdue_last_sent');
    if ($lastrun_o > 0) {
        $body_o = get_string('overdue_last_run_value', 'local_airpay_exams',
            (object) ['time' => userdate($lastrun_o), 'count' => $lastsent_o]);
    } else {
        $body_o = get_string('overdue_last_run_never', 'local_airpay_exams');
    }
    $settings->add(new admin_setting_heading(
        'local_airpay_exams/overdue_status',
        get_string('overdue_status', 'local_airpay_exams'),
        $body_o));
