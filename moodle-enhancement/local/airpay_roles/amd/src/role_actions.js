...
const openEditCapForm = async (roleid, capability, current, returnFocus) => {
    const title = await getString('form_edit_cap', 'local_airpay_roles');
    ...
    modalForm.addEventListener(modalForm.events.FORM_SUBMITTED, async () => {
        const msg = await getString('cap_updated_success', 'local_airpay_roles', capability);
