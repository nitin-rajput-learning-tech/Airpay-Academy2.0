...
class list_roles extends external_api {
    ...
    public static function execute(string $search = '', string $archetype = 'all',
        ...
        $viewbase   = new \moodle_url('/local/airpay_roles/view.php');
        $auditbase  = new \moodle_url('/local/airpay_roles/view.php');
        foreach ($rows as &$r) {
            $actions = [];
            ...
            $actions[] = '<a href="' . s($viewurl->out(false)) . '" '
                ...
                . 'title="' . s(get_string('btn_view_role', 'local_airpay_roles')) . '">'
            ...
            if ($can_manage) {
                ...
                $actions[] = '<a href="' . s($editurl->out(false)) . '" '
                    . 'class="btn btn-sm btn-link text-muted p-1" '
                    . 'title="' . s(get_string('btn_edit_caps', 'local_airpay_roles')) . '">'
            ...
            if ($can_audit) {
                ...
                $actions[] = '<a href="' . s($audurl->out(false)) . '" '
                    . 'class="btn btn-sm btn-link text-muted p-1" '
                    . 'title="' . s(get_string('audit_filter_role', 'local_airpay_roles')) . '">'
            ...
            $r['archetype_label'] = $r['archetype'] !== '' ? $r['archetype']
                : '<em class="text-muted">' . s(get_string('ov_archetype_custom', 'local_airpay_roles')) . '</em>';
