...
namespace local_airpay_roles\form;
...
class edit_capability_dynamic_form extends dynamic_form {
    ...
    protected function definition(): void {
        ...
        if ($roleid > 0) {
            ...
            $mform->addElement('static', 'rolename',
                get_string('col_name', 'local_airpay_roles'),
        ...
        if ($capability !== '') {
            $mform->addElement('static', 'capability_label',
                get_string('form_capability', 'local_airpay_roles'),
                '<code>' . s($capability) . '</code>');
        }

        // Permission select.
        $perms = [
            'inherit'  => get_string('cap_perm_inherit',  'local_airpay_roles'),
            'allow'    => get_string('cap_perm_allow',    'local_airpay_roles'),
            'prevent'  => get_string('cap_perm_prevent',  'local_airpay_roles'),
            'prohibit' => get_string('cap_perm_prohibit', 'local_airpay_roles'),
        ];
        $mform->addElement('select', 'permission',
            get_string('form_permission', 'local_airpay_roles'), $perms);
        ...
        $mform->addElement('textarea', 'reason',
            get_string('form_reason', 'local_airpay_roles'),
            ['rows' => 2, 'cols' => 60, 'maxlength' => 1024]);
        $mform->setType('reason', PARAM_TEXT);
        $mform->addHelpButton('reason', 'form_reason', 'local_airpay_roles');
    }
    ...
    protected function check_access_for_dynamic_submission(): void {
        require_capability('local/airpay_roles:manage', $this->get_context_for_dynamic_submission());
    }
    ...
    protected function get_page_url_for_dynamic_submission(): moodle_url {
        $roleid = $this->optional_param('roleid', 0, PARAM_INT);
        return new moodle_url('/local/airpay_roles/view.php',
