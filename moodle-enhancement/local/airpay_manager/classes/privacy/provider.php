...
/**
 * Privacy Subsystem for local_airpay_manager.
 *
 * Stores PII in:
...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table('local_airpay_mgr_requests', [
        ...
        $collection->add_database_table('local_airpay_mgr_allocations', [
    ...
    public static function get_users_in_context(userlist $userlist): void {
        ...
        $sql = "SELECT userid FROM {local_airpay_mgr_requests}
                 UNION
                SELECT managerid FROM {local_airpay_mgr_requests} WHERE managerid > 0
                 UNION
                SELECT decided_by FROM {local_airpay_mgr_requests} WHERE decided_by > 0
                 UNION
                SELECT userid FROM {local_airpay_mgr_allocations}
                 UNION
                SELECT managerid FROM {local_airpay_mgr_allocations} WHERE managerid > 0";
    ...
    public static function delete_data_for_all_users_in_context(\context $context): void {
        ...
        $DB->delete_records('local_airpay_mgr_requests');
        $DB->delete_records('local_airpay_mgr_allocations');
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist): void {
        ...
        $DB->delete_records('local_airpay_mgr_requests', ['userid' => $userid]);
        // Anonymise rows where they were manager or decider on others' requests.
        $DB->execute("UPDATE {local_airpay_mgr_requests}
        ...
        $DB->delete_records('local_airpay_mgr_allocations', ['userid' => $userid]);
        // Anonymise allocations they CREATED for others.
        $DB->execute("UPDATE {local_airpay_mgr_allocations}
    ...
    public static function delete_data_for_users(approved_userlist $userlist): void {
        ...
        $DB->execute("DELETE FROM {local_airpay_mgr_requests} WHERE userid $insql", $inparams);
        $DB->execute("UPDATE {local_airpay_mgr_requests}
                         SET managerid = 0, decision_reason = NULL
        ...
        $DB->execute("DELETE FROM {local_airpay_mgr_allocations} WHERE userid $insql", $inparams);
        $DB->execute("UPDATE {local_airpay_mgr_allocations}
                         SET managerid = 0, note = NULL
                       WHERE managerid $insql", $inparams);
    }
