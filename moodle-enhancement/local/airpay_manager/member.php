...
if (!\local_airpay_manager\team_manager::can_view_member((int) $USER->id, $userid)) {
    throw new \moodle_exception('nopermission', 'error', '', null,
        'You can only view direct reports or skip-level reports under you.');
}

$detail = \local_airpay_manager\team_manager::get_member_detail($userid);
$user = $detail['user'];

$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/local/airpay_manager/member.php', ['id' => $userid]));
$PAGE->set_title('Team Member — ' . fullname($user));
$PAGE->set_heading('Team Member — ' . fullname($user));
$PAGE->set_pagelayout('standard');
$PAGE->set_secondary_navigation(false);
$PAGE->navbar->add('My Team', new moodle_url('/local/airpay_manager/index.php'));
...
$data = [
    ...
    'back_url'      => (new moodle_url('/local/airpay_manager/index.php'))->out(false),
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_manager/member', $data);
echo $OUTPUT->footer();
