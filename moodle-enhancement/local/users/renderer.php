...
class local_users_renderer extends plugin_renderer_base {
    ...
    public function employees_profile_view($id) {
        ...
        // Inject gamification data (points, badges, rank, streak).
        global $CFG;
        if (file_exists($CFG->dirroot . '/local/airpay_gamification/lib.php')) {
            try {
                require_once($CFG->dirroot . '/local/airpay_gamification/lib.php');
                $usersviewContext['ap_gamification'] = local_airpay_gamification_get_summary($id);
            } catch (\Throwable $e) {
                $usersviewContext['ap_gamification'] = null;
            }
        }

        // Inject skills summary (readiness %, met/total).
        if (file_exists($CFG->dirroot . '/local/airpay_skills/classes/skills_manager.php')) {
            try {
                $analysis = \local_airpay_skills\skills_manager::get_gap_analysis($id);
                if ($analysis['has_data']) {
                    $usersviewContext['ap_skills_summary'] = $analysis['summary'];
                }
            } catch (\Throwable $e) {
