...
/**
 * CLI smoke test for airpay_cart.
 *
...
require_once(__DIR__ . '/../lib.php');  // For local_airpay_cart_get_course_price()

global $DB;

echo "=== airpay_cart smoke test ===\n\n";
...
$price = local_airpay_cart_get_course_price($course->id);
$check('Course price set to ₹1500', $price === 1500.00, "got $price");

// === 2. Add to cart ===
echo "\n=== 2. Add to cart ===\n";
$cart = \local_airpay_cart\cart_manager::add_item($user->id, $course->id);
...
echo "\n=== 3. Checkout ===\n";
$cart = \local_airpay_cart\cart_manager::checkout($user->id, [
...
$ok = \local_airpay_cart\cart_manager::mark_paid($historyid, 'SMOKE-REF-' . time());
$check('mark_paid returns true', $ok === true);

$cart_after = $DB->get_record('local_airpay_cart_history', ['id' => $historyid]);
$check('Status = paid', $cart_after->status === 'paid');
$check('timepaid set', !empty($cart_after->timepaid));

// Ledger row?
$ledger = $DB->get_records('local_airpay_cart_ledger', ['historyid' => $historyid]);
...
// Invoice issued?
$invoice = $DB->get_record('local_airpay_cart_invoices', ['historyid' => $historyid]);
...
// === 5. Refund (full) ===
echo "\n=== 5. Refund ===\n";
$ok = \local_airpay_cart\cart_manager::refund($historyid, 0, 'Smoke test refund', $user->id);
$check('refund returns true', $ok === true);
$cart_after = $DB->get_record('local_airpay_cart_history', ['id' => $historyid]);
$check('Status = refunded', $cart_after->status === 'refunded');
$ledger_count = $DB->count_records('local_airpay_cart_ledger', ['historyid' => $historyid]);
$check('Ledger has 2 entries (payment + refund)', $ledger_count === 2);
$check('User unenrolled', !is_enrolled($context, $user->id));

// === 6. Audit log integrity ===
echo "\n=== 6. Audit log ===\n";
$ledger_rows = $DB->get_records('local_airpay_cart_ledger',
