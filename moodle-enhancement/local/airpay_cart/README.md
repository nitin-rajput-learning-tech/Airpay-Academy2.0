...
1. Maintain one open cart per user (`local_airpay_cart_history` row, `status='open'`).
...
|---|---|---|---|
| `local/airpay_cart:view` | read | user, manager, editingteacher, teacher, student | system |
| `local/airpay_cart:purchase` | write | user, manager, student | system |
| `local/airpay_cart:viewallorders` | read | manager | system |
| `local/airpay_cart:refund` | write | _(siteadmin only)_ | system |
| `local/airpay_cart:manageprices` | write | manager, editingteacher | **course** ← Phase 8.1 B9 fix |
...
| Table | Purpose |
|---|---|
| `local_airpay_cart_history` | One row per cart/order (open → pending → paid/failed/refunded) |
| `local_airpay_cart_id` | Sequential order id reservation table |
| `local_airpay_cart_ledger` | Immutable INSERT-only payment events (payment_received, refund_full, refund_partial) |
| `local_airpay_cart_invoices` | GST-compliant invoices (per-year sequential numbering) |
| `local_airpay_cart_credits` | Customer credits / wallet balance |
...
| Function | Purpose |
|---|---|
| `local_airpay_cart_add_item` | Add a course to user's cart |
| `local_airpay_cart_remove_item` | Remove a course from user's cart |
| `local_airpay_cart_get_cart` | Fetch current cart with totals |
| `local_airpay_cart_checkout` | Reserve order + transition status |
| `local_airpay_cart_get_order` | Fetch a specific order (tenant-scoped) |
| `local_airpay_cart_list_orders` | Paginated order list (tenant-scoped for non-admin) |
| `local_airpay_cart_refund_order` | Admin/manager refund (tenant-scoped) |
| `local_airpay_cart_daily_sums` | Finance report (tenant-scoped) |
| `local_airpay_cart_set_course_price` | Cap at CONTEXT_COURSE ← Phase 8.1 B9 |
| `local_airpay_cart_get_course_price` | Read current price for a course |
| `local_airpay_cart_my_invoices` | List own invoices |
| `local_airpay_cart_get_invoice` | Render one invoice (HTML + print-to-PDF UI) |
...
```powershell
php "C:/xampp/htdocs/moodle5/public/local/airpay_cart/cli/smoke_cart.php"
