...
class airpay_gateway implements gateway_interface {
    ...
    public function initiate_payment(\stdClass $cart): array {
        global $CFG;
        $endpoint   = (string) get_config('local_airpay_cart', 'airpay_endpoint');
        $merchantid = (string) get_config('local_airpay_cart', 'airpay_merchantid');
        $secret     = (string) get_config('local_airpay_cart', 'airpay_secret');

        if (empty($endpoint) || empty($merchantid) || empty($secret)) {
            throw new \moodle_exception('error_gatewaydown', 'local_airpay_cart',
        ...
        $params = [
            ...
            'callback_url'     => $CFG->wwwroot . '/local/airpay_cart/callback.php',
            'success_url'      => $CFG->wwwroot . '/local/airpay_cart/return.php?orderid=' . urlencode($orderid),
            'failed_url'       => $CFG->wwwroot . '/local/airpay_cart/return.php?orderid=' . urlencode($orderid) . '&fail=1',
        ];
