...
class list_orders extends external_api {
    ...
    public static function execute(string $search = '', string $sort = 'timecreated',
        ...
        $can_view_all = has_capability('local/airpay_cart:viewallorders', $ctx);
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_airpay_cart_history} WHERE $wheresql",
            $sqlparams);
        ...
        if ($total > 0) {
            $records = $DB->get_records_sql(
                "SELECT id, orderid, userid, costcenterid, total_amount, currency,
                   ...
                   FROM {local_airpay_cart_history}
                  WHERE $wheresql
