...
class cart_manager {
    ...
    public static function is_enabled_for_user(\stdClass $user): bool {
        $enabled = get_config('local_airpay_cart', 'enabled_tenants');
    ...
    public static function get_or_open_cart(int $userid): \stdClass {
        ...
        $cart = (object) [
            ...
            'tax_amount'      => 0,
            'total_amount'    => 0,
            'currency'        => get_config('local_airpay_cart', 'currency') ?: 'INR',
            'status'          => 'open',
            'timecreated'     => time(),
            'timemodified'    => time(),
        ];
        $cart->id = $DB->insert_record('local_airpay_cart_history', $cart);
        return $cart;
    }
    ...
    public static function add_item(int $userid, int $courseid): \stdClass {
        ...
        $price = local_airpay_cart_get_course_price($courseid);
        if ($price === null) {
        ...
        if (is_enrolled($context, $userid)) {
            throw new \moodle_exception('error_alreadyenrolled', 'local_airpay_cart');
        }
    ...
    public static function recompute_totals(\stdClass $cart): \stdClass {
        ...
        $DB->update_record('local_airpay_cart_history', $cart);
        return $cart;
    }
    ...
    public static function checkout(int $userid, array $billing, string $gateway): \stdClass {
        ...
        $items = json_decode($cart->items_json ?: '[]', true) ?: [];
        if (empty($items)) {
            throw new \moodle_exception('error_emptycart', 'local_airpay_cart');
        }
        ...
        $orderid = $DB->insert_record('local_airpay_cart_id',
    ...
    public static function mark_paid(int $historyid, string $gateway_ref, array $payload = []): bool {
        ...
        try {
            ...
            // 1. Ledger.
            $DB->insert_record('local_airpay_cart_ledger', (object) [
                'historyid'   => $cart->id,
    ...
    public static function refund(int $historyid, float $amount, string $reason, int $initiatedby): bool {
        ...
        $previousrefunds = (float) $DB->get_field_sql(
            "SELECT COALESCE(SUM(ABS(amount)), 0) FROM {local_airpay_cart_ledger}
