...
$PAGE->set_url(new moodle_url('/local/airpay_cart/return.php', ['orderid' => $orderid]));
$PAGE->set_pagelayout('standard');
$PAGE->set_title(get_string('orderconfirmation', 'local_airpay_cart'));
$PAGE->set_heading(get_string('ordernumber', 'local_airpay_cart', $orderid));

$status_label = get_string('status_' . $cart->status, 'local_airpay_cart');
...
$data = [
    ...
    'total_str'      => \local_airpay_cart\invoicer::currency_symbol($cart->currency)
                      . number_format((float) $cart->total_amount, 2),
    'invoice_url'    => $invoice
        ? (new moodle_url('/local/airpay_cart/invoice.php',
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_cart/return', $data);
echo $OUTPUT->footer();
