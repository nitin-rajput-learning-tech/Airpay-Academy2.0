 ...
 */
function local_airpay_cart_extend_navigation_user_settings(navigation_node $navigation, $user, $context) {
    global $USER;
    if ($USER->id != $user->id) {
        return;
    }
    ...
    $url = new moodle_url('/local/airpay_cart/index.php');
    $navigation->add(get_string('mycartlong', 'local_airpay_cart'), $url,
 ...
 */
function local_airpay_cart_get_course_price(int $courseid): ?float {
    global $DB;
