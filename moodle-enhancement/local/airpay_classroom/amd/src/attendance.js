 ...
 *
 * @module     local_airpay_classroom/attendance
...
const saveAttendance = async (sessionid, root) => {
    ...
    if (marks.length === 0) {
        const empty = await getString('no_attendance_yet', 'local_airpay_classroom');
    ...
    try {
        const response = await Ajax.call([{
            methodname: 'local_airpay_classroom_bulk_mark_attendance',
