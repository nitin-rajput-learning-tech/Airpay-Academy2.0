...
class list_classrooms extends external_api {
    ...
    public static function execute(string $search = '', string $sort = 'name', string $sortdir = 'asc',
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_airpay_classroom} c WHERE $wheresql", $sqlparams);

        $records = [];
        if ($total > 0) {
            $records = $DB->get_records_sql(
                "SELECT c.* FROM {local_airpay_classroom} c
                  WHERE $wheresql
               ORDER BY $orderby",
        ...
        foreach ($records as $c) {
            ...
            $viewurl = (new \moodle_url('/local/airpay_classroom/view.php', ['id' => $c->id]))->out(false);
