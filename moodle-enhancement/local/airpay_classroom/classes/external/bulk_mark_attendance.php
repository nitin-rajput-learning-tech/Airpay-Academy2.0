...
class bulk_mark_attendance extends external_api {
    ...
    public static function execute(int $sessionid, array $marks): array {
        ...
        require_capability('local/airpay_classroom:attendance', $context);

        if (count($params['marks']) > self::MAX_MARKS) {
            throw new \moodle_exception('toomanymarks', 'local_airpay_classroom');
        }

        $count = \local_airpay_classroom\session_manager::bulk_mark_attendance(
