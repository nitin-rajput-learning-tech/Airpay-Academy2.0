...
use local_airpay_classroom\session_manager;
...
final class sessions_external_test extends \advanced_testcase {
    ...
    private function seed_classroom_and_session(): array {
        ...
        if (!$DB->get_manager()->table_exists('local_airpay_classroom')) {
            $this->markTestSkipped('local_airpay_classroom not present.');
        }
        $now = time();
        $cid = (int) $DB->insert_record('local_airpay_classroom', (object) [
            'name'         => 'Cap Test',
            'description'  => '',
            'costcenterid' => 0,
            'open_path'    => '/1',
    ...
    public function test_list_sessions_requires_view(): void {
        ...
        $this->setAdminUser();
        \local_airpay_classroom\session_manager::create_session($ids['classroomid'], (object) [
