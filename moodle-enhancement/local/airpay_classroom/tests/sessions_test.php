...
final class sessions_test extends \advanced_testcase {
    ...
    public function test_create_session_persists(): void {
        ...
        global $DB;
        $row = $DB->get_record('local_airpay_classroom_sessions', ['id' => $sid], '*', MUST_EXIST);
    ...
    public function test_delete_session_cascades_attendance(): void {
        ...
        $this->assertTrue($DB->record_exists('local_airpay_classroom_attendance',
            ['sessionid' => $sid, 'userid' => $u->id]));

        session_manager::delete_session($sid);

        $this->assertFalse($DB->record_exists('local_airpay_classroom_sessions', ['id' => $sid]));
        $this->assertFalse($DB->record_exists('local_airpay_classroom_attendance',
            ['sessionid' => $sid, 'userid' => $u->id]));
    }
    ...
    public function test_unenrol_user_clears_attendance(): void {
        ...
        $this->assertFalse($DB->record_exists('local_airpay_classroom_users',
    ...
    public function test_mark_attendance_creates_then_updates(): void {
        ...
        session_manager::mark_attendance($sid, (int) $u->id, session_manager::ATT_PRESENT);
        $this->assertSame(1, $DB->count_records('local_airpay_classroom_attendance',
            ['sessionid' => $sid, 'userid' => $u->id]));
        $this->assertEquals(session_manager::ATT_PRESENT,
            (int) $DB->get_field('local_airpay_classroom_attendance', 'status',
                ['sessionid' => $sid, 'userid' => $u->id]));
    ...
    public function test_delete_classroom_cascades_to_roster(): void {
        ...
        $this->assertTrue($DB->record_exists('local_airpay_classroom_users',
            ['classroomid' => $cid, 'userid' => $u->id]));
