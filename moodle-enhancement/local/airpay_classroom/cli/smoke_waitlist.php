...
echo "=== airpay_classroom waitlist smoke ===\n\n";
...
$classroom = $DB->get_record_sql(
    "SELECT * FROM {local_airpay_classroom} ORDER BY id LIMIT 1");
if (!$classroom) {
    ...
    $classroom->id = $DB->insert_record('local_airpay_classroom', $classroom);
} else {
    // Ensure capacity = 2 for the test.
    $classroom->capacity = 2;
    $DB->update_record('local_airpay_classroom', $classroom);
}
...
\local_airpay_classroom\session_manager::unenrol_user($classroom->id, $u1);
$check('u1 removed from roster',
    !$DB->record_exists('local_airpay_classroom_users',
        ['classroomid' => $classroom->id, 'userid' => $u1]));
$check('u3 promoted to roster',
    $DB->record_exists('local_airpay_classroom_users',
        ['classroomid' => $classroom->id, 'userid' => $u3]));

$w3_after = $DB->get_record('local_airpay_classroom_waitlist', ['id' => $w3->id]);
...
$w4_after = $DB->get_record('local_airpay_classroom_waitlist', ['id' => $w4->id]);
$check('u4 renumbered to position 1', (int) $w4_after->position === 1,
    "position={$w4_after->position}");

// === 4. Leave waitlist ===
echo "\n=== 4. Leave waitlist ===\n";
$ok = \local_airpay_classroom\waitlist_manager::leave($w4->id, $u4);
$check('Leave returns true', $ok);
$w4_left = $DB->get_record('local_airpay_classroom_waitlist', ['id' => $w4->id]);
$check('u4 waitlist status = removed', $w4_left->status === 'removed');

// 4b. Other user can't remove
try {
    $w5_id = \local_airpay_classroom\waitlist_manager::join($classroom->id, $u4)->id;
    \local_airpay_classroom\waitlist_manager::leave($w5_id, $u1);
...
echo "\n=== 5. List waitlist ===\n";
$rows = \local_airpay_classroom\waitlist_manager::list_waiting($classroom->id);
...
$DB->delete_records('local_airpay_classroom_waitlist', ['classroomid' => $classroom->id]);
$promoted = \local_airpay_classroom\waitlist_manager::auto_promote($classroom->id);
