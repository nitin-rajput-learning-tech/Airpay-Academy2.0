...
/**
 * Upgrade helpers for local_sentientia_live.
 *
...
/**
 * ADR-031: take local/sentientia_live:manage_all back from every role that
 ...
 */
function local_sentientia_live_revoke_manage_all(): int {
    global $DB;
    $syscontext = \context_system::instance();
    $roleids = $DB->get_fieldset_select('role_capabilities', 'DISTINCT roleid',
        'capability = :cap AND contextid = :ctx',
        ['cap' => 'local/sentientia_live:manage_all', 'ctx' => $syscontext->id]);
    foreach ($roleids as $roleid) {
        unassign_capability('local/sentientia_live:manage_all', (int) $roleid, $syscontext->id);
    }
