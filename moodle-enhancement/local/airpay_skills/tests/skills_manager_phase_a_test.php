...
final class skills_manager_phase_a_test extends \advanced_testcase {
    ...
    public function test_save_skill_level_inserts_then_updates(): void {
        ...
        $row = $DB->get_record('local_airpay_skill_levels', ['id' => $id1], '*', MUST_EXIST);
    ...
    public function test_delete_designation_skill_removes_row(): void {
        ...
        $this->assertTrue($DB->record_exists('local_airpay_role_skills', ['id' => $id]));

        skills_manager::delete_designation_skill($id);
        $this->assertFalse($DB->record_exists('local_airpay_role_skills', ['id' => $id]));
    }
    ...
    public function test_delete_skill_cascades_to_levels(): void {
        ...
        $this->assertTrue($DB->record_exists('local_airpay_skill_levels',
            ['skillid' => $skillid]));

        skills_manager::delete_skill($skillid);

        $this->assertFalse($DB->record_exists('local_airpay_skill_levels',
            ['skillid' => $skillid]),
