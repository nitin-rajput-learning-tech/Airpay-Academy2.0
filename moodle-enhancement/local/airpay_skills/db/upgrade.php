...
function xmldb_local_airpay_skills_upgrade(int $oldversion): bool {
    ...
    //
    // Adds `local_airpay_user_skill_hist` — append-only history of every
    ...
    if ($oldversion < 2026051901) {
        $tbl = new xmldb_table('local_airpay_user_skill_hist');
        if (!$dbman->table_exists($tbl)) {
        ...
        upgrade_plugin_savepoint(true, 2026051901, 'local', 'airpay_skills');
    }
