 ...
 */
function xmldb_local_airpay_skills_install() {
    global $DB;

    if ($DB->count_records('local_airpay_skill_cats') > 0) {
        return;
    }
    ...
    foreach ($categories as $cat) {
        $cat['description'] = '';
        $cat['timecreated'] = $now;
        $catids[$cat['name']] = $DB->insert_record('local_airpay_skill_cats', (object)$cat);
    }
    ...
    foreach ($skills as $catname => $skilllist) {
        ...
        foreach ($skilllist as $skillname) {
            $DB->insert_record('local_airpay_skills', (object)[
                'categoryid'  => $catid,
                'name'        => $skillname,
                'description' => '',
                'max_level'   => 5,
