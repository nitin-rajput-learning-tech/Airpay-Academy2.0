...
$skill = $DB->get_record('local_airpay_skills', ['id' => $id], '*', MUST_EXIST);
$category = $DB->get_record('local_airpay_skill_cats',
    ['id' => $skill->categoryid]);

$ctx = context_system::instance();
$PAGE->set_context($ctx);
$PAGE->set_url(new moodle_url('/local/airpay_skills/view.php',
    ['id' => $id, 'tab' => $tab]));
$PAGE->set_pagelayout('admin');
$PAGE->set_title('Skill: ' . format_string($skill->name));
$PAGE->set_heading('Skill: ' . format_string($skill->name));
require_capability('local/airpay_skills:view', $ctx);

// Counts for tab badges.
$count_levels = (int) $DB->count_records('local_airpay_skill_levels',
    ['skillid' => $skill->id]);
$count_designations = (int) $DB->count_records('local_airpay_role_skills',
    ['skillid' => $skill->id]);
$count_courses = (int) $DB->count_records('local_airpay_course_skills',
    ['skillid' => $skill->id]);
$count_learners = (int) $DB->count_records('local_airpay_user_skills',
    ['skillid' => $skill->id]);
...
switch ($tab) {
    case 'levels':
        $levels = $DB->get_records('local_airpay_skill_levels',
        ...
        $tab_data['edit_url'] = (new moodle_url('/local/airpay_skills/level_definitions.php',
    ...
    case 'designations':
        ...
        $tab_data['matrix_url'] = (new moodle_url('/local/airpay_skills/designation_matrix.php'))->out(false);
        break;
    ...
    case 'courses':
        ...
        $tab_data['courses'] = array_values(array_map(fn($r) => [
            ...
            'view_url'  => (new \moodle_url('/local/airpay_courses/enrolledusers.php',
                ['id' => (int) $r->courseid]))->out(false),
        ], $records));
        $tab_data['has_courses'] = count($records) > 0;
        $tab_data['map_url'] = (new moodle_url('/local/airpay_skills/course_mapping.php'))->out(false);
        break;
...
$data = [
    ...
    'tab_overview_url'    => (new moodle_url('/local/airpay_skills/view.php', ['id' => $id, 'tab' => 'overview']))->out(false),
    'tab_levels_url'      => (new moodle_url('/local/airpay_skills/view.php', ['id' => $id, 'tab' => 'levels']))->out(false),
    'tab_designations_url' => (new moodle_url('/local/airpay_skills/view.php', ['id' => $id, 'tab' => 'designations']))->out(false),
    'tab_courses_url'     => (new moodle_url('/local/airpay_skills/view.php', ['id' => $id, 'tab' => 'courses']))->out(false),
    'tab_learners_url'    => (new moodle_url('/local/airpay_skills/view.php', ['id' => $id, 'tab' => 'learners']))->out(false),

    'back_url' => (new moodle_url('/local/airpay_skills/index.php'))->out(false),
