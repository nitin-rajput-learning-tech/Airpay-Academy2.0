...
class edit_category extends \core_form\dynamic_form {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'hdr_basic', get_string('heading_category', 'local_airpay_skills'));

        $mform->addElement('text', 'name', get_string('category_name', 'local_airpay_skills'),
        ...
        // ── Visual ────────────────────────────────────────────────────
        $mform->addElement('header', 'hdr_visual', get_string('heading_visual', 'local_airpay_skills'));
        ...
        $mform->addElement('select', 'icon', get_string('icon', 'local_airpay_skills'), $icon_options);
        $mform->setType('icon', PARAM_TEXT);
        $mform->setDefault('icon', 'fa-cogs');

        $mform->addElement('text', 'color', get_string('color', 'local_airpay_skills'),
            ['size' => 10, 'maxlength' => 10, 'placeholder' => '#0066A7']);
        $mform->setType('color', PARAM_TEXT);
        $mform->setDefault('color', '#0066A7');
        $mform->addHelpButton('color', 'color', 'local_airpay_skills');
    ...
    public function validation($data, $files) {
        ...
        if (!empty($data['color']) && !preg_match('/^#[0-9a-fA-F]{6}$/', $data['color'])) {
            $errors['color'] = get_string('color_invalid', 'local_airpay_skills');
        }
        return $errors;
    }
    ...
    public function process_dynamic_submission() {
        ...
        if ($categoryid === 0) {
            $newid = \local_airpay_skills\skills_manager::create_category($data);
            return ['categoryid' => $newid, 'message' => get_string('categorycreated', 'local_airpay_skills')];
        } else {
            \local_airpay_skills\skills_manager::update_category($categoryid, $data);
            return ['categoryid' => $categoryid, 'message' => get_string('categoryupdated', 'local_airpay_skills')];
        }
    }
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        $c = $DB->get_record('local_airpay_skill_cats', ['id' => $categoryid], '*', MUST_EXIST);
