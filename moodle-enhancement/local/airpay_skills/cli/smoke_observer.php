...
//
// Run: php public/local/airpay_skills/cli/smoke_observer.php
//
...
$skill = $DB->get_record_sql(
    "SELECT id, max_level FROM {local_airpay_skills} ORDER BY id ASC LIMIT 1");
...
$mapid = \local_airpay_skills\skills_manager::save_course_skill(
...
\local_airpay_skills\skills_manager::update_from_course(
    (int) $user->id, (int) $course->id);

$us = $DB->get_record('local_airpay_user_skills', [
    'userid'  => (int) $user->id,
...
$DB->delete_records('local_airpay_user_skills',
    ['userid' => (int) $user->id, 'skillid' => (int) $skill->id]);
$DB->delete_records('local_airpay_course_skills',
