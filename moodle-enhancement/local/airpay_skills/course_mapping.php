...
// {@see local_airpay_skills\observer::course_completed} consumes these
...
$PAGE->set_url(new moodle_url('/local/airpay_skills/course_mapping.php',
...
$PAGE->navbar->add('Skills',
    new moodle_url('/local/airpay_skills/admin.php'));
...
$top_courses_rows = $DB->get_records_sql(
    "SELECT c.id, c.fullname, c.shortname,
            (SELECT COUNT(*) FROM {local_airpay_course_skills} cs
...
$selected_course = $courseid
    ? \local_airpay_skills\skills_manager::get_course_summary($courseid) : null;
$mapped_rows = $courseid
    ? \local_airpay_skills\skills_manager::list_course_skills($courseid) : [];
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_skills/course_mapping', $data);
