 ...
 */
function local_airpay_gamification_seed_badges() {
    global $DB;

    // Only seed if no badges exist.
    if ($DB->count_records('local_airpay_badges') > 0) {
        return;
    }
    ...
    foreach ($badges as $badge) {
        ...
        $DB->insert_record('local_airpay_badges', (object)$badge);
    }
}
 ...
 */
function local_airpay_gamification_get_summary(int $userid): array {
    $total = \local_airpay_gamification\points_manager::get_total($userid);
    $today = \local_airpay_gamification\points_manager::get_today($userid);
    $level = \local_airpay_gamification\points_manager::get_level($total);
    $badges = \local_airpay_gamification\badge_manager::get_user_badges($userid);
    $rank = \local_airpay_gamification\leaderboard::get_rank($userid);
