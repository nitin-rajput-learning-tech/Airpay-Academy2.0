...
class points_manager {
    ...
    public static function award(int $userid, string $action, ?int $courseid = null, string $description = ''): int {
        ...
        $recent = $DB->record_exists_select('local_airpay_points_log',
        ...
        $DB->insert_record('local_airpay_points_log', $record);
    ...
    public static function get_today(int $userid): int {
        ...
        return (int)$DB->get_field_sql(
            "SELECT COALESCE(SUM(points), 0) FROM {local_airpay_points_log}
    ...
    private static function update_total_points(int $userid): void {
        ...
        if ($streak) {
            ...
            $DB->insert_record('local_airpay_streaks', (object)[
                'userid'          => $userid,
                'current_streak'  => 0,
                'longest_streak'  => 0,
                'last_login_date' => '',
