 ...
 * from \local_costcenter\lib\accesslib to \local_sentientia_org\accesslib
...
class accesslib {
    ...
    public static function get_costcenterpath_context(\context $context): string {
        ...
        $org = $DB->get_record('local_sentientia_org', ['id' => $rootcatid]);
    ...
    public static function get_costcenter_info(int $orgid) {
        ...
        $record = $DB->get_record('local_sentientia_org', ['id' => $orgid]);
        if ($record) {
            return $record;
        }
     ...
     *
     * During migration, local_sentientia_org may not have data yet.
     ...
     * the new `local_sentientia_org` table first, falling back to the
    ...
    public static function costcenterpath_contextdata(string $costcenterpath): \context {
        ...
        // local_sentientia_org has no `category` column — we don't need one because
     ...
     *   2. **Sentientia-native fallback** — match `local_sentientia_org.shortname`
    ...
    public static function get_tenant_category_id(string $open_path): ?int {
        ...
        try {
            $shortname = $DB->get_field('local_sentientia_org', 'shortname',
                ['id' => $top_id, 'depth' => 1]);
            if (!empty($shortname)) {
            ...
            // local_sentientia_org or column missing — fall through.
        }
