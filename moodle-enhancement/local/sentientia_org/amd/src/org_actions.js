...
// @module     local_sentientia_org/org_actions
...
const openOrgForm = async (orgid, parentid, returnFocus) => {
    ...
    const title = await getString(titleKey, 'local_sentientia_org');
    const modalForm = new ModalForm({
        formClass: 'local_sentientia_org\\form\\edit_org',
...
const confirmToggle = async (orgid, name, isVisible, returnFocus) => {
    ...
    const [title, message, success] = await Promise.all([
        getString(titleKey, 'local_sentientia_org'),
        getString(msgKey, 'local_sentientia_org', name),
        getString('orgvisibilitychanged', 'local_sentientia_org'),
    ]);
    Notification.confirm(title, message, title, null, () => {
        Ajax.call([{methodname: 'local_sentientia_org_toggle_visibility',
                    args: {orgid: orgid}}])[0]
            .then(() => {
...
const confirmDelete = async (orgid, name, returnFocus) => {
    const [title, message, label, success] = await Promise.all([
        getString('deleteorg', 'local_sentientia_org'),
        getString('confirmdelete', 'local_sentientia_org', name),
        getString('delete', 'core'),
        getString('orgdeleted', 'local_sentientia_org'),
    ]);
    Notification.deleteCancelPromise(title, message, label, returnFocus).then(() => {
        Ajax.call([{methodname: 'local_sentientia_org_delete_org',
                    args: {orgid: orgid}}])[0]
            .then(() => {
                Notification.addNotification({message: success, type: 'success'});
                window.location.reload();
