...
/**
 * Privacy provider lock-in tests for local_airpay_challenge.
 *
...
final class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function test_get_metadata_declares_three_tables(): void {
        $collection = new \core_privacy\local\metadata\collection('local_airpay_challenge');
        $collection = provider::get_metadata($collection);
        $items = $collection->get_collection();
        $names = array_map(fn($i) => $i->get_name(), $items);
        $this->assertContains('local_airpay_challenge_challenges', $names);
        $this->assertContains('local_airpay_challenge_attempts', $names);
        $this->assertContains('local_airpay_challenge_leaderboard', $names);
    }
    ...
    public function test_export_user_data_includes_attempts(): void {
        ...
        $contextlist = new approved_contextlist($u, 'local_airpay_challenge', [$sysctx->id]);
    ...
    public function test_delete_data_for_user_nukes_attempts_and_leaderboard(): void {
        ...
        $this->assertTrue($DB->record_exists('local_airpay_challenge_attempts',
            ['userid' => $u->id]));
        $this->assertTrue($DB->record_exists('local_airpay_challenge_leaderboard',
            ['userid' => $u->id]));

        $contextlist = new approved_contextlist($u, 'local_airpay_challenge',
            [\context_system::instance()->id]);
        provider::delete_data_for_user($contextlist);

        $this->assertFalse($DB->record_exists('local_airpay_challenge_attempts',
            ['userid' => $u->id]));
        $this->assertFalse($DB->record_exists('local_airpay_challenge_leaderboard',
            ['userid' => $u->id]));
    }
    ...
    public function test_delete_data_for_user_anonymises_authored_challenges(): void {
        ...
        $row_before = $DB->get_record('local_airpay_challenge_challenges', ['id' => $cid]);
        ...
        $row_after = $DB->get_record('local_airpay_challenge_challenges', ['id' => $cid]);
