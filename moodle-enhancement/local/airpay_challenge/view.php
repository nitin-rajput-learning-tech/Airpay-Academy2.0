...
$challenge->participants = (int) $DB->count_records(
    'local_airpay_challenge_attempts', ['challengeid' => $id]);
$challenge->completed = (int) $DB->count_records(
    'local_airpay_challenge_attempts',
    ['challengeid' => $id, 'status' => \local_airpay_challenge\challenge_engine::ATTEMPT_COMPLETED]);

$myattempt = $DB->get_record('local_airpay_challenge_attempts',
    ['challengeid' => $id, 'userid' => $USER->id]);
$row = \local_airpay_challenge\challenge_engine::format_challenge_row($challenge, $myattempt ?: null);

$baseurl = new moodle_url('/local/airpay_challenge/view.php', ['id' => $id]);
$PAGE->set_url($baseurl);
$PAGE->navbar->add(get_string('heading_index', 'local_airpay_challenge'),
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_challenge/view', $data);
