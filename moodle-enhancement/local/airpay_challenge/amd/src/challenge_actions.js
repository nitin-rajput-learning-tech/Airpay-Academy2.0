...
// @module     local_airpay_challenge/challenge_actions
...
const openChallengeForm = async (challengeid, returnFocus) => {
    ...
    const title = await getString(titleKey, 'local_airpay_challenge');
    const modalForm = new ModalForm({
        formClass: 'local_airpay_challenge\\form\\edit_challenge_dynamic_form',
...
const join = async (challengeid, returnFocus) => {
    const success = await getString('joined_challenge', 'local_airpay_challenge');
    Ajax.call([{
        methodname: 'local_airpay_challenge_join_challenge',
        args: {challengeid: challengeid},
    }])[0].then(() => {
...
const leave = async (challengeid, returnFocus) => {
    const [title, message, ok, success] = await Promise.all([
        getString('btn_leave', 'local_airpay_challenge'),
        getString('btn_leave', 'local_airpay_challenge'),
        getString('continue', 'core'),
        getString('left_challenge', 'local_airpay_challenge'),
    ]);
    Notification.confirm(title, message + '?', ok, null, () => {
        Ajax.call([{
            methodname: 'local_airpay_challenge_leave_challenge',
            args: {challengeid: challengeid},
        }])[0].then(() => {
            Notification.addNotification({message: success, type: 'success'});
            window.location.reload();
...
const confirmDelete = async (challengeid, name, returnFocus) => {
    const [title, message, label, success] = await Promise.all([
        getString('btn_delete', 'local_airpay_challenge'),
        getString('challenge_deleted', 'local_airpay_challenge', name),
        getString('delete', 'core'),
        getString('challenge_deleted', 'local_airpay_challenge', name),
    ]);
    Notification.deleteCancelPromise(title, message, label, returnFocus).then(() => {
        Ajax.call([{
            methodname: 'local_airpay_challenge_delete_challenge',
            args: {id: challengeid},
        }])[0].then(() => {
            Notification.addNotification({message: success, type: 'success'});
            window.location.reload();
