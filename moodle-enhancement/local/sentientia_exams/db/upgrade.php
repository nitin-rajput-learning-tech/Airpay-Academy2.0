...
function xmldb_local_sentientia_exams_upgrade(int $oldversion): bool {
    global $DB;
    $dbman = $DB->get_manager();

    // 2026051901 — P1 #23: exam categories.
    //
    // Adds `categoryid` to `local_sentientia_exams` so admins can tag exams
    ...
    if ($oldversion < 2026051901) {
        $table = new xmldb_table('local_sentientia_exams');
        $field = new xmldb_field('categoryid', XMLDB_TYPE_INTEGER, '10',
            null, XMLDB_NOTNULL, null, '0', 'departmentid');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2026051901, 'local', 'sentientia_exams');
    }
    ...
    if ($oldversion < 2026052001) {
        $table = new xmldb_table('local_sentientia_exams_remind_sent');
        ...
        upgrade_plugin_savepoint(true, 2026052001, 'local', 'sentientia_exams');
    }
