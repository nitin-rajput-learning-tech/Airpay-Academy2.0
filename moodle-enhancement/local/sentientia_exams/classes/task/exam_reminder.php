...
namespace local_sentientia_exams\task;
...
class exam_reminder extends \core\task\scheduled_task {
    ...
    public function get_name(): string {
        return get_string('task_exam_reminder', 'local_sentientia_exams');
    }

    public function execute(): void {
        global $DB;

        if (!(int) get_config('local_sentientia_exams', 'reminder_enabled')) {
            mtrace('local_sentientia_exams exam_reminder: disabled '
                . '(reminder_enabled = 0)');
            return;
        }

        $buckets = $this->parse_buckets((string) get_config(
            'local_sentientia_exams', 'reminder_days_before') ?: '7,3,1');
        if (empty($buckets)) {
            mtrace('local_sentientia_exams exam_reminder: no valid buckets');
            return;
        }
        sort($buckets);

        $max = max(1, min(5000, (int) get_config(
            'local_sentientia_exams', 'reminder_max_per_run') ?: 500));

        mtrace('local_sentientia_exams exam_reminder: starting. '
        ...
        set_config('reminder_last_run', $now, 'local_sentientia_exams');
        set_config('reminder_last_sent', $sent, 'local_sentientia_exams');

        mtrace('local_sentientia_exams exam_reminder: finished. '
    ...
    private function send_one_reminder(\stdClass $row, int $bucket,
        ...
        if ($DB->record_exists('local_sentientia_exams_remind_sent', [
        ...
        $a = (object) [
            ...
            'exam_url'       => (new \moodle_url('/local/sentientia_exams/view.php',
                ['id' => (int) $row->examid]))->out(false),
        ];

        $msg = new \core\message\message();
        $msg->component   = 'local_sentientia_exams';
        ...
        if (class_exists('\\local_sentientia_pwa\\notification_bridge')) {
            \local_sentientia_pwa\notification_bridge::also_push(
                ...
                get_string('reminder_push_title', 'local_sentientia_exams', $a),
                get_string('reminder_push_body',  'local_sentientia_exams', $a),
                $a->exam_url,
        ...
        try {
            $DB->insert_record('local_sentientia_exams_remind_sent', (object) [
                'userid'   => (int) $row->userid,
                'examid'   => (int) $row->examid,
                'days_before_deadline' => $bucket,
                'deadline_ts'          => $deadline_ts,
