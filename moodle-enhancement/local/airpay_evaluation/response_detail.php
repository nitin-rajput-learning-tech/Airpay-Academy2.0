...
$response = $DB->get_record('local_airpay_evaluation_responses',
    ['id' => $id], '*', MUST_EXIST);
$evaluation = $DB->get_record('local_airpay_evaluation',
    ['id' => $response->evaluationid], '*', MUST_EXIST);

$ctx = context_system::instance();
$PAGE->set_context($ctx);
$PAGE->set_url(new moodle_url('/local/airpay_evaluation/response_detail.php', ['id' => $id]));
$PAGE->set_pagelayout('admin');
$PAGE->set_title('Response detail');
$PAGE->set_heading('Response detail — ' . format_string($evaluation->name));
require_capability('local/airpay_evaluation:view', $ctx);
...
$questions = $DB->get_records('local_airpay_evaluation_questions',
    ['evaluationid' => $evaluation->id], 'sortorder ASC');

// Aggregate stats per question for comparison.
$all_responses = $DB->get_records('local_airpay_evaluation_responses',
...
$data = [
    ...
    'back_url'      => (new moodle_url('/local/airpay_evaluation/responses.php',
        ['id' => $evaluation->id]))->out(false),
    'analysis_url'  => (new moodle_url('/local/airpay_evaluation/analysis.php',
        ['id' => $evaluation->id]))->out(false),
];

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_evaluation/response_detail', $data);
echo $OUTPUT->footer();
