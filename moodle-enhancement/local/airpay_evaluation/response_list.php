...
$PAGE->set_url(new moodle_url('/local/airpay_evaluation/response_list.php',
    ['id' => $evaluationid]));
...
$rows = $DB->get_records_sql(
    "SELECT r.id, r.userid, r.courseid, r.programid, r.classroomid,
       ...
       FROM {local_airpay_evaluation_responses} r
...
foreach ($rows as $r) {
    $shape[] = [
        ...
        'detail_url'   => (new moodle_url('/local/airpay_evaluation/response_detail.php',
            ['id' => $r->id]))->out(false),
    ];
}

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_evaluation/response_list', [
    'eval_name'     => format_string($evaluation->name),
    'is_anonymous'  => $is_anonymous,
    'total'         => count($shape),
    'rows'          => $shape,
    'has_rows'      => !empty($shape),
    'aggregate_url' => (new moodle_url('/local/airpay_evaluation/responses.php',
