...
//
// Phase Z.1 (2026-05-08) — privacy provider for airpay_evaluation.

namespace local_airpay_evaluation\privacy;
...
class provider implements
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        if ($DB->record_exists('local_airpay_evaluation_responses',
                ['userid' => $userid])) {
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        $rows = $DB->get_records_sql(
          ...
          LEFT JOIN {local_airpay_evaluation} e ON e.id = r.evaluationid
              WHERE r.userid = :uid",
            ['uid' => $userid]);
        if (empty($rows)) {
            return;
        ...
        \core_privacy\local\request\writer::with_context(
            ...
            ->export_data(['airpay_evaluation_responses'],
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $userids = $DB->get_fieldset_select(
            'local_airpay_evaluation_responses', 'DISTINCT userid',
            'userid > 0');
        if (!empty($userids)) {
    ...
    public static function delete_data_for_users(
        ...
        $DB->delete_records_select('local_airpay_evaluation_responses',
