...
class list_evaluations extends external_api {
    ...
    public static function execute(string $search = '', string $sort = 'name', string $sortdir = 'asc',
        ...
        if (strlen($params['filters']) > 4096) throw new \moodle_exception('filterstoolong', 'local_airpay_evaluation');
        ...
        if (!is_siteadmin()) {
            ...
            if ($top > 0) {
                $where[] = '(e.open_path = :eorgexact OR e.open_path LIKE :eorgprefix)';
                $sqlparams['eorgexact']  = '/' . $top;
                $sqlparams['eorgprefix'] = $DB->sql_like_escape('/' . $top . '/') . '%';
            }
        }
        ...
        $total = (int) $DB->count_records_sql(
            "SELECT COUNT(*) FROM {local_airpay_evaluation} e WHERE $wheresql", $sqlparams);
