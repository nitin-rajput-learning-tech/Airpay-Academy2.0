...
class expire_assignments extends \core\task\scheduled_task {
    ...
    public function get_name(): string {
        return get_string('task_expire_assignments', 'local_airpay_evaluation');
    }
    ...
    public function execute(): void {
        ...
        $sql = "UPDATE {local_airpay_evaluation_assign}
                   SET status = 'expired',
                       timemodified = :nowmod
        ...
        $count = $DB->count_records_sql(
            "SELECT COUNT(1) FROM {local_airpay_evaluation_assign}
              WHERE status = 'assigned'
                AND due_at IS NOT NULL
                AND due_at > 0
        ...
        mtrace('local_airpay_evaluation expire_assignments: '
