 ...
 *
 * URL: /local/airpay_emails/editor.php?template=compliance/deadline_warning&tenant=1
 *
...
if (!is_siteadmin() && !has_capability('local/airpay_emails:manage_templates', $context)) {
...
if (empty($templatekey)) {
    redirect(new moodle_url('/local/airpay_emails/manage.php', ['tab' => 'templates']));
}

$PAGE->set_url(new moodle_url('/local/airpay_emails/editor.php',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && confirm_sesskey()) {
    ...
    \local_airpay_emails\template_manager::save_override($templatekey, $tenantid, $subject, $bodyhtml);
    redirect(new moodle_url('/local/airpay_emails/editor.php',
...
if (optional_param('revert', 0, PARAM_INT) && confirm_sesskey()) {
    global $DB;
    $DB->delete_records('local_airpay_email_overrides', [
...
$override = \local_airpay_emails\template_manager::get_override($templatekey, $tenantid);
...
if ($override && !empty($override->body_html)) {
    ...
    $filepath = $CFG->dirroot . '/local/airpay_emails/templates/' . $templatekey . '.mustache';
...
$label = $templatekey;
$category = '';
foreach (\local_airpay_emails\email_renderer::get_template_list() as $cat) {
...
$smartblocks = [
    ...
    ['id' => 'course_card', 'label' => 'Course Card', 'icon' => 'fa-book',
     'html' => '{{> local_airpay_emails/partials/course_info_box}}'],
    ...
    ['id' => 'footer_note', 'label' => 'Support Footer', 'icon' => 'fa-life-ring',
     'html' => '{{> local_airpay_emails/partials/footer_note}}'],
...
foreach ([0, 1, 77, 177] as $tid) {
    $ov = \local_airpay_emails\template_manager::get_override($templatekey, $tid);
    if ($ov) {
...
$editordata = [
    ...
    'sesskey'          => sesskey(),
    'save_url'         => (new moodle_url('/local/airpay_emails/editor.php',
                            ['template' => $templatekey, 'tenant' => $tenantid]))->out(false),
    'back_url'         => (new moodle_url('/local/airpay_emails/manage.php', ['tab' => 'templates']))->out(false),
    'preview_url'      => (new moodle_url('/local/airpay_emails/preview.php',
                            ['template' => $templatekey, 'tenant' => $tenantid]))->out(false),
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_airpay_emails/manage/editor', $editordata);
echo $OUTPUT->footer();
