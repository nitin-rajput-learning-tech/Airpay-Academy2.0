...
namespace local_airpay_emails\external;
...
class template_api extends external_api {
    ...
    public static function get_template(string $templatekey, int $tenantid = 0): array {
        ...
        $override = \local_airpay_emails\template_manager::get_override($params['templatekey'], $params['tenantid']);
        ...
        if ($override) {
            ...
            $samplecontext = \local_airpay_emails\email_context::get_sample($params['templatekey']);
            $data = [
            ...
            $filepath = $CFG->dirroot . '/local/airpay_emails/templates/' . $params['templatekey'] . '.mustache';
    ...
    public static function save_template(string $templatekey, int $tenantid, string $subject, string $bodyhtml): array {
        ...
        if (!is_siteadmin()) {
            self::validate_context($context);
            require_capability('local/airpay_emails:manage_templates', $context);
        }

        $id = \local_airpay_emails\template_manager::save_override(
    ...
    public static function preview_template(string $templatekey, int $tenantid = 1,
        ...
        if (!empty($params['bodyhtml'])) {
            ...
            $fullname = 'local_airpay_emails/' . $params['templatekey'];
            $body = \local_airpay_emails\email_renderer::render($fullname, $samplecontext);
        }
