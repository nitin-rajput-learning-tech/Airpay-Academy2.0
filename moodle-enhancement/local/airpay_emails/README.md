...
- **`certificate_helper`** wraps `tool_certificate\template::get_issue_file()`
  and materialises the PDF into `$CFG->tempdir/airpay_emails/` for
...
- **`course_incomplete` rule type** in `process_rules` — ramping
  ...
  the user finishes.
- **Schema additions** to `local_airpay_email_log`:
  `attachment_filename` (char 255 nullable), `certificate_issue_id`
  (int 10 nullable). To `local_airpay_email_rules`:
...
| Table | Notes |
|-------|-------|
| `local_airpay_email_log` | + Sprint B: `attachment_filename`, `certificate_issue_id`, `status` widened to char(32) |
| `local_airpay_email_rules` | + Sprint B: `cadence_days_json`, `max_reminders_per_user`, `auto_stop_on_completion` |
| `local_airpay_email_overrides` | per-tenant template overrides |
| `local_airpay_email_prefs` | per-user opt-out per rule type |
