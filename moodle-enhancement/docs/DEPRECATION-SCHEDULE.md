...
| 7 | `$USER->open_path` tenant identifier (24+ files) | Hard BizLMS tenant coupling | `local_sentientia_core\tenant_identity` seam — **scaffold SHIPPED (ADR-019, default-ON legacy flag)**; ~24 callers not yet migrated | 2 | 🟢 seam shipped · 🟠 caller-migration needs-human |
| 8 | `local_costcenter` org hierarchy + `open_supervisorid` | BizLMS org/manager model | `local_sentientia_core\org` seam — **manager accessor SHIPPED (ADR-020 W3.1)**; full org model + ZEEA-first migration is W3.2+ | 3 | 🟢 manager seam shipped · 🟠 migration needs-human |
