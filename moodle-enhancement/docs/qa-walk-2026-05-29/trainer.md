...
### T-05 — trainer redirect never fires (RESOLVED — by-design, 2026-05-29)
Root cause confirmed (same shape as T-01): the redirect gate `block/airpay_trainer:viewtrainerslist` (`core_renderer.php:1254-1265`) grants `editingteacher`+`manager`, not `teacher`; the BizLMS fallback cap `block/trainerdashboard:viewtrainerslist` isn't registered locally. **Decision (owner delegated the call): option (a) — by-design, no code change.** With T-01/T-02, trainers now land on the rich `/my/` shell + Live Sessions link; granting the cap would force-redirect them to the minimal classroom-sessions block page — a regression. The redirect remains live for editingteacher/manager (unchanged prod behavior), and T-04 ensures that page no longer fatals. Considered but rejected: (b) grant the `teacher` archetype the cap; (c) retarget the redirect to Sentientia Live `trainer/index.php`. Revisit only if a dedicated trainer landing is later desired.
