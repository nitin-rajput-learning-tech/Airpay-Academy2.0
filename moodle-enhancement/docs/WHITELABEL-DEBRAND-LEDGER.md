...
| D3 | Support-email token | `welcome_mailer` DEFAULT_BODY `academy@airpay.co.in` | Functional contact address, not customer-name. Needs a `[support_email]` token + config for full white-label. |
