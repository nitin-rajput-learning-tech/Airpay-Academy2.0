...
- **Status:** **Proposed — awaiting Nitin's sign-off.** The eight open decisions this
  roadmap depends on are packaged for signature in
  [`docs/business/DECISION-MEMO-2026-08-04.md`](../business/DECISION-MEMO-2026-08-04.md);
  this ADR flips to Accepted when that memo comes back signed (with any amendments
  folded in here).
- **Date:** 2026-08-04
...
For (at least) the next two quarters, engineering capacity goes to **shipping what
exists, proving what is claimed, and closing last-mile stubs** — not to net-new
feature construction. The mock-mode AI/LXP cohort is *sold as demo-able roadmap*
("human-review-gated, mock-first by design" — for a BFSI buyer, AI-off-by-default is
a feature) until its promotion criteria in Phase 2/3 are met.
...
|---|---|---|---|
| 1.1 | **Ship customer-zero** | Execute rollout-gate Phase 2 (ninja rehearsal on the final 2026-08-04 package) → Nitin-gated live cutover. Drop-dead rule per Decision B: if IT's PHP 8.3/MySQL 8.4 gates don't land in ~8 weeks, cut the product layer over on 5.1.3 and take 5.2 later as a routine upgrade. | Package final: `Sentientia-LMS-5.2-Complete-Standalone-2026-08-04.zip`, SHA `a4944f11…753d` |
...
| 1.3 | **Trust evidence, not prose** | Execute the already-written VAPT plan (CERT-In-empanelled firm per Decision 6); close the two named ISO Gap controls (key management, backup procedures); run one evidenced backup-restore rehearsal **including filedir**; re-date the lapsed trust-track milestones honestly. | `docs/security/` readiness pack; known empty-filedir clone artifact |
...
| 2.3 | **One AI gateway, then ONE live feature** | Build `local_sentientia_ai`: consolidate the six duplicated `anthropic_client` classes onto the `core_ai` bridge pattern (already in `sentientia_assistant`); central key management, spend-ledger with per-customer/tenant quotas, golden-set eval harness. Close the two last-mile stubs (aiquiz→mod_quiz/question-bank publisher; authoring→course builder). Then flip **aiquiz only** live under the ADR-012 4-layer cost defence (per Decision A). |
...
| 3.3 | **PWA hardened; native parked** | Offline course-list caching, push wired to the nudge engine, install-prompt on the mobile bottom nav. Capacitor scaffold stays unstaffed until a signed customer requires store presence (Decision 5). |
...
5. **Feature freeze scope:** the freeze applies to *net-new construction*; it does
   NOT block Phase 1/2 items that are configuration, evidence, last-mile closure, or
   promotion of existing scaffolds.

## 4. Alternatives considered

- **Build-out-the-gap-cohort-first** (the implicit GAP-ANALYSIS §8 path): rejected —
  it deepens the dark-capability paradox; nothing in the flag-off LXP layer wins a
  BFSI deal while the trust layer loses it, and AI demos that dead-end at stubs are
  worse than no AI demo.
...
- **SOC 2 first:** rejected for the beachhead — CERT-In VAPT + ISO 27001 track wins
  India BFSI faster and cheaper; SOC 2 starts when a global prospect requires it.
...
**Negative / accepted costs:** one quarter of visible feature-velocity slowdown
(the freeze); VAPT + cert spend must actually be funded (memo Q3) or Phase 1 loses
its teeth; single-tenant-managed increases per-customer ops load until the control
plane matures — accepted as the price of credible data-residency answers.
