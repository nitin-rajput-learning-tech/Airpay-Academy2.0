...
**Status:** **Accepted — executing** (2026-06-01, Nitin's "do not defer anything" go). Wave 4.1 (seam + schema + seed/parity CLIs + `:managetenants` admin UI) BUILT additive + default-legacy + locally rehearsed (PHPUnit). **2026-08-20: Gate B EXECUTED on local dev** — registry seeded (100% parity vs the allow-list), `tenant_registry_legacy` flipped OFF locally, and `customer::current()` de-hardwired through `tenant_registry::customer_of()` (platform 2026082000, ADR-028 Phase 2.1): a second customer ("[DEMO] Meridian Financial Services", tenant root 500) resolves end-to-end incl. per-customer feature flags. Evidence: docs/visual-evidence/2026-08-20/. The live cutover (flipping `tenant_registry_legacy` OFF on production) remains gated on Nitin's deploy + clone-DB rehearsal. · **Owner:** Nitin Rajput
