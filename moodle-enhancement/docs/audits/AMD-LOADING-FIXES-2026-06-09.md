...
| **§2** AMD `define()` rename | **✅ APPLIED (deploy overlay)** | The stale `theme_airpayux/*` define names live in **built** `amd/build/*.js`. Fixed durably in `overlay-airpay-customs.ps1` via a new `Repair-AmdModuleNames` step (runs right after the theme `Copy-Tree`): idempotent `theme_airpayux`→`theme_sentientia` rewrite over `theme/sentientia/amd/build/**/*.js` (UTF-8 no-BOM) with a **fail-loud post-condition gate** that aborts the deploy if any token survives. Scope is limited to the theme build dir, so it cannot touch the ADR-025-excluded `airpay_ratings`/`paygw_airpay`. Verified on a scratch deploy seeded from git's branded `theme/airpayux` build: **25/25 rewritten, 0 survivors, no-op when already clean, gate proven to fire**. Theme-side sibling of ADR-025 follow-up (c). |
