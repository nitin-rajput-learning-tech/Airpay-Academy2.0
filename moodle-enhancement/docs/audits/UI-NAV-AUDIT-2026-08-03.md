...
| **N-05** | **P2** | **Logout confirm is a UX hazard** — on `/login/logout.php` the first `.btn-primary` is NOT the logout action; a scripted "click the primary" stayed logged in. Users pressing the visually-primary button don't log out | reproduced in walk (needed `?sesskey=` to actually log out) |
...
| **N-07** | **P2** | **Trainer sees "Compliance"** in the sidebar — likely nav leakage vs the compliance-officer surface (trainer roles: employee+trainer). Needs a capability check on the link's gate | qa_trainer sidebar: My Team, Compliance, Live Sessions… |
