...
| Phase 1 tenant isolation | PASS on the learner-facing surface: ZEEA learner catalog leaks no Airpay courses; Airpay/ZEEA admin course+user lists are AJAX datatables, DB-level scoping correct (Airpay admin 6 courses/8 users, ZEEA admin 4/2). Datatable rows are WS-scoped, not exercised over HTTP here - spot-check in Nitin's browser pass |
| Phase 1 accessibility quick pass | Guest + persona pages: lang correct (en/hi), skip-link present, all images have alt, headings present, no empty unlabelled buttons. One real WCAG 4.1.2 gap: the global search box had a placeholder but no label in three render paths (dashboard.mustache, topbar.mustache, and the PHP-built shell topbar in core_renderer.php) - all three now carry role=search + aria-label; re-scan shows 0 unlabelled inputs on dashboard/course/catalog |
...
| 2.1/2.5/2.6 re-confirm (post-deploy) | 0 failing scheduled tasks, adhoc queue 0, cron current; OPcache **On** in FPM; Apache log clean. Note: PHP-FPM logged one `pm.max_children=5` warning under the earlier browser walk (capacity → resize + raise for Stage B). Schema check flags F-8 tables (`local_sentientia_user_type`, `..._partner_employee_profile`) as "not expected" — pre-existing/by-design (created via `install.php` not XMLDB). **Verified Stage-B-safe (2026-09-07):** created on BOTH the fresh path (`install.php` → `user_type_tables::ensure()`) AND the upgrade path (`upgrade.php` `create_table` guarded by `!table_exists`, gated `<2026052801`), so the live→5.2 migration creates them either way; the "not expected" note is cosmetic (deliberate programmatic schema per the code comment). No `install.xml` change needed. Scout also confirms `sentientia_platform` is the ONLY plugin using the programmatic-`install.php` pattern, and UAT's clean fresh 5.2 install (schema check shows extras, zero "missing") already validates the fresh-install path for all plugins. |
