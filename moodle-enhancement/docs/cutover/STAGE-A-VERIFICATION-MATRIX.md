...
| F-10 | Course page 500 for any learner who completed a certificate course: `theme/sentientia core_renderer::course_context_header_settings_menu()` queried `tool_certificate_issues.moduleid/moduletype` — columns that exist only in production's BizLMS-customised certificate tool (11,415 rows keyed that way, `courseid` empty); the stock tool in the 5.2 package has `courseid/component` only | Blocker (5.2 package) → fixed | schema-aware lookup: BizLMS columns when present (production byte-identical), else stock `courseid` + `archived = 0`; found by the persona link gate |
