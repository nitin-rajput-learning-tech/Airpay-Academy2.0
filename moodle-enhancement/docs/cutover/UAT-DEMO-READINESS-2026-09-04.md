...
| F-12 residue (dashboard) | On-screen Hindi re-check 2026-09-08 showed the admin dashboard's *Top Courses* widget rendering "AML **&amp;** KYC"; audit found the same `format_string()`-through-`{{ }}` pattern in the team member name, achievement title, recommendation title, learner course-progress card, deadline tile and course-editing sidebar names, plus ~20 template-level English fragments the P3 pass had left | `{{{ }}}` on every format_string/get_string slot; fragments routed through 20 new en+hi strings. Theme 2026090801, **deployed 2026-09-09 and confirmed on screen** (Top Courses and course cards render "&"; streak/rank/completion strings in Hindi — `docs/visual-evidence/2026-09-09/uat-hindi-cascade-recheck/`). The re-check found the last layer — 9 literal section headings, the PHP-built Recent Activity lines, the recommendation card's category/summary slots and the login page copy — fixed as theme **2026090803** (26 more en+hi strings), deploy pending |
| Org-cascade filter i18n | The 5-level Organisation → Department → … filter on Manage Users / Manage Courses stayed English under Hindi (labels, "All …" defaults, and the AMD module's rebuilt options) | `local_sentientia_org` `cascade_*` strings (en+hi, both trees; ME copy 1.4.2 is what UAT runs), theme component + both plugin templates via `{{#str}}`, `data-cascade-all-label` read by the rebuilt `org_cascade` bundle. **Deployed 2026-09-09, confirmed on screen incl. a live cascade rebuild in Hindi** |
