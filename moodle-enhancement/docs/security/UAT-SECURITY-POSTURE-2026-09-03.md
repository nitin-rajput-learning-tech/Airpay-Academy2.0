...
| H1 signup enumeration | **FIXED** - `signup_service::validate()` no longer reports email/username collisions; `register()` returns the existing account's id and (mail permitting) notifies that address, same response shape as a fresh signup; format/policy errors unchanged; tests updated | `local_sentientia_users` 2026090302 |
| H3 LRS rate limiting | **FIXED** - per-client fixed window (`lrs_rate_limit` 600 / `lrs_rate_window` 60 s, admin-configurable), new table `local_sentientia_xapi_lrs_rate` (install.xml + upgrade), site-wide credentials metered via sentinel ids, 429 + Retry-After before any parsing; live check on UAT: 401 / 200 x3 / 429 with a limit of 3 | `local_sentientia_xapi` 2026090302 |
| H4 SSE concurrency | **FIXED** (2026-09-04) - `sse_connection_registry` (new table `local_sentientia_live_sse`, install.xml + upgrade): global cap `sse_max_connections` (default 8) + per-actor cap 2 (trainer userid / participant id / IP fallback), stream lifetime `sse_max_seconds` (default 60, was 300 hard-coded), 503 + Retry-After 5 before any SSE byte, release on shutdown, heartbeat prune; trainer AMD gains a reload-on-fatal retry; 137/137 tests; live check on UAT: 2nd stream refused at cap 1. Sizing note: UAT runs PHP-FPM with `pm.max_children = 5`, so the cap is 3 there and the shipped default is 4 - keep it below the FPM pool. Verification also exposed that `mod_proxy_fcgi` buffered the whole stream until exit; vhost fixed with `ProxySet flushpackets=on` (see deploy checklist) | `local_sentientia_live` 2026090302 |
| M1 KeKa employee id in URL path | **FIXED** (2026-09-04) - `rawurlencode()` before the path segment | `local_sentientia_integrations` |
| M2 landing-page site-wide fallback | **CLARIFIED** - the fallback only runs on a vanilla single-tenant schema where site-wide == the only tenant; comment corrected, behaviour unchanged | `theme_sentientia` |
| M3 fingerprint files served | **FIXED** (2026-09-04) - `deploy/moodle-htaccess.template` denies `upgrade.txt`, `environment.xml`, `UPGRADING.md`, `CHANGES.md`, composer/package manifests; UAT: both now 404 | htaccess |
| M4 missing security headers | **FIXED** (2026-09-04) - `mod_headers` enabled on UAT; template sets `X-Content-Type-Options: nosniff`, `Referrer-Policy`, `Permissions-Policy`, and HSTS (1 y, includeSubDomains) conditioned on the forwarded-https flag; CSP deliberately deferred (needs a nonce strategy for Moodle's inline scripts) | htaccess |
| M5 gateway landing without session binding | Open - rides on C1 (fail-closed verifier branch, Nitin-gated) | `paygw_airpay` |
| H2 CAPTCHA | Open; closes with the IT reCAPTCHA keys | config |
