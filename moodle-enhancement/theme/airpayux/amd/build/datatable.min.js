...
define("theme_airpayux/datatable", [
    ...
    Datatable.prototype.renderHead = function() {
        ...
        head.innerHTML = this.columns.map(function(c) {
    ...
    Datatable.prototype.renderBody = function() {
        ...
        if (this.state.rows.length === 0) {
            var span = this.columns.length + (this.actionsHtml ? 1 : 0);
            var msg = this.state.search
                ? 'No results for "' + escapeHtml(this.state.search) + '"'
                : "No records found";
            body.innerHTML = '<tr><td colspan="' + span + '" class="text-center py-5 text-muted">'
        ...
        body.innerHTML = this.state.rows.map(function(row) {
            ...
            return '<tr data-row-id="' + escapeHtml(row.id != null ? row.id : "") + '">' + cells + actionCell + '</tr>';
        }).join("");
    };
