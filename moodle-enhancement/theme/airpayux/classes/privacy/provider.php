...
/**
 * Privacy Subsystem implementation for theme_airpayux.
 *
...
namespace theme_airpayux\privacy;
...
/**
 * The airpayux theme stores a user preference data.
 *
 * @copyright  2018 Andrew Nicols <andrew@nicols.co.uk>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class provider implements
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if (isset($draweropennavpref)) {
            $preferencestring = get_string('privacy:drawernavclosed', 'theme_airpayux');
            if ($draweropennavpref == 'true') {
                $preferencestring = get_string('privacy:drawernavopen', 'theme_airpayux');
            }
        ...
        if (isset($draweropenindexpref)) {
            $preferencestring = get_string('privacy:drawerindexclosed', 'theme_airpayux');
            if ($draweropenindexpref == 1) {
                $preferencestring = get_string('privacy:drawerindexopen', 'theme_airpayux');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_airpayux',
        ...
        if (isset($draweropenblockpref)) {
            $preferencestring = get_string('privacy:drawerblockclosed', 'theme_airpayux');
            if ($draweropenblockpref == 1) {
                $preferencestring = get_string('privacy:drawerblockopen', 'theme_airpayux');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_airpayux',
