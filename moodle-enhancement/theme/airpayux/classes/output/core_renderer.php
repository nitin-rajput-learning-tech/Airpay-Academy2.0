...
JS;
    ...
    public function custom_language_menu(){
        ...
        if(count($langs) > 1){
            ...
            return $this->render_from_template('theme_airpayux/language_menu_dropdown', $select);
        }
    }
    // context_header() and render_context_header() moved to
    // \theme_airpayux\output\traits\context_header in Engineering 30.
    ...
    public function left_navigation_quick_links(){
        ...
        $block_content .= html_writer::start_tag('ul', array('class'=>'pull-left row-fluid user_navigation_ul'));
            ...
            $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_dashboard', 'class'=>'pull-left user_nav_div dashboard'));
                $button1 = html_writer::link($CFG->wwwroot.'/my/dashboard.php', '<i class="fa fa-home" aria-hidden="true"></i><span class="user_navigation_link_text">'.get_string('leftmenu_dashboard', 'theme_airpayux').'</span>', array('class'=>'user_navigation_link'));
...
//                 $gamification = html_writer::link($gamification_url, '<i class="fa fa-trophy"></i><span class="user_navigation_link_text">'.get_string('leftmenu_gmleaderboard','theme_airpayux').'</span>',array('class'=>'user_navigation_link'));
...
//             }
            ...
            if(is_siteadmin()){
                $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_adminstration', 'class'=>'pull-left user_nav_div adminstration'));
                    ...
                    $admin = html_writer::link($admin_url, '<i class="fa fa-cogs"></i><span class="user_navigation_link_text">'.get_string('leftmenu_adminstration','theme_airpayux').'</span>',array('class'=>'user_navigation_link'));
                    $block_content .= $admin;
                $block_content .= html_writer::end_tag('li');
            }
    ...
    */
    ...
    public function full_header() {
        ...
        return $this->render_from_template($show_course_header? 'theme_airpayux/course_full_header' : 'theme_airpayux/full_header', $header);
    }
        /**
...
//            if(is_null($userrole) || $userrole == 'user'){
        ...
        return $this->render_from_template('theme_airpayux/course_context_header', $course_context);
    }
    function theme_airpayux_user_get_user_navigation_info($user, $page, $options = array()) {
    ...
    // user_menu() moved to theme_airpayux.output.traits.user_menu in Engineering 32.
    ...
    public function quickaccess_links() {
        ...
        $block_content .= html_writer::start_tag('ul', array('class'=>'quickpop_over_ul'));
            //======= Dasboard link ========//
            // $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_dashboard', 'class'=>'pull-left user_nav_div dashboard'));
            //     $button1 = html_writer::link($CFG->wwwroot, '<i class="fa fa-home" aria-hidden="true"></i><span class="user_navigation_link_text">'.get_string('leftmenu_dashboard', 'theme_airpayux').'</span>', array('class'=>'user_navigation_link'));
            //     $block_content .= $button1;
            // $block_content .= html_writer::end_tag('li');
...
//             }
            $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_home', 'class'=>'pull-left user_nav_div adminstration'));
                    ...
                    $admin = html_writer::link($admin_url, '<i class="fa fa-home" aria-hidden="true"></i><span class="user_navigation_link_text">'.get_string('home','theme_airpayux').'</span>',array('class'=>'user_navigation_link'));
                    $block_content .= $admin;
                $block_content .= html_writer::end_tag('li');
            $pluginnavs = array();
            foreach($local_pluginlist as $key => $local_pluginname){
            ...
            if(is_siteadmin()){
                ...
                //     $admin = html_writer::link($admin_url, '<span class="image_repository_icon dypatil_cmn_icon icon"></span><span class="user_navigation_link_text">'.get_string('repositoryfiles','theme_airpayux').'</span>',array('class'=>'user_navigation_link'));
                //     $block_content .= $admin;
                // $block_content .= html_writer::end_tag('li');
                $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_adminstration', 'class'=>'pull-left user_nav_div adminstration'));
                    $admin_url = new moodle_url('/admin/search.php');
                    $admin = html_writer::link($admin_url, '<i class="fa fa-cogs" aria-hidden="true"></i></span><span class="user_navigation_link_text">'.get_string('leftmenu_adminstration','theme_airpayux').'</span>',array('class'=>'user_navigation_link'));
                    $block_content .= $admin;
                $block_content .= html_writer::end_tag('li');
    ...
    public function courseformat_drawer_content(){
        ...
        if(empty($percentage)){
            ...
            //print
            return $this->render_from_template('theme_airpayux/core_courseformat/local/courseindex/course_drawer_header', $coursedata);
        }
    }
