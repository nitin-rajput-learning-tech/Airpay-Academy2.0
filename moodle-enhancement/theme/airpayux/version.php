...
//
// Chip P0-B (2026-05-24) — :focus-visible on _bizlms-admin.scss. Closes
// the deferred Chip H follow-up from the P1 #12 re-apply chip plus
// extends WCAG 2.4.7 coverage to interactive selectors that previously
// had only :hover. 7 new :focus-visible rule blocks (13 selectors total)
// added to _bizlms-admin.scss: 1 byte-identical sibling for the existing
// form-control :focus block (mirrors Chip H exactly), plus 6 new outline
// rules on admin nav-tabs, admin category links, admin .btn-primary,
// static-page nav link, quick-link card, and static-page TOC link.
// New rules use the WCAG 2.4.7 pattern: outline 2px solid + 2px offset +
// --ap-radius-sm. Existing :hover declarations untouched (zero non-focus
// regression). Brace balance verified (339 open == 339 close).
...
$plugin->release   = '1.0.36-beta';  // P0 #1+#2+#3+#4+#5+#6+#7+#9 + F-13 + P1 #10+#11+#12+#13+#14+#17 + P2 #18+#19+#20+#21+#23 + cart_badge audit + P1 #12 re-apply + Chip P0-B (bizlms-admin :focus-visible)
