...
## 🤖 WAVE H — WORKSTREAM G AI FEATURES, SESSION 2 (2026-05-25)

**Two parallel AI-plugin MVPs shipped, both mirroring the `local_sentientia_aiquiz` (ADR-012) scaffolding pattern: feature flags default OFF, per-call [CONFIRM] gate, deterministic mock client, per-customer daily cost cap, 100% Hindi parity, no live API calls in tests.**

### H2.1 — `local_sentientia_recommendations` + `block_sentientia_recommendations` (Phase H.0) — ✅ SHIPPED

AI course-recommendations engine. Learners see the top 3-5 personalised
course recommendations on their dashboard, generated by Anthropic Claude
from completion history + skills + role + tenant, each with a "Why this?"
rationale.

- **Plugin** `local_sentientia_recommendations` 0.1.0-alpha (2026052500),
  MATURITY_ALPHA, depends on `local_airpay_core` + `local_airpay_courses`.
- **Schema:** 1 table `local_sentientia_rec_log` (userid, courseid,
  batchid, score, rank_order, reasoning, mode, tokens, status lifecycle
  active → dismissed/enrolled/expired, customerid + costcenterid).
- **Classes:** `anthropic_client` (call_mock + call_live), `prompt_builder`
  (v1, catalogue-aware), `response_parser` (catalogue-bounded — drops
  invented course IDs), `recommendation_engine` (profile build + batch
  persist/expire + retrieve + ownership-checked status flip), privacy
  provider.
- **Block** `block_sentientia_recommendations` 0.1.0-alpha — read-only
  dashboard block; renders nothing when flag OFF / no batch / lacks :view.
- **3 flags** (all default OFF): `.enabled`, `.live_api`, `.auto_cron`.
- **3 caps:** view (learners), generate (manager), manage_all (manager).
- **Cost defence:** master flag + live-API flag + per-call [CONFIRM] +
  **per-customer** daily token cap (correct unit for a fan-out feature).
- **Tests:** 4 plugin test classes (~45 methods) + block render test, all
  mock-only. CLI smoke `cli/mock_smoke.php` PASS.
- **Hindi:** 79 keys, 100% parity (block 4 keys, 100% parity).
- **ADR-015** + state card shipped.

### H2.2 — `local_sentientia_translate` (Phase T.0) — ✅ SHIPPED

AI content-translation engine with brand-name preservation. Admin pastes
English source, picks a target language (hi/mr/kn/sw), Claude returns a
native-script translation, brand names are preserved/rendered per a
per-customer override, admin reviews a side-by-side diff before saving.

- **Plugin** `local_sentientia_translate` 0.1.0-alpha (2026052500),
  MATURITY_ALPHA, depends on `local_airpay_core`.
- **Schema:** 2 tables — `local_sentientia_tr_log` (translation lifecycle
  pending → translated → saved/discarded/failed) + `local_sentientia_tr_brand`
  (per-customer per-language brand-override map, unique-keyed).
- **Classes:** `anthropic_client`, `prompt_builder` (v1, script-aware),
  `response_parser`, `translate_engine` (orchestrate + deterministic brand
  post-process + persist), `brand_manager` (DEFAULT_PROTECTED always-on
  list + per-customer script overrides), privacy provider.
- **Brand preservation:** prompt instruction (keep verbatim) + deterministic
  whole-token, longest-first find/replace (e.g. "Airpay" → the Kannada-script
  form for `kn`). Guaranteed regardless of model output → unit-testable.
  Customer-scoped, so brand maps never leak across customers.
- **UI:** `translate.php` (form + [CONFIRM] gate + diff view + save/discard)
  and `brands.php` (per-customer brand-override manager).
- **2 flags** (default OFF): `.enabled`, `.live_api`.
- **3 caps:** translate, manage_brands, manage_all (all manager).
- **Tests:** 5 test classes (~55 methods) incl. a dedicated brand-name
  preservation suite (WITH-override substitution, WITHOUT-override verbatim,
  whole-token guard, longest-first, customer-scoped delete). CLI smoke PASS.
- **Hindi:** 99 keys, 100% parity.
- **ADR-016** + state card shipped.

### ADR numbering note
The chip brief paired these as ADR-014 (recommendations) + ADR-015
(translation). ADR-014 was already taken by real-time leaderboards earlier
in the build wave, so the pair landed as **ADR-015** (recommendations) +
**ADR-016** (translation) — content otherwise exactly as briefed.

### Verification (both plugins)
- ✅ `php -l` clean on every PHP file; `install.xml` schema-valid (xmllint).
- ✅ Mock-mode end-to-end pipelines PASS via CLI smoke (no DB, no network).
- ✅ Hindi parity verified by `array_diff_key` (all 3 lang-pack pairs).
- ✅ Feature flags default OFF; [CONFIRM] gate enforced; privacy providers declared.
- ⏳ `php admin/cli/upgrade.php` install + PHPUnit run pending on local XAMPP.
- ⏳ Live-API smoke (manual, [CONFIRM], real budget) gates promotion.
