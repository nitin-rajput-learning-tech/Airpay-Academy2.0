...
**Updated:** 2026-05-20 (afternoon) — **Wave 1 COMPLETE (10/10 P0s) + 29 Wave 2 P1 fixes shipped.** Latest commit `26fadb324` on `production` completes the course-deadline workflow: P1 #29 ships the overdue manager-escalation cron, sibling of yesterday's P1 #28 reminder. The same `_remind_sent` table now holds both pre-deadline reminders (positive buckets) AND post-deadline escalations (negative buckets) — single audit trail, no schema fork. Five batches today.
