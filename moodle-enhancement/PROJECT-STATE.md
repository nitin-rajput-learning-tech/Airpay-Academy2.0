...
(`docs/audits/SENTIENTIA-CAPABILITY-AND-GAP-AUDIT-2026-06-09.md`). **9 commits, all pushed to
...
- **Gate 0 (static template lint).** New `tools/scan_stale_theme_refs.php` (flags quoted
  ...
  tree + per-line opt-out) alongside the existing Mustache comment-leak scanner. Both wired into the
  pre-commit hook (now **14 CHECKS**) AND CI (`.github/workflows/ci.yml`). CHECK 3 refined to skip
  `tests/`+`cli/` (legitimate superglobal patterns).
...
**Owner-gated / deferred** (autonomous-safe work is exhausted — these need a decision or an unblock):
...
3. **Gate 2 (visual-diff/a11y) + Gate 3 (coverage matrix)** — designed in ADR-027, not yet built.
