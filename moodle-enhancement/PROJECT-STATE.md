...
**Updated:** 2026-09-26 (**ADR-031 cross-tenant authority deployed to UAT (735 files, upgrade OK, W2-02 PASS, WS smoke 0 errors); production read-only audit 37 confirmed — see the 2026-09-25/26 section**) **Previous:** 2026-09-25 (**Wave 2 offline: erasure rebuilt across 38 providers, compliance team scope for L2 managers, N1–N15 closed; UAT deploy pending VPN — see the 2026-09-24/25 section**) **Previous:** 2026-09-16 (**09-16 deploy screen-checked PASS 10/10; compliance report tenant-scope + Hindi fixed (1.0.2), footer private notice (theme 2026090805) — deployed 2026-09-17; see the 2026-09-16 section; previous: package refreshed + last UAT residues fixed — see the 2026-09-10 section; previous: UAT HARDENING DAY — see the 2026-09-08 section below: T-01 author fix, stale AMD bundle root cause, Hindi dashboard parity, tenant-scoped Manage Courses, team siteadmin — all deployed + on-screen PASS.**) **Previous (2026-09-03):** (**UAT STAGE A EXECUTED — Sentientia 5.2 INSTALLED + RUNNING on UAT, VERDICT PASS-WITH-NOTES (P5 runtime validation done).** Fresh CLI install of the 08-19 package on `UAT-Sentientia-LMS` (https://academy2.airpay.ninja, Moodle 5.2+ Build 20260519, 480 plugins, MySQL 8.4 / PHP 8.3) driven by Claude over VPN → `ssh uat-tunnel` (TOTP) → `ssh uat-lms`, with `tools/uat/stage-a-install.sh` (+ new `finish_install.php`). Matrix A 6/6 · B 4/4 · C 6/7 · D 4/5 · E 3/5 (the ⚠ rows need Nitin's browser: console/dark-mode/590px screenshots; E5 = DB password rotation deferred by Nitin "let's go through install first"). **Six runtime findings the P4 static pass could not see, ALL fixed same day + committed:** (F-1) MySQL 8 reserved word `stored` in `local_sentientia_xapi` install.xml → `timestored` (1.0.1); (F-2) an aborted `install_database.php` resumed via `upgrade.php` never runs the install tail (admin = literal `adminsetuppending`, `rolesactive=0`, "finish from original IP") → `finish_install.php`, auto-detected by the installer; (F-3) **product gap: the `open_*` tenant substrate was provisioned in `local_sentientia_core/db/upgrade.php` only — a fresh install has NO open_path** (analytics task `Unknown column`, landing page fallback, org/audience queries) → NEW `db/install.php` (2026090301), UAT repaired via `bootstrap_substrate.php` (55 cols), phpunit drop+init fresh-install gate re-run; (F-4) **Moodle 5.2 fresh-install defaults changed: `forcelogin=1` + `enablemyhome=0` ("Enable site home") → guests bounce `/` → login** (core `index.php`; upgraded sites are preserved by a core upgrade step so Stage B is unaffected) → installer posture step (`forcelogin=0`, `enablemyhome=1`, `frontpage=''`, `theme=sentientia`) — landing page now at `/`; (F-5) router unconfigured → rewrite block in `deploy/moodle-htaccess.template` + `$CFG->routerconfigured`, 6/6 self-tests OK; (F-6) no core `hi` pack on a fresh install (no langimport CLI in 5.x) → `tool_langimport\controller` step. Environment 64 OK/0 error; plugins 480/70 additional clean; cron in `www-data` crontab, 157 tasks 0 failing; flags: only the guidebook's one-click-enrol ON for /1 + /177. Functional: hidden course `UAT-SMOKE-01` + Page activity + `uat_smoke_learner` (onboarding + hi dashboard 200). Notes → Stage B: no HSTS at LB, `Server:` version header, no `vendor/` (dev deps only), `db_user` = rds_superuser unrotated, stray `/var/www/html/moodledata`, onboarding title hard-coded EN, UAT package = 08-19 build (ADR-030 + today's fixes need a refreshed package). Credentials (admin / LMS / learner) in the session scratchpad → Nitin's vault, never in git. Evidence: `docs/visual-evidence/2026-09-03/uat-stage-a/` + `docs/cutover/STAGE-A-VERIFICATION-MATRIX.md` §G. **Nitin's first look → 3 fixes same day (`e30f61973`, deployed UAT + local, Playwright screenshots in the evidence folder):** (a) the mobile-width login showed the stock "moodle BizLMS" mark — `pix/login_logo.png` was the fallback whenever no login logo is uploaded (production state too) → `branding_assets::loginlogo()` falls back to the brand asset; (b) `/#ap-courses` was a dead anchor when no featured courses exist → the section always renders with an empty-state card → public catalog; (c) footer "Get the mobile app" + "Data retention summary" = Moodle 5.2 fresh-install defaults (`tool_mobile/setuplink`, `tool_dataprivacy/showdataretentionsummary`) that airpay.academy has off → set on UAT + installer posture step. Guest-page browser pass: zero console errors, dark mode + 390px verified (logged-in surfaces still Nitin's pass; N-8: FA4 `fa-sun-o` toggle icon renders as a generic glyph under FA6). **UAT VALIDATION PLAN + PHASE 0 EXECUTED (loop session, same day):** plan = `docs/cutover/UAT-VALIDATION-PLAN-2026-09-03.md` (+ artifact) — Phase 0 fit-for-testing → 1 persona testing ∥ 2 ops validation → 3 integrations → 4 Stage B (Nitin-gated) → 5 sign-off. Phase 0 done: (0.1) UAT refreshed to current git (api 1.3.0 + 9 tables, core 2026090301, gamification, platform, users; old dirs in `/var/backups/sentientia-uat/`); (0.2) **`tools/uat/provision_test_users.php`** (idempotent, staged) built in-house after the research/critique workflow lost its agents to the session limit twice — provisions the fresh-install gaps, 6 roles (administrator forced to prod's id 9, trainer, sentientiaauthor, complianceofficer, NEW linemanager for approve/allocate, employee alias at system so learners reach the Sentientia caps), 10 org rows with prod ids, 3 categories, 31 mock-mode flags, certificate template, 5 skills, 14 courses each with Page + 3-question quiz (5.x mod_qbank), 10 personas (Airpay L&D admin / manager with 3 reports / 2 learners incl. Hindi+overdue+onboarding / trainer / author / compliance officer; public learner; ZEEA learner + admin), enrolments, completions via the completion API, 7 certificates, 2 learning paths, program, 2 classrooms, exam, evaluation, compliance snapshot, leaderboard, live poll; verify stage shows every persona in its intended role_detector tier and all 10 log in via the form; (0.3) **master credentials doc** `docs/cutover/uat-credentials/UAT-TEST-ACCOUNTS.filled.md` + `.docx` (gitignored) with 12 guided scripts + cross-persona scenario + issue template — handed to Nitin; (0.4) DB hygiene: app-scoped `sentientia_app` user in `config.php`, `db_user` rotated (values in the session scratchpad → vault). **New findings fixed same day:** F-7 `tool_certificate` fatals on 5.2 (`reset_caches(): void`, vendor patch + core-mod record); F-8 platform user-type tables + org branding columns were upgrade-only (`sentientia_platform/db/install.php` + schema class; 9 fields into `sentientia_org/db/install.xml`); F-9 prod custom roles created by no plugin (seed recreates; first-party role installer = follow-up). **Security posture (Phase 2.4, `docs/security/UAT-SECURITY-POSTURE-2026-09-03.md`): 2 critical — C1 forgeable gateway payment hash (the June fix branch `fix/airpay-payment-verification` is NOT merged), C2 cross-tenant self-enrol via the commerce path — **C2 FIXED** (catalog tenant gate `assert_course_visible_to_viewer`, 18/18 tests, regression on UAT), **H1 FIXED** (signup enumeration, users 2.7.6), **H3 FIXED** (xAPI LRS per-client rate limiter with its own table, live 429 check on UAT, xapi 1.0.2); **H4 FIXED 2026-09-04** (SSE connection registry: global/per-actor caps, 60 s lifetime, 503 + Retry-After; live 0.2.3-alpha; 137/137 tests; live check on UAT); open: C1 (Nitin-gated merge), H2 (reCAPTCHA keys from IT), 5 mediums; UAT verdict CONDITIONAL PASS, production BLOCK until closed.** Phase 2 ops (same day): link gate over ~170 dashboard links found + fixed F-10 (course page 500 for learners with a certificate — BizLMS-only `moduleid/moduletype` columns on the stock tool_certificate; now schema-aware); backup/restore drill RTO 44 s at UAT volume; 20-user load baseline 0 errors, p95 ≤ 1 s, memory flat; daily log-scan cron on UAT; tasks healthy. Ask drafts for Amul/Cloud.in + IT: `docs/cutover/UAT-ASKS-2026-09-03.md`. Stage B stays Nitin-gated.) Prior 2026-09-02 (**ADR-030 WAVE C SHIPPED — SCIM Groups + provisioning attestation; ADR-030 (Phase 2.4) is now COMPLETE across all three waves, everything flag-OFF.** `local_sentientia_api` 1.3.0 (2026090200): `/Groups` mirrors the organisation tree (id/displayName/externalId=shortname, members = users placed at the org path), structure read-only via SCIM (501) with writable membership PATCH that moves users between orgs inside the client's tenant (add → org, remove → tenant root), `/ResourceTypes`+`/Schemas` advertise Group; new attestation table `scimevt` records created/reactivated/deactivated/updated/moved per client+user+externalId on every provisioning change, surfaced on `scim.php` with a sesskey'd CSV export — the deprovisioning evidence ADR-028 named; pruned by cleanup, privacy-declared; 24 en+hi pairs (parity 0 fail). Gates: lint 12/12, install.xml well-formed (9 tables) + XMLDB generator OK for all, upgrade clean, schema 0 discrepancies, endpoints smoke OK. Tests: `scim_groups_test` 5 cases; full plugin suite re-run on a fresh phpunit DB (result recorded in the state card). Phase 2.4 engineering therefore done; ADR-030 status → Implemented. Stage A UAT still blocked solely on the RDS credential; IT Claude-allowlist pending.) Prior same day (**`sentientia_gamification` vanilla-schema crash FIXED — `badge_manager`, 2026090200 / 1.0.2-beta.** The "pre-existing notice" logged on 08-29 root-caused: `check_badges()` selected the BizLMS-injected `{user}.open_path` for the seeded `compliance_complete` / `leaderboard_top10` badges; on a schema without that column (PHPUnit DB, any non-BizLMS Customer-N) the `dml_read_exception` escaped the `course_completed` observer, `core\event\manager` swallowed it into `debugging()` ("Unexpected debugging() call detected" in `local_sentientia_api` webhooks_test) and the award chain aborted after the first 100 points — no badge check, no `first_course` bonus. Fix = schema probe via DDL `field_exists()` + site-wide fallback; SQL byte-identical when the columns exist. First plugin PHPUnit (`tests/badge_manager_test.php`, **2 tests / 9 assertions green**; the vanilla path is exercised by dropping the column for the test's duration because `bizlms_fixture` DDL survives `resetAfterTest()`); state card refreshed (was still titled `local_airpay_gamification`); both duplicate trees updated. **Same defect class found, NOT fixed here (task chips spawned):** `local_sentientia_evaluation\observer::course_completed` debugs on a vanilla schema; legacy `gamification/classes/leaderboard.php` reads `open_path` unguarded. The "1 PHPUnit deprecation" seen in every run = PHPUnit 11.5 flagging `@covers` doc-comment metadata (repo-wide; the new test uses `#[CoversClass]`). Harness lesson: two sessions ran `init.php` concurrently on the shared `phpu_` DB → mid-install "Table context does not exist"; Moodle's lock serialises test RUNS only — check for a running `init.php` / `util.php --install` before re-initing. Nothing deployed to live; XAMPP gamification `version.php` intentionally left at 2026080401 (a bump would invalidate the shared phpunit env + force a site upgrade) — run `admin/cli/upgrade.php` when convenient.) Prior 2026-08-29 (**ADR-030 WAVE B SHIPPED — SCIM 2.0 Users provisioning in `local_sentientia_api` 1.2.0 (2026082900), flag-OFF.** Raw-HTTP endpoint `scim/v2.php` (xAPI-LRS bootstrap pattern — core WS can't emit real status codes) driving a transport-neutral `scim\handler` (gate: bearer client → `sentientia.api.enabled`+`sentientia.api.scim.enabled` per (customer, tenant) → per-client rate window → route). Users: list (filter userName/externalId/id/emails.value `eq`, 1-based paging), POST 201 (+Location/ETag), GET, PUT, PATCH (Entra-style ops incl. `active`, name, emails, userName; unknown paths ignored), DELETE = soft deactivate (suspend + destroy sessions + unmap); re-POST of a deactivated externalId reactivates (200); collisions 409 `uniqueness`; discovery docs served. Clients table `scimcli` (sha256 token, tenant root, auth plugin for created users, self-contained rate counter) + mapping `scimmap` (per-client, 191-char, unique); admin page `scim.php` (cap `:scim_manage`) issues/rotates tokens shown once. Writes via `user_manager` (open_path=/root only when the BizLMS column exists; vanilla stays clean); tenant scoping rebuilt without `$USER`. Privacy + en/hi (40 pairs, parity 0 fail). **PHPUnit: scim 16/16 (76 asserts); full plugin suite 55/55 (164 asserts) on a FRESH phpunit DB** after two test-hygiene fixes: (a) the June v1 WS tests assumed BizLMS `open_path` columns and only passed when an earlier test had applied `bizlms_fixture` DDL (persists until re-init) → now `use` the fixture trait themselves; (b) flag-static leak → invalidate after `set()` and in tearDown. Gotcha: `bizlms_fixture` is a TRAIT with a protected instance method — `class_exists()` is false for traits, so a static-call guard silently skips. Build lessons logged in the state card: XMLDB rejects `fk_user` + separate `idx_user` on one field; CHAR NOT NULL must not carry `DEFAULT=""`; PHP 8 nested ternary needs parens; a killed phpunit init wedges the `phpu_` tables ("Can not use database for testing") → drop `phpu_%` + re-init. Side fix `sentientia_users` 2.7.5 (`destroy_user_sessions`). Local endpoint verified 401 via PATH_INFO and `?path=`. Next: Wave C (Groups + attestation report).) Prior same day (**ADR-030 WAVE A SHIPPED — outbound webhooks in `local_sentientia_api` 1.1.0 (2026082800), flag-OFF.** Per-subscription HMAC-SHA256 secrets (`X-Sentientia-Signature: t=<ts>,v1=<hmac("t.body")>`, 5-min replay window), delivery queue with backoff 60s→300s→1800s→7200s → dead after 5, every-minute `webhook_drain` task registered DISABLED (triple opt-in), observers on course_completed / user_enrolment_created / certificate_issued (fail-safe, id-only payloads), admin page `webhooks.php` (cap `:webhooks_manage`), SSRF guard (https-only + `curl_security_helper` at save + `\core\http_client` policy at send, redirects off), flag-OFF tenant or disabled sub ⇒ dead-letter without sending; privacy provider extended; 55 en+hi pairs (parity 0 fail). **PHPUnit 15/15, 57 assertions.** Fresh-install lesson: an `install.xml` COMMENT with a raw `<` silently broke XMLDB parsing (all plugin tables "not expected" in `check_database_schema.php`; phpunit init failed) while the local `upgrade.php` path masked it — phpunit init IS the fresh-install gate, run it after every schema change. Pre-existing notice surfaced in `sentientia_gamification` badge_manager on course_completed (task chip spawned). Design: ADR-030 (3 waves; B = SCIM Users router on the xAPI-LRS raw-HTTP pattern since core WS can't return real status codes; C = Groups + attestation). **UAT Stage A still blocked solely on the RDS credential** (emailed db_user password dead-on-arrival, reset requested via Ganesh/Cloud.in); VPN egress `114.143.225.62` now allowlisted on the jump SG and the tunnel + key access verified; **operational constraint discovered: the full-tunnel corporate VPN blocks Claude for non-approved users → IT allowlist requested; interim = batch-mode install (prep off-VPN, one command on-VPN, review off-VPN).** Server prep completed by us over SSH: PHP 8.3 extensions installed + ini, Apache directory listing off, LMS password rotated.) Prior 2026-08-28 (**UAT ACCESS SAGA + approval-annex batch (loop session).** Access: domain `academy2.airpay.ninja` LIVE + verified (ACM cert, LB public, HTTP→HTTPS); Ganesh switched the docroot — site root 302s into Moodle, `/install.php` serves the open wizard (build string confirms 20260519 = our package, a free integrity check); browser-wizard route REJECTED by Nitin → CLI install. Access blockers walked: jump `35.154.8.154` filtered from home IP AND from the new corporate VPN (Matt approved VPN 08-27; tested 08-28 — VPN routes into the tunnel but office network has NO route into the AWS VPC; RDS private-resolves 10.0.137.64). **Fastest fix requested via Amul ("sure will do"): allowlist VPN egress `114.143.225.62` on UAT-Jump SG:22**; background port-watcher armed (8h, 90s cadence). Staged for the moment it opens: `tools/uat/stage-a-install.sh` (config.php-FIRST closes the wizard; fail-fast preflight; evidence log; LF-forced via .gitattributes), ssh config (jump/tunnel/uat-lms), ed25519 key. NEW DOCS this batch: `docs/business/SECURITY-SPEND-PLAN-ANNEX-2026-08.md` (6 line items ₹52–109L indicative inside the ₹80–120L envelope, sequencing + governance — deck slide-16 "spend plan to follow" delivered, approval still PENDING) + `docs/cutover/STAGE-A-VERIFICATION-MATRIX.md` (27-row A–E pass/fail walk + triage protocol + verdict block). AI-budget annex drafting runs in the spawned side-session. NEXT in loop: ADR-028 Phase 2.4 (SCIM 2.0 + outbound webhooks) unless the port opens first — then Stage A takes priority (needs Nitin's TOTP for the tunnel).) Prior 2026-08-24 (**C-SUITE DECK v2 — Nitin's factual corrections applied** (same file, replaced in place): (1) **Sentientia has NO live users** — every user record on it is a TEST import of the live airpay.academy DB backup; all "live in production / serving 3,500+ employees" claims rewritten to "built & proven on a full production data copy; real users load only after the UAT migration rehearsal passes" (slides 2/5/8/11/12/17/19 + notes); (2) tenants named correctly: **airpay internal / public / ZEEA Mafunzo (Tanzania)**; (3) security ₹80–120L + AI operating budget rows reframed **Pending approval** (amber) with "detailed spend plan to follow" — the 2026-08-04 memo was Nitin's signed recommendation, NOT company budget approval; both need line-item spend-plan annexes; (4) AI monthly cap = to be ESTIMATED, with a **self-hosted local-model option (Ollama or similar on our own server) under evaluation** to shrink it — NEW engineering TODO. Also delivered: consolidated pending-task list for Airpay DevOps + Cloud.in. Memory note `project_sentientia_not_live_yet` records the framing rules.) Prior 2026-08-21 (**C-SUITE INVESTMENT DECK SHIPPED** — `docs/business/SENTIENTIA-EXECUTIVE-DECK-2026-08-21.pptx`, 20 slides for the budget/resource approval meeting (MD/Founder, CHRO, CTO, CFO, Marketing — addressed implicitly). Built from the Task-1 feature/improvement inventory + engineering record: purchased-baseline → transformation stats (17→52 plugins, 514 theme files, 100+ vendor-code defects incl. the payment bypass, 100% hi parity, 28 ADRs) → live-in-production vs built-behind-flags (A/B honesty discipline preserved) → Meridian Customer-N proof → trust runway (re-baselined Sep/Oct/Nov/Q1 dates) → market/pricing (draft tier anchors ₹300–1,800/user/yr, labelled DRAFT) → **slide 16 investment-ask table** (trust ₹80–120L envelope + AI monthly cap = approved-in-principle per the signed 2026-08-04 memo; infra/people/GTM = new asks with `[to confirm]` placeholders Nitin must fill before presenting) → risks → 6 decisions-requested. Speaker notes on all 20 slides; brand palette; QA'd slide-by-slide via real PowerPoint COM render (2 defect rounds fixed: bullet-array options nesting, footer collision, UTF-8 mojibake from a PS round-trip). No numbers invented — every figure traces to the repo record; internal build cost deliberately framed as "existing capacity + AI-assisted engineering, vendor RFP will price the market comparison". Docs-only session; nothing deployed.) Prior 2026-08-20 (**CUSTOMER-N DEMONSTRATED — ADR-021 Gate B executed locally, ADR-028 Phase 2.1 closed.** `customer::current()` de-hardwired (platform 2026082000): resolves via `tenant_registry::customer_of()` when the registry is live; byte-identical Phase-0 (AIRPAY) while dormant — the default + all prod. Local execution: registry seeded (100% parity vs allow-list), `tenant_registry_legacy` OFF locally, `[DEMO] Meridian Financial Services` (id 2) + tenant 500 + qa_demo_learner — customer resolution correct per-user (demo→2, airpay→1) and **per-customer feature flags proven** (same flag true/false by customer — first real ADR-002 customer-layer exercise). PHPUnit `customer_resolution_test.php` **4/4 green (9 assertions)** incl. dormant-identity + suspended-tenant fallback. Evidence: `docs/visual-evidence/2026-08-20/`. Remaining for the full SALES demo: DEMO-TENANT-PLAN dressing (Meridian branding row, seed courses/personas) + Nitin's Option A-vs-B call. Same session earlier: privacy-provider salvage (7 real providers merged + 49-string core hi-pack parity closure + local ratings-clash reverted), trust-track milestones re-baselined on UAT, ADR-024/CLAUDE.md records corrected. Nothing deployed to live; production registry stays legacy-ON.) Prior 2026-08-19 (**UAT INFRA PROVISIONED — both hard gates MET.** Matt approved via Jitesh (Cloud.in ticket HS-20260819-79876); EC2 `UAT-Sentientia-LMS` (t3a.small, Ubuntu 24.04, PHP 8.3.6 ✅, 40GB enc, private 10.0.135.185 via UAT-Jump) + RDS `lms-sentientia-UAT-db` (**MySQL 8.4.9 ✅**, db.t3.small, encrypted) are LIVE — **P5 runtime validation is unblocked** for the first time since June. Infra-bound checklist: `docs/cutover/UAT-SENTIENTIA-DEPLOY-CHECKLIST.md` (Stage A fresh-install smoke = P5; Stage B live-backup rehearsal = rollout-gate Phase 2, Nitin-gated). Blockers on OUR side: LB domain (wwwroot must be fixed pre-install) + SSH user list; caveats: EC2 2GB RAM tight for rehearsal, ticket's '4GB' RDS RAM is wrong for db.t3.small (2GB actual), live moodledata size unknown vs 40GB disk. Package to deploy REFRESHED same day for the Ganesh/OneDrive handoff: `Sentientia-LMS-5.2-Complete-Standalone-2026-08-19.zip` (65,417 files, 162,022,806 bytes / 154 MB, SHA-256 `8b8676f177e0588c6b4ad8abc98fc22e5c97ae55f6be8b528312baec080a7ebc`) — now carries ADR-029 KeKa hardening + skills-first recs (both flag-OFF dormant); README prereqs block updated to point at the UAT env + checklist; guidebook PDF regenerated + pypdf-validated; 08-05 zip `.superseded`. ⚠ The PDF attached to the sent ticket reply carries the OLD 08-05 SHA — follow-up line on the ticket + OneDrive guidebook are canonical.) Prior 2026-08-07 (**KeKa JML hardening SHIPPED — ADR-029** (`local_sentientia_integrations` 1.2.0-beta + `local_sentientia_lifecycle` 1.1.0-beta): all 10 items of the 2026-08-05 investigation closed. Webhook gated behind NEW flag `sentientia.hrms.webhook.enabled` + `hrms_enable` (was live the moment `webhook_secret` was set), `hash_equals` on the `X-Webhook-Secret` header ONLY (`?secret=` GET path removed — access-log leak), log-row `MAX(id)` race fixed. ONE canonical upsert (`keka_client::upsert_employee`) now serves webhook + the REINSTATED nightly reconcile task `keka_reconcile` (flag `sentientia.hrms.reconcile.enabled` + setting + task-disabled = triple opt-in; NOT the 2026-05-07-deleted duplicate — same code path, §3.2 hazard can't recur). Identity `open_employeeid`-first/email-second (KeKa email change no longer forks a duplicate user); writes via `user_create_user`/`user_update_user` (real events, no forged `user_created`); leavers: employeeId fallback + session destroy + `timemodified`; joiners: dept-code→org-shortname tenant mapping with validated `keka_default_orgpath` default `/1` (tenantless users impossible; default never re-tenants existing users); `reportsTo`→`open_supervisorid` two-pass manager sync. Lifecycle auto-enrol heuristic RETIRED (enrolled every joiner into every dated course platform-wide): new definition = visible course tagged `mandatory_tag` (default "mandatory"), tenant-scoped via `open_path`, behind NEW flag `sentientia.lifecycle.autoenrol.enabled` (ADR-029 records tag-vs-customfield rationale). PHPUnit: keka 12 methods + NEW `observer_test` 5 methods — **17/17 green, 50 assertions** on the re-inited env (the first run flushed a real product bug my own fixture exposed: the tenant-placement last resort hardcoded org id=1, which no Customer-N install would have → now "first root org in the tree"); deployed webhook smoke-tested live → 403 `HRMS webhook is disabled` with flags OFF; lang-parity green (en+hi, KeKa settings de-hardcoded onto lang strings); README false claims fixed (keka_webhook.php / sync_keka_users / dry-run never existed); both state cards rewritten (lifecycle card was self-contradictory since 05-28). **OPEN (external, blocks production flag flips): live KeKa contract verification** — event names/payload shapes/`get_employee` envelope/egress IPs are ASSUMPTIONS from public docs, marked in code + README; reverse-proxy IP allowlist recommended. All three flags default OFF — production behaviour unchanged. Deployed local + upgraded (2026080700 ×2) + mirrored (ME ↔ top-level ↔ webroot verified identical). Nothing deployed to live.) Prior same day (**OAuth2 (XOAUTH2) outbound SMTP × Microsoft 365 — IT-facing runbook shipped**: `docs/operations/OAUTH2-SMTP-M365-RUNBOOK.md`, every step grounded in the 5.1.3+/5.2 code (verified 2026-08-05): `admin/settings/server.php:475-532` — XOAUTH2 auth-type + `smtpoauthservice` selector only exist when an ENABLED OAuth2 issuer does; `moodle_phpmailer::process_oauth()` (lib/phpmailer/moodle_phpmailer.php:152-175) — send-time auth = the issuer's connected SYSTEM ACCOUNT refresh token via `\core\oauth2\api::get_system_oauth_client()`, `smtpuser` as XOAUTH2 userName; `issuer.php:44` SMTPWITHXOAUTH2 usage mode + `systememail` field. Covers: Entra app registration (delegated **Office 365 Exchange Online → SMTP.Send** — NOT the Graph one — + offline_access/openid/profile/email/User.Read, admin consent, `{wwwroot}/admin/oauth2callback.php`, may be a SEPARATE app from the SSO one), Exchange Authenticated-SMTP prerequisite on a licensed service mailbox (tenant-wide disablement/Security Defaults ⇒ 535 even with a valid token), the **append-`https://outlook.office365.com/SMTP.Send`-to-offline-scopes-BEFORE-Connect-a-system-account gotcha** (stock Microsoft template lacks any SMTP scope — microsoft.php:42-43; `grantedscopes` frozen at connect time — api.php:643 → is_system_account_connected() scope-containment check issuer.php:228-253; fix = edit scopes then RECONNECT), connect AS the sending mailbox, smtp.office365.com:587 TLS + empty `smtppass` config table, verification (testoutgoingmailconf + `refresh_system_tokens_task` enabled) + 9-row troubleshooting table, and the local-dev exclusion (`noemailever=1` stays — 151-email incident rule; ninja/production only, live config Nitin-gated). `docs/OAUTH2-SSO-SETUP.md` cross-links the runbook. Docs-only session; nothing deployed to live.) Prior 2026-08-05 second wave (**Gate #3 aiquiz half CLOSED + ninja package rebuilt for the DevOps handoff.** (1) **Phase G.4 real mod_quiz publisher shipped** (`8e64d16a1`, aiquiz 0.3.0-alpha/2026080500): the G.0 "quiz id 0" stub is replaced — approved questions land in the course's default shared question bank (5.x `question_bank_helper` + GIFT import pipeline, control chars escaped), a REAL quiz is created HIDDEN via `add_moduleinfo` (module id supplied — the mod_form normally injects it) and populated via `grade_calculator::recompute_quiz_sumgrades` (`quiz_update_sumgrades` hard-deprecated since 4.2), all in one delegated transaction; the draft records the ACTUAL quiz id. review.php gains a target-course selector for course-less drafts; capability gates manageactivities+question:add; 15 en+hi pairs (parity green). **PHPUnit 5/5 green, 19 assertions** (hidden cm, slots, bank entries, pushed_quizid, rejected-question exclusion, status gate via errorcode, transactional-rollback capability gate, GIFT escaping). `sentientia.aiquiz.auto_push` stays default OFF — flipping it is the ninja-rehearsal verification step. **Addendum-A engineering gates: ALL THREE CLOSED** (#3 completed same day: authoring course_builder `03941c297`, 0.2.0-alpha/2026080501 — approved drafts become real hidden courses [book of cards + mastery quiz, all 3 qtypes incl. canonical MRQ weights + match pairs, gradepass from mastery_score], 7/7 PHPUnit/40 assertions, behind NEW flag `sentientia.authoring.publish.enabled` default OFF); still awaiting Nitin's cap figure + ANTHROPIC_API_KEY. (2) **Approvals: Priyanka APPROVED the ninja provisioning request; Matt (CTO) pending.** Package rebuilt as `Sentientia-LMS-5.2-Complete-Standalone-2026-08-05.zip` (identity in the dist guidebook PDF; 08-04 zip superseded) — the OneDrive pair for Amul/DevOps is refreshed and final. Nothing deployed to live.) Prior same day (**AI GATEWAY CONSUMER FLEET COMPLETE — all six consumers routed (ADR-028 Phase 2.3 follow-up).** skillsai / recommendations / translate / authoring / assistant migrated onto `local_sentientia_ai` per the README recipe (reference: aiquiz 2026080402), all at **2026080500**: OPT-IN delegation only when the gateway class exists AND `sentientia.ai.gateway.enabled` is ON — default OFF keeps every plugin **byte- and side-effect-identical** to its pre-gateway build (local mock, NO ledger writes; the 17-test aiquiz lesson applied from the start); each plugin's own mock passed down as the `'mock'` callable (v2-hindi skills / candidate-aware recs / `[MOCK <lang>]` banner / full-module cards+questions fidelity preserved); `legacy_component` key fallback everywhere (**caveat:** authoring's legacy key is `anthropic_api_key` — outside the gateway's `api_key` fallback, central key applies on its gateway path); gateway `denied` → the plugin's `failed` semantics everywhere. assistant covers BOTH surfaces — `ai_client::ask()` (purpose `assistant_chat`; no local mock → gateway generic mock when live OFF; only LIVE responses cached so mock text can't linger across a flag flip; system prompt extracted to a shared builder for byte-identical payloads) + `agent\agent_client::propose()` (purpose `agent_reasoning`; guard chain untouched — gateway carries text, never authority); `core_ai_bridge` untouched as the alternative backend. **PHPUnit 236/236 green across 6 suites** (skillsai 43t/191a · recommendations 47/125 · translate 58/144 · authoring 55/103 · assistant 22/56 · gateway 11/35; env re-inited for the version bumps). The campaign flushed one **PRE-EXISTING P1.3 bug**: `agent\context_builder::build()` named `open_path`/`open_designation` in an explicit user SELECT — fatal on any vanilla/Customer-N schema, 5 `agent_loop_test` errors → schema-portable `SELECT *` + null-safe reads (same class as the aiquiz `draft_manager` fix); `ai_client::build_context()` has the same coupling + unguarded `local_costcenter`/`open_supervisorid`/`tool_certificate_issues` reads (no test hits it) → follow-up task spawned. **Duplicate-tree drift closed while mirroring:** top-level `local/` was MISSING `sentientia_skillsai` + `sentientia_authoring` entirely and carried a stale 1.1.2-era assistant (no P1.3 agent surface) → full-dir synced from ME; top-level `recommendation_engine`/`translate_engine` still had the pre-ADR-018 `open_path` parse → seam versions copied over; webroot assistant `external.php`/`external_agent.php` still legacy `\external_api` → ME's `core_external\` versions deployed; 2 whitespace-only webroot `access.php` normalized. All 3 trees verified in parity (`diff -rq` clean × 6 plugins). Lang-parity gate green (57 files, 0 fail, 8 known en-only warns — no string changes). Local site upgraded to 2026080500 ×5 + caches purged. Live flags remain OFF; gate #3 (last-mile stubs) + Nitin's Addendum-A cap figure + ANTHROPIC_API_KEY still pending before any flip. Nothing deployed to live.) Prior 2026-08-04 (**UI-NAV residue closed + ninja package rebuilt to carry the UI wave.** (1) NEW `theme/sentientia/pix/course_default.svg` — the `course_bannerimage()` fallback referenced a never-shipped pix asset, 404ing on every course without an overview image; branded gradient banner, verified 200 `image/svg+xml`. (2) `dark_mode.scss` component rules tokenized — 104 hexes → `var(--ap-color-*)`; compile-proven byte-identical + runtime-proven token resolution under `body.dark-mode`; core remap block + high-contrast section deliberately untouched (no token remap exists under `body.high-contrast` — its pre-existing dark-value bug stays flagged for the audit follow-up). Theme **2026080301** deployed local; evidence `docs/visual-evidence/2026-08-04/`. (3) **Ninja handover rebuilt as 2026-08-04**: overlay re-run (65,349 files at 5.2 target, AMD gate 0 stale tokens), UI-wave artifacts spot-verified in the 5.2 tree, repackaged → `Sentientia-LMS-5.2-Complete-Standalone-2026-08-04.zip` (65,374 files, **161,913,184 bytes / 154 MB**, SHA-256 `a4944f11f6c6a8b48f6b9994ff7af2f48bb6e2bc46024918b2e6914cd643753d`), guidebook PDF regenerated + pypdf-validated (11pp, new SHA in, old out), 08-03 zip renamed `.superseded`. Commits `d104d995d` + `57c4b4ce9`. **OneDrive pair for Amul is now final** — still awaiting Matt/Priyanka approval; P5 runtime validation still IT-gated (PHP 8.3 + MySQL 8.4). Nothing deployed to live. (4) Same day, second wave — **strategy reconciliation**: 7-agent product-maturity audit (4 inventories + 3 strategy lenses → `docs/audits/PRODUCT-MATURITY-AUDIT-2026-08-04.md`; headline: the platform's constraint is *dark* capability — modern UX local-only, AI cohort mock/flag-OFF with two last-mile stubs, `customer::current()` hardwired, trust track prose-only) → **ADR-028 "reconciled product roadmap: ship-and-prove before build"** (Proposed; supersedes-as-roadmap PLATFORM-EVOLUTION-ROADMAP-2026-2027 + GAP-ANALYSIS §8 + ADR-018's wave-list role; 3 phases: Lights-on+Trust sprint → Prove the claims (Customer-N, skills-first home, one AI gateway + one live feature, SCIM/webhooks) → Experience compounders) + **`docs/business/DECISION-MEMO-2026-08-04.md`** packaging the six never-answered gap-analysis §9 questions + 2 unrecorded decisions (Anthropic live-API budget; 5.2-vs-5.1.3 go-live drop-dead) for Nitin's signature. (5) **Memo SIGNED same day — ADR-028 ACCEPTED (amended).** Nitin's calls, four overriding the proposed defaults: Q1=a **freeze rejected → parallel tracks** (trust+ship ∥ AI/cohort promotion; the 3 engineering gates survive: T-01 back-fill, AI gateway before live, last-mile stubs before flag flips); Q2=a+b marketplace dual-track (Go1 talk + certify Coursera connector); Q3=b **full ₹80–120L trust roadmap funded** (ISO cert + SOC 2 + load tests); Q4=b+a TTS evaluation now, ElevenLabs incumbent; Q5=b+a **native Capacitor app funded now** (prereq: REST v1 exercised) + PWA hardening; Q6=a+b India-BFSI beachhead AND global parallel; Addendum A=b broader multi-feature AI budget (⚠ cap figure + ANTHROPIC_API_KEY pending); Addendum B=**"5.2 now"** — single cutover path, no 5.1.3 interim, IT gates critical path. Open follow-ups: AI cap+key, exact Q3 envelope, IT committed dates. (6) **ADR-028 Phase 1.5 quick-win batch EXECUTED** (owner: "start"): **T-01 residue closed with evidence** — aiquiz `:generate/:review` + leaderboard `:manageboard` gain teacher archetype + upgrade back-fill (aiquiz also grants `sentientiaauthor`), both 0.2.1-alpha/2026080400, verified on the prod-import DB; translate/xapi/learningpath grants confirmed BY-DESIGN (docblock evidence), NOT bugs. **Shell-chrome i18n restored** — 31+4 en+hi pairs (sidebar 40 call-sites, course player, topbar; 2 extra template emit sites + 5 aria-labels caught in browser verification), theme 2026080302, en+hi browser-verified. **Lang-parity now MECHANICAL**: `tools/check-lang-parity.php` + CI job `lang-parity-check` + pre-commit CHECK 17 (now [N/17]); first run caught 6 pre-existing FAILs → all closed (hi +79: catalog 16/m365 33/translate 30; en +3 real bugs: classroom cap+privacy, programs privacy) → gate green `56 files, 0 fail, 8 warn` (warns = en-only pack backlog). **Privacy registry**: 8 null providers added (evidence-checked), 9 components bumped 2026080400; 8 plugins flagged needing REAL providers → spawned follow-up task. **reCAPTCHA reclassified**: signup already ships honeypot+reCAPTCHA v2 (P1 #59) — gap is unset prod keys → `docs/security/ENTERPRISE-IDENTITY-PACK.md` (NEW: Entra SSO runbook + MFA policy/config sequence + reCAPTCHA keys; local stays MFA-off by design — QA harness; ninja runs the sequence first). `sentientia_request` duplicate-tree drift resolved (newer AMD builds → ME, de-branded pluginname → ME+webroot). All deployed local + upgraded (one stale `upgraderunning` lock from a killed CLI run cleared via DB) + both trees mirrored. Nothing deployed to live. (7) **AI GATEWAY SHIPPED — `local_sentientia_ai` 0.1.0-alpha (ADR-028 Phase 2.3, engineering gate #2 of 3 for the signed Addendum-A budget).** One gateway replacing the six duplicated per-plugin Anthropic clients: central `api_key`, spend ledger (`local_sentientia_ai_ledger` + Reports ▸ AI spend ledger admin page), **fail-closed quotas** (daily global/customer tokens + monthly est-cost; 0/empty = live BLOCKED, never unlimited), mock-first routing (both flags default OFF; live-intent failures surface as failed/denied — never silent mock), REAL privacy provider (ledger user-attributed; prompt text never stored), **structural PHPUNIT/BEHAT no-spend guard inside `call_live()`** (added after the first test run actually POSTed a fake key to Anthropic — install-applied setting defaults gave quota headroom). Reference consumer migrated: **aiquiz 2026080402 with OPT-IN routing** via `sentientia.ai.gateway.enabled` (first-cut unconditional delegation broke 17 aiquiz tests → corrected same-day; default OFF = byte- and side-effect-identical legacy path; v2-hindi mock fidelity verified byte-faithful through the gateway). The test campaign also flushed out + fixed **3 PRE-EXISTING aiquiz bugs** (all Phase G.1-era, in the designated first-live feature): `draft_manager` `open_path` SELECT fatal on any vanilla/Customer-N schema (→ schema-portable `SELECT *`); **`load_for_actor` DML string-vs-int strict-compare denying every non-manage_all owner access to their own draft** (masked in QA by admin accounts); prompt_builder word-count test miscount; + draft_manager_test wired to the shared `bizlms_fixture` (June pattern aiquiz never got). **PHPUnit: 93/93 green** (aiquiz 82 + gateway 11 incl. 2 golden mock fixtures; phpunit env re-inited twice for version bumps). One-click enrol (2): already ON for /1 locally + already in the ninja cutover packet step 5 — no action needed. Ops: local Apache died silently mid-verification (console-mode fragility, opcache off) → restarted via httpd.exe. Remaining gate #3 before any live flip: last-mile stubs (aiquiz→mod_quiz publisher, authoring→course builder) + Nitin's cap figure + ANTHROPIC_API_KEY. Follow-up: migrate skillsai/recommendations/translate/authoring/assistant per the README recipe. Nothing deployed to live.) Prior 2026-08-03 second wave (**UI-NAV-AUDIT executed, ALL phases A–F.** **A:** course/activity pages re-shelled into the app shell — the dual-navigation split (N-01) is CLOSED; `course_editing.mustache` preserves the drawers world for edit mode (round-trip verified); single in-course TOC incl. section 0; core `edit_switch()` surfaced; reactive-component console errors eliminated. **B:** shell breadcrumbs on nested pages. **C:** 305 dark-mode hexes tokenized across 11 partials (agent-executed, dart-sass-verified); white-quiz-card dark bug fixed at root (`modules.scss` activity-header). **D:** notifications+messages in the shell topbar; N-05/N-07 re-examined → works-as-designed, audit amended. **E:** 3 page-title fixes (double-pipe, User Engine→Manage Users, Skills Matrix→My Skills; en+hi) + sr-only h1 a11y. **F:** per-persona fingerprint re-walk green. Commits `ba736b7a2`+`d34f6f870` pushed; theme 1.0.50-beta. Deployed local only.) Prior same day (**Demo-prep + UI-repair wave + pending-item sweep.** Signup page repaired (stacked layout wired, `passwordunmask`→`password`, sitewide mform `element-template` fixes: `new_req` GIF→FA `req` icon + label-attr leak + `float-sm-rights` typo) then **redesigned to the corporate split-panel login design** (new `signup_page.mustache` sharing the `.airpay-login` BEM system; browser-verified e2e incl. live-DB hero stats). **Course-player UI repaired** (sticky bar pinned flush under navbar — was floating 64px mid-air; track polish; learner header compaction 643→193px, edit-mode untouched). **Featured-for-you rail gated to curators** (`local/sentientia_courses:manage`) — hidden from user roles, per-persona verified. Demo credential sheet verified (8 personas, 2 drifted passwords realigned, local-only). **Pending-item sweep:** June's stranded work landed (`local/sentientia_request` 1.3.3 drift-sync; 2026-06-17 gap-test-pass fixes: xapi timestamp, content_market IN(), learningpath adaptive DDL/privacy + tests; QA-password redacted from the committed gap-test report; `tools/gap-test/reports/` gitignored). Commits `52fe9182b`→`f730a5b14` pushed. **Ninja handover ready:** 5.2 standalone zip + RDS Guidebook PDF in `docs/cutover/dist/` (email to DevOps drafted; awaiting Matt/Priyanka approval). **P5 runtime validation still blocked** — no local PHP 8.3 (`.tools/php84` is a stub); prod RDS MySQL 8.4 + PHP 8.3 remain open IT items. Nothing deployed to live.). Prior 2026-06-19 (**Moodle 5.2 deploy-candidate built + statically validated** — overlay-built the 5.2 core + the current 46-plugin Sentientia layer into a 154 MB standalone package; **P4 static validation PASS-WITH-NOTES, 0 confirmed blockers** (1,567 `php -l` clean · schema/utf8mb4-DDL clean · 0 `theme_airpayux` survivors · 5 `external_*` "highs" verified false-positive); completed the `core_external\` migration (reconciliation #3 closed) + closed AMD webroot-drift + excluded stale cruft from the package. **Deploy gated on prod MySQL 8.4 + PHP 8.3 (IT change requests); local runtime validation (P5) gated on PHP 8.3.** See top H2 + `docs/cutover/MOODLE-5.2-RECONCILIATION-PLAN.md` + `…-P4-STATIC-VALIDATION-2026-06-19.md`. Commits `5d2185ce9`→`9e10ff03c`→`f9ef2485b`→`0fb2258a3` on `claude/gap-integration`, pushed. **Nothing deployed to live.**). Prior 2026-06-18 (milestone `sentientia-milestone-2026-06-18` packaged for IT/ninja — see top H2). Prior 2026-06-10 (**Sentientia quality-gate + de-brand + audit-action wave (Opus 4.8 standing-loop)** — closed all 3 post-import-audit P0s + advanced P1/P2; **12 commits pushed to production, none deployed to live**. **P0-1** canonicalised the 40-plugin Sentientia product backend into git (was webroot-only — a redeploy-from-git would have lost the entire backend) + merged. **P0-3/P0-4** retargeted stale `local_airpay_*` refs (privacy provider table + structured_logger label) and caught a latent FATAL the audit under-scoped — the rule_engine SQL filtered a non-existent `lu.timemodified` column → fixed to `lu.timecreated`. **ADR-027 gate system** (the durable answer to "we run visual audits and still ship UI bugs"): Gate-0 static scanners (Mustache comment-leak + a NEW stale-`theme_airpayux` reference scanner `tools/scan_stale_theme_refs.php`) wired into BOTH the pre-commit hook (now **14 CHECKS**) and CI; Gate-1 `render-smoke` Playwright spec (per persona×surface: AMD-booted + no raw `{{ }}` + landmark + 0 console errors) → all 3 recurring bug classes now gated. **De-brand (P2)**: course/view hero (`#305D94`→Sentientia gradient + i18n), 31 plugin `pluginname` display labels, course-share events + `privacy:metadata` strings — **KEEPING** legitimate customer/company refs (Airpay Academy / Airpay Payment Services / Airpay tenant / Airpay Gateway). **P1-3 resolved structurally** — in-course activity views (quiz/scorm/assign/forum) inherit the already-validated `course.php` layout (`incourse`→`course.php`), no per-surface work needed. **Owner-gated remainder**: broad lang-string de-brand has genuine gateway/customer/product ambiguity (needs a scope call); fresh in-course screenshots blocked by local-tooling + Chrome `localhost:8080` permission; Gate-2 (visual-diff) / Gate-3 (coverage-matrix) not yet built. See the H2 immediately below. **Prior 2026-06-09:** **Durable deploy-pipeline AMD de-brand fix** — made the `theme_airpayux`→`theme_sentientia` AMD-module rename survive a clean redeploy-from-git by adding a `Repair-AmdModuleNames` post-copy step to `tools/overlay-airpay-customs.ps1` (idempotent `.Contains`-guarded literal rewrite of `theme/sentientia/amd/build/**/*.js`, UTF-8 no-BOM, + fail-fast `grep…==0` gate that aborts the deploy on any surviving token); closes the durable half of **F-LOAD-02** / theme-side **ADR-025 follow-up (c)**. Verified end-to-end on a scratch deploy seeded from git's branded `theme/airpayux` build — 25/25 bundles rewritten, **0 `theme_airpayux` remain**, relative `./` deps + no-BOM intact, idempotent no-op when source already de-branded, gate proven to fire. Scoped to the theme build tree only (`airpay_ratings`/`paygw_airpay` excluded per ADR-025). Tool-only, nothing deployed; live webroot left as-is (already hot-fixed, 0 console errors/19 surfaces). Flagged residual: the from-git *directory* de-brand (`theme/airpayux`→`theme/sentientia`) is still not scripted. See top H2. Prior same-day (**ADR-018 Wave-2 theme code-resilience** — guarded the *sole* `open_*` SQL query in the canonical git theme (`theme/airpayux/layout/dashboard.php` `$teammembers` / `open_supervisorid=:mgrid`) behind a `field_exists` auto-detect guard; manager *detection* (`$ismanager`) already uses the native `moodle/site:viewreports` capability; every other theme `open_*` ref is a null-safe object-property read → **the theme now renders on a vanilla / column-less Moodle via code-resilience alone** (Nitin's 4 decisions: code-resilience / single-implicit-tenant / native-roles / `field_exists`-no-flag). `php -l` clean; authenticated `/my/` **200**, 0 exception, 0 token leak (data-rich Airpay schema → guard true → identical query → **zero-regression**); ADR-018 + this file updated; committed `debrand/bizlms-eabyas`. Flagged-not-blocking: the live webroot theme (`theme/sentientia`, 1059-line dashboard) is a **stale pre-`role_detector`-refactor artifact on no git branch** — recommend a `git→webroot` redeploy to sync. Prior 2026-06-08 de-brand follow-up — **ADR-025 is COMPLETE** (36 components, incl. `theme_airpayux`→`theme_sentientia`; origin/production @ `6d550dd57`): executed ADR-025 follow-up (b) — purged the `local$name` junk dir from the local clone (2,514 untracked files, never loaded; 664 phantom grep hits gone); and surfaced a **NEW gap not in ADR-025** — ~30 committed `amd/build/*.min.js` bundles still call renamed-away `local_airpay_*` WS funcs (the rename `sed` skipped compiled bundles; every `db/services.php` is alias-free) → broken AJAX on those actions until a `grunt amd` rebuild; folded into ADR-025 follow-up (c). See top H2. Prior 2026-06-03: **ADR-022 batch-1 rename `local_airpay_ratings`→`local_sentientia_ratings` rehearsed-on-branch + verified 12/12 on the local prod-import (the **7 role-cap grants preserved**, en+hi 12/12 strings, WS re-registered, classes autoload, 0 residue); hardened the reusable procedure: NEW `tools/rename/handover.php` (re-points `capabilities.component` too — the duplicate-cap fix — + deletes old WS rows + broad component sweep), `codemod.php` lang-file fix (all language packs, not just `en`), version-bump + no-op `db/upgrade.php` to rebuild the classmap; the "class won't autoload" chase was a false negative in the verify harness (`"\external"`→`\e`=ESC). **Kept rehearsed-on-branch `rename/airpay_ratings-batch1`: it was briefly merged to `production` (`4e7f8b398`) then REVERTED (`f9ffcd242`) per Nitin's course-correction — the rename rollout + merge-to-production is Nitin's gated call, not autonomous. DB hand-over unrun; nothing deployed to live.**; prior 2026-06-02: **paygw_airpay payment-bypass FIXED (Critical) on `fix/airpay-payment-verification` — guard restored + fail-closed verifier, proven 5/5, NOT deployed (sandbox-gated); ADR-023 recurring-subscription design (per-instance scope, flag OFF); ADR-022 batch-1 `airpay_ratings` rename rehearsal scoped + handoff written — execution deferred to a fresh session; see top H2. Nothing deployed to live.**; prior 2026-05-30: **ADR-018 Wave 2 — `open_path` caller migration COMPLETE + merged to production**: the `tenant_identity` seam extended + ~22 product caller sites across 11 plugins migrated off direct `$USER->open_path` onto `local_sentientia_core\tenant_identity`; fast-forwarded `8bee68407`→`fe9184f08` (7 commits, PR-1 seam + batches 1-6); behaviour-identical + flag-gated; `_PATCHED` vendor + entity-path writes deferred; `compliance_report` engine migrated (owner WIP untouched); see top H2. Prior: **ADR-018 Wave-1 overnight stabilization loop (Opus 4.8) — 9 chips shipped + pushed to `production`, NONE deployed to prod**: dark-mode WCAG-AA ×3 (catalogue chips, Bootstrap `text-*` incl. BS5.3, a real `.text-black-50` 1.06:1 fix), white-label de-Moodle ×3 — all GPL-safe (OTP login button, `privacy:metadata`, `@copyright` docblocks preserving the eAbyas notice), decouple docs ×3 (`DEPRECATION-SCHEDULE.md`, `BIZLMS-MIGRATION-NARRATIVE.md`, `_bizlms-*.scss` deprecation headers) + a stylelint guard banning new `.costcenter_data`/`.content_right` coupling; commits `dd44503aa`→`f8632b169`; owner's compliance_report WIP untouched; see top H2 + ADR-018 execution log. Prior 2026-05-29: (**QA-walk 2026-05-29 backlog FULLY CLOSED** — every actionable finding fixed or verified NOT-A-BUG. Closeout wave: **SA-04** (admin course-mgmt redirect now capability-gated → siteadmins reach native management), **P-01** (sidebar "My Cart" → the session cart that holds items), **P-02** (cart-badge AMD wired on the columns2/'standard' layout), **SA-05** ("Epsilon" → "Airpay Academy UX (Sentientia)" across 5 lang packs), **C-004** verified NOT-A-BUG (server-side filter re-aggregates KPIs + table). 5 branches pushed (e01 / e02[+P-01] / sa04 / sa05 / p02), **none merged or deployed to prod** — PR-merge + the E-01 flag-enable are the owner's calls. See top H2. Prior same-day: Employee-walk E-02 P2 FIXED — cap-gated "My Skills" sidebar link added to the Learner+Manager shells (`theme_airpayux\sidebar_navigation`) so learners can reach the existing `/local/airpay_skills/index.php` dashboard; the page itself was fine (transient QA repro, same shape as M-01). Theme class-only, no flag/version bump; branch `fix/qa-walk-e02-learner-skills-nav` pushed (not merged). M-01 P3 also verified NOT-A-BUG (team-KPI tile = `count($team_data)`, same source as the table; live tile = 1). Employee/manager QA-walk cluster now fully closed. See top H2. Prior same-day: Employee-walk E-01 P1 FIXED (+ E-03 P3) — one-click free self-enrolment for internal tenants. Root cause (verified, corrected the BUG-LOG's "no self-enrol/auto-enrol" guess): the catalog "Enroll" button routed free courses to `?action=addtocart` (never enrolled), and the cart's `enrollfree` called core `enrol_self()` which silently no-ops on key-gated courses (course 71 has an enrolment key) while falsely reporting success. Fix: new `local_airpay_catalog\enrolment` — `should_offer_oneclick()` policy (logged-in internal-tenant + free + flag; Public /77 + guests keep the cart) + `enrol_now()` mechanism (idempotent MANUAL enrol, bypasses the key, mirrors `cart_manager`); new `course.php?action=enrolnow`, grid/detail button routing, and a fix to the `cart.php` silent-success lie. Behind `sentientia.catalog.free_oneclick_enrol.enabled` (default OFF; enabled for /1 locally — PROD must enable per internal tenant). `local_airpay_catalog` 2026052901→2026052902; +8-case PHPUnit suite (CI) + 4 lang strings ×5 langs. Verified end-to-end: CLI (manual key bypass, idempotent) + real-browser (qa_employee one-click-enrolled 71+403, My Courses=2 — was the empty-page symptom) + 3 screenshots desktop/mobile. NOT pushed/deployed to prod. See top H2. Prior same-day: Trainer-walk T-01 + T-02 FIXED — Sentientia Live now reachable by the BizLMS `trainer` role: added `teacher` archetype to `:create`/`:run` in `access.php` + a `db/upgrade.php` back-fill onto existing teacher-archetype roles (Moodle only auto-applies archetype defaults on a cap's *first* install), and a capability + `live.enabled`-flag-gated "Live Sessions" sidebar link in the Manager + Learner shells. `local_sentientia_live` 2026052504→2026052900; verified qa_trainer `:create`/`:run` → YES + sidebar link rendered. Same session also fixed T-04 (block_airpay_trainer dashboard "Class not found") and decided T-05 by-design (trainers keep the richer /my/ shell, no forced redirect). Committed locally on production (0773c2e96 + c92fbdc2a + 18830886e); not pushed/deployed. See top H2.**; **C4 follow-up — fixed the legacy flag-OFF catalog add-to-cart double-question-mark URL bug + version bump; owner chose merge-now, landed via PR #3 → production (live file-copy + cache-purge still manual).**; **Resumed-session wave (Opus 4.8) — F-024 analytics visual walk + C10 P1 certificate polish + C4 public-storefront LXP restyle ALL SHIPPED, then a Bucket F verify close-out (F-030/F-031/F-032/F-033 — all verified-good, no code change; paygw security confirmed on the live path), then 5 owner-reported signup-flow UI fixes (honeypot field, tall-card zoom, duplicate success message, close-button glyph, unstyled confirm page) — all real-browser verified**; commits `7328c6364` / `6368f7617` / `76b5f9abc` / `ea00d36ab` + Bucket-F-verify + signup-fixes; C4 + C10-tenant-scope both feature-flagged default OFF; see top H2s. **Prior 2026-05-28:** **Stabilization Audit ALL 3 PHASES COMPLETE + 3 BLOCKERS FIXED** — `docs/audits/PLATFORM-STABILIZATION-AUDIT-2026-05-28.md` is the audit; `docs/adr/ADR-017-polymorphic-user-types.md` is the architectural drop. **Stream 1 (deploy-unblock fixes) shipped in commit `e32473e58`**: cap rename `local/courses:manage` → `local/airpay_courses:manage` in 5 files + drop `local/users:edit` fallback in user_manager (F-080/F-088, F-097); back-port 30 drift files across `local/airpay_pages` (17 incl. version.php + 11 CLI + EN lang + 3 HTML + qr_scan), `local/airpay_lifecycle` (6 incl. version.php + db/events + db/tasks + observer + compliance_check), `local/airpay_core` (2 CLI helpers), `theme/airpayux` (9 AMD files); deployed + cache purged + Apache log verified clean. **Stream 2 (ADR-017 polymorphic user-type)** drafted in Proposed status — schema (`local_airpay_user_type` + `local_airpay_employee_profile` + `local_airpay_consumer_profile`), `user_type_provider` interface (employee_provider + consumer_provider), 5-phase migration path, 7 open questions for Nitin to resolve. Closes F-001/F-003/F-004/F-005/F-006/F-007. **Stream 3 (Phase 3 consolidation)** organised 97 findings into 7 buckets: A (✅ shipped, 12 items) / B (Ship-small, 20) / C (Finish-large, 17) / D (Remove, 6) / E (Redesign, 4) / F (Investigate, 10) / §5 (v2 lock, 21). Estimated v1 close-out ~10-12 weeks. Prior on this same day:) (Stabilization Audit Phase 0 + Phase 1 COMPLETE — `docs/audits/PLATFORM-STABILIZATION-AUDIT-2026-05-28.md`. 97 findings logged (F-001–F-097). Phase 0 was desk-research across PROJECT-STATE, ADRs, state-cards, 4 prior audits, plugin maturity stamps, naming-transition. Phase 1 was runtime probing — Apache error log mining, DB capability + feature-flag inspection, workspace↔deployed file-count drift survey, plugin URL routing probes. **3 stop-the-bus 🔴 BLOCKING findings surfaced in Phase 1**: F-080/F-088 (caps `local/courses:manage` + `local/users:edit` referenced in 5 files but NOT registered — every page render logs Notice + silently locks L&D Admins out of course-management sidebar); F-091 (`local/airpay_pages` workspace missing 17 files including `version.php`, EN lang pack, 11 CLI scripts — IT deploy from workspace = broken plugin); F-092 (`local/airpay_lifecycle` workspace missing 6 files including the entire runtime: `version.php`, `db/events.php`, `db/tasks.php`, `classes/observer.php` — deploying from workspace = lifecycle observer + compliance cron stop firing). Phase 2 (ADR-017 polymorphic user-type) and Phase 3 (consolidate + triage) still to come. Prior 2026-05-27:) (**Sidebar role switcher** — surfaced BizLMS role switching in the airpayux shell sidebar for multi-role users; backend already worked but the shell discarded the switcher HTML. Verified Admin↔Learner round-trip for Nitin. See top H2. Prior 2026-05-25:) (**Goal C CLOSED** — four full per-persona user guides shipped under `docs/user-guides/` (Tenant Admin, Course Author, Compliance Officer, Learner), each ≥20 pages with login + full walkthrough + mobile + troubleshooting + v1.0.37-beta changelog; plus a README index with chooser flowchart and 4 screenshot manifests. See the "GOAL C CLOSED" H2 immediately below. Prior update 2026-05-24:) (Three parallel-chip MVPs shipped: **Tier 2.6 Calendar Sync** — `local_sentientia_calendar` with token-URL ICS feed, 4 feature flags, 28 PHPUnit assertions, ADR-013, Hindi 100%; **Tier 1 #4 AI Quiz Generation Phase G.0** — `local_sentientia_aiquiz` with 4-layer cost defence and mock-mode demoable pipeline, ~47 PHPUnit tests, ADR-012, Hindi 100%; **Tier 2 #7 Real-time Leaderboards Phase L.0** — `local_sentientia_leaderboard` + `block_sentientia_leaderboard` with SSE-driven live ranking across quiz/completion/skill board types, GDPR-compliant opt-out, ADR-014, Hindi 100%. **Platform Visual Audit v4.1.0** shipped from mobile-app session — 14 surfaces audited (9 P0 / 8 P1 / 6 P2 findings), CONDITIONAL PASS verdict; full report at `docs/audits/PLATFORM-VISUAL-AUDIT-2026-05-24.md`. Earlier today the night-run autonomous batch shipped 16 items: Phase B.12 cutover-day mechanical fixes (A1-A8), plugin PHPUnit coverage (B1-B2), Goal C user guides for 6 personas (C1-C6); cutover-day TODO list is now mostly empty modulo NVDA verification + activity_header runtime test. **Paygw security follow-up shipped earlier this session** — MD5 deprecated, require_login() at file scope removed, sandbox/live URL clarified, 13 new PHPUnit tests added. Phase B Moodle 5.2 upgrade is code-complete; production stays on 5.1 until customer-driven cutover decision. ADR-001 records the strategic pivot from "patch Moodle deployment" to "build saleable enterprise LMS product" — Airpay Academy is customer-zero. See `docs/adr/ADR-001-fork-strategy-and-product-pivot.md`.
...
## 🔐 2026-09-25/26 (overnight) — ADR-031 cross-tenant authority SHIPPED to UAT; production audit (Opus 5.5)

**Nitin connected the VPN and asked for the night to be used end-to-end.** Everything below is on
`claude/gap-integration` (pushed) and **deployed to UAT** (https://academy2.airpay.ninja). Production
(airpay.academy) is untouched — production deploys stay Nitin-gated.

**What ADR-031 is** (`docs/adr/ADR-031-cross-tenant-authority.md`, accepted 2026-09-25): exactly one
cross-tenant authority — site admin or `local/sentientia_platform:crosstenant` (no archetype default,
nobody holds it yet); plugin capabilities say WHAT, never WHERE; a tenant that does not resolve sees
nothing (fail closed); every write checks its target's tenant (`tenant::require_same_tenant_user()` /
`require_path_access()`); site-admin behaviour unchanged.

**How it was built (all adversarially reviewed before merge):**
- Sweep → 73 hits, 70 confirmed (`docs/audits/CROSS-TENANT-AUTHORITY-SWEEP-2026-09-25.md`).
- 8 plugin groups in worktrees (identity, courses, learning, engagement, assessment, comms, aicontent,
  integration), each fixed then reviewed; then **5 fix-forward rounds** closing every review must-fix and
  the security-relevant should-fixes (courses role picker, SCIM account takeover of in-tenant site
  admins, v1 API enrol roles, request approval into another tenant, analytics `:viewallorgs`,
  `viewer_can_access(0===0)`, evaluation anonymity bypass, skills gap cross-tenant links, ...).
- A **cross-cutting review** of the whole merged change set (upgrade integrity, cross-plugin
  contracts, residual unscoping across all plugins, backwards compatibility per persona) found 34 more;
  every confirmed one is fixed except the three decisions below.
- Also fixed on the way: no-reports managers keep in-tenant allocation (production behaviour); welcome
  email never sent since 2026-05-16 (now `email_to_user`, no plaintext password stored; white-label
  `[support_email]` restored after 29d25542c reverted it); `lp_adaptive_log` scores rounded to
  integers (NUMBER(6,2) + PostgreSQL-safe widening); classroom fresh-install schema parity; pwa
  upgrade-step order; 19 pre-existing test failures (calendar 9, courses 1, users/learningpath 9).
- Tools: `tools/uat/deploy_to_uat.sh` now fails loudly on a failed upgrade (it printed a green check);
  `adr031_interim_lockdown.php --release`; `adr031_predeploy_probe.php`; `adr031_ws_smoke.php`.

**UAT Window A — done tonight:**
1. Interim lockdown applied at ~20:35 (35 P0 caps PROHIBITed for role 9 while the code was pending).
2. Pre-deploy probe: 0 recompletion rules (so no costcenterid-0 reset risk), 0 path-less items, both
   UAT site admins (2, 14) sit at /1, org after_config hook present.
3. **Deploy 04:06 IST: 735 files, sha256-verified, upgrade.php OK on Moodle 5.2 / MySQL 8.4**, caches
   purged. Backup of the replaced files: `/tmp/uat-predeploy-backup-20260926-040629.tgz`. Full upgrade log:
   `/tmp/uat-upgrade-20260926-040629.log`. **No DB backup was taken** (the DB is remote and config.php is
   not readable by the SSH user; Claude does not handle DB credentials) — ask IT for an RDS snapshot
   before the next large deploy.
4. Lockdown **released** with the ADR-031-aware `--release`: 22 in-tenant grants restored, 12
   cross-tenant switches left removed by the upgrade, 1 cleared (recompletion:manage, never held).
5. **W2-02 erasure probe PASS** on UAT: 8 seeded tables emptied for a throwaway account, request
   honestly `completed`.
6. Post-deploy **WS smoke** (53 read web services × tenant admin, /1 learner, /177 learner, line
   manager on real UAT data): **0 errors**; refusals exactly where ADR-031 intends; the /1 tenant admin
   sees 7 courses / 8 users / their tenant's programs, paths, classrooms, exams, evaluations.
7. Public pages 200; no PHP fatals in the Apache/FPM logs after the deploy.

**Local verification (MariaDB, fresh PHPUnit init = fresh-install gate passed 3× tonight):** see the
test table appended below this section when the overnight run finishes.

**Production (airpay.academy, BizLMS 4.1.2) — READ-ONLY audit:**
`docs/audits/PRODUCTION-CROSS-TENANT-AUDIT-2026-09-25.md` — **37 confirmed findings (24 P0)** in
partitions 1–2: several AJAX web services and pages check only `require_login()` before deleting or
editing org, course, classroom, program, group and user records by id (any logged-in account can reach
them), `PARAM_RAW` concatenated into SQL on some list/download paths, and the scoping helper fails open
for users without a `local_userdata` row. Interim actions listed for Nitin. Partition 3 (exams,
evaluation, notifications, skills, cart, ratings, forum, report blocks): see below.

**Needs Nitin (decisions — nothing was changed for these):**
1. **P0 role configuration on UAT (and later production):** role 9 "administrator" holds core
   `moodle/role:manage` (so a tenant admin can grant their own role `:crosstenant` — or anything) and
   core `moodle/user:loginas|update|delete|create` at system context (core pages have no tenant
...
3. **Cart**: can a learner buy (and be enrolled in) another tenant's priced course? Today yes; ADR-031
   left it as a product decision (recommend: buyer's tenant or explicitly public courses, flagged).
4. Product follow-ups flagged by reviews: per-tenant DPO panel (privacy), tenant-scoped notification
   rules, a learner-facing learning-path page (links now go to My courses), welcome email now REALLY
   sends (checkbox defaults ticked — testers use addresses they own), `rule_engine` LIMIT precedence
   bug (fixing it starts real sends — decide first), manager `idx_user_course` unique index blocks a
   second typed allocation per user (pre-existing).
5. **Browser pass (Window B, VPN off, Chrome):** the 48-item persona screen-check list and the
   screenshots CLAUDE.md s.13 requires are in `docs/visual-evidence/2026-09-25/README.md` — no
   authenticated screenshot was possible overnight (Chrome dies on VPN; Claude never types passwords).
6. Stale worktrees under `.claude/worktrees/` (≈40) can be deleted once this is merged — needs your OK.

---

## 🔒 2026-09-24/25 — Wave 2: erasure rebuilt, compliance team scope, 30+ defects closed offline (Opus 5.5)

The VPN was down for most of the day, so Window A (the UAT deploy and the on-box W2-02 erasure probe)
has **not run**. Everything below is committed and pushed on `claude/gap-integration` and verified
locally. Nothing has been deployed to UAT or production. Evidence and the defect table are in
`docs/visual-evidence/2026-09-24/README.md` (N1–N15).

**Browser pass (Window B, before the deploy):**
- W2-05 is proven live: the tiles showed 10 against the table's 8 (ZEEA leak), and Fatma appeared in
  Priya's leaderboard.
- W2-08 (TLS) is settled locally.
- Seven new defects were found, N1–N7.

**Shipped (all adversarially reviewed before merge; workflows ran 3–25 agents each):**
- **W2-06 white-label rename.** 36 plugins no longer show "Airpay" in their names (`718363a14`).
- **N1: cross-tenant profile reads closed** (merged branch `claude/wave2-n1-profile-access`).
- **N5/N6: refusals render real messages** (merged `claude/wave2-n5-refusal-strings`; analytics is
  now 2026092401, so the `:viewallorgs` revoke still runs).
- **N7: the compliance Manager Report separates people from assignments.**
- **Right-to-erasure rebuilt** (`local_sentientia_privacy` 1.0.2):
  - **Step 0:** erasure used to clear 8 hand-listed tables and still report `completed`. It now asks
    every `local_sentientia_*` provider.
  - **New hook `anonymise_data_for_user()`** (duck-typed). Completions, attendance, exemptions, cmi5
    attempts, proctoring verdicts, program and recompletion records are kept against the anonymised
    user row.
  - **DPDP panel:** Approve fataled on every click since 2026-04-13 (a missing `$adminid`); now fixed.
  - **Provider audit:** all 38 providers were audited and every confirmed defect fixed. The worst:
    - xapi redacted every unresolved statement in the LRS;
    - evaluation's `$userid` was never set, so it rewrote other people's rows (my own `951b20982`);
    - WhatsApp deleted other employees' consent audit;
    - calendar never deleted anything;
    - authoring, aiquiz, translate, courses and the AI ledger deleted shared or financial records;
    - the platform provider missed 5 runtime-created profile tables;
    - classroom missed the waitlist, and fresh installs lacked that table (upgrade step, 2026092400).
- **Compliance team scope (Nitin's rule):** an L2 manager sees direct reports **and extended teams**,
  within their tenant, and nobody else. `viewer_scope` makes the single access decision for both the
  page and the export.
  - **Also fixed:** configuration is site-admin only on the server (any viewer could exclude users in
    any tenant), `?dept` / `?subdept` are clamped (they leaked other tenants' department names), and
    non-admins with no tenant are refused (they used to see the whole site).
  - **Portability and correctness:** PostgreSQL-safe SQL, and defaulters keyed per assignment.
  - **Checked on the imported data:** trees match a brute-force walk for all 258 managers. The largest
    manager now sees 20 people instead of the tenant's 916.
- Five AI-client tests asserted an error the no-spend guard now pre-empts; fixed.

**Tests (local PHPUnit, 2026-09-24):**
- **Pass:** compliance_report 44/44, privacy 9/9, authoring 72, programs 44, aiquiz 92, translate 63,
  recommendations 49, assistant 22.
- **Stale deployed test copies (errors):** evaluation. Redeployed; the rerun is pending after the
  classroom rebuild.
- **Ten PRE-EXISTING failures:** whatsapp 2, recompletion 5, challenge 4, platform 2. None of those
  test files changed today. They are being fixed in worktrees; the feature-flag tenant override may be
  a real code bug.
- **Not yet run:** users, analytics, gamification, pages, calendar, manager, courses, proctoring, xapi,
  learningpath, classroom, live, ai, notifications, cart, skills, and core_privacy.

**Needs Nitin:**
1. **VPN for Window A.** Re-run the dry run: the range has grown well beyond 165 files. Then run
   `deploy_to_uat.sh --yes --prefer-me --range c74bd55be..HEAD`, check FPM `opcache.validate_timestamps`,
   then run `tools/uat/w202_erasure_probe.php --i-am-uat`. The probe now sends nothing to admins, seeds
   the new tables too, and needs every seeded table emptied.
2. **Decision:** erasure currently deletes challenge attempts (the programs implementer's call; the
   reviewer agreed). Keep or erase?
3. **Decision:** line managers keep "no configuration". Tenant-admin configuration needs tenant-scoped
   engine methods first.
4. **Known and not fixed:** the CI PostgreSQL PHPUnit job uses pre-5.1 paths (no `public/`), so it cannot
   run these suites.
5. Site-admin screen checks: the plugins overview (W2-06), the privacy registry, and the DPDP panel.
...
- **Found while capturing deck screenshots (2026-09-17):** the Hindi login page rendered username/password placeholders as literal `य…` escapes — `{{#quote}}` JSON-encodes non-ASCII inside HTML attributes. Fixed in theme **2026090806** (`5124cc605`, core 5.2 `cleanstr` pattern; CLI render en+hi clean). **Deployed to UAT 2026-09-22 07:05** (4 files, checksums OK, upgrade Success); the served Hindi login page shows real placeholders (curl-verified). No UAT deploy is pending.
- **2026-09-22 07:12 — `local_sentientia_emails` 1.1.3 deployed** (`6eab7095f`, 3 files): the "New setting: default_cadence_days_json" smell was a real bug — `setting_cadence_json::validate()` returned `''` instead of `true`, so `write_setting()` never stored the value (install default or admin edits). UAT now holds `[1,3,7,14,21]`; a second `upgrade.php` run prints no "New setting". Both trees + test updated.
...
- **New finding, fixed same day:** the **Compliance report** was under-scoped — ZEEA in the Airpay admin's Business Unit filter (`LIKE '/1%'`), any `?bu=` accepted, and every tenant's mandatory course shown as a matrix/export column — and only half-localised. `local_sentientia_compliance_report` **1.0.2** (both trees; +5 tests, 89/89 en+hi). Also F-12 in the content-market card title (1.0.1-beta) and the scoped-admin dashboard subtitle.
- **Footer (Nitin):** "Licensed under GPL v3" badge → `footer_private_notice` ("Private & confidential · For authorised users of Airpay Payment Services Pvt. Ltd. and Airpay Academy only", en+hi); UI wording only, code stays GPL v3 — ADR-001 §5 addendum. Sidebar literals localised. Theme **2026090805**.
- **2026-09-17 08:27 — deployed** compliance_report 1.0.2 + theme 2026090805 + content_market 1.0.1-beta to UAT (`c14c36e85`, 16 files, checksums OK, upgrade Success; UAT's compliance lang file had mixed CRLF/LF endings, so the tree check was done content-wise — it is the ME copy). Served footer (privacy page, homepage) shows the private notice in en + hi; no "GPL" string on any served page. **Re-look PASS 09:00 (VPN off):** Meera = Airpay-only BU filter "(9)", 3 course columns, "Head of L&D", 0 English residue under Hindi; Juma = "ZEEA (2)" + Tanzania column only; private footer notice en + hi. Two residues found and fixed in the repo: *Browse Airpay Library* (`local_sentientia_courses` **1.11.5**, F-12 + full i18n — this, not the content market, was yesterday's double-escaped page) and the content-market "not enabled" string (1.0.2-beta, no `&`). **Deployed 10:16** (`8aca24621`, `--prefer-me` — UAT's courses lang files match the ME tree content-wise; 8 files, checksums OK, upgrade Success). **Pending:** a one-minute Chrome look at *Browse Airpay Library* as Juma (VPN off); tester email is safe to send now (draft in `docs/cutover/uat-credentials/`, gitignored). Observation: every UAT upgrade run prints "New setting: local_sentientia_emails/default_cadence_days_json" — the default is not sticking; harmless, to look at.

---

## ✅ 2026-09-10 — last UAT residues fixed + Sentientia 5.2 package refreshed (Fable 5.1)

- **Admin dashboard tenant scoping** (theme 2026090804, `35cd2a48b`): only the 4 KPI counts had been scoped, and with `LIKE '/1%'` (matches `/177…`) — root cause of both the "over-count by the ZEEA users" note and the ZEEA admin seeing Airpay figures. Every widget now takes one exact-or-child `$tenantscope()` fragment; System Health = site admins only; the Compliance Overview widget queried non-existent `local_sentientia_compl_*` tables and never rendered — fixed (`local_compliance_*`). Verified by CLI-rendering the layout on the prod import as Airpay admin (743 users, was 749), ZEEA admin (6 / 17 / ZEEA-only) and site admin.
- **Remaining i18n/F-12 layer** (theme 2026090803, `71dae4f18`): 9 literal section headings, PHP-built Recent Activity lines (now `get_string` + unescaped course name), recommendation-card slots, login page copy (19 strings). Plus gamification level names (1.0.3-beta) and compliance-report filter defaults (1.0.1) via lang strings; org-cascade filter localised end-to-end (`3a2d93d1c`, org 1.4.2 ME tree); core card templates overridden for screen-reader course names. All on-screen checks PASS (visual-evidence 2026-09-09, 2026-09-10).
- **Deployer safety:** `deploy_to_uat.sh` aborts on drifted duplicate-tree files (UAT runs the `moodle-enhancement/` copy of `local_sentientia_org`).
- **Package:** `Sentientia-LMS-5.2-Complete-Standalone-2026-09-10.zip` — 162 MB, 65,588 files, SHA-256 `775ac465…f170`, tag `v4.2.0-sentientia-5.2-package-2026-09-10`, recipe scripted in `tools/packaging/build-5.2-standalone.sh`. Findings: the 08-03 zip had shipped the dev `config.php`; GNU tar's fake-zip trap. Pins updated in `UAT-SENTIENTIA-DEPLOY-CHECKLIST.md`, `MOODLE-5.2-RECONCILIATION-PLAN.md`, `SENTIENTIA-MIGRATION-PLAN` D-3.
- **Still pending:** UAT deploy of theme 2026090803/04 + gamification + compliance_report (next tunnel window; the package is otherwise byte-identical to UAT); tester email ready to send after that.

---

## ✅ 2026-09-08 — UAT hardening day: T-01 author fix, stale AMD bundles, Hindi dashboard, tenant-scoped Manage Courses, team siteadmin (Fable 5.1)

**Landed on UAT (https://academy2.airpay.ninja), all checksum-verified via `tools/uat/deploy_to_uat.sh` and re-checked on screen the same day:**
- **T-01 course-author gap CLOSED** (`1dc599466`): `local_sentientia_authoring\author_role::ensure()` seeds the archetype-less `sentientiaauthor` role with exactly the 7 authoring caps on BOTH fresh install and upgrade (the recurring install.php-vs-upgrade.php parity class); theme `role_detector` gains an additive capability-based `isauthor`, `sidebar_navigation` an Authoring Studio / AI Quiz / Skills AI group gated on plugin flag AND page capability. Nitin's scope rule: the author creates and manages **own** content (coursecreator + editingteacher on created courses); tenant-admin Manage Courses stays excluded. PHPUnit 5 new tests green. Visual PASS as Sneha. The 3 authoring flags were already ON globally from the 3 Sept provisioning — Nitin: leave on; **standing rule: never flip a flag without his call.**
- **P1 "invalidrecordunknown" modal root-caused — NOT tenant scoping** (`c7f085dd8`): the compiled `theme/sentientia/amd/build/org_cascade.min.js` (and `user_status_badge.min.js`) still called the retired `local_airpay_*` web-service names; the parity gate is existence-only so a stale build passes. Rebuilt with grunt (node at `<repo>/.tools/node-v22`, grunt at the XAMPP dirroot). Every admin on every org-cascade page had been hitting it.
- **Dashboard Hindi parity** (`2bddc7fd2` + `8549d4f89`): all ~50 dashboard-body literals and ~20 template-level fragments now go through `get_string()` / `{{#str}}` — 74 new en+hi key pairs, parity 301/301, placeholder sets verified; F-12 residue (format_string values re-escaped by `{{ }}`) fixed in 6 more template slots. Theme 2026090701 → **2026090801 / 1.0.52-beta** (2026090801 deploy pending the next tunnel window). Remaining English: chart axis months (library), org-cascade filter labels (component + `sentientia_courses/manage.mustache`, separate pass).
- **Manage Courses tenant scoping** (`e37013e9a`, `local_sentientia_courses` 1.11.4): KPI tiles + category filter now use the datatable's exact scope via `course_manager::manage_kpi_counts()` / `manage_category_options()`; site admins keep global figures. Policy kept: legacy NULL-`open_path` courses stay visible (test-locked; production has 2 — CTI002, BC001_1). UAT's only one (UAT-SMOKE-01) moved to `/1` as data. PHPUnit 5/5 new + 5/5 existing. Probe: admin 15 / Airpay 7 / ZEEA 4.
- **Accounts:** shareable `uat_siteadmin_team` (id 14) created + `admin` password reset (both only in gitignored `uat-credentials/UAT-TEST-ACCOUNTS.filled.md`; `.docx` regenerated via new `tools/uat/md_to_docx.py`). Flag audit: all 37 UAT overrides trace to the 3 Sept provisioning batch (incl. `sentientia.pwa.install.enabled` → the install banner is expected).
- **Tooling:** `tools/uat/deploy_to_uat.sh` (stage→tar→scp→backup→sudo extract→sha256→upgrade→purge; fixed a multi-source-scp bug); pre-commit CHECK 9 component-vs-directory heuristic fixed (`${component#*_}`) — it had been warning on all 134 multi-word plugin version.php files.
- **Process lessons:** Claude-in-Chrome dies while the corporate VPN is on → browser work (VPN off) and SSH work (VPN on) must be sequenced; "Log in as" works from the L&D admin account and needs a coordinate click on the sign-in icon; spawned-task hand-backs integrate cleanly via `git -C <worktree> diff | git apply` + copying untracked files (state cards need a hand merge).

**Evidence:** `docs/visual-evidence/2026-09-08/uat-author-t01/` and `.../uat-courses-hindi-recheck/`. **Docs:** `docs/cutover/UAT-DEMO-READINESS-2026-09-04.md` (all persona findings now resolved or scheduled), `T-01-AUTHOR-CAPS-FIX-2026-09-07.md`, state cards for theme_sentientia / sentientia_courses / sentientia_authoring. **Open (external):** C1 payment-hash merge (Nitin), H2 reCAPTCHA keys (IT), Phase 1 testers, Stage B go + DevOps sizing.

---

## 🧭 2026-06-19 — Moodle 5.2 deploy-candidate built + P4-validated (overlay approach)

**What:** moved the deploy candidate from 5.1.3+ to **Moodle 5.2** via the overlay strategy (5.2 core +
current Sentientia layer, NOT a 641-commit merge). Built the complete standalone package
`docs/cutover/dist/Sentientia-LMS-5.2-Complete-Standalone-2026-06-19.zip` (154 MB, 65,368 files; secret
`config.php` excluded; PHP 8.3 + MySQL 8.4 prereqs front-and-centre in `DEPLOY-README.txt`).

**Validation — P4 static, PASS-WITH-NOTES (0 confirmed blockers):** 13-agent workflow over the 46-plugin
layer → `php -l` 1,567/0 errors · 45 `install.xml` well-formed · 0 utf8mb4 DDL-bug-class hits (worst
index 1,220 B vs 3,072 B — the old 333-char/1333-VARCHAR class is gone) · 0 `theme_airpayux` survivors ·
55/55 version components match · the 5 `external_*` "highs" adversarially confirmed false-positive (5.2
`class_alias` shim resolves them). Full report: `docs/cutover/MOODLE-5.2-P4-STATIC-VALIDATION-2026-06-19.md`.

**Fixes shipped (committed + pushed):** reconciliation #1 otplogin · #2 BS5 badge shim (`badge-secondary`,
covers all sites) · #3 `core_external\` migration **completed** (assistant + paygw earlier; emails ×2 +
leaderboard ×3 this pass) · #5 compliance CSV export hardened (`require_sesskey` + `\core\dataformat` +
sesskey'd UI button, `f9ef2485b`) · #6 `my/*` core-overlay record. Package-only (dist is gitignored): the
10 cart/emails/proctoring `min.js` builds (exist in git, were webroot drift) patched in; the non-git
nested `local/sentientia_live/sentientia_live/` duplicate excluded.

**HARD GATES (unchanged):** prod AWS RDS → **MySQL 8.4** + **PHP 8.3** on prod & local (IT change
requests). Until those land, 5.2 **cannot run/deploy** — this candidate is for code review + staging prep
only. **Nothing deployed to live; the 5.2 cutover + ninja rehearsal remain Nitin-gated.** `course_view.php`
`external_format_text()` migration deferred (shim-functional, low risk to leave). Tooling (overlay
+ package scripts) is in the session scratchpad; the deployable source on `claude/gap-integration` is correct.
...
Windows-local artifact does NOT apply to the Linux server — enable OPcache there).

**This milestone bundles:** the 11-gap competitive cohort (feature-flagged default-OFF, mock mode;
the 2 install-blocking XMLDB DDL bugs fixed) + the scoped **Sentientia Author** system-context role
(authoring 2026061701) + brand-book seed migrations + **the 2026-06-18 link-stress-test QA fixes**:
**F1** content_market blank-page (MOODLE_INTERNAL-before-config), **F7** org-role `/my/↔/` infinite
redirect loop (one-shot `$SESSION` guard in `user_menu.php` — deploy-critical for multi-role users),
**F3** legacy-cap "Capability not found" debug-noise (guarded in `accesslib`+`core_renderer`).
Commits `e8f37b474` (F1+F7) · `5562494d1` (link-test harness+report) · `ec234347a` (F3) + the
milestone packet, **pushed to `claude/gap-integration`**. **Closed the pending ledger** (#400 packet
delivered, #401 foolproof gate delivered; #398 Gate-2/white-label/ADR-017 + #399 customer-#2
productization explicitly deferred post-milestone). Stress-test verdict: links overwhelmingly
healthy; residuals (F4 debug-only get_string; F5 quiz53/forum2 empty-500 = incomplete-`filedir`
clone artifact, re-verify on ninja; F3 residual in legacy vendor blocks) are NOT deploy blockers.
**Nothing deployed to live — ninja Phase-2 + Phase-3 are Nitin-gated.**
