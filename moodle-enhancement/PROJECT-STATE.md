...
- **Pending:** deploy 1.0.2 + 2026090805 + content_market 1.0.1-beta at the next tunnel window (`deploy_to_uat.sh --yes --prefer-me --range 4aab9bde3..HEAD`), one-minute re-look at the compliance report as Meera/Juma, then the tester email goes out (draft in `docs/cutover/uat-credentials/`, gitignored).
