...
**Updated:** 2026-05-20 (afternoon) — **Wave 1 COMPLETE (10/10 P0s) + 30 Wave 2 P1 fixes shipped.** Latest commit `15beb1a24` on `production` ships the conditional question display back-end for airpay_evaluation. Survey authors can now build branching flows; server-side enforcement ensures hidden questions skip required-answer validation. Six batches today (#25 → #30).
...
- **P1 #31 — airpay_evaluation conditional display UI** — back-end shipped in P1 #30; needs the front-end JS show/hide on respond.mustache + data attributes carrying dep info into the DOM. Small scope (~150 LOC).
- **airpay_evaluation show-non-respondents** (audit #20) — blocked on assignments table (separate P1 first).
- **airpay_exams reminder + overdue tasks** — mirror P1 #28/#29's pattern but for exams. Schema would need a new `_remind_sent` table on exams since they wrap quizzes not courses.
- **Hindi packs for `airpay_skills`, `airpay_courses`, `airpay_exams`** — symmetric work to P1 #27 (~180 strings combined).
