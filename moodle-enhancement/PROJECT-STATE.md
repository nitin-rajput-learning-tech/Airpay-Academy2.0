...
- **F-032 (🟠 paygw_airpay security) — confirmed on the live path.**
  ...
  code, not a live vuln, but recommend removal. **✅ removed 2026-05-29**
  (see top H2): deleted from both trees, and the served-tree F-032
  hardening drift (served was still pre-hardening `.09`) was promoted in
  the same `[CONFIRM]`-gated chip.
