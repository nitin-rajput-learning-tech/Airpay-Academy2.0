...
**Updated:** 2026-05-20 (evening) — **Wave 1 COMPLETE (10/10 P0s) + 31 Wave 2 P1 fixes shipped.** Latest commit `a68756b67` on `production` completes the conditional-question feature: P1 #31 ships the JS show/hide that consumes P1 #30's back-end. Authors can now build branching surveys end-to-end. Seven batches today (#25 → #31).
...
- **airpay_exams reminder + overdue tasks** — clone the P1 #28/#29 pattern but for exams. Schema needs a new `_remind_sent`-style table on exams since they wrap quizzes not courses. Same buckets concept (positive = pre-deadline, negative = post-deadline). Medium scope (~700 LOC for both halves).
