 ...
 *
 * Same design and a11y pattern as block_sentientia_cron_health:
 ...
 *
 * @package block_sentientia_cert_health
 */
class block_sentientia_cert_health extends block_base {

    public function init() {
        $this->title = get_string('pluginname', 'block_sentientia_cert_health');
    }
    ...
    public function get_content() {
        ...
        // Build the HTML. Same a11y/design pattern as block_sentientia_cron_health:
        ...
        $html = \html_writer::start_tag('section', [
            ...
            'role'       => 'region',
            'aria-label' => get_string('region_label', 'block_sentientia_cert_health'),
        ]);
        ...
        $html .= $this->kpi_card(
            get_string('kpi_sent', 'block_sentientia_cert_health'),
            $sent, $sent_severity);
        ...
        $html .= $this->kpi_card(
            get_string('kpi_failed', 'block_sentientia_cert_health'),
        ...
        $html .= $this->kpi_card(
            get_string('kpi_suppressed', 'block_sentientia_cert_health'),
        ...
        $html .= \html_writer::tag('div',
            \html_writer::link(
                new \moodle_url('/local/sentientia_emails/manage.php',
                    ['tab' => 'logs']),
                get_string('view_full_log', 'block_sentientia_cert_health')),
     ...
     * block_sentientia_cron_health::kpi_card() — see that file's docblock
    ...
    private function kpi_card(string $label, int $value, string $severity): string {
        ...
        $a_label = get_string('kpi_aria_label', 'block_sentientia_cert_health',
            (object) [
                'value'    => $value,
                'label'    => $label,
