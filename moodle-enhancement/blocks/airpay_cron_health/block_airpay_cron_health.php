...
class block_airpay_cron_health extends block_base {
    ...
    public function get_content() {
        ...
        // i18n-friendly (lang/en/block_airpay_cron_health.php).
        $html = \html_writer::start_tag('section', [
            'class'      => 'airpay-cron-health',
            'role'       => 'region',
            'aria-label' => get_string('region_label', 'block_airpay_cron_health'),
        ]);
        ...
        if (!empty($stuck)) {
            ...
            foreach ($stuck as $t) {
                ...
                $a_label = s($t->classname) . ', '
                    . get_string('overdue_label', 'block_airpay_cron_health', $overdue);
    ...
    private function kpi_card(string $label, int $value, string $severity): string {
        ...
        $severity_text = get_string("severity_$severity", 'block_airpay_cron_health');

        // Combined aria-label so the card reads as one unit.
        $a_label = get_string('kpi_aria_label', 'block_airpay_cron_health',
            (object) [
                'value'    => $value,
                'label'    => $label,
