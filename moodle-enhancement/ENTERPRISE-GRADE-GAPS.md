...
| **Native enrol modal (vs deep-link)** | ✅ | ✅ | ✅ | ✅ | ⚠ | Phase F.5 (2026-05-08) v1.6.0 — `enrol_users_modal` dynamic_form with role + multi-user picker (excludes already-enrolled + tenant-scoped); `enrol-users-modal` action wired in row's enrol icon → opens in-page modal; deep-link kept as Shift-click/middle-click fallback (`<a href>` preserved); workflow + a11y green |
...
| **Mass-enrol tool (CSV upload)** | ✅ | ✅ | ✅ | ✅ | ⚠ | Phase F.4 (2026-05-08) v1.5.0 — `enrol_csv.php` filepicker form + `enrol_csv_processor::process()` reads `email,courseshortname[,role]` rows; ROLE_ALIASES map ('student'→'employee') keeps cross-Moodle CSVs working; uses enrol_manual::enrol_user; tenant-scope guard; smoke_enrol_csv.php tests 6-row mixed CSV (succeed/skip-ghost/skip-bad-course/fail-bad-role/fail-missing) + idempotent re-run |
