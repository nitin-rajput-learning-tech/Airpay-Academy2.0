...
class enrol_learningplan_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_learningplan_enrol($instance, false) !== true) {
        ...
        $icons = array();
            ...
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_learningplan'), 'enrol_learningplan');
    ...
    public function show_enrolme_link(stdClass $instance) {
        ...
        if (true !== $this->can_learningplan_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/learningplan:config', $context)) {
            return false;
        }
     ...
     */
    public function enrol_learningplan(stdClass $instance, $data = null) {
        global $DB, $USER, $CFG;
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once("$CFG->dirroot/enrol/learningplan/locallib.php");

        $enrolstatus = $this->can_learningplan_enrol($instance);

        if (true === $enrolstatus) {
            // This user can self enrol using this instance.
            $form = new enrol_learningplan_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_learningplan($instance, $data);
                }
            }
        } else {
            ...
            $form = new enrol_learningplan_empty_form($url, $data);
        }
     ...
     */
    public function can_learningplan_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($checkuserenrolment) {
            ...
            if ($DB->get_record('user_enrolments', array('userid' => $USER->id, 'enrolid' => $instance->id))) {
                return get_string('canntenrol', 'enrol_learningplan');
            }
        }
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_learningplan_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_learningplan');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_learningplan_get_instance_info';
        }
        return $instanceinfo;
    }
     ...
     * as defined by sort_by_roleassignment_authority() having 'enrol/learningplan:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/learningplan:manage')) {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/learningplan:unenrol", $context)) {
            $url = new moodle_url('/enrol/unenroluser.php', $params);
            $actions[] = new user_enrolment_action(new pix_icon('t/delete', ''), get_string('unenrol', 'enrol'), $url, array('class'=>'unenrollink', 'rel'=>$ue->id));
        }
        if ($this->allow_manage($instance) && has_capability("enrol/learningplan:manage", $context)) {
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/learningplan:config', $context)) {
            return false;
        }
