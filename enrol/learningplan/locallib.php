 ...
 * @since  Moodle 3.0
 */
function enrol_learningplan_check_group_enrolment_key($courseid, $enrolpassword) {
    global $DB;
...
class enrol_learningplan_enrol_form extends moodleform {
    protected $instance;
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'learningplanheader', $heading);

        if ($instance->password) {
            // Change the id of self enrolment key input as there can be multiple self enrolment methods.
            $mform->addElement('passwordunmask', 'enrolpassword', get_string('password', 'enrol_learningplan'),
                    array('id' => 'enrolpassword_'.$instance->id));
            $context = context_course::instance($this->instance->courseid);
            $keyholders = get_users_by_capability($context, 'enrol/learningplan:holdkey', user_picture::fields('u'));
            $keyholdercount = 0;
            foreach ($keyholders as $keyholder) {
                $keyholdercount++;
                if ($keyholdercount === 1) {
                    $mform->addElement('static', 'keyholder', '', get_string('keyholder', 'enrol_learningplan'));
                }
            ...
            $mform->addElement('static', 'nokey', '', get_string('nopassword', 'enrol_learningplan'));
        }
        ...
        if($coursecostcenterid ==$usercostcenter)
           $this->add_action_buttons(false, get_string('enrolme', 'enrol_learningplan'));
        else 
            $mform->addElement('static', 'notforyour','',get_string('notforyour', 'enrol_learningplan'));
        ...
        if ($instance->password) {
            if ($data['enrolpassword'] !== $instance->password) {
                if ($instance->customint1) {
                    ...
                    if (!enrol_learningplan_check_group_enrolment_key($instance->courseid, $data['enrolpassword'])) {
                        // We can not hint because there are probably multiple passwords.
                        $errors['enrolpassword'] = get_string('passwordinvalid', 'enrol_learningplan');
                    }
                    ...
                    if ($plugin->get_config('showhint')) {
                        ...
                        $errors['enrolpassword'] = get_string('passwordinvalidhint', 'enrol_learningplan', $hint);
                    } else {
                        $errors['enrolpassword'] = get_string('passwordinvalid', 'enrol_learningplan');
                    }
                }
