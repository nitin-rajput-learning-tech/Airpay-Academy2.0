...
class enrol_program_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_program_enrol($instance, false) !== true) {
        ...
        $icons = array();
            ...
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_program'), 'enrol_program');
    ...
    public function get_instance_name($instance) {
        ...
        if (empty($instance)) {
            ...
            $enrol = $this->get_name();
            $program = $DB->get_record('local_program', array('id'=>$instance->customint1));
            if (!$program) {
                return get_string('pluginname', 'enrol_'.$enrol);
            }
            ...
            if ($role = $DB->get_record('role', array('id'=>$instance->roleid))) {
                ...
                return get_string('pluginname', 'enrol_'.$enrol) . ' (' . $program->name . ' - ' . $role .')';
            } else {
                return get_string('pluginname', 'enrol_'.$enrol) . ' (' . $program->name . ')';
            }
    ...
    public function show_enrolme_link(stdClass $instance) {
        ...
        if (true !== $this->can_program_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/program:config', $context)) {
            return false;
        }

        //  if($DB->record_exists('enrol', array('courseid'=>$courseid, 'enrol'=>'program','customint1'=>$programid))) {
     ...
     */
    public function enrol_program(stdClass $instance, $data = null) {
        global $DB, $USER, $CFG;
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once("$CFG->dirroot/enrol/program/locallib.php");

        $enrolstatus = $this->can_program_enrol($instance);

        if (true === $enrolstatus) {
            // This user can self enrol using this instance.
            $form = new enrol_program_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_program($instance, $data);
                }
            }
        } else {
     ...
     */
    public function can_program_enrol(stdClass $instance, $checkuserenrolment = true) {
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_program_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_program');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_program_get_instance_info';
        }
        return $instanceinfo;
    }
     ...
     * as defined by sort_by_roleassignment_authority() having 'enrol/program:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/program:manage')) {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/program:unenrol", $context)) {
            $url = new moodle_url('/enrol/unenroluser.php', $params);
            $actions[] = new user_enrolment_action(new pix_icon('t/delete', ''), get_string('unenrol', 'enrol'), $url, array('class'=>'unenrollink', 'rel'=>$ue->id));
        }
        if ($this->allow_manage($instance) && has_capability("enrol/program:manage", $context)) {
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/program:config', $context)) {
            return false;
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_program'), $roles);
        $programoptions = $this->get_program_options($context,$instance);
        $programs = array(null => get_string('select_program',
                        'enrol_program')) + $programoptions;
        $mform->addElement('select', 'customint1', get_string('program', 'enrol_program'), $programs);
        if ($instance->id) {
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        $sql = "roleid = :roleid AND customint1 =:customint1 AND courseid = :courseid AND enrol = 'program' AND id <> :id";
    ...
    public function get_program_options($context,$instance){
        global $DB;
        $sqlclass = "SELECT programid FROM {local_program_courses} as lcc  WHERE lcc.courseid =:courseid ";
        $programidsarr = $DB->get_fieldset_sql($sqlclass,array("courseid"=>$instance->courseid));
        if(!empty($programidsarr)){
            list($insql, $inparams) = $DB->get_in_or_equal($programidsarr);
            $sql = "SELECT id,name FROM {local_program}  WHERE id $insql";
            $programoptions=$DB->get_records_sql_menu($sql,  $inparams, $sort='', $fields='*', $limitfrom=0, $limitnum=0);
        }else{
