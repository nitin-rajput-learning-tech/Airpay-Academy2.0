...
class get_form extends external_api
{
    ...
    public static function execute(string $component, string $paymentarea, int $itemid, string $description): array
    {
        ...
        $order = airpay_helper::get_unprocessed_order($component, $paymentarea, $itemid);

        if ($order) {
            if (airpay_helper::check_payment($config, $order)) {
                airpay_helper::process_payment($order);
                // This order has already been paid - prevent them from paying again.
                return [
                    'airpayform' => '',
                    'warning' => \html_writer::div(get_string('orderalreadycomplete', 'paygw_airpay')),
                ];
            }
        }
        ...
        if (empty($order)) {
            // Create a new order.
            $order = airpay_helper::create_order($component, $paymentarea, $itemid, $payable->get_account_id());
        }

        $script = airpay_helper::get_payment_script($config, $order, $description, $cost);
    ...
    public static function execute_returns(): external_single_structure
    {
        return new external_single_structure([
            'airpayform' => new external_value(PARAM_RAW, 'airpay form for payment'),
            'warning' => new external_value(PARAM_TEXT, 'airpay warnings'),
        ]);
    }
}
